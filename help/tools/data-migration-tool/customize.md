...
有时，Magento 1与Magento 2之间由[扩展](https://commercemarketplace.adobe.com//extensions.html)或自定义代码创建的数据格式和结构会有所不同。 使用[!DNL Data Migration Tool]中的扩展点迁移此数据。 如果数据格式和结构相同，则该工具可以自动迁移数据，而无需用户干预。
