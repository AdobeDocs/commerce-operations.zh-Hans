...
* **ACSD-56979** （适用于 Adobe Commerce 和 Magento Open Source >=2.4.3 &lt;2.4.7) - Fixes the issue where product images are removed after deleting a staging update.
* **ACSD-57086** （适用于 Adobe Commerce 和 Magento Open Source >=2.4.3 &lt;2.4.7) - Fixes the issue where the orders placed from non-default websites with terms and conditions enabled are not processed correctly.
* **ACSD-57588** （适用于 Adobe Commerce 和 Magento Open Source >=2.4.6 &lt;2.4.7) - Fixes the issue where shipping an order to multiple addresses triggers an error during region ID processing.
* **ACSD-57643** （适用于 Adobe Commerce 和 Magento Open Source >=2.4.6 &lt;2.4.7) - Fixes the issue where products with custom options are incorrectly added to the shopping cart via GraphQL.
* **ACSD-57846** （适用于 Adobe Commerce 和 Magento Open Source >=2.4.2 &lt;2.4.7) - Fixes the issue where GraphQL products search with a filter for zero prices does not return any results due to an exception.
* **ACSD-57941** （适用于 Adobe Commerce 和 Magento Open Source >=2.4.6 &lt;2.4.7) - Fixes the issue where product options are incorrectly assigned to the admin store instead of their respective stores.
* **ACSD-58375**(适用于Adobe Commerce和Magento Open Source>=2.4.2 &lt;2.4.7) — 修复了在商店视图级别添加[!DNL YouTube]视频时，错误&#x200B;*[!DNL YouTube API Key]*&#x200B;配置导致错误的问题。
* **ACSD-58739**(适用于Adobe Commerce和Magento Open Source>=2.4.7 &lt;2.4.8) — 修复了部分重新索引引发错误的问题。
...
* **ACSD-57337**(适用于Adobe Commerce >=2.4.4 &lt;2.4.6) — 修复了以下问题：对特定网站具有访问限制的管理用户能够在&#x200B;**[!UICONTROL Companies]**&#x200B;网格中查看所有网站中的公司。
...
* **ACSD-55241**(适用于Adobe Commerce和Magento Open Source>=2.4.2 &lt;2.4.7) — 修复了在使用多个地址进行签出期间，*[!UICONTROL Used]*&#x200B;和&#x200B;*[!UICONTROL Times Used]*&#x200B;属性显示的生成优惠券值不正确的问题。
* **ACSD-56760**(适用于Adobe Commerce >=2.4.6 &lt;2.4.7) — 修复了以下问题：如果仅限特定网站的管理员用户在Webstore有自己的根类别时，无法对类别内的产品进行排序或添加新产品。
* **ACSD-56858**(适用于Adobe Commerce >=2.4.2 &lt;2.4.7) — 修复了为受限制的公司管理员错误地显示B2B公司角色权限的问题。
* **ACSD-57074**(适用于Adobe Commerce和Magento Open Source>=2.4.6 &lt;2.4.7) — 修复了以`price_`开头的`attrbute_code`的&#x200B;*是/否*&#x200B;自定义属性无法正确使用索引以及具有此类属性的产品在前端不可用的问题。
...
* **ACSD-56621** （对于 Adobe Commerce >=2.4.2 &lt;2.4.7) - Fixes the issue where the updated first name and last name are not reflected in the greetings header section for the company admin user.
...
* **ACSD-56415**(适用于Adobe Commerce和Magento Open Source>=2.4.5 &lt;2.4.7) — 修复了在数据库包含大量要索引的部分价格数据时，由于`DELETE`查询，部分价格索引的性能会下降的问题。
* **ACSD-54965**(适用于Adobe Commerce >=2.4.5 &lt;2.4.6) — 修复了将产品仅分配给自定义Stock时，Visual Merchandising网格无法显示正确Stock的问题。
* **ACSD-52824**(适用于Adobe Commerce >=2.4.5 &lt;2.4.7) — 修复了在公司设置中禁用公司客户的PayPal Express、Google Pay和Apple Pay按钮时，显示此类付款方式按钮的问题。
* 更新的修补程序：ACSD-56193

## 版本1.1.44 {#v1-1-44}

* **ACSD-56790**(适用于Adobe Commerce和Magento Open Source>=2.4.6 &lt;2.4.7) — 修复了使用“**从Stock移至底部**”选项对类别产品进行排序时，用户被重定向到管理员仪表板且屏幕顶部显示`Invalid security or form key. Please refresh the page`错误的问题。
* **ACSD-56280** （适用于 Adobe Commerce >=2.4.4 &lt;2.4.7) - Fixes the issue where ordering items from a gift registry leads to an exception.
* **ACSD-56246** （适用于 Adobe Commerce 和 Magento Open Source >=2.4.6 &lt;2.4.7) - Fixes the issue where data is removed from the custom multi-select attribute when a scheduled update for a product becomes active.
...
* **ACSD-54885** （适用于 Adobe Commerce 和 Magento Open Source >=2.4.2 &lt;2.4.7) - Fixes the issue with the multiple address checkout when the admin user is using the *以客户* 身份登录功能。
* **ACSD-55610** （适用于 Adobe Commerce 和 Magento Open Source >=2.4.4 &lt;2.4.7) - Fixes the issue where a partially canceled order has an incorrect discount amount.
* **ACSD-55334** （适用于 Adobe Commerce 和 Magento Open Source >=2.4.3 &lt;2.4.7) - Fixes translations for labels through Translation dictionaries in GraphQL response.
...
* **ACSD-55305**(适用于Adobe Commerce >=2.4.4 &lt;2.4.7) — 修复了&#x200B;**[!UICONTROL myAccount]** > **[!UICONTROL Company Structure]**&#x200B;页面上的&#x200B;*[!UICONTROL Edit Company User]*&#x200B;弹出窗口在屏幕上因加载程序而冻结的问题。
* 更新的修补程序： ACSD-49013
...
* **ACSD-54067**(适用于Adobe Commerce和Magento Open Source>=2.4.0 &lt;2.4.7) — 修复了在移动设备上不播放产品视频的问题。
* **ACSD-55414**(适用于Adobe Commerce和Magento Open Source>=2.4.0 &lt;2.4.6) — 改进了MariaDB尝试将EAV entity_id从字符串转换为整数时的性能。
* **ACSD-51819**(适用于Adobe Commerce >=2.4.4 &lt;2.4.4-p4) — 修复了可以使用同一报价ID下达多个订单的问题。
* **ACSD-53118**(适用于Adobe Commerce >=2.4.0 &lt;2.4.7) — 修复了在产品具有空属性时，使用优惠券代码应用&#x200B;*[!UICONTROL Cart Price Rule]*&#x200B;的问题。
* **ACSD-54324**(适用于Adobe Commerce >=2.4.5 &lt;2.4.7) — 修复GraphQL requisition_lists请求不考虑分页设置并返回所有结果的问题。
...
* **ACSD-54319**(适用于Adobe Commerce和Magento Open Source>=2.4.2 &lt;2.4.6) — 修复了&#x200B;*[!UICONTROL Product in Cart]*&#x200B;报告中产品价格为零的问题。
* **ACSD-53378**(适用于Adobe Commerce和Magento Open Source>=2.4.5 &lt;2.4.7) — 为拥有大型通讯簿的客户缩短了结帐页面加载时间。
...
* **ACSD-54472** （适用于 Adobe Commerce >=2.4.6 &lt;2.4.7) - Fixes the issue where customers of a rejected company can still authenticate, and customers of a blocked and a rejected company can still place orders. 该修补程序为 GraphQL 端点添加了额外的验证。
* **ACSD-52801** （适用于 Adobe Commerce 和 Magento Open Source >=2.4.4 &lt;2.4.7) - Adds the option to do a partial match when searching for products in GraphQL.
* **ACSD-55004** （适用于 Adobe Commerce 和 Magento Open Source >=2.4.6 &lt;2.4.7) - Fixes the issue where the validator crashes while uploading an import file larger than the value configured in `php.ini`.
...
* **ACSD-53704**(适用于Adobe Commerce >=2.4.0 &lt;2.4.7) — 修复了在奖励点数过期后错误计算奖励点数平衡历史记录的问题。
* **ACSD-53583**（适用于 Adobe Commerce 和 Magento Open Source >=2.4.4 *&lt;2.4.7) - Improves partial reindex performance for  类别产品和**产品类别*&#x200B;索引器。
* **ACSD-54026** （对于 Adobe Commerce >=2.4.6 &lt;2.4.7) - Fixes an incorrect error message for an `updateCompanyRole` 针对非授权用户的 GraphQL 请求。
* **ACSD-54106** （适用于 Adobe Commerce 和 Magento Open Source >=2.4.1 &lt;2.4.5) - Fixes the issue where category product sorting by name for Turkish accented characters is incorrect.
* **ACSD-52219** （适用于 Adobe Commerce 和 Magento Open Source >=2.4.5 &lt;2.4.7) - Fixes the issue where Admin grids saved filters don&#39;t work as expected when frequently switching between bookmark views.
* **ACSD-54342** （适用于 Adobe Commerce 和 Magento Open Source >=2.4.0 &lt;2.4.7) - Fixes an incorrect error message *） 数据结构错误：导入不含有效数据的 CSV 文件时，值会混合* 。
* **ACSD-54660**(适用于Adobe Commerce和Magento Open Source>=2.4.4 &lt;2.4.6) — 添加了一个新的输入属性&#x200B;*sort*，用于按`sort_field`和`sort_direction`对GraphQL中的客户订单进行排序。
* **ACSD-54776**(适用于Adobe Commerce >=2.4.5 &lt;2.4.7) — 修复了未选中的&#x200B;*[!UICONTROL Use Default Value]*&#x200B;和非默认产品字段值不会为第二个网站、商店和商店视图保存的问题。
* **ACSD-53998**(适用于Adobe Commerce和Magento Open Source>=2.4.4-p2 &lt;2.4.5) || >=2.4.5-p1 &lt;2.4.7) — 修复了从客户帐户注销后，基于&#x200B;**[!UICONTROL Customer Segment]**&#x200B;的&#x200B;**[!UICONTROL Dynamic Block]**&#x200B;无法正常工作的问题。
...
* **ACSD-47657**(对于Adobe Commerce和Magento Open Source>=2.4.4 &lt;2.4.7) — 为AWS凭据添加了缓存机制。 凭据提供程序现在使用Magento缓存来缓存从AWS中检索到的凭据，以进行EC2配置。
...
* **ACSD-54111**(适用于Adobe Commerce和Magento Open Source>=2.4.2 &lt;2.4.6) — 修复了水印图像的长宽比与产品图像不匹配时不会在店面上显示产品缩略图的问题。
...
* **ACSD-51574**(适用于Adobe Commerce和Magento Open Source>=2.4.2 &lt;2.4.7) — 修复了将图像替换为具有相同名称的其他图像后，图像在前端未更新的问题。
...
* **ACSD-47875**(适用于Adobe Commerce和Magento Open Source>=2.3.7 &lt;2.4.7) — 修复了管理员用户无法从具有库存管理的特定商店视图范围的Admin将产品添加到客户购物车的问题。
* **ACSD-53176**(适用于Adobe Commerce >=2.3.7 &lt;2.4.5) — 修复了具有&#x200B;*的*&#x200B;相关产品规则&#x200B;*是*&#x200B;条件之一，与产品不匹配的问题。
* **ACSD-51666**(适用于Adobe Commerce和Magento Open Source>=2.3.7 &lt;2.4.7) — 修复了错误&#x200B;*会话已过期，请重新登录。在客户尝试登录后发生的*。
* 添加了MDVA-39305-v2的新版本。
* 更新了ACSD-19640的要求。

## 1.1.35 版 {#v1-1-35}

* **ACSD-51899** （适用于 Adobe Commerce 和 Magento Open Source >=2.4.0 &lt;2.4.7) - Fixes the issue where the default shipping address on the checkout shipping step is auto-populated with a previously selected in-store pickup address.
* **ACSD-52041** （对于 Adobe Commerce 和 Magento Open Source >=2.4.4 &lt;2.4.7) - Fixes the issue where the error message: *[ERROR] [!DNL Page Builder] 渲染了 5 秒钟而不释放锁定。保存使用[!DNL Page Builder]编辑的内容时，Chrome浏览器中会显示*。
...
* **ACSD-48070**(适用于Adobe Commerce >=2.3.7 &lt;2.4.7) — 修复了编辑计划的更新时触发异常的问题。
* **ACSD-51890**(适用于Adobe Commerce和Magento Open Source>=2.4.0 &lt;2.4.7) — 修复了在没有[!DNL Google reCAPTCHA] v3验证的情况下多次单击[!UICONTROL Submit review]按钮的问题。
...
* **ACSD-52202** （适用于 Adobe Commerce 和 Magento Open Source >=2.4.3 &lt;2.4.7) - Fixes the issue where the salable qty of default stock wrongly changes to 0 when a non-default stock is changed to 0 qty on order fulfillment.
* **ACSD-51265** （适用于 Adobe Commerce 和 Magento Open Source >=2.4.2 &lt;2.4.7) - Fixes the issue with `catalog_product_price` 系统中捆绑产品过多时的重新索引性能。
* **ACSD-52831**(适用于Adobe Commerce >=2.3.7 &lt;2.4.7) — 修复了启用[!DNL Google reCAPTCHA v3 Invisible]后客户无法下达可转让报价单的问题。
* **ACSD-51845**(适用于Adobe Commerce和Magento Open Source>=2.4.4 &lt;2.4.7) — 修复了无法通过异步批量REST API更新具有层价格和其他属性集的后续产品的问题。
* **ACSD-52815**(适用于Adobe Commerce和Magento Open Source>=2.3.7 &lt;2.4.7) — 修复了非默认来源的数量字段输入最多支持6位数字的问题，不同于默认Stock的8位。
* **ACSD-51149**(适用于Adobe Commerce >=2.3.7 &lt;2.4.7) — 修复了启用目录权限的计划导入导出使索引器无效，然后由cron缓存刷新的问题。
* **ACSD-50815**(适用于Adobe Commerce，Magento Open Source>=2.4.5 &lt;2.4.6) — 修复了简单产品的小数数量不能用于新的捆绑产品选项的问题。
* ACSD-47803的更新版本。
* 更新了ACSD-51892的标题。
* 更新了ACSD-51379。
* 更新了ACSD-49970-v2。
...
* **ACSD-50813**(适用于Adobe Commerce >=2.4.5 &lt;2.4.7) — 修复了管理员无法在包含[!UICONTROL Add Products by SKU]功能的SKU中将包含斜杠的捆绑产品添加到管理员订单的问题。
* **ACSD-51630**(适用于Adobe Commerce和Magento Open Source>=2.4.3 &lt;2.4.7) — 修复了大量系统消息导致下载管理页面速度慢的问题。
* **ACSD-51853**(适用于Adobe Commerce和Magento Open Source>=2.4.1 &lt;2.4.7) — 修复了在使用[!UICONTROL Page Builder]时未应用复制的文本样式的问题。
* **ACSD-52160**(适用于Adobe Commerce和Magento Open Source>=2.4.4 &lt;2.4.7) — 修复了以下问题：根据购物车价格规则，产品验证结果未根据规则条件正确评估“如果在购物车中找到/未找到项目，且所有条件/上述任一条件为true”。
* **ACSD-51636**(Adobe Commerce >=2.4.5 &lt;2.4.7) — 修复了公司管理员尽管拥有所有必要的角色和权限，但不能从“客户帐户”部分添加新用户的问题。
* **ACSD-51739**(适用于Adobe Commerce >=2.4.6 &lt;2.4.7) — 修复了在CompanyTeam GraphQL请求中请求`structure_id`时返回错误的问题。
* **ACSD-51857**(适用于Adobe Commerce和Magento Open Source>=2.4.0 &lt;2.4.7) — 修复了大型sales_order和`sales_order_item`数据库表上的`aggregate_sales_report_bestsellers_data` cron报表由于主数据查询的写入方式而性能缓慢的问题。
* **ACSD-48448**(适用于Adobe Commerce和Magento Open Source>=2.4.2 &lt;2.4.7) — 修复了在订单取消期间发生争用条件问题，导致`inventory_reservation`表中出现重复项的问题。
* **ACSD-52689**(适用于Adobe Commerce和Magento Open Source>=2.4.3 &lt;2.4.6) — 修复了使用REST API无法将图像上传到Amazon S3存储空间的问题。
...
* **ACSD-50368**(适用于Adobe Commerce，Magento Open Source>= 2.4.3 &lt;2.4.5) — 修复了在通过Async REST API或Async Bulk REST API创建客户时，忽略客户的`group_id`的问题。
* **ACSD-51497**(适用于Adobe Commerce和Magento Open Source>=2.3.7 &lt;2.4.0) || >= 2.4.1 &lt;2.4.7) — 修复了客户无法按下拉类型的自定义属性对目录页面进行排序的问题。
* **ACSD-51408**(适用于Adobe Commerce和Magento Open Source>=2.3.7 &lt; 2.4.7) — 修复了将订单项状态错误地设置为&#x200B;*[!UICONTROL Backordered]*&#x200B;的问题。
* **ACSD-51735**(适用于Adobe Commerce和Magento Open Source>=2.4.4 &lt;2.4.5) — 修复了在产品素材库为&#x200B;*0*&#x200B;时，将订单项状态错误地设置为&#x200B;*[!UICONTROL Ordered]*&#x200B;的问题。
...
* **ACSD-51120**(对于Adobe Commerce >=2.3.7 &lt;2.4.3) — 修复了以下问题：对于包含通过暂存更新更新更新的GraphQL块的CMS页面，未清除CMSGET的缓存。
* **ACSD-51240**(对于Adobe Commerce >=2.4.4 &lt;2.4.6) — 修复了通过公司注册表进行注册时上传文件缺失的问题。
* **ACSD-51907**(适用于Adobe Commerce >=2.4.2 &lt;2.4.3) — 修复了受限管理员用户无法创建具有脱机退款的贷项通知单的问题。
* **ACSD-52148**(适用于Adobe Commerce和Magento Open Source>=2.4.0 &lt;2.4.4) — 修复了[!DNL Google V3 reCAPTCHA]管理员登录偶尔失败的问题。
* **ACSD-51431**(对于Adobe Commerce和Magento Open Source>=2.3.7 &lt;2.4.7) — 修复了即使更改日志中没有新条目，索引器状态为&#x200B;*正在工作*&#x200B;的问题。
* **ACSD-51892** （适用于 Adobe Commerce 和 Magento Open Source >=2.4.6 &lt;2.4.7) - Fixes the performance issue where config files load multiple times.
* 已弃用的 ACSD-51114。

## 1.1.32 版 {#v1-1-32}

* **ACSD-49628** （适用于 Adobe Commerce 和 Magento Open Source >=2.4.2 &lt;2.4.7) - Fixes the issue where the [!UICONTROL Page Builder's] ）多个错误阻止管理员在没有内容权限的情况下保存产品。
* **ACSD-51305** （适用于 Adobe Commerce 和 Magento Open Source >=2.4.6 &lt;2.4.7) - Fixes the issue where out-of-stock configurable child products are not available in the GraphQL response.
...
* **ACSD-50336**(适用于Adobe Commerce和Magento Open Source>=2.4.4-p1 &lt;2.4.4-p3) — 修复了当产品恢复库存或价格发生变化时，不会发送产品警告电子邮件的问题。
* **ACSD-50367**(适用于Adobe Commerce和Magento Open Source>=2.3.7 &lt;2.4.7) — 修复了创建没有值的多选客户地址属性时，客户地址导出不起作用的问题。
* **ACSD-49877**(适用于Adobe Commerce和Magento Open Source>=2.3.7 &lt;2.4.7) — 修复了视频直接链接到远程视频文件而不是流服务时，视频自动播放在移动设备[!DNL Safari]上不起作用的问题。
* **ACSD-50165**(适用于Adobe Commerce和Magento Open Source>=2.4.0 &lt;2.4.7) — 修复了错误&#x200B;*无法删除该文件。 警告！unlink：从管理员刷新JS/CSS缓存时，没有此类文件或目录*。
...
* **ACSD-50527**(适用于Adobe Commerce >=2.3.7 &lt;2.4.7) — 修复了保存包含空动态块的页面时出现的错误。
* **ACSD-49973**(适用于Adobe Commerce和Magento Open Source>=2.4.4 &lt;2.4.5) — 改进通过GraphQL获取捆绑产品的性能。
...
* **ACSD-48771**(适用于Adobe Commerce和Magento Open Source>=2.4.5 &lt;2.4.7) — 修复了从早期的[!DNL Page Builder]版本升级列块内容类型的问题。
...
* **ACSD-49835**(适用于Adobe Commerce和Magento Open Source>=2.4.5 &lt;2.4.7) — 修复了使用默认复选框值未在多选属性的商店级别正确保存的问题。
...
* **ACSD-48807**(适用于Adobe Commerce和Magento Open Source>=2.4.1 &lt;2.4.7) — 修复了通过GraphQL的storeview未筛选客户产品评论的问题。
* **ACSD-49433**(适用于Adobe Commerce和Magento Open Source>=2.4.3 &lt;2.4.7) — 修复了在购物车中显示未结金额礼品卡默认金额的小计的问题。
* **ACSD-48866**(适用于Adobe Commerce和Magento Open Source>=2.3.7 &lt;2.4.7) — 修复了请求类别的RSS源时出错的问题。
...
* **ACSD-48362**(适用于Adobe Commerce >=2.4.1 &lt;2.4.7) — 修复了在使用可转让报价下单时使用默认收货地址而不是新收货地址的问题。
* **ACSD-48059**(适用于Adobe Commerce >=2.3.7 &lt;2.4.7) — 修复了商家无法将“[!UICONTROL Match product by rule]”保存在类别中的问题。
...
* **ACSD-48164**(适用于Adobe Commerce和Magento Open Source>=2.3.7 &lt;2.4.7) — 修复了受限管理员无法保存网站级别值的问题。
* **ACSD-48404**(适用于Adobe Commerce和Magento Open Source>=2.4.0 &lt;2.4.4) — 修复了当按浏览器的返回按钮时，“记住类别分页=是”会导致错误的问题。
...
* **ACSD-49042**(适用于Adobe Commerce和Magento Open Source>=2.4.4 &lt;2.4.5) — 修复了无法从店面订购具有无限延期交货的产品的问题。
* 更新的修补程序：ACSD-48366、ACSD-48661。

## 版本1.1.26 {#v1-1-26}

* **ACSD-47937**(适用于Adobe Commerce和Magento Open Source2.4.4) || >=2.4.5 &lt;2.4.6) — 修复了由于应用程序级缓存而并非始终发送价格下降通知的问题。
* **ACSD-48661**(适用于Adobe Commerce和Magento Open Source>=2.3.7 &lt;2.4.7) — 修复了如果公司的信用额度大于999，且逗号分隔符会因验证错误而导致公司无法挽留的问题。
* **ACSD-48773**(适用于Adobe Commerce和Magento Open Source>=2.4.2 &lt;2.4.7) — 修复了奖励积分电子邮件模板从错误商店获取的问题。
* **ACSD-48587**(适用于Adobe Commerce和Magento Open Source>=2.3.7 &lt;2.4.7) — 修复了在产品小组件匹配规则中HTML特殊字符导致无法显示匹配产品的问题。
* **ACSD-48212**(适用于Adobe Commerce和Magento Open Source>=2.3.7 &lt;2.4.6) — 修复了产品导入将产品分配给错误源的问题。
...
* **ACSD-46541**(适用于Adobe Commerce和Magento Open Source>=2.4.0 &lt;2.4.4) — 修复了删除订单项时管理员用户无法创建贷项通知单的问题。
* **ACSD-46581**(适用于Adobe Commerce和Magento Open Source>=2.4.0 &lt;2.4.6) — 修复了在购物车中选择国家/地区后，估计税额合计未更新的问题。
* **ACSD-46618**(适用于Adobe Commerce和Magento Open Source>=2.4.0 &lt;2.4.6) — 修复了产品列表小组件为登录客户显示不正确缓存价格的问题。
* **ACSD-46674**(适用于Adobe Commerce和Magento Open Source>=2.4.0 &lt;2.4.6) — 修复了图像类型的自定义选项在客户电子邮件中显示为HTML的问题。
* **ACSD-46988**(适用于Adobe Commerce和Magento Open Source>=2.4.4 &lt;2.4.6) — 修复GraphQL“currency”API请求为自定义货币返回NULL值的问题。
...
* 更新了修补程序：MDVA-42584。
* 已替换的修补程序：MDVA-36572、ACSD-45241。

## 版本1.1.20 {#v1-1-20}

* **ACSD-46520**(*对于Adobe Commerce和Magento Open Source>=2.4.0 &lt;2.4.5*) — 修复了用户在使用商店积分退款时获得错误订单状态的问题。
* **ACSD-46703**(*Adobe Commerce和Magento Open Source>=2.4.4 &lt;2.4.6*) — 修复了在产品编辑页面上无法拖放自定义选项的问题。
* **ACSD-44851**(*对于Adobe Commerce和Magento Open Source>=2.4.0 &lt;2.4.6*) — 修复了包含子类别的类别无法打开或扩展的问题。
* **ACSD-46815**(*Adobe Commerce和Magento Open Source>=2.4.5 &lt;2.4.6*) — 修复了类别树请求仅限于20个类别的问题。
* **ACSD-45675** (*Adobe Commerce和Magento Open Source>=2.4.0 &lt;2.4.6*) — 修复了产品导出使用&#x200B;*默认商店视图*&#x200B;作用域中的类别名称的问题。
* **ACSD-46869**(*Adobe Commerce和Magento Open Source>=2.4.4 &lt;2.4.6*) — 修复了购物车中的可配置产品未通过&#x200B;*PUTREST API*&#x200B;请求更新而不更改产品数量的问题。
* **MDVA-42768-V2**(*Adobe Commerce和Magento Open Source>=2.4.2 &lt;2.4.3*) — 修复了当&#x200B;*显示缺省*&#x200B;为&#x200B;*是*&#x200B;时，可配置产品将正常价格显示为&#x200B;*0*&#x200B;的问题。
...
* **ACSD-45520**(*Adobe Commerce和Magento Open Source>=2.3.0 &lt;2.4.6*) — 修复了当用户从购物车编辑可配置产品时，未在产品详细信息页面上预先选择色板选项的问题。
* **ACSD-45169**(*Adobe Commerce和Magento Open Source>=2.4.1 &lt;2.4.6*) — 修复了在应用暂存更新后，[!DNL Visual Merchandiser]未显示可配置产品的正确Stock和价格的问题。
* **ACSD-45424**(*对于Adobe Commerce和Magento Open Source>=2.3.4 &lt;2.4.6*) — 修复了在部分退款（贷项通知单）后创建错误保留补偿的问题。
...
* **MDVA-44044**(*Adobe Commerce和Magento Open Source>=2.3.0 &lt;2.4.2-p2*) — 修复了将产品分配给新网站后未在页上显示产品的问题。
* **MDVA-42509**(*Adobe Commerce和Magento Open Source>=2.3.3 &lt;2.4.4*) — 修复了无法上传CSV以快速订购导致&#x200B;*无法发送Cookie*&#x200B;错误的问题。
* 更新的修补程序：MDVA-41061、MDVA-42584。
...
* **MDVA-44660**（*适用于 Adobe Commerce 和 Magento Open Source >=2.4.2-p1*&lt;2.4.5）- 修复了无法将重音字符``` ` ```用于客户名字和姓氏的问题。
* **MDVA-40896**（*适用于 Adobe Commerce 和 Magento 开源 >=2.4.3*&lt;2.4.4） - 修复了以下错误：*类型错误：在异步产品批量 API 中，将参数 3 传递给 Magento* 错误。
* **MDVA-38559（适用于 Adobe Commerce 和 Magento Open Source >=2.4.0&lt;2.4.3 *）- 修复了*具有多个订阅的客户 /V1/customers/search API *错误。***
* **MDVA-44533** （*适用于 Adobe Commerce 和 Magento Open Source >=2.3.1 &lt;2.4.4*）- 修复了折扣错误地应用于捆绑子产品的问题。
* 更新的修补程序：MDVA-41061、MDVA-42269。

## 1.1.14 版 {#v1-1-14}

* **MDVA-43983**（*适用于 Adobe Commerce 和 Magento Open Source >=2.4.2&lt;2.4.5*）- 修复了“不单独&#x200B;*可见”产品*&#x200B;仍显示在目录高级搜索结果中的问题。
* **MDVA-44100** （*适用于 Adobe Commerce 和 Magento 开源 >=2.4.3 &lt;2.4.5*） - 修复了所有 FPT 都分配给购物车中的最后一个产品并为其他产品重置的问题。
* **MDVA-43605** （*适用于 Adobe Commerce 和 Magento Open Source >=2.3.1 &lt;2.4.5*）- 修复了使用 Rest API 时订单数据返回行总计负值的问题。
* **MDVA-43102** （*适用于 Adobe Commerce 和 Magento Open Source >=2.3.1 &lt;2.4.5*）- 修复了通过 REST API 完成退款时无法正确更新可售数量的问题。
...
* **MDVA-42790** （*适用于 Adobe Commerce 和 Magento Open Source >=2.4.3 &lt;2.4.5*）- 修复了无法通过 REST API 为特定网站更新产品价格属性的问题。
* **MDVA-41350** （*适用于 Adobe Commerce 和 Magento Open Source >=2.3.0 &lt;2.4.5*）- 修复了当具有受限访问权限的管理员用户按订单中的 SKU 添加超出其角色范围的产品时引发异常的问题。
* **MDVA-42269**（*适用于 Adobe Commerce 和 Magento Open Source >=2.4.3-p1*&lt;2.4.5）- 修复了管理员用户由于 TypeError 而&#x200B;*无法登录到 Admin 的问题：strtotime（） 期望参数 1 为字符串，给定*&#x200B;错误为 null。
* **MDVA-40830** （*适用于 Adobe Commerce 和 Magento Open Source >=2.3.0 &lt;2.4.5*）- 修复了在下单期间多次应用商店信用的问题。
* **MDVA-42237** （*适用于 Adobe Commerce 和 Magento Open Source >=2.4.2 &lt;2.4.5*）- 修复了可配置产品特价在其子产品价格更改后不更新的问题。
...
* **MDVA-40816**(*对于Adobe Commerce和Magento Open Source>=2.3.0 &lt;2.4.5*) — 修复了相关产品规则显示来自规则条件中未定义的类别中的产品的问题。
...
* **MDVA-27239**(*对于Adobe Commerce和Magento Open Source>=2.3.0 &lt;2.3.6*) — 修复了未显示交叉销售产品的问题。
...
* 已改进[!DNL Quality Patch Tool]的索引页。 在工具的最新版本中为[!DNL quality patches]添加了方便的搜索和筛选。
* 更新的修补程序：MDVA-33382、MDVA-39482。

## 1.1.5 版 {#v1-1-5}

* **MDVA-41236** （*适用于 Adobe Commerce 和 Magento Open Source >=2.3.0 &lt;2.4.4*）- 修复了以下问题：如果之前删除了“结束日期”，则无法为产品创建新的更新或编辑现有的计划更新。
* **MDVA-41046** （*适用于 Adobe Commerce 和 Magento Open Source >=2.3.0 &lt;2.4.4*）- 修复了具有自定义选项的简单产品可用于分配给可配置/分组产品的问题。
* **MDVA-40545** （*适用于 Adobe Commerce 和 Magento Open Source >=2.3.0 &lt;2.4.4*）- 修复了即使同一页面有多个节点，也只能检索页面的第一个节点的问题。
* **MDVA-41164** （*适用于 Adobe Commerce 和 Magento Open Source >=2.4.2 &lt;2.4.3-p1*）- 修复了管理员用户无法使用文件或图像类型自定义客户属性保存或编辑公司的问题。
* **MDVA-39229** (*Adobe Commerce和Magento Open Source>=2.3.0 &lt;2.4.4*) — 修复了导致在更新目录规则暂存更新开始时间后出现以下错误的问题： *Cron Job staging_synchronize_entities_period出错：无法删除活动更新。*
* **MDVA-40619**(*对于Adobe Commerce和Magento Open Source>=2.3.0 &lt;2.4.4*) — 修复了尝试在CMS页面上执行内联编辑时，对CMS页面层次结构所做的更改会导致500错误的问题。
* **MDVA-41061**(*对于Adobe Commerce和Magento Open Source>=2.4.2 &lt;2.4.3*) — 修复了从管理员那里保存产品时Stock状态重置为可销售的问题。
...
* **MDVA-29400** (*对于Adobe Commerce和Magento Open Source>=2.3.0 &lt;=2.3.7-p1 || >=2.4.0 &lt;=2.4.0-p1*) — 修复了对[!DNL PayPal Express Checkout]下达的重复订单的问题。
...
* **MDVA-37234** (*对于Adobe Commerce和Magento Open Source>=2.3.5 &lt;2.4.0 || >=2.4.1 &lt;=2.4.2-p2*) — 修复了将项目多次添加到购物车（并行请求）以请求同一SKU为同一购物车ID创建重复行项目的问题。
* **MDVA-33606**(*对于Adobe Commerce和Magento Open Source>=2.4.1 &lt;=2.4.2-p2*) — 修复了在保存分配给层次结构的CMS页面时，用户收到&#x200B;*唯一约束冲突*&#x200B;错误的问题。
* **MDVA-31590**(*对于Adobe Commerce和Magento Open Source>=2.4.0 &lt;=2.4.1-p1*) — 修复了用户无法使用MySQL异步队列批量更新属性的问题。
* **MDVA-36309**(*Adobe Commerce和Magento Open Source>=2.4.2 &lt;=2.4.2-p2*) — 修复了管理网格中按属性搜索产品时速度慢的问题。
...
* **MDVA-39043**（*适用于 Adobe Commerce 和 Magento Open Source >=2.3.4&lt;=2.4.3*）- 修复了具有有限访问权限的管理员用户在尝试将“产品&#x200B;*”小组件添加到* CMS 页面时收到错误的问题。
...
* **MDVA-36021** (*Adobe Commerce和Magento Open Source>=2.4.0 &lt;2.4.4*) — 修复了管理员的订单详细信息页面上显示&#x200B;*调用成员函数getId()*&#x200B;错误的问题。
* **MDVA-34948** (*对于Adobe Commerce和Magento Open Source>=2.3.6 &lt;=2.3.6-p1 || >=2.4.0 &lt;=2.4.0-p1*) — 修复长时间运行的查询（如`GET_LOCK`）的问题。
* **MDVA-39305**(*对于Adobe Commerce和Magento Open Source>=2.4.0 &lt;=2.4.2-p1*) — 修复了注册客户无法使用启用的Google ReCaptcha登录的问题。
* **MDVA-37897**(*Adobe Commerce和Magento Open Source>=2.3.0 &lt;2.4.4*) — 修复了客户尝试使用“最近查看”小组件中的选项添加产品时重定向不正确的问题。

## 版本1.1.0 {#v1-1-0}

* 引入了修补程序类别，以改善用户体验并使客户更轻松地搜索所需的修补程序。
* `patches.json`文件已重命名为`support-patches.json`。
* **MDVA-38799** （*适用于 Adobe Commerce >=2.3.0 &lt;2.4.3*）- 修复了创建暂存更新后无法保存可下载产品的问题。
...
* **MDVA-38468** (*Adobe Commerce >=2.3.2 &lt;=2.3.5-p2*) — 修复了保存CMS页面时的错误： *具有相同ID PAGE_ID的项目已存在*。
* **MDVA-34680** (*对于Adobe Commerce >=2.3.6 &lt;=2.3.7 || >=2.4.1 &lt;2.4.3*) — 修复了在客户网格中无法正确筛选客户帐户创建时间的问题。
...
* **MDVA-36572** （*适用于 Adobe Commerce >=2.3.5 &lt;2.4.3*）- 修复了当贷项通知单更新不再导致数据库中出现重复产品预订更新的问题。
* **MDVA-38132**（*适用于 Adobe Commerce >=2.3.3&lt;2.4.3*）- 修复了以下问题：由于重定向错误过多&#x200B;*而*&#x200B;无法访问管理面板。
* **MDVA-38270** （*适用于 Adobe Commerce >=2.4.1 &lt;2.4.3*）- 修复了 GraphQL 中订单总计中缺少礼品卡信息的问题。

## 1.0.24 版 {#v1-0-24}

* **MDVA-37779** （*适用于 Adobe Commerce >=2.4.2 &lt;=2.4.4*）- 修复了当网站 ID 与商店 ID 不一致时通过 GraphQL 将可配置产品添加到购物车的问题。
...
* **MDVA-19640** (*Adobe Commerce >=2.3.0*) — 修复[!DNL Advanced Reporting]未显示任何数据的问题。
* **MDVA-11189** (*对于Adobe Commerce >=2.3.0 &lt;2.3.5*) — 修复了在导入CSV文件以更新产品Stock后，`cataloginventory_stock`表中的行被删除的问题。
* **MDVA-26639** (*Adobe Commerce >=2.3.3-p1 &lt;2.3.6*) — 修复了如果创建了新的订单确认电子邮件模板，订单邮件中缺少订单项的问题。
* **MDVA-15546** (*Adobe Commerce >=2.3.0*) — 修复了创建订单后，异常日志中显示&#x200B;*Column entity_id where子句不明确*&#x200B;错误的问题。
...
* **MDVA-33289**（*适用于 Adobe Commerce >=2.4.0*&lt;2.4.3）- 修复了原始 JavaScript 代码在结帐时在帐单地址 UI 中显示的问题（如果[!DNL Google Tag Manager]已启用）。
* **MDVA-35982** （*适用于 Adobe Commerce >=2.3.0 &lt;2.4.3*）- 修复了仅限于特定网站的管理员用户无法为在同一网站上下达的订单创建货件的问题。
* **MDVA-35155** （*适用于 Adobe Commerce >=2.3.0 &lt;2.3.6*）- 修复了在更改选项标题后无法购买捆绑产品的问题。
...
* **MDVA-31307** (*Adobe Commerce >=2.4.0 &lt;2.4.2*) — 修复了由于缓存块的动态CSP白名单存在问题，导致某些类别上出现&#x200B;*内存不足*&#x200B;错误的问题。

## 1.0.18 版 {#v1-0-18}

* **MDVA-32655**（*适用于 Adobe Commerce >=2.3.0*&lt;2.4.3）- *修复了在删除多个产品后，不正确的正在进行*&#x200B;消息状态为正确的&#x200B;*消费者完整*`quoteItemCleaner`消息。
* **MDVA-34102** （*适用于 Adobe Commerce >=2.3.0 &lt;2.4.3*）- 修复了“管理”区域中“产品网格”和“编辑产品”页面上已禁用产品的默认库存数量为零的问题。
* **MDVA-35286** （*适用于 Adobe Commerce >=2.4.0 &lt;2.4.2*）- 修复了以下问题：如果客户在购物车中捆绑了产品并从多个地址结帐切换到单页结帐，则会出现错误。
* **MDVA-35312** (*对于Adobe Commerce >=2.4.1-p1 &lt;2.4.2*) — 修复了空GraphQL请求时的响应代码500。
* **MDVA-34189** (*对于Adobe Commerce >=2.3.4 &lt;2.4.3*) — 修复了在加载“管理类别”页面时对[!DNL Visual Merchandiser]个查询执行的503个首字节超时。
* **MDVA-34695** (*对于Adobe Commerce >=2.3.0 &lt;2.4.1*) — 在删除类别后修复了负`children_count`。
...
* **MDVA-34023** (*对于Adobe Commerce >=2.3.0 &lt;2.4.3*) — 修复了错误&#x200B;*没有地址为ID的此类实体*&#x200B;在访客的浏览器上随机显示的问题。
* **MDVA-32759** (*适用于Adobe Commerce >=2.3.1 &lt;2.4.3，扩展为B2B*) — 修复了共享目录删除现有层定价的问题。
* **MDVA-33482**(适用于Adobe Commerce ^2.3.5 *的*) — 修复了针对部分发票生成贷项通知单导致对总订单计税而不是对该部分发票计税的问题。
* **MDVA-33393** (*Adobe Commerce >=2.3.0 &lt;2.4.2*) — 修复错误&#x200B;*提供的countryId不存在*。
...
* **MDVA-34100**(*对于Adobe Commerce >=2.3.4 &lt;2.4.2，具有B2B扩展名*) — 修复了无法从公司结构页面创建公司帐户的问题。
...
* **MDVA-24201**(*对于Adobe Commerce >=2.3.0 &lt;2.3.5*) — 修复了价格在手动重新索引之前无法反映计划的购物车价格规则的问题。
* **MDVA-32694** (*对于Adobe Commerce >=2.3.0 &lt;2.3.6 || >= 2.4.0 &lt;2.4.2*) — 修复了管理员用户无法将产品添加到可转让报价的问题（如果它与非默认商店相关）。
...
* **MDVA-30858**(*对于Adobe Commerce >=2.3.0 &lt;2.4.2*) — 修复了[!DNL PayPal]结算报告在&#x200B;**报告** > **销售** > **[!DNL PayPal]**&#x200B;结算下不可用的问题。
* **MCP-87** (*对于Adobe Commerce >=2.3.1 &lt;2.4.2*) — 改进了用于大型配置文件的类别产品和库存索引器的索引时间。
* **MDVA-33106** (*对于Adobe Commerce >=2.3.0 &lt;2.4.2*) — 修复了在执行cron `run`命令后删除重新计划的产品更改的问题。
* **MDVA-19391（适用于 Adobe Commerce >=2.3.0&lt;2.3.5 *）- 修复了由于表中的说明`catalog_category_entity_text`记录为 NULL 而引发错误的问题`analytics_collect_data`。***
...
* **MDVA-23426**(*Adobe Commerce >=2.3.3 &lt;2.3.5*) — 修复了Adobe Commerce发送的通知电子邮件中包含空白正文且内容被添加为附件的问题。
...
* **MDVA-32133** （*Adobe Commerce >=2.4.0 &lt;2.4.1*）- 修复了在某些情况下无法从页面生成器加载媒体库的问题。
* **MDVA-12304** （*适用于 Adobe Commerce >=2.3.0*）- 将 Cookie 的最大数量从 50 增加到 200。
* **MDVA-32632** （*Adobe Commerce >=2.3.2 &lt;2.3.5*）- 修复了订单显示在支付系统中，但不显示在 Adobe Commerce 中的问题。
...
* **MDVA-31363**(*适用于Adobe Commerce >=2.3.2 &lt;2.4.2*) — 修复了在使用&#x200B;*针对整个购物车的固定金额折扣*&#x200B;操作时，包含优惠券的购物车价格规则不通过GraphQL应用的问题。
* **MDVA-30889**(*对于Adobe Commerce >=2.3.0 &lt;2.4.2*) — 修复了使用虚拟和简单产品作为选项对捆绑包开具发票后出现错误的问题。
* **MDVA-31791** (*对于Adobe Commerce >=2.3.4 &lt;2.3.5*) — 在使用目标规则或链接的产品时提高产品页的性能。
* **MDVA-31168** (*对于Adobe Commerce >=2.3.0 &lt;2.4.2*) — 修复了产品导出CSV文件未显示以及存在内存分配错误的问题。
* **MDVA-32313** (*Adobe Commerce >=2.3.0 &lt;2.3.4*) — 修复了使用错误配置选项将可配置产品添加到愿望列表的问题。
* **MDVA-31759** (*Adobe Commerce >=2.3.0 &lt;2.4.2*) — 修复了在CSV导入期间未使用&#x200B;*下拉菜单*&#x200B;和&#x200B;*textarea*&#x200B;属性值更新产品的问题。
...
* **MDVA-28651** (*对于Adobe Commerce >=2.3.0 &lt;2.3.6 || >2.4.0（具有B2B扩展*） — 修复了通过REST API加载可转让报价时出现性能问题的问题。
...
* **MDVA-29148** (*对于Adobe Commerce >=2.3.0 &lt;2.4.2*) — 修复了通过API调用创建产品时的问题，如果在负载中未提供值，则`\Magento\Eav\Model\Entity\Attribute\Backend\ArrayBackend`（如Multiselect）类型的产品自定义属性不使用其默认值。
* **MDVA-30837** (*对于Adobe Commerce >=2.3.1 &lt;2.3.5*) — 在免费运送方法配置中添加了配置设置&#x200B;*将税包含到金额：是/否*。 当&#x200B;*含税金额*&#x200B;设置为&#x200B;*是*&#x200B;时，最小订单金额的计算方式为小计+税费。 当&#x200B;*含税金额*&#x200B;设置为&#x200B;*否*&#x200B;时，最小订单金额计算为小计
* **MDVA-25028** (*对于Adobe Commerce >=2.3.2 &lt;2.3.3 || >=2.3.5 &lt;2.3.6*) — 修复了在触发欺诈筛选器时，使用[!DNL PayPal Payflow Pro]下达的订单未设置为“涉嫌欺诈”状态的问题。
...
* **MDVA-31021** (*对于Adobe Commerce >=2.3.0 &lt;2.4.2*) — 修复`module-catalog-import-export/Model/Import/Product/Option.php`中存在的性能问题。 如果`catalog_product_option`表中的记录数超过100k，则使用单个产品的新CSV进行验证所花的时间少于10秒。
* **MDVA-31007** (*适用于Adobe Commerce >=2.4.0 &lt;2.4.1*) — 修复了自定义地址属性未正确显示在“我的帐户”区域和后端的订单详细信息页面中的问题。
* **MDVA-29389**（*适用于 Adobe Commerce >=2.3.0&lt;2.4.2*）- 修复了高级报告的问题，其中 cronjob 显示`analytics_collect_data`：*必须在主机参数中配置端口（如 localhost：3306）。*
* **MDVA-31343**（*适用于 Adobe Commerce >=2.3.4*&lt;2.3.6）- 修复了在安排类别时已删除的正文类`page-layout-category-full-width`的问题。
* **MDVA-30945**（*适用于 Adobe Commerce >=2.3.0*&lt;2.4.2）- 修复了更新购物车`Call to a member function getValue() on null in module-configurable-product CartItemProcessor.php`时收到致命错误消息的问题。
...
* **MDVA-30977** （*适用于 Adobe Commerce >=2.3.4 &lt;=2.3.5-p2*）- 修复了重新编制索引后类别中随机产品缺失的问题。
...
* **MDVA-28661**(*对于Adobe Commerce >=2.3.0 &lt;2.4.2，具有B2B扩展名*) — 修复了在更改公司管理员后，公司用户公司帐户部分引发错误的问题。

## 版本1.0.4 {#v1-0-4}

* **MDVA-30195**(*对于Adobe Commerce 2.3.1 - 2.3.4-p2*) — 修复了如果数据库名称太长，则cron作业失败，导致在前端不更新类别的问题。
* **MDVA-30106**(*对于Adobe Commerce ^2.3.0*) — 修复了在结账期间付款未加载的问题，其中&#x200B;*无法在JS控制台中读取null的属性“length”*&#x200B;错误。
* **MDVA-28656** (*对于Adobe Commerce >=2.3.1 &lt;2.3.6 || >=2.4.0 &lt;2.4.2*) — 修复了如果下单时不需要付款信息（例如，100%折扣），并且为订单创建了发票，则订单状态将更改为&#x200B;*已关闭*&#x200B;而不是“完成”的问题。
...
* **MDVA-30444** (*Adobe Commerce >=2.3.0 &lt;2.4.1*) — 修复了使用GraphQL下达的订单没有发送确认电子邮件的问题。
...
* **MDVA-29965** (*对于Adobe Commerce >=2.3.3 &lt;2.4.0*) — 修复了客户在将产品添加到购物车时收到“表单密钥无效”错误的问题。
* **MDVA-30008**(*对于Adobe Commerce >=2.3.0 &lt;2.4.2*) — 修复了B2B问题，即无法通过Admin API为公共目录中的产品下订单。
