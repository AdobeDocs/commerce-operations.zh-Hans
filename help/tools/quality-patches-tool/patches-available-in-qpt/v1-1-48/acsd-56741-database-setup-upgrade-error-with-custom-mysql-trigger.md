...
由于数据库中的自定义MySQL触发器与索引和&#x200B;*无关，因此在*&#x200B;期间出现错误消息`setup:upgrade`尝试访问类型为null[!DNL MView]的值上的数组偏移。
