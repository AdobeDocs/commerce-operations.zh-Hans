...
# ACSD-67089：在设置`page_size`时，`inventory/export-stock-salable-qty` API返回错误的`total_count`
...
设置`page_size`时，`inventory/export-stock-salable-qty` API返回错误的`total_count`值。
...
当`page_size`设置为&#x200B;*5*&#x200B;时，`inventory/export-stock-salable-qty` API返回错误的&#x200B;*5*&#x200B;的`total_count`，而不是实际的&#x200B;*22*&#x200B;总数。
