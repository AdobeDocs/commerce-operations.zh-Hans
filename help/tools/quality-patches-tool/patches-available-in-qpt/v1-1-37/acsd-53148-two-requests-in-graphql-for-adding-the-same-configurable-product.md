...
# ACSD-53148：GraphQL中用于添加同一可配置产品的两个并行请求

ACSD-53148修补程序修复了以下问题：GraphQL中将同一可配置产品添加到购物车的两个并行请求在购物车上产生了具有相同产品SKU的两个单独项目。 安装[[!DNL Quality Patches Tool (QPT)]](/help/tools/quality-patches-tool/quality-patches-tool-to-self-serve-quality-patches.md) 1.1.37时，此修补程序可用。 修补程序ID为ACSD-53148。 请注意，该问题计划在Adobe Commerce 2.4.7中修复。
...
在GraphQL中，对将相同的可配置产品添加到购物车发出两个并行请求，导致购物车上有两个单独的具有相同产品SKU的项目。

<u>重现步骤</u>：

1. 创建带有SKU *Test*&#x200B;的可配置产品以及带有SKU *Test-A*&#x200B;的简单产品。
1. 通过GraphQL创建新的空购物车（使用您自己的[!DNL URL]更改位置）：
...
1. 运行两个并发请求以将同一产品添加到购物车（更改两个请求的&#x200B;*cartID*&#x200B;和位置）：
