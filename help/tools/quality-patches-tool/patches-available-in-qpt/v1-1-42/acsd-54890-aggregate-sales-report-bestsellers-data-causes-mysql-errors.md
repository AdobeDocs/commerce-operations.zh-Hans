...
由于`aggregate_sales_report_bestsellers_data`空间不足，**[!DNL MySQL]**&#x200B;导致`/tmpdisk`错误。

<u>重现步骤</u>：

当`aggregate_sales_report_bestsellers_data`表具有大量记录（如数千万条记录）时，执行`sales_bestsellers_aggregated_daily` cron作业。
