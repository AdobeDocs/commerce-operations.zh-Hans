...
许多问题是由于MySQL在分配给MySQL的存储中耗尽存储所引起的(`datadir` MySQL配置设置，默认值为 `/data/mysql`)或 `tmpdir` 空间不足。 默认 `tmpdir` （MySQL设置）为 `/tmp`. 此 **[!UICONTROL MySQL% free storage by node]** 框架查看 `/, /tmp` （如果定义为单独的装载）和 `/data/mysql` 可用存储的百分比。 从MySQL版本5.7（MariaDB版本10.2）开始，未压缩 `tmp` 表格会写入 `tmp` 表空间 `/data/mysql` 目录(ibtmp1)。 默认情况下，此文件将自动展开，无限制。 由于它是一个表空间，因此不会减小其大小，在MySQL重新启动时将重置为12MB。
...
此 **[!UICONTROL Galera Number of Nodes in cluster]** 框架显示来自MySQL日志的信息。 当节点加入并离开群集时，将仅显示选定时间范围的消息。 如果节点在时间范围之前离开群集，则该时间范围内将不存在消息。 如果您怀疑数据库可能缺少节点，请将时间范围扩展到更长的时间段，以查看是否可以看到其他信息。 如果时间段内有信息指示小于中的所有节点， [!DNL Galera] 群集时，请展开时间范围以查看是否可以确定节点何时离开群集。
...
此 **[!UICONTROL Galera Log by Host]** 帧与 **[!UICONTROL Galera log]** 框架，但会按节点划分，以帮助进行故障排除。

## [!UICONTROL Database performance]

![数据库性能](../../assets/tools/observation-for-adobe-commerce/mysql-tab-8.jpg)

此 **[!UICONTROL Database performance]** 框架显示特定请求期间的数据库性能。 您可以通过单击图表下方的彩色图标中的各个指标来查看它们。 中调用的许多指标 [使用New Relic监视MySQL数据库性能](https://newrelic.com/blog/how-to-relic/how-to-monitor-mysql) 在此帧中找到。
...
此 **[!UICONTROL Transaction Database Call Count]** 框架显示每个事务Facet进行的数据库调用的数量。 这似乎是着重于行，而不是语句。

## [!UICONTROL Cron_schedule table updates]

![Cron_schedule表更新](../../assets/tools/observation-for-adobe-commerce/mysql-tab-10.jpg)

此 **[!UICONTROL Cron_schedule table updates]** frame显示所选时段内cron_schedule表数据库更新的最长持续时间。

## [!UICONTROL Slow Query Traces]

![查询跟踪缓慢](../../assets/tools/observation-for-adobe-commerce/mysql-tab-11.jpg)

此 **[!UICONTROL Slow Query Traces]** 框架显示存在慢查询跟踪的表和请求类型。 为耗时超过五秒的查询事务创建了慢速查询跟踪。 更新查询对于此框架非常重要。 如果表正在由更新 `UPDATE`， `DELETE`、和 `INSERT` 语句时，它们可能会将表锁定一段时间。

平均 `SELECT` 如果与FOR UPDATE一起使用，语句可能会锁定行。
...
此 **[!UICONTROL Cron table change]** frame查找“无法获取cron作业的锁定：”错误消息，以及特定的PHP内存错误和涉及 `cron_schedule` 表格。 如果 `cron_schedule` 表被锁定(例如，被 `DELETE` 查询正在针对它运行)，它将阻止其他cron运行。

## [!UICONTROL Deadlocks]

![死锁](../../assets/tools/observation-for-adobe-commerce/mysql-tab-14.jpg)

此 **[!UICONTROL Deadlocks]** frame查看从MySQL日志中解析的以下字符串：

* “%PHP严重错误：允许的内存大小为%”)，作为php_mem_error
* &#39;%get lock；尝试重新启动事务，查询为：DELETE自\&#39;cron_schedule%&#39;)为cron_sched_lock_del
* &#39;% lock for cron job： indexer_reindex_all_invalid%&#39;)作为&#39;lock_indexer_reindex_all_invalid%&#39;
...
* &#39;% lock for cron job： indexer_update_all_views%&#39;)作为&#39;lock_indexer_update_all_views&#39;
...
* “% lock for cron job： sales_grid_order_shipment_async_insert%”)作为“lock_sales_grid_order_shipment_async_insert”
...
* “% lock for cron job： staging_synchronize_entities_period%”)作为“lock_staging_synchronize_entities_period”
* &#39;% lock for cron job： indexer_clean_all_changelogs%&#39;)作为&#39;lock_indexer_clean_all_changelogs&#39;
* 用于cron作业的“% lock： magento_targetrule_index_reindex%”)作为“lock_magento_targetrule_index_reindex”
* “% lock for cron job： newsletter_send_all%”)作为“lock_newsletter_send_all”
* “% lock for cron job： newsletter_send_all%”)作为“lock_newsletter_send_all”
* “% lock for cron job： sales_send_order_emails%”)，作为“lock_sales_send_order_emails”
...
* &#39;%锁定cron作业：sales_send_order_invoice_emails%&#39;)，作为&#39;lock_sales_send_order_invoice_emails&#39;
* &#39;%锁定cron作业：sales_send_order_invoice_emails%&#39;)，作为&#39;lock_sales_send_order_invoice_emails&#39;
* &#39;% lock for cron job： captcha_delete_expired_images%&#39;)作为&#39;lock_captcha_delete_expired_images&#39;
* “% lock for cron job： magento_newrelicreporting_cron%”)作为“lock_magento_newrelicreporting_cron”
* “% lock for cron job： outded_authentication_failures_cleanup%”)作为“lock_outded_authentication_failures_cleanup”
* “% lock for cron job： send_notification%”)作为“lock_send_notification”
* &#39;%锁定cron作业：magento_giftcardaccount_generation_codes_pool%&#39;)为&#39;lock_magento_giftcardaccount_generation_codes_pool&#39;
* 用于cron作业的“%锁定：catalog_product_frontend_actions_flush%”)，作为“lock_catalog_product_frontend_actions_flush”
* “% lock for cron job： mysqlmq_clean_messages%”)作为“mysqlmq_clean_messages”
* “% lock for cron job： catalog_product_attribute_value_synchronize%”)作为“lock_catalog_product_attribute_value_synchronize”
* “% lock for cron job： ddg_automation_importer%”)作为“lock_ddg_automation_importer”
...
* “% lock for cron job： catalog_product_outdoated_price_values_cleanup%”)作为“lock_catalog_product_outdoated_price_values_cleanup”
...
* &#39;% lock for cron job： get_amazon_capture_updates%&#39;)作为&#39;lock_get_amazon_capture_updates&#39;
* &#39;% lock for cron job： get_amazon_authorization_updates%&#39;)作为&#39;lock_send_get_amazon_authorization_updates&#39;
* “% lock for cron job： temando_process_platform_events%”)作为“lock_temando_process_platform_events”
* “% lock for cron job： ddg_automation_status%”)作为“lock_ddg_automation_status”
* “% lock for cron job： ddg_automation_status%”)作为“lock_ddg_automation_status”
...
* &#39;% lock for cron job： magento_reward_balance_warning_notification%&#39;)作为&#39;lock_magento_reward_balance_warning_notification&#39;
* “% lock for cron job： analytics_update%”)作为“lock_analytics_update”
* “% lock for cron job： messagequeue_clean_outdated_locks%”)作为“lock_messagequue_clean_outdated_locks”
* “% lock for cron job： messagequeue_clean_outdated_locks%”)作为“lock_messagequue_clean_outdated_locks”
* “% lock for cron job： staging_apply_version%”)作为“lock_staging_apply_version”
...
* “% lock for cron job： ddg_automation_campaign%”)作为“lock_ddg_automation_campaign”
* “% lock for cron job： visitor_clean%”)作为“lock_visitor_clean”
* “% lock for cron job： scconnector_verify_website%”)作为“lock_scconnector_verify_website”
* “% lock for cron job： ddg_automation_email_templates%”)，作为“lock_ddg_automation_email_templates”
* “% lock for cron job： aggregate_sales_report_order_data%”)作为“lock_aggregate_sales_report_order_data”
...
此 **[!UICONTROL DB Statistics]** 帧显示每秒的删除、写入、读取行、更新和查询缓慢。
...
* “%42S22%”)作为“sql_42S22”
* “%InnoDB： Error （重复键）%”)，因为“innodb_dup_key”来自日志时间序列

## [!UICONTROL DB Error Table]

![数据库错误表](../../assets/tools/observation-for-adobe-commerce/mysql-tab-18.jpg)

此 **[!UICONTROL DB Error Table]** 框架显示的信息与 **[!UICONTROL Database Errors]** 框架，但您可以按节点和表格格式查看它。 参见 [MariaDB错误代码](https://mariadb.com/kb/en/mariadb-error-codes/) 了解更多信息。

## [!UICONTROL Database Traces]

![数据库跟踪](../../assets/tools/observation-for-adobe-commerce/mysql-tab-19.jpg)

此 **[!UICONTROL Database Traces]** frame按类型显示选定时间轴上的数据库跟踪。

## [!UICONTROL Database processes]

![数据库进程](../../assets/tools/observation-for-adobe-commerce/mysql-tab-20.jpg)

此 **[!UICONTROL Database processes]** 框架显示数据库进程、环境和节点标识符。
...
此 **[!UICONTROL MySQL Running and Sleeping Threads by environment]** 框架显示与数据库之间的活动连接和休眠连接。 如果存在到数据库的连接，其中慢速查询进入休眠状态，则存在休眠连接。 休眠连接可以是被锁定的行或表阻止的数据库查询。 这些休眠连接还保持PHP工作程序连接。

## [!UICONTROL MySQL mem used by node]

![节点使用的MySQL记忆体](../../assets/tools/observation-for-adobe-commerce/mysql-tab-23.jpg)

此 **[!UICONTROL MySQL mem used by node]** frame显示MySQL内存的节点使用情况。 在较大的网站上，此框架可以是连续条，其中使用的内存为GB数。
