...
| **命令** | **可用选项** |
|----------------|-----------------|
| `upgrade:check` | <ul><li>—help:返回所有可用选项。</li><li>—current-version:当前Adobe Commerce版本。 此参数是必需的，必须始终使用。</li><li> — 最小问题级别：您可以根据最小问题级别过滤问题（默认值为“警告”）。</li><li>—ignore-current-version-compatibility-issues（或 — i）：如果您不想在报表中包含当前版本的严重问题、错误和警告。</li><li> — 即将发布的版本（或 — c）：定位特定的Adobe Commerce版本。 如果忽略，则将使用最新的可用版本。</li></ul> |
...
可以运行 `--help` 作为选项。 它返回 `--help` 指定命令的选项。
