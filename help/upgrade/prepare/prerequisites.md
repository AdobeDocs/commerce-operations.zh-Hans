...
确保更新了环境中的所有系统要求和依赖项。 查看目标PHP版本的[PHP迁移附录](https://www.php.net/manual/en/appendices.php)和[所需的PHP设置](../../installation/prerequisites/php-settings.md#php-settings)。

>[!NOTE]
>
>对于云基础架构Pro项目上的Adobe Commerce，您必须创建[支持](https://experienceleague.adobe.com/zh-hans/docs/support-resources/adobe-support-tools-guide/adobe-commerce-support/adobe-commerce-help-center-user-guide#submit-ticket)票证，以在暂存环境和生产环境中安装或更新服务。 指示所需的服务更改，并在票证中包含更新的`.magento.app.yaml`和`services.yaml`文件以及PHP版本。 Cloud Infrastructure团队更新项目最多可能需要48小时。 请参阅[支持的软件和服务](https://experienceleague.adobe.com/zh-hans/docs/commerce-on-cloud/user-guide/architecture/cloud-architecture#supported-software-and-services)。
...
* [正在安装Elasticsearch](https://www.elastic.co/docs/deploy-manage/deploy/self-managed/installing-elasticsearch)
...
有关在部署到生产环境之前备份数据、检测潜在迁移问题和测试升级的完整说明，请参阅[升级Elasticsearch](https://www.elastic.co/docs/deploy-manage/upgrade/deployment-or-cluster)。 根据您当前的Elasticsearch版本，可能需要也可能不需要完全重新启动群集。

Elasticsearch需要Java开发工具包(JDK) 1.8或更高版本。 请参阅[安装Java软件开发工具包(JDK)](../../installation/prerequisites/search-engine/overview.md#install-the-java-software-development-kit)以检查安装的JDK版本。
...
OpenSearch需要JDK 1.8或更高版本。 请参阅[安装Java软件开发工具包(JDK)](../../installation/prerequisites/search-engine/overview.md#install-the-java-software-development-kit)以检查安装的JDK版本。
...
>[!NOTE]
>
>这些步骤仅适用于Adobe Commerce 2.4.6和2.4.7。Adobe Commerce 2.4.8及更高版本不再支持Elasticsearch；请改用OpenSearch。

1. 将Elasticsearch 7.x服务器升级到8.x，并确保已启动并正在运行。 请参阅[Elasticsearch文档](https://www.elastic.co/docs/deploy-manage/deploy/self-managed/installing-elasticsearch)。
...
1. 将Elasticsearch 8.x服务器降级为7.x，并确保其已启动并正在运行。 请参阅[Elasticsearch文档](https://www.elastic.co/docs/deploy-manage/deploy/self-managed/installing-elasticsearch)。
...
Adobe建议将打开的文件[ulimit](https://www.gnu.org/software/bash/manual/html_node/Bash-Builtins.html#index-ulimit)设置为`65536`或更多的值，但如有必要，您可以使用更大的值。 您可以在命令行上设置限制，也可以将其设置为用户shell的永久设置。
