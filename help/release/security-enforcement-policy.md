...
badgePaas: label="仅限Adobe Commerce on Cloud" type="Informative" url="https://experienceleague.adobe.com/zh-hans/docs/commerce/user-guides/product-solutions" tooltip="仅适用于云项目上的Adobe Commerce 。"
hide: true
...
Adobe公司安全策略要求，适用于Adobe Commerce on Cloud的所有Adobe托管环境都可以在安全且兼容的软件上运行。

1. 所有第三方软件依赖项(PHP、MariaDB、Elasticsearch/OpenSearch、Redis、RabbitMQ)的支持版本

1. Cloud上的Adobe Commerce（版本2.4.8、2.4.9或最新版本）
...
在下表中，找到您的Adobe Commerce on Cloud版本，以了解您需要执行的操作。

**表1：版本**&#x200B;必需的操作和截止日期

| **您的版本** | **[操作1：<br>升级第三方软件依赖项](#action-1-upgrade-third-party-software-dependencies)**&#x200B; | &#x200B;** 操作2：<br>[升级或迁移您的Adobe Commerce版本](#action-2-upgrade-to-a-supported-adobe-commerce-version)** |
| --- | --- | --- |
| 2.4.4或2.4.5 | 要求在2026年10月30日之前采取行动。 | 要求在2027年6月1日之前采取行动 |
| 2.4.6或2.4.7 | 要求在2026年10月30日或2027年5月31日之前执行操作，具体取决于软件。 | 要求在2028年6月1日之前采取行动 |
| 2.4.8或2.4.9 | 要求在2026年10月30日或2027年5月31日之前执行操作，具体取决于软件。 | 此时不需要执行任何操作 |

## 谁也不需要采取行动

本通知不适用于：

* 使用[!DNL Adobe Commerce as a Cloud Service]的客户
* 在所有环境中使用支持软件依赖项的Cloud版本2.4.8或2.4.9上的Adobe Commerce的客户

### 验证当前版本

您需要电子商务管理员的帮助来执行以下步骤，以检查您在云环境的每个Adobe Commerce中运行的版本。

#### 步骤1：检查云版本的Adobe Commerce

1. 登录到Adobe Commerce管理面板。

   当前版本应显示在任何Admin页面的右下角。

1. 如果版本未显示在管理员中，请使用[Adobe Commerce命令行工具](https://experienceleague.adobe.com/zh-hans/docs/commerce-operations/configuration-guide/cli/config-cli){target="_blank"}运行version命令：
...
已超过供应商支持终止的软件不再接收安全修补程序，这意味着无法修复该软件中的已知安全问题。 此外，根据[Adobe生命周期策略](https://experienceleague.adobe.com/zh-hans/docs/commerce-operations/release/planning/lifecycle-policy)：
...
>[!WARNING]
>
>如果您的环境在[所需操作和截止日期表](#determine-your-required-actions)中详述的截止日期之前未满足要求，则Adobe将被强制挂起到受影响环境的入站流量。 您的电子商务店面将下线，不再为购物者服务。

## 每个操作需要什么

### 操作1：升级第三方软件依赖项

根据软件的不同，所有不支持的软件依赖项必须根据下表共享的时间表进行升级。 您可以在[Cloud Console](https://console.adobecommerce.com/)中查看环境。 要验证在每个环境中运行的依赖项版本，请参阅[检查软件依赖项版本](#check-software-dependency-versions)。 软件依赖项升级适用于Cloud版本2.4.4到2.4.9上的所有Adobe Commerce。
...
### 操作2：升级到支持的Adobe Commerce版本

如果您需要升级Adobe Commerce on Cloud版本，则有两个选项：

1. [升级到支持的云版Adobe Commerce](#upgrade-to-adobe-commerce-on-cloud-version-249)
1. [迁移到Adobe Commerce as a Cloud Service （SaaS平台）](#migrate-to-adobe-commerce-as-a-cloud-service)

无论您选择哪个选项，当前版本的强制实施日期均适用。

**表3：在Cloud版本**&#x200B;上升级到支持的Adobe Commerce的准则和截止日期
...
要确定哪个选项适合您的需求，请参阅下表对Cloud版本2.4.9上的Adobe Commerce与Adobe Commerce as a Cloud Service的比较。

**表4：云上的Adobe Commerce与Adobe Commerce as a Cloud Service**

| | Cloud上的Adobe Commerce版本2.4.9 | Adobe Commerce as a Cloud Service |
| --- | --- | --- |
| 内容 | 最新的Adobe Commerce版本，包含全面的安全保护、质量修复和平台依赖关系更新。 | Adobe完全托管的commerce平台，为持续创新而构建，无需升级开销。 [了解详情](https://experienceleague.adobe.com/zh-hans/docs/commerce/cloud-service/overview)。 |
...
如果环境在[中的实施日期之前未满足这些要求，则确定您所需的操作](#determine-your-required-actions)，Adobe将采取适当操作。 这包括暂停流向受影响的基础结构的流量，因此，您的电子商务店面将脱机。
...
### 升级到Adobe Commerce on Cloud版本2.4.9
...
Adobe提供了一些工具，可降低迁移到Adobe Commerce as a Cloud Service的成本和时间。 这些工具仅适用于迁移。 它们不用于Adobe Commerce on Cloud上的版本升级。 有关完整的迁移指南（包括迁移路径和阶段），请参阅[迁移概述](https://experienceleague.adobe.com/zh-hans/docs/commerce/cloud-service/migration/overview)。
...
* **店面和扩展迁移：** Adobe的[AI辅助迁移和开发人员工具](https://developer.adobe.com/commerce/extensibility/developer-agent/)，包括[!DNL Adobe Developer App Builder]和[!DNL Commerce Storefront powered by Edge Delivery Services]，有助于加快店面现代化和扩展重新平台。

如果您有任何问题，请联系您的客户团队、解决方案客户经理、续订专家或联系[支持服务](https://experienceleague.adobe.com/zh-hans/docs/support-resources/adobe-support-tools-guide/adobe-commerce-support/adobe-commerce-help-center-user-guide?lang=en#submit-ticket)。
