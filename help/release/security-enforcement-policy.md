...
网络安全格局正在发生根本性变化，企业已建立的防御机制需要迅速演变。 安全对于电子商务企业至关重要，因为在线交易要求他们处理敏感的个人和业务数据，在出现违规时使他们面临财务和身份风险。 PaaS电子商务环境在Adobe和我们的客户之间采用了一种共享的安全责任模型，客户负责维护应用层依赖项、与第三方软件的集成以及部署管道。

在Adobe，我们主动应对不断演变的风险，并确保按照最高安全标准在云上设置Adobe Commerce客户。 这包括：

* 每月进行单独的安全修复，以针对关键漏洞提供更快、更可预测的保护
* 年度补丁发布，提供长期支持
* 通过3年支持窗口简化每个版本的生命周期策略

虽然Adobe会采取必要步骤保护客户的安全，但Adobe Commerce on Cloud的[共享责任模型](../security-and-compliance/shared-responsibility.md)要求我们的客户始终使用受支持的Adobe Commerce on Cloud版本和第三方软件、应用应用程序修补程序、审核第三方扩展和安全自定义代码。 已超过供应商支持终止的软件不再接收安全修补程序，软件中的安全问题得不到解决。 继续在不支持的软件上运行电子商务店面会产生真实且不断增加的安全风险。

本页概述了Adobe Commerce（版本2.4.4到2.4.9）上所有客户为确保其电子商务环境的安全而需要采取的操作、实施日期以及未满足安全要求时的预期情况。

## 维护安全、合规的环境所需的操作

要确保电子商务环境的安全性和合规性，Adobe Commerce on Cloud上的所有客户都需要使用：

1. 支持所有第三方软件依赖项的版本：PHP、MariaDB、Elasticsearch/OpenSearch、Redis和RabbitMQ

1. 云上的Adobe Commerce的安全且受支持的版本

遵循以下准则以检查您是否需要采取操作来保护云环境上的Adobe Commerce。 如果在表1中列出的截止日期前未满足安全要求的环境，将会暂停入站流量，从而使店面离线。 如果您担心难以在截止日期前完成工作，需要延长有效期，请联系您的客户团队或Adobe [支持](https://experienceleague.adobe.com/zh-hans/docs/support-resources/adobe-support-tools-guide/adobe-commerce-support/adobe-commerce-help-center-user-guide#support-case)。
...
1. 登录到[云控制台](https://experienceleague.adobe.com/zh-hans/docs/commerce-on-cloud/start/cloud-console)。
2. 打开相关项目，然后选择要查看的环境。
3. 在`.magento/services.yaml`文件中检查该环境的服务配置，该文件定义了Adobe Commerce on Cloud支持的服务名称和版本。

有关详细说明，请参阅[配置服务](https://experienceleague.adobe.com/zh-hans/docs/commerce-on-cloud/user-guide/configure/service/services-yaml)。

所有不支持的软件依赖项都必须升级到下表2中时间表列出的版本。
...
| MariaDB/Galera | 大于10.5但小于10.11 | 版本10.11或更高版本 | 2027年5月31日 |
...
* **兼容性：**&#x200B;这些更改大多是同一软件中的版本升级，风险较低。 以下变化值得密切关注：
  ...
  * 从&#x200B;**PHP 8.1升级到8.2**&#x200B;可能会在自定义代码和第三方扩展中显示弃用警告。

如果您使用第三方扩展，请与供应商确认其当前版本支持您的目标软件版本。 如果您与解决方案集成商合作，请他们在升级计划、测试和验证的早期阶段参与其中。

### 操作2：检查Commerce on Cloud版本并升级到支持的版本

检查您的环境运行的云上的Adobe Commerce版本。 如果任何环境不在受支持的版本上，则可以升级到版本2.4.9或最新受支持的版本，或迁移到[!DNL Adobe Commerce as a Cloud Service]。

#### 步骤1：检查您的Adobe Commerce on Cloud版本和所需操作

1. 登录到您的Adobe Commerce管理面板。

   当前版本将显示在任何Admin页面的右下角。

1. 如果从“管理”面板中隐藏了版本：

   * 连接到[远程环境](https://experienceleague.adobe.com/zh-hans/docs/commerce-on-cloud/user-guide/develop/secure-connections#connect-to-a-remote-environment)。
   * 使用Adobe Commerce [命令行工具](../configuration/cli/config-cli.md)检查版本。

     ```shell
     bin/magento --version
     ```

在下表中，查看您的Adobe Commerce版本所需的操作。

**表3： Adobe Commerce on Cloud版本升级要求**

| 云上的Adobe Commerce的当前版本 | 必需操作 | 截止日期 |
|---|---|---|
| 版本2.4.4或2.4.5 | 升级到Cloud上的Adobe Commerce版本2.4.9（或最新版本）或迁移到[!DNL Adobe Commerce as a Cloud Service]。<br>原因： v2.4.4和2.4.5仅在2027年5月31日之前接收核心应用程序的有限且独立的安全修复 — 这不包括质量修复、对应用程序依赖项（例如PHP）的兼容性支持或平台依赖项更新。 请参阅Adobe的[生命周期策略](https://experienceleague.adobe.com/zh-hans/docs/commerce-operations/release/planning/lifecycle-policy)。 | 2027年6月1日 |
| 版本2.4.6或2.4.7 | 升级到Cloud上的Adobe Commerce版本2.4.9（或最新版本）或迁移到[!DNL Adobe Commerce as a Cloud Service]。<br>原因：版本2.4.6在2027年8月30日之前获得扩展支持，并且在2028年5月31日之前仅获得核心应用程序的有限且独立的安全修复。 版本2.4.7在2027年5月31日之前获得标准支持，并在2028年5月31日之前扩展支持。 请参阅Adobe的[生命周期策略](https://experienceleague.adobe.com/zh-hans/docs/commerce-operations/release/planning/lifecycle-policy)。 | 2028年6月1日 |
| 版本2.4.8或2.4.9 | 无需Adobe Commerce on Cloud版本升级操作。 操作1中的第三方软件依赖项截止日期仍然适用。<br>原因：未设置截止日期。 | 当前非必需 |
...
Adobe将继续致力于帮助您采取必要步骤，以升级到云和第三方软件上的Adobe Commerce受支持版本。

如果环境在上面共享的实施日期之前未满足安全要求，Adobe将被强制采取适当的措施来保证较大安装群的安全性。 这包括暂停流向受影响的基础结构的流量，因此，您的电子商务店面将脱机。
...
* **升级兼容性报告：** Adobe提供了一个详细报告，用于准确识别升级到Adobe Commerce版本2.4.9所需的内容，包括成本范围。 [生成升级兼容性报告](https://supportinsights.adobe.com/commerce/tab/main)。
...
如有疑问，请与帐户团队联系或联系[支持服务](https://experienceleague.adobe.com/zh-hans/docs/support-resources/adobe-support-tools-guide/adobe-commerce-support/adobe-commerce-help-center-user-guide?lang=en#submit-ticket)。
