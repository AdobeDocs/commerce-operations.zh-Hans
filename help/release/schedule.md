...
- **安全修补程序** — 至少每年为所有[受支持](lifecycle-policy.md)发行行发布[个安全修补程序](versioning-policy.md#security-patch-release)。 这些修补程序包括所有以前发布的安全性、合规性和质量修补程序。  如有必要，Adobe可能会发布其他安全修补程序，但不保证一定会发布。
...
>[!NOTE]
>
> 有关每个次要、补丁和安全版本的发行日期以及常规支持结束日期，请参阅[已发行版本](/help/release/versions.md)。
