...
- **安全修补程序** — 至少每年为所有[受支持](versioning-policy.md#security-patch-release)发行行发布[个安全修补程序](lifecycle-policy.md)。 这些修补程序包括所有以前发布的独立安全修补程序。 如有必要，Adobe可能会发布其他安全修补程序，但不保证一定会发布。

- **Patch** — 每年（5月）发布适用于Adobe Commerce 2.4.x LTS版本行的完整[修补程序](versioning-policy.md#patch-release)（3年支持期）。

- **Beta修补程序**—Adobe Commerce 2.4.x LTS版本线的两个[Beta版修补程序](versioning-policy.md#beta-patch-release)每年发布两次。
...
- [Adobe安全公告与建议](https://helpx.adobe.com/cn/security/security-bulletin.html#magento)
- 电子邮件
- 产品内警报

>[!NOTE]
>
> 有关每个次要、补丁和安全版本的发行日期以及常规支持结束日期，请参阅[已发行版本](https://experienceleague.adobe.com/zh-hans/docs/commerce-operations/release/versions)。
