...
* 现在，在将产品添加到购物车时权限不足时，当`addProductsToCart`突变失败时，Magento Open Source会返回信息性消息。
...
* 现在，当没有与客户地址关联的增值税ID时，自动客户组分配将起作用。 如果地址不含增值税ID，则被视为属于非增值税注册客户，而Magento Open Source会将该客户组更改为非增值税组。 以前，启用&#x200B;**[!UICONTROL Enable Automatic Assignment to Customer Group]**&#x200B;后，当从客户帐户中删除VAT ID时，客户帐户不会自动分配给`customerGroup`。
