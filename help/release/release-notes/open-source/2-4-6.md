...
PHP 7.4支持已从此版本中删除。 无法在PHP 7.4上运行Magento Open Source2.4.6。
...
* Magento Open Source现在，当 `addProductsToCart` 将产品添加到购物车时权限不足，无法进行变异。
...
* Magento Open Source不再错误地基于类别路径假定一个类别是另一个类别的父类别。 以前，当两个类别相邻，并且一个类别ID以另一个类别的类别ID开头时，Adobe Commerce会假定这些类别具有相同的父类别。 [GitHub-32464](https://github.com/magento/magento2/issues/32464)
