...
Magento Open Source2.4.6引入了对PHP 8.2的支持。PHP 8.1仍完全受支持，但已删除对PHP 7.4的支持。 它包括显着的性能和可扩展性增强。 GraphQL批量购物车操作和类别树渲染响应操作已得到优化。
...
尽管这些功能的代码与Magento Open Source核心代码的发布捆绑在一起，但其中一些项目也是独立发布的。 这些项目的错误修复会在特定于项目的单独发行信息中进行记录，该信息可在每个项目的文档中找到。
...
请参阅 [Adobe安全公告](https://helpx.adobe.com/security/products/magento/apsb23-17.html) 以了解这些修复问题的最新讨论。
...
* **新的系统配置，用于在管理员用户更改其电子邮件时要求确认电子邮件**. 为了减少站点漏洞，添加了新的系统配置设置（默认情况下处于禁用状态），以要求在管理员用户更改其电子邮件时进行电子邮件确认： **[!UICONTROL Stores]** > **[!UICONTROL Settings:Configuration]** > **[!UICONTROL Customers]** > **[!UICONTROL Customer Configuration]** > **[!UICONTROL Account Information Options]** > **[!UICONTROL Require email confirmation if an email has been changed]**. 启用此选项后，每次管理员用户更改其电子邮件地址时，应用程序都会请求电子邮件确认。 以下模块受此更改的影响： `Magento_Customer`. <!-- AC-3064 -->

### 平台增强功能

Magento Open Source2.4.6引入了对PHP 8.2的支持。PHP 8.1仍完全受支持。 <!-- AC-6598 -->

此版本中移除了对PHP 7.4的支持。 不能在PHP 7.4上运行Magento Open Source2.4.6。

Magento Open Source现在支持：

* 编辑器2.2.x。已删除编辑器1.x。 <!-- AC-2839 -->

* Redis 7.0.x。尽管与Redis 6.2的兼容性仍然保持不变，但我们建议将此版本与Redis 7.0.x一起部署，因为Redis 6.2的生命周期预计将在2024年终止。 <!-- AC-6606 -->

* 现在支持将OpenSearch作为本地Magento Open Source和云部署的默认搜索引擎。 它支持作为独立的搜索引擎进行Magento Open Source。 此版本支持OpenSearch v2.x，并且已通过OpenSearch 2.5测试。尽管与OpenSearch 1.x的兼容性仍然保持不变，但我们建议将此版本与OpenSearch 2.x结合使用。您可以在云和内部部署上安装并运行带OpenSearch 2.x的Magento Open Source和Magento Open Source2.4.6 [搜索引擎先决条件](https://experienceleague.adobe.com/docs/commerce-operations/installation-guide/prerequisites/search-engine/overview.html). <!-- AC-449 3555 -->
...
* PHP 7.4兼容性已从商务和Magento Open Source代码库中删除。 <!-- AC-2841-->
...
* `jquery-migrate` 已从商务和Magento Open Source代码库中删除。 [GitHub-21406](https://github.com/magento/magento2/issues/21406) <!-- AC-1199 -->
...
* 的 `laminas/laminas-dependency-plugin` 已删除编辑器插件。 [GitHub-36515](https://github.com/magento/magento2/issues/36515) <!-- AC-7100-->

* 已更换 `Zend_Filter` with `laminas/laminas-filter`. <!-- AC-5922-->
...
以下Adobe IMS模块已与Adobe Commerce和Magento Open Source核心代码分离，现在打包为单独的元包：

* `adobe-ims`
* `adobe-ims-api`
* `admin-adobe-ims`

新 `adobe-ims-metapackage` v2.2.0与Adobe Commerce 2.4.6和Magento Open Source2.4.6捆绑在一起。此版本中没有任何功能发生更改。<!-- CABPI-505-->
...
**改进了查询启用了类别权限的类别时的响应时间**. 查询启用了类别权限且包含许多客户组或共享目录的类别中的产品时，响应时间已缩短。 支持此性能改进的代码更改包括： <!-- B2B-2179 2404 2329-->

* GraphQL解析程序操作已优化。
* 数据提供程序不再为正在处理的每个新产品重新设置运行时缓存的数据，这改进了性能。 <!-- B2B-2452-->
* 在GraphQL解析程序中优化了产品权限处理。 现在，产品权限将应用于产品收藏集，而不是按产品应用。  <!-- B2B-2404-->
* 优化了在 `addProductsToCart` 变异。 <!-- B2B-2424-->
* 产品权限已通过产品内联片段进行优化。 <!-- B2B-2459-->

**类别树渲染的优化性能**. 的 `categoryList` 已优化查询以支持具有大量类别的客户。 使用此查询搜索1500个以上具有最多五个嵌套子类别的类别的商家将注意到性能改进。 通过删除不必要的方法调用、改进类别树的缓存以及递归加载类别数据，改进了重构代码对类别子代的加载。  支持此性能改进的代码更改包括： <!-- B2B-2484 2469 2472-->

* 类别模型的自定义属性元数据获取已得到改进。 <!-- B2B-2463-->
* 类别ID获取已得到改进， `getChildren` 呼叫已减少。 <!-- B2B-2464-->
...
**客户查询现在支持订单排序**. 的 `orders` 输出对象 `customer` 查询现在包含排序字段。 您现在可以按顺序号或创建日期以及升序或降序对订单进行排序。 <!-- AC-6903-->
...
页面生成器版本1.7.3与Magento Open Source2.4.6兼容。

### PWA Studio

PWA Studiov.13.0.x与Magento Open Source2.4.6兼容。它包含多项增强功能，可改进辅助功能。 有关错误修复的信息，请参阅 [PWA Studio版本](https://github.com/magento/pwa-studio/releases). 请参阅 [版本兼容性](https://developer.adobe.com/commerce/pwa-studio/integrations/adobe-commerce/version-compatibility/) 以获取PWA Studio版本及其兼容的Adobe Commerce核心版本列表。


### 已知问题

**问题**:New Relic当前与PHP 8.2不兼容。运行New Relic的部署可以在PHP 8.1上运行Adobe Commerce 2.4.6，但不能在PHP 8.2上运行。 **解决方法**:在PHP 8.1上运行Adobe Commerce 2.4.6，直到发布支持PHP 8.2的新New Relic版本为止。该版本预计在2023年夏季发布。 <!-- AC-7769-->

## 已修复的问题

我们修复了Magento Open Source2.4.6核心代码中的数百个问题。
...
* 现在，可以从管理员中成功过滤捆绑产品 **[!UICONTROL Catalog]** > **[!UICONTROL Products]** 使用荷兰语区域设置时。 以前，当您尝试在 `Type: Bundle Product`: `Something went wrong with processing the default view and we have restored the filter to its original state`. [GitHub-35704](https://github.com/magento/magento2/issues/35704)

<!-- ACP2E-1038-->

* 当捆绑产品的特殊价格超过1000时，产品网格不再引发异常。 以前，当您按SKU搜索产品或导航到 **[!UICONTROL Catalog]** > **[!UICONTROL Products]**,Magento Open Source抛出此错误： `Non Numeric value encountered`. [GitHub-36284](https://github.com/magento/magento2/issues/36284))
...
* 在GraphQL中，包含 `BundleProduct` 片段， `items.options.product` 对象包含预期数据。 以前，对象在某些情况下可能为空。
...
* GraphQL产品查询现在只返回要查询的存储范围内的捆绑产品选项。

<!-- B2B-2455-->

* 未分配到类别但是属于捆绑产品或分组产品的产品选项的产品现在在响应中返回，当 **[!UICONTROL Allow Category Browsing]** 为发送查询的客户启用了配置设置。
...
* 的导出值 **[!UICONTROL Maximum Qty Allowed in Shopping Cart]** 现在与“管理员配置”设置中设置的默认值匹配。

<!-- ACP2E-889-->

* Magento Open Source现在可以正确计算带运费折扣的订单退款总计。

<!-- ACP2E-876-->

* Google reCAPTCHA现在可按预期用于店面订单。 以前，当购物者首次尝试下订单之前未先选择Google ReCAPTCHA时，Magento Open Source会显示以下错误： `ReCaptcha validation failed, please try again` 和 `No such cart with id = ID`.
...
* 现在，当购物者在购物车页面上更改产品数量而不更新购物车，然后尝试导航到下一页时，Magento Open Source在将购物者重定向到下一页之前，会按预期显示一个确认弹出窗口。

<!-- AC-5987-->

* 现在，在管理员中产品数量减少后，购物者可以从购物车中减少产品数量。 以前，当购物者尝试减少产品数量时，Magento Open Source抛出以下错误： `The requested qty is not available`. [GitHub-35780](https://github.com/magento/magento2/issues/35780)

<!-- AC-6744-->

* 当产品数量超过在多次发运结帐期间配置的最大值时，Magento Open Source不再在JavaScript控制台中引发错误。 现在，Magento Open Source会显示一则信息性错误消息。 [GitHub-36184](https://github.com/magento/magento2/issues/36184)
...
* 购物者不能再将类别中的产品添加到购物车，现在，在更改购物者所属的客户组后，该类别不允许添加。 Magento Open Source现在会在权限配置设置发生更改时检查特定类别的客户组权限访问权限。
...
* 现在，当送货地址包含扩展属性时，结帐可成功完成。 以前，Magento Open Source抛出了以下错误： `Object of class Magento\Quote\Api\Data\AddressExtension could not be converted to string`. [GitHub-34202](https://github.com/magento/magento2/issues/34202)

<!-- AC-2793-->

* 的 `addBundleProductsToCart` 如果所需的产品选项没有值，变异将不再将产品添加到报价中。 [GitHub-25676](https://github.com/magento/magento2/issues/25676)

<!-- B2B-2458-->

* Magento Open Source现在会在 `addProductsToCart` 当将产品添加到购物车时，如果权限不足，突变会失败。

<!-- ACP2E-1242-->

* Magento Open Source在使用快速订购添加数量超过可用库存的产品时，不再在购物车页面上引发错误。
...
* Magento Open Source在重新加载产品详细信息页面时不再显示上一页的数据。 以前，重新加载产品详细信息页面时，第一个选项卡中的数据会与新加载的数据合并一秒钟。
...
* 现在，在 **[!UICONTROL Display Out of Stock Products]** 配置设置已启用。 [GitHub-35899](https://github.com/magento/magento2/issues/35899)

<!-- AC-6384-->

* 当 **[!UICONTROL Allow All Products Per Page]** 设置。 [GitHub-35900](https://github.com/magento/magento2/issues/35900)

<!-- AC-6330-->

* 的 **[!UICONTROL Display Out of Stock Products]** 现在，配置选项在启用后可按预期工作。 以前，当管理员上启用此选项时，所有产品在店面上都显示为无现货。 [GitHub-35898](https://github.com/magento/magento2/issues/35898)
...
* 基于SKU且 **[!UICONTROL is not one of]** 条件现在可按预期工作。

<!-- AC-729-->

* Magento Open Source不再基于类别路径错误地假设一个类别是另一个类别的父类别。 以前，当两个类别相邻且一个类别ID以另一个类别的类别ID开头时，Adobe Commerce会假定这些类别具有相同的父类别。 [GitHub-32464](https://github.com/magento/magento2/issues/32464)
...
* Magento Open Source在管理员用户将产品手动添加到已分配了许多简单产品的可配置产品时，不再引发错误。 以前，Magento Open Source显示以下错误： `Something went wrong with processing the default view and we have restored the filter to its original state`.

<!-- AC-2877-->

* 在禁用后退订单的部署中，购物者无法再将可售数量为0的可配置产品添加到其购物车。 Magento Open Source现在表示产品无现货。 [GitHub-35319](https://github.com/magento/magento2/issues/35319)
...
* 已在“管理优惠券代码”工作流中添加验证逻辑，以在生成优惠券时强制执行优惠券代码设置。 以前，优惠券代码生成会忽略 **[!UICONTROL Code Length]** 设置时间 **优惠券数量** 值大于2500。
...
* 的 `catalog_index_refresh_price` cron作业现在成功执行。 以前，Magento Open Source在代码执行期间记录了此错误： `report.ERROR: Cron Job catalog_index_refresh_price has an error: SQLSTATE[HY093]: Invalid parameter number: number of bound variables does not match number of tokens`.
...
* Magento Open Source在从管理员创建订单后不再发送客户重复的电子邮件。

<!-- ACP2E-1200-->

* 管理员订单电子邮件在 **[!UICONTROL Email order confirmation]** 设置处于禁用状态。 [GitHub-36256](https://github.com/magento/magento2/issues/36256)
...
* 管理员用户现在可以成功打开管理员 **[!UICONTROL Store]** > **[!UICONTROL Configuration]** 页面中的国家/地区 **欧盟国家** 设置(**[!UICONTROL Stores]** > **[!UICONTROL Configuration]** > **[!UICONTROL General]** > **[!UICONTROL Country options]** > **[!UICONTROL European Union Countries]**)。 以前，当管理员用户尝试打开此页面时，Magento Open Source抛出了以下错误： `TypeError: explode(): Argument #2 ($string) must be of type string, null given in /var/www/php/folder/vendor/magento/module-config/ViewModel/CountryFilter.php:46`. [GitHub-35901](https://github.com/magento/magento2/issues/35901)
...
* 商户现在能够指定自定义外部SMTP提供程序。 [GitHub-26104](https://github.com/magento/magento2/issues/26104)

<!-- AC-3752-->

* 现在，在运行Magento Open Source2.4.3-p2的部署中，可以按预期重新授权OAuth集成。 以前，重新授权失败，并出现以下错误： `The attempt to post data to consumer failed due to an unexpected error. Please try again later`. Magento Open Source还尝试解密未加密的值。 [GitHub-35313](https://github.com/magento/magento2/issues/35313)
...
* AMQP消费者不再因弃用而失败 `trim()` 函数。以前，当AMQP使用者在安装后启动时，Magento Open Source会引发以下错误： `Deprecated Functionality: trim(): Passing null to parameter #1 ($string) of type string is deprecated in .../vendor/magento/framework-amqp/Config.php on line 143`. [GitHub-35380](https://github.com/magento/magento2/issues/35380)

<!-- AC-2882-->

* 的 `getFilteredTimerIds` 方法现在可按预期工作。  类 `Magento\Framework\Profiler\Driver\Standard\Stat` 现在与PHP 8.1兼容。以前，Magento Open Source在调用此方法时会引发异常。 [GitHub-35307](https://github.com/magento/magento2/issues/35307)
...
* Magento Open Source现在可在管理员用户选择 **[!UICONTROL Last 24 Hours]** 选项。 以前，Magento Open Source显示的时间范围不正确。 发生此情况是因为数据库的时区保持不变。
...
* CMS页面层次结构中的痕迹导航在 **[!UICONTROL Enable Hierarchy Functionality]** 已禁用。 以前，会显示痕迹导航，当购物者单击痕迹导航时，会显示404页面。 [GitHub-35513](https://github.com/magento/magento2/issues/35513)
...
* 的 `customerOrders` 查询不再引发 `\"model\" value should be specified`错误。 [GitHub-35852](https://github.com/magento/magento2/issues/35852)
...
* 的 `categoryList` 查询现在可为已定位和未定位的父类别返回准确的产品计数。 新 `is_anchor` 属性已添加到“类别集合选定的列”列表。 以前，请求返回的产品计数为0。 [GitHub-36300](https://github.com/magento/magento2/issues/36300)
...
* 在 `products` 查询现在包含价格类型属性的正确值 `attribute_code` 和 `label`. [GitHub-28878](https://github.com/magento/magento2/issues/28878)
...
* 导入与现有产品具有相同数字URL键但没有其终端字母字符的产品时，不再导致验证错误 **[!UICONTROL Product URL Suffix]** 在管理员中设置为空。
...
* 的 `Fixer.io` 导入服务现在可按预期工作。 货币汇率现在已成功导入。 [GitHub-36010](https://github.com/magento/partners-magento2ee/issues/36010)
...
* 状态为 **[!UICONTROL Closed]** 现在，在发出贷项通知单并退回客户后，会按预期分配给虚拟订单。 以前，虚拟产品显示为 **[!UICONTROL Complete]** 在“状态”下拉列表中，但 **[!UICONTROL Closed]** 在从管理员添加评论时出现的订单注释页脚中。 [GitHub-35283](https://github.com/magento/magento2/issues/35283)
...
* 现在，在 **[!UICONTROL Apply discount on prices]** 配置选项设置为 **[!UICONTROL including taxes]**.
...
* 现在，在启用异步索引时，可以按预期为免费产品下达自动开票订单。 不会向 `sales_order_grid or sales_invoice_grid` 表格。 以前，Magento Open Source会引发异常，因为应用程序尝试在订单放置期间索引网格，而不是依赖 `cron`. [GitHub-36334](https://github.com/magento/magento2/issues/36334)
...
* Magento Open Source在商家单击 **[!UICONTROL Reports]** > **[!UICONTROL Braintree Settlement]**: `Error: Undefined constant Braintree\PaymentInstrumentType::ANDROID_PAY_CARD in /var/www/html/vendor/paypal/module-braintree-core/Ui/Component/Report/Listing/Column/PaymentType.php:49`. [GitHub-35434](https://github.com/magento/magento2/issues/35434)
...
* 管理员用户现在可以在启用Braintree付款方法时创建订单或重新排序。 以前，Magento Open Source抛出了以下错误： `report.CRITICAL: Error: Call to a member function getMethodInstance() on null in /app/vendor/paypal/module-braintree-core/Block/Form.php:174`.

<!-- BUNDLE-3139-->

* 现在，在启用BraintreePayPal并且 **[!UICONTROL Enable Card Payments]** 已禁用。 以前，进度栏从未解析，并且Magento Open Source显示此控制台错误： `adapter.js:66 Uncaught TypeError: Cannot read properties of undefined (reading 'clientToken')at Object.getClientToken (adapter.js:66:66)`. [GitHub-35961](https://github.com/magento/magento2/issues/35961)
...
* 现在，在 **[!UICONTROL Send Line Items]** 配置设置已从管理员中启用。
...
* 现在，在启用Braintree付款方法后，管理员用户可以为客户创建订单或重新排序。 以前，Magento Open Source抛出了以下错误： `report.CRITICAL: Error: Call to a member function getMethodInstance() on null in /app/vendor/paypal/module-braintree-core/Block/Form.php:174`

<!-- BUNDLE-3139-->

* 购物者现在可以在店面中使用Braintree付款方法， **[!UICONTROL Enable Card Payments]** 已被禁用。 以前，Magento Open Source抛出此JavaScript错误： `_adapter.js:66 Uncaught TypeError: Cannot read properties of undefined (reading 'clientToken')at Object.getClientToken (adapter.js:66:66).`
...
* 现在，在 **[!UICONTROL Minimum Terms to Match]** 配置设置已启用。 以前，Magento Open Source抛出了以下错误： `{"error":{"root_cause":[{"type":"parsing_exception","reason":"[match_phrase] query does not support [minimum_should_match]' ...`.
...
* 搜索结果现在包括所有可用产品的无现货选项输出，当 **[!UICONTROL Display Out Of Stock Products]** 已启用 [GitHub-36055](https://github.com/magento/magento2/issues/36055)
...
* Magento Open Source现在尊敬管理员客户 **[!UICONTROL Show Company]** 配置设置。 禁用此设置后，发运公司将不再包含在发运、客户地址或数据库订单记录中。 以前，尽管设置了禁用，仍会显示信息。 [GitHub-33729](https://github.com/magento/magento2/issues/33729)
...
* Magento Open Source在对发运成本应用折扣时不再引发最小订单金额错误。
...
* 添加了 `price_excl_tax` 和 `price_excl_tax` 输出的类型 `setShippingMethodsOnCart` 变异。 [GitHub-31206](https://github.com/magento/magento2/issues/31206)
...
* 的 **[!UICONTROL Display Zero Tax Subtotal]** 现在，对于提供多种发运方法的部署中的订单，可按预期应用配置设置。 以前，在选择送货方法之前，即使禁用了此设置，结帐期间购物车页面上也会显示零税字段。 [GitHub-35561](https://github.com/magento/magento2/issues/35561)

<!-- AC-3157-->

* Magento Open Source现在可以在运行PHP 8.1的部署中，在店面类别页上显示带税的分组产品。以前，Magento Open Source不显示产品列表，因此引发了以下异常： `main.CRITICAL: Exception: Deprecated Functionality: ucfirst(): Passing null to parameter #1 ($string) of type string is deprecated in /var/www/m24/vendor/magento/module-tax/Pricing/Render/Adjustment.php on line 188 in /var/www/m24/vendor/magento/framework/App/ErrorHandler.php:61`. [GitHub-35500](https://github.com/magento/magento2/issues/35500)
...
* 现在，当没有与客户地址关联的增值税ID时，即可自动分配客户组。 如果地址不包含增值税ID，则它将被视为属于非增值税注册客户，并且Magento Open Source会将客户组更改为非增值税组。 以前， **[!UICONTROL Enable Automatic Assignment to Customer Group]** 启用，则客户帐户未自动分配给 `customerGroup` 从客户帐户中删除增值税ID时。
...
* 用户角色网格的过滤器功能（管理员） **[!UICONTROL System]** > **[!UICONTROL Permissions]** > **[!UICONTROL User roles]** > **[!UICONTROL Administrators]**)现在可以按预期工作。 数组值现在单独处理。 以前，当您尝试在此网格中输入查询时，Magento Open Source引发了此控制台错误，并且页面未加载： `Uncaught TypeError: Cannot read properties of null`. [GitHub-35993](https://github.com/magento/magento2/issues/35993)
...
* 主题标题设置 `Welcome Text` 现在可以包含单引号字符。 以前，Magento Open Source抛出此JavaScript错误： `Uncaught SyntaxError: Unable to process binding "ifnot: function(){return customer().fullname }"`.

### URL重写

<!-- ACP2E-808-->

* 将产品可见性更改为 **[!UICONTROL Not Visible Individually]** 现在，在全局“所有商店视图”范围中，只删除未覆盖可见性属性的商店视图的URL重写。 以这种方式更改产品可见性还会为所有商店视图(包括那些已覆盖URL键属性（但不是可见性设置）的视图)生成URL重写。 以前，在可见性设置发生更改时，不会应用URL重写规则。
...
* 商家现在可以使用REST API为零总计订单创建贷项通知单。 以前，Magento Open Source显示以下错误： `The order does not allow a credit memo to be created. The credit memo's total must be positive. We can't create a credit memo for the invoice/order`.
...
* 现在，修复器货币服务可通过ApiLayer生成的密钥按预期工作，并且货币汇率可正确导出。 以前，当商家尝试通过管理员导入货币汇率时，Magento Open Source会抛出错误。
...
**问题**:启用免费发运后，您不能创建贷项通知单(**[!UICONTROL Stores]** > **[!UICONTROL Configuration]** > **[!UICONTROL Sales]** > **[!UICONTROL Delivery Methods]**)，且显示的发运价格包括税。 当您尝试创建贷项通知单时，Adobe Commerce会引发此异常： `"Division by zero".vendor/magento/module-sales/Model/Order/Creditmemo/Total/Tax.phpLine 139`. [GitHub-36800](https://github.com/magento/magento2/issues/36800)

**解决方法**:此问题的热修复程序现已可用。 转到 [质量修补程序工具：搜索修补程序](https://experienceleague.adobe.com/tools/commerce-quality-patches/index.html) 并搜索ACSD-50814补丁。
