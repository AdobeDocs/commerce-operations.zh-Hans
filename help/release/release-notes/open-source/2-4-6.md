...
此版本的安全改进改进了与最新安全最佳实践的兼容性，包括：

* 通过在网格视图、批量操作和导出中对操作进行更具体的审核，填补了管理操作日志中的空白。<!-- AC-1553 -->

* 当在付款处理期间发生意外错误时，reCAPTCHA验证在结账期间不再失败。 [GitHub-35093](https://github.com/magento/magento2/issues/35093) <!-- AC-2642 -->
...
* 现在，支持将OpenSearch作为Magento Open Source内部部署和云部署的默认搜索引擎。 它支持作为独立的Magento Open Source搜索引擎。 此版本支持OpenSearch v2.x，并已经过OpenSearch 2.5测试。尽管与OpenSearch 1.x的兼容性仍然存在，但我们建议将此版本与OpenSearch 2.x一起使用。您可以在云和本地[Magento Open Source引擎先决条件](https://experienceleague.adobe.com/docs/commerce-operations/installation-guide/prerequisites/search-engine/overview.html)上安装并运行带有OpenSearch 2.x的搜索引擎和Magento Open Source2.4.6。<!-- AC-449 3555 -->
...
**已优化类别树呈现的性能**。 已优化`categoryList`查询，以支持具有大量类别的客户。 使用此查询搜索包含最多五个嵌套子类别的1500多个类别的商家将注意到性能改进。 通过删除不必要的方法调用、改进类别树的缓存和递归加载类别数据，改进了通过重构代码加载类别子级的过程。  支持此性能改进的代码更改包括： <!-- B2B-2484 2469 2472-->
...
PWA Studiov.13.0.x与Magento Open Source2.4.6兼容。它包括多项增强功能，可改善辅助功能。 有关错误修复的信息，请参阅[PWA Studio版本](https://github.com/magento/pwa-studio/releases)。 有关PWA Studio版本及其兼容的Adobe Commerce核心版本的列表，请参阅[版本兼容性](https://developer.adobe.com/commerce/pwa-studio/integrations/adobe-commerce/version-compatibility/)。
...
* `composer install`命令不再覆盖`.gitignore`个文件。 [GitHub-32888](https://github.com/magento/magento2/issues/32888)
...
* `bin/magento setup:upgrade`现在在首次执行`composer install --no-dev`后成功执行。 [GitHub-36351](https://github.com/magento/magento2/issues/36351)
...
* 使用荷兰语言环境时，现在可以从管理员&#x200B;**[!UICONTROL Catalog]** > **[!UICONTROL Products]**&#x200B;成功筛选捆绑产品。 以前，当您尝试筛选`Type: Bundle Product`时，Magento Open Source显示此错误： `Something went wrong with processing the default view and we have restored the filter to its original state`。 [GitHub-35704](https://github.com/magento/magento2/issues/35704)
...
* 当为发送查询的客户启用&#x200B;**[!UICONTROL Allow Category Browsing]**&#x200B;配置设置时，对于未分配给类别但属于捆绑产品或分组产品的产品选项的产品，现在将在响应中返回。
...
* 在更改购物者所属的客户组后，购物者不能再将现在不允许的类别中的产品添加到购物车中。 当权限配置设置发生更改时，Magento Open Source现在会检查特定类别的客户群组权限访问权限。
...
* 如果所需的产品选项没有值，`addBundleProductsToCart`突变将不再向报价中添加产品。 [GitHub-25676](https://github.com/magento/magento2/issues/25676)

<!-- B2B-2458-->

* 当将产品添加到购物车中的权限不足时，`addProductsToCart`突变失败时，Magento Open Source现在会返回信息性消息。

<!-- ACP2E-1242-->

* 当使用“快速订购”添加的产品数量超过可用库存时，Magento Open Source不会再在“购物车”页面上引发错误。
...
* 优惠券代码现在可以成功导出到Excel XML。 以前，生成的文件不包含所有选定的优惠券代码，而只包含在“每页的项目数”选择器中选定的项目数。[GitHub-36073](https://github.com/magento/magento2/issues/36073)
...
* 从管理员处创建订单后，Magento Open Source不会再向客户发送重复电子邮件。
...
* 现在，可以在运行Magento Open Source2.4.3-p2的部署中按预期重新授权OAuth集成。 以前，重新授权失败，错误为： `The attempt to post data to consumer failed due to an unexpected error. Please try again later`。 Magento Open Source还尝试解密未加密的值。 [GitHub-35313](https://github.com/magento/magento2/issues/35313)
...
* `getFilteredTimerIds`方法现在按预期工作。  类`Magento\Framework\Profiler\Driver\Standard\Stat`现在与PHP 8.1兼容。以前，在调用此方法时，Magento Open Source会引发异常。 [GitHub-35307](https://github.com/magento/magento2/issues/35307)
...
* 现在，当管理员用户从管理员统计数据仪表板中选择&#x200B;**[!UICONTROL Last 24 Hours]**&#x200B;选项时，Magento Open Source会显示准确的时间范围。 以前，Magento Open Source显示的时间范围不正确。 发生这种情况是因为数据库的时区保持不变。
...
* **[!UICONTROL My Quotes]**&#x200B;页面上的&#x200B;**[!UICONTROL Show my quotes]**&#x200B;筛选器现在可按预期工作。 以前，筛选结果包括其他管理员用户的报价。
...
* `customer`查询现在对具有指定`from`和`to`范围的订单进行正确筛选。 [GitHub-31537](https://github.com/magento/magento2/issues/31537)
...
* 启用BraintreePayPal并禁用&#x200B;**[!UICONTROL Enable Card Payments]**&#x200B;时，使用Braintree付款方式的签出现在成功完成。 以前，进度条从未解析，Magento Open Source显示此控制台错误： `adapter.js:66 Uncaught TypeError: Cannot read properties of undefined (reading 'clientToken')at Object.getClientToken (adapter.js:66:66)`。 [GitHub-35961](https://github.com/magento/magento2/issues/35961)
...
* 现在，如果在对装运应用折扣后应用税，则装运退款会正确计算。

<!-- ACP2E-910 AC-3182-->

* 将同时返回通过GraphQL `cart`查询请求的购物车的所有适用促销。 促销活动包括行项目折扣、购物车规则/折扣、优惠券、礼品卡和商店积分。 [GitHub-35545](https://github.com/magento/magento2/issues/35545)
...
* 现在，在运行PHP 8.1的部署中，Magento Open Source可以在店面类别页上显示含税的分组产品。以前，Magento Open Source不显示产品列表，并引发此异常： `main.CRITICAL: Exception: Deprecated Functionality: ucfirst(): Passing null to parameter #1 ($string) of type string is deprecated in /var/www/m24/vendor/magento/module-tax/Pricing/Render/Adjustment.php on line 188 in /var/www/m24/vendor/magento/framework/App/ErrorHandler.php:61`。 [GitHub-35500](https://github.com/magento/magento2/issues/35500)

<!-- ACP2E-1258-->

* 商家无法再将计划的更新移动到包含相同规则ID的另一个更新。 以前，当更新移至包含相同规则ID的其他更新时，计划更新购物车价格规则会创建重复的计划。

<!-- ACP2E-1295-->

* 当没有与客户地址关联的增值税ID时，现在可以自动分配客户组。 如果地址不包含增值税ID，则被视为属于非VAT注册客户，而Magento Open Source会将客户组更改为非VAT组。 以前，启用&#x200B;**[!UICONTROL Enable Automatic Assignment to Customer Group]**&#x200B;后，从客户帐户中删除增值税ID时，不会自动将客户帐户分配给`customerGroup`。

<!-- ACP2E-493-->

* 现在，在暂存预览模式下为暂存目录规则显示正确的动态块。 以前，显示不相关的动态块。

<!-- ACP2E-1080-->

* 在结帐过程中应用多发时，发票和贷项通知单的“税务汇总”部分现在可以正确显示多税信息。

<!-- ACP2E-1076-->

* 当订单小计（含税）以及固定产品税额超过购物车页面上的购物车规则条件时，不再应用折扣。 以前，即使总金额超出了购物车规则条件，也会应用折扣。
...
* 开发人员现在可以指示系统在特定的存储视图中执行数据夹具。 新的可选参数`scope`接受与存储夹具别名对应的`string`。

<!-- ACP2E-1133-->

* 添加了使用可选的`count`参数生成数据夹具的多个实例的功能。
...
* 现在，将全局“所有存储视图”作用域中的产品可见性更改为&#x200B;**[!UICONTROL Not Visible Individually]**，将只删除未覆盖visibility属性的存储视图的URL重写。 以这种方式更改产品可见性也会为所有商店视图(包括那些已覆盖URL键属性（但不是可见性设置）生成URL重写。 以前，更改可见性设置时未应用URL重写规则。

### Web API框架

<!-- ACP2E-1207-->

* 现在，如果映像由于分配给其他存储视图中的某个角色而未从产品中删除，REST API `DELETE /rest/V1/products/:sku/media/:entryId`会返回错误消息。 以前，即使未成功删除图像，API也会返回成功的响应。
...
* 现在，在来宾客户通过确认电子邮件确认其帐户后，产品会按预期从产品列表添加到愿望列表和查看页面中。 以前，产品在帐户确认后不会添加到愿望列表中。

<!-- ACP2E-1175-->

* 当购物者尝试向期望列表中添加一项时，`addProductsToWishlist`查询现在返回信息性错误消息，该项对产品`select`自定义选项具有无效值。 以前，下拉自定义选项值的服务器验证缺失，这会导致数据库中的值无效。 因此，终结点返回此内部服务器错误： `GraphQL endpoint returns Internal server error with "Call`。

## 已知问题

**问题**：如果启用了免运费(**[!UICONTROL Stores]** > **[!UICONTROL Configuration]** > **[!UICONTROL Sales]** > **[!UICONTROL Delivery Methods]**)，并且显示的运费含税，则无法创建贷项通知单。 当您尝试创建贷项通知单时，Adobe Commerce会引发此异常： `"Division by zero".vendor/magento/module-sales/Model/Order/Creditmemo/Total/Tax.phpLine 139`。 [GitHub-36800](https://github.com/magento/magento2/issues/36800)
...
您可以使用[编辑器](../../../installation/composer.md)安装Magento Open Source2.4.6。
