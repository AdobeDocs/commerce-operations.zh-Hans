...
* 当产品缺货时，`cart`查询不再返回null响应。 已将包含错误消息的新`errors`元素引入响应。 以前，当您使用缺货产品运行查询时，应用程序Open Source在响应中的`null`部分下显示`items`值。 查看[购物车查询](https://developer.adobe.com/commerce/webapi/graphql/schema/cart/queries/cart/)。
...
* 当CSP设置为`banner`时，您现在可以将Vimeo视频设置为在`restrict mode`元素的后台运行。 以前，applicationOpen Source引发JavaScript错误。
