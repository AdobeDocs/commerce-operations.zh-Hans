...
* `bin/magento config:set graphql/session/disable 1`命令允许商家完全禁止为所有GraphQL操作创建会话Cookie。 默认情况下，应用程序Open Source会创建这些Cookie并依赖它们进行授权，这会影响性能。 将来，我们建议使用令牌作为授权GraphQL请求的唯一形式。 我们不建议单独使用会话Cookie或将其与授权令牌结合使用。 查看[GraphQL授权](https://developer.adobe.com/commerce/webapi/graphql/authorization-tokens.html)。<!-- B2B-2204 -->
...
* 您现在可以重命名数据修补程序，并将旧类名作为别名添加到`patch_list`数据库表中。 现在，Magento Open Source在应用补丁程序之前会检查数据库中是否已存在数据补丁程序别名。 以前，在这些情况下，applicationOpen Source会引发错误。
...
* 刷新Magento Open Source缓存并启用`use_stale_cache`后，Admin中不再显示已禁用的全页缓存。
...
* 当产品缺货时，`cart`查询不再返回null响应。 已将包含错误消息的新`errors`元素引入响应。 以前，当您使用缺货产品运行查询时，应用程序Open Source在响应中的`items`部分下显示`null`值。 查看[购物车查询](https://developer.adobe.com/commerce/webapi/graphql/queries/cart.html)。
...
* 现在，产品列表在Admin中正确呈现。 以前，产品列表未呈现，并且Magento Open Source显示此错误： `Item (Magento\Catalog\Model\Product\Interceptor) with the same ID "<ID>" already exists`。 [GitHub-33145](https://github.com/magento/magento2/issues/33145)
...
* 现在，在通过修补程序脚本创建全局`select`属性期间，Magento Open Source会按预期显示可配置属性。 以前，符合条件的全局属性会被隐藏。
...
* 当购物者单击&#x200B;**下单**&#x200B;后输入无效的电子邮件格式时，Magento Open Source现在在“送货”页面上显示错误消息。 以前，错误消息显示在“付款”页面上。 [GitHub-33590](https://github.com/magento/magento2/issues/33590
...
* Magento Open Source在管理员上不再显示`preg_replace()`错误。 第三个参数(`$subject`)现在的类型为`array|string`，而不是`bool`。
...
* 现在，当您导航到店面上的`/checkout/sidebar/updateItemQty/?item_qty=error`时，Magento Open Source返回404错误，而不是500错误。 以前，引发此错误： `Warning: A non-numeric value encountered in /vendor/magento/module-checkout/Controller/Sidebar/UpdateItemQty.php on line 69`。 [GitHub-34380](https://github.com/magento/magento2/issues/34380)
...
* 在单击&#x200B;**共享愿望清单**&#x200B;之前，当您激活&#x200B;**在此处选中以将RSS馈送链接到愿望清单**&#x200B;复选框时，Magento Open Source不再引发错误。 [GitHub-34998](https://github.com/magento/magento2/issues/34998)
...
* 发出GraphQLMagento Open Source后，请求会话不再结束。 以前，`ClearCustomerSessionAfterRequest`插件会注销购物者。 [GitHub-34550](https://github.com/magento/magento2/issues/34550)
...
* 现在，在运行PHP 7.4的部署中，Magento Open Source在贷项通知单网格页上显示所请求存储中的记录。以前，在您创建贷项通知单并尝试查看它后，applicationOpen Source引发以下错误： `The store that was requested wasn't found. Verify the store and try again`。

<!-- AC-2442-->

* 对于从名称前面带有数字的存储区视图创建的订单，“Magento Open Source”现在在“贷项通知单网格”页上显示贷项通知单。 以前，applicationOpen Source显示错误： `The store that was requested wasn't found. Verify the store and try again. Exception in /var/www/html/vendor/magento/module-store/Model/StoreRepository.php:75`。 [GitHub-35122](https://github.com/magento/magento2/issues/35122)

<!-- ACP2E-31-->

* Magento Open Source现在在管理员和店面发票页总计上显示免运费(0)。 以前，当订单的装运量为零时，应用产品打开Source不会在发票页装运总数中显示装运总金额。
...
* 现在，产品详细信息页面上列出的价格与层价格在结账工作流中列出的价格相同，层价格因所选数量而异（例如，某个产品的定价因购买2件产品而非5件产品而有所不同）。 以前，结帐价格反映的是最低产品数量的价格。
...
* 当“Magento Open Source资源”选项卡上的资源访问设置为&#x200B;**自定义**&#x200B;时，**存储** > **配置** > **服务**&#x200B;页面现在会按预期显示Role Web API信息。 [GitHub-35506](https://github.com/magento/magento2/issues/35506)
...
* 将`int`配置为可搜索的后端`type`属性时，Elasticsearch查询现在可按预期工作。 以前，applicationOpen Source引发了`Elasticsearch\Common\Exceptions\BadRequest400Exception`异常。

<!-- ACP2E-99-->

* 现在，您可以在Elasticsearch查询中将搜索同义词与&#x200B;**要匹配的最少搜索项**&#x200B;参数一起使用。 以前，如果在设置中指定此参数，并且为特定关键字添加了搜索词，则搜索不会返回任何结果。

<!-- ACP2E-72-->

* 现在，在启用了“Magento Open Source建议”和&#x200B;**显示每个建议的结果计数**&#x200B;设置的部署中，搜索会在店面显示准确的搜索结果建议计数。 以前，关键字旁边显示的计数为零。

<!-- AC-700-->

* 现在，在“目录”页面上按自定义属性排序的产品会按预期顺序显示。 以前，产品按其属性选项值ID排序，该ID反映产品添加到属性的顺序。 [GitHub-33810](https://github.com/magento/magento2/issues/33810)

<!-- AC-645-->

* 在分层导航中按色板过滤产品现在会显示正确的产品图像。 以前，分层导航`PageCache`键不包含可配置产品的筛选器参数。

<!-- AC-2051-->

* 修复了`catalogsearch/advanced/result`上的PHP错误，并且  `catalogsearch/advanced/index`页。  以前，当数组传入任何高级Magento Open Source字符串时，搜索会显示此错误： `Warning: trim() expects parameter 1 to be string, array given | magento/module-catalog-search`。 [GitHub-33586](https://github.com/magento/magento2/issues/33586)
...
* 现在，当在生产模式下启用JavaScript捆绑包时，管理员可以访问管理员菜单选项。  [GitHub-35325](https://github.com/magento/magento2/issues/35325)
...
您可以使用[编辑器](../../../installation/composer.md)安装Magento Open Source2.4.5。
