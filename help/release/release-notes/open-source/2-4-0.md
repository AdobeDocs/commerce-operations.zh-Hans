---
title: Magento Open Source 2.4.0发行说明
...
# Magento Open Source 2.4.0发行说明

Magento Open Source 2.4.0引入了对PHP 7.4、Elasticsearch 7.6.x和MySQL 8.0的支持。重大的安全更改包括默认情况下在Admin中启用双重身份验证。

**在此版本中，已从核心代码中删除Authorize.Net方法集成。 商家应该迁移到Commerce Marketplace**&#x200B;上可用的官方扩展。
...
虽然这些功能的代码与季度版本捆绑在一起，但其中几个项目(例如Inventory management和Progressive Web Applications (PWA) Studio)也单独发布。 每个项目的文档中都提供了特定于项目的单独发行信息，其中记录了这些项目的错误修复。
...
此扩展取代了我们的核心Braintree集成。 它提供与Braintree核心集成相同的功能。 请参阅[Braintree支付](https://commercemarketplace.adobe.com//paypal-module-braintree.html)。
...
在`config.php`中预定义了存储配置时，Magento Open Source安装现在会成功完成，并会按预期创建存储。
...
* 现在，所有导出的网格数据（Magento Open Source和自定义数据）都具有选项标签，而不是类型为`Select`或`Multiselect`的所有列的选项值。 以前，在导出此数据时，导出的电子表格包含具有ID的列，而不是网格中显示的值。 _由Maksym Novik在拉取请求[中提交的修复26523](https://github.com/magento/magento2/pull/26523)_。 [GitHub-25963](https://github.com/magento/magento2/issues/25963)
...
* 现在，付款系统和Magento Open Source中都会显示已完成的订单。 以前，订单出现在支付系统中，但不出现在Magento Open Source中。 [GitHub-25862](https://github.com/magento/magento2/issues/25862)
...
* Elasticsearch 7.6.x现在是Adobe Commerce和Magento Open Source的默认目录搜索引擎。 如果不安装Elasticsearch 7.6.x，则无法安装或升级到版本2.4.0。已删除Elasticsearch版本2.x的代码。 Elasticsearch版本5.x和6.x已被弃用，不再受支持。 请参阅[搜索引擎先决条件](../../../installation/prerequisites/search-engine/overview.md)。
...
* Magento Open Source `cron`现在用于在创建测试数据后重新编制索引，在适用的情况下，`cache:flush config`将取代`cache:flush`。 Lukasz Bajsarowicz在拉取请求&#x200B;_中提交的[修复26990](https://github.com/magento/magento2/pull/26990)_。 [GitHub-26989](https://github.com/magento/magento2/issues/26989)
...
**问题**：安装PHP 7.4.2的部署中出现storefront错误消息异常。 使用PHP 7.4.2部署2.4.0时，店面错误消息中的空格符号将替换为加(+)字符。  此错误是PHP 7.4.2的固有错误，Magento Open Source无法更正此错误。 **解决方法**：我们建议使用其他版本的PHP 7.4.x。请参阅[原始消息数据显示在店面](https://support.magento.com/hc/en-us/articles/360045804332)知识库文章。<!-- MC-34170-->
...
您可以使用编辑器安装Magento Open Source 2.4.0。
