...
DHL已引入架构版本6.2，并且将在不久的将来弃用架构版本6.0。 支持DHL集成的Adobe Commerce 2.4.4及更早版本仅支持版本6.0。部署这些释放的商家应在方便时尽早申请`AC-3022.patch`以继续提供DHL作为装运承运人。 有关下载和安装修补程序的信息，请参阅[应用修补程序以继续将DHL作为运输运营商提供](https://support.magento.com/hc/en-us/articles/7707818131597-Apply-a-patch-to-continue-offering-DHL-as-shipping-carrier)知识库文章。
...
* **保护云SSH访问**。 云基础架构上的Adobe提供了多重身份验证(MFA)实施，用于管理对云环境的SSH访问的身份验证要求。 默认情况下，项目上未启用2FA的多重身份验证。  Adobe强烈建议启用此功能。 请联系支持人员以获取帮助。 请参阅[为SSH访问启用多重身份验证](https://experienceleague.adobe.com/en/docs/commerce-cloud-service/user-guide/project/multi-factor-authentication)。
...
* **新`\Magento\Framework\Escaper`类**。 为负责生成HTML的`.phtml`模板和PHP类提供了此类。 此类包含与多个上下文相关的HTML整理方法。 `$escaper`局部变量在`.phtml`模板内可用，应使用局部变量来代替已弃用的`$block->escape{method}`。 使用`$escaper`而非`$block`，因为`$block->escape{method}`的使用已被弃用。
...
* **支持PHPUnit 9.x并弃用PHPUnit 6.5**。 PHP 7.4需要使用最新的PHPUnit测试框架，即PHPUnit 9.x。Commerce Marketplace扩展供应商必须确认所有新的扩展版本都与PHP 7.4兼容，并且所有单元和集成测试都配置为使用PHPUnit 9运行。
...
* **正在删除MySQL目录搜索引擎**。 已从2.4.0中删除MySQL搜索引擎，并将其替换为Elasticsearch的默认搜索引擎。 Elasticsearch提供卓越的搜索功能以及目录性能优化。  所有商家都必须有Elasticsearch安装和部署2.4.0。请参阅[验证Elasticsearch是否已安装](https://experienceleague.adobe.com/docs/commerce-operations/upgrade-guide/prepare/prerequisites.html)。
...
* **删除Signifyd欺诈保护代码的核心集成**。 不再支持此核心功能。 商家应迁移到Commerce Marketplace上可用的[Signifyd Fraud &amp; Chargeback Protection扩展](https://marketplace.magento.com/signifyd-module-connect.html)。
...
* **编辑器更新插件**。 Composer插件通过解析在更新到新版本之前必须对根项目`composer.json`文件进行的更改来简化升级过程。 此插件可防止覆盖自定义设置。 请参阅[使用编写器根插件进行升级](https://experienceleague.adobe.com/docs/commerce-operations/upgrade-guide/implementation/perform-upgrade.html)。

* **卖家协助购物**。 此功能允许商家代表其客户查看店面。 客户选择允许店面访问其帐户。 此社区开发的功能包括由[MAGEFAN](https://magefan.com/)开发的原始扩展。 查看[卖方辅助购物](https://experienceleague.adobe.com/en/docs/commerce-admin/config/customers/login-as-customer)。 功能包括：

   * ACL可以针对每个网站配置哪些管理员可以登录到客户帐户
   * 与多个网站和客户帐户范围的兼容性
   * 代表客户下达的订单会登录店面并由管理员管理
   * 管理员注销后，所有会话都将被销毁，管理员无法访问客户密码。

### 性能改进

* **客户数据节无效逻辑的改进**。 此版本引入了使所有客户部分数据失效的新方式，从而避免了在自定义`sections.xml`失效处于活动状态时本地存储出现已知问题。  (以前，当具有操作无效的自定义&#x200B;*etc/frontend/sections.xml*&#x200B;时，未正确填充私有内容（本地存储）。) 查看[私人内容](https://developer.adobe.com/commerce/php/development/cache/page/private-content/)。
...
* `pickupLocations`查询支持库存入库提货功能
* `categories`查询返回与指定筛选器匹配的类别列表。 此查询与`categoryList`查询的不同之处在于它支持分页。
* `reorderItems`变异允许登录用户将先前订单中的所有产品添加到其购物车中。

有关此增强功能和其他增强功能的详细信息，请参阅[GraphQL开发人员指南](https://developer.adobe.com/commerce/webapi/graphql/)。 有关最近的GraphQL错误修复的详细讨论，请参阅[发行说明](https://developer.adobe.com/commerce/webapi/graphql/release-notes.html)。
...
* Cookie和单元测试
* 折扣
* 配置设置现在包含“现场消息”部分，用于控制Klarna促销消息的店面显示

#### 顶点

此版本的Vertex包括以下新增功能和增强功能：

* 对管理员配置用户体验的改进
* 用XML架构文件和修补程序替换安装和升级脚本
...
* 解决了使用替代的LESS编译器时，Magento的Luma主题中的LESS编译问题。 _Pieter Hoste在拉取请求[中提交的修复24003](https://github.com/magento/magento2/pull/24003)_。 [GitHub-23619](https://github.com/magento/magento2/issues/23619)
...
* 如果在部署中配置了数据库缓存引擎，`health_check.php`不再失败。 _由Andrii Beziazychnyi在拉取请求[中提交的修复25722](https://github.com/magento/magento2/pull/25722)_。 [GitHub-25669](https://github.com/magento/magento2/issues/25669)

### 购物车和结帐

<!-- MC-23870-->

* 启用&#x200B;**与后端同步**&#x200B;配置设置后，应用程序在签出期间不再引发错误。 [GitHub-23833](https://github.com/magento/magento2/issues/23833)

<!-- MC-29786-->

* 现在，装运方法的单选按钮已在签出工作流程中按预期启用。
...
* 现在，当您尝试在要上传的文件损坏时通过单击&#x200B;**按SKU排序**&#x200B;添加产品时，应用程序会显示信息性错误消息。 以前，应用程序显示空白页面。
...
* 结帐工作流程中现在会应用基于付款方式的购物车价格规则。 [GitHub-24206](https://github.com/magento/magento2/issues/24206)
...
* 为帐单地址和送货地址同时选择了单个地址时，更改帐单地址不再更改客户的送货地址。 _由Yurii Tvardyi在拉取请求[26279](https://github.com/magento/magento2/pull/26279)_&#x200B;中提交的修复。 [GitHub-26276](https://github.com/magento/magento2/issues/26276)
...
* 价格条件使用`catalog_product_entity_decimal`按价格筛选。 但是，此表不包含可配置、捆绑动态和分组产品的记录，因为这些产品没有固定价格。 用于按价格筛选的正确表为`catalog_product_index_price`。

<!-- ENGCOM-7104-->

* 管理目录和产品列表现在在MSRP、成本和特殊价格列中显示货币符号。 _由Sergiy Vasiutynskyi在拉取请求[27261](https://github.com/magento/magento2/pull/27261)_&#x200B;中提交的修复。 [GitHub-21910](https://github.com/magento/magento2/issues/21910)，[GitHub-20906](https://github.com/magento/magento2/issues/20906)，[GitHub-20472](https://github.com/magento/magento2/issues/20472)

<!-- ENGCOM-6375-->

* 单击“关闭”图标时，“为客户组添加价格”弹出窗口（**管理员** > **目录** > **产品** > **新产品** > **高级定价**）现在将按预期关闭。 _由Ashna-Jahan在拉取请求[中提交的修复25759](https://github.com/magento/magento2/pull/25759)_。 [GitHub-25433](https://github.com/magento/magento2/issues/25433)

<!-- ENGCOM-6300-->

* 为新产品的说明和标题提供的示例说明不再包含双引号。  _Nathan Morgan在拉取请求[中提交的修复25655](https://github.com/magento/magento2/pull/25655)_。 [GitHub-25654](https://github.com/magento/magento2/issues/25654)
...
* 现在，`CatalogWidget`产品列表对锚点类别可按预期工作，并且锚点类别中的产品现已匹配和显示。 以前，当您选择的父类别作为锚点但不包含分配的产品时，产品在小组件中不可见。
...
* 更正了结帐工作流程的“送货方式”部分中的单选按钮未对齐的问题。  _Hitesh在拉取请求[中提交的修复25966](https://github.com/magento/magento2/pull/25966)_。 [GitHub-25962](https://github.com/magento/magento2/issues/25962)
...
* 从图库图像中选择的图像缩略图现在会按预期进行缓存。 以前，这些图像的大小是动态调整的。

<!-- MC-32275-->

* 当您保存已分配给多个商店的CMS页面时，应用程序不再引发错误。 以前，当您创建或编辑CMS页面时，应用程序保存了该页面，但也引发了以下错误： `Item (Magento\VersionsCms\Model\Hierarchy\Node) with the same ID "PAGE_ID" already exists`。

<!-- MC-32452-->

* 新的CMS页面现在会按预期添加到网站的商店页面层次结构中。
...
* 现在，您可以从交叉销售选项卡中将可配置产品添加到购物车中。 当您选择产品并从此选项卡单击&#x200B;**添加到购物车**&#x200B;时，您现在将进入产品详细信息页面，您可以在其中选择特定的产品选项。 以前，应用程序会将您重定向到404错误页面。
...
* “我的帐户”仪表板现在在移动视图中正确显示电子邮件地址值。 _由Eden Duong在拉取请求[25942](https://github.com/magento/magento2/pull/25942)_&#x200B;中提交的修复。 [GitHub-25935](https://github.com/magento/magento2/issues/25935)
...
* 注册新帐户后，以访客身份订阅新闻稿的客户不再取消订阅。

<!-- MC-29841-->

* 应用程序现在对每次密码更改都使用新的`PHPSession`。

<!-- MC-30650-->

* 您现在可以成功创建客户，并使用&#x200B;**客户** > **所有客户** > **添加新客户**&#x200B;上的“关联到网站”下拉菜单将其与特定网站关联。 以前，当您尝试在多站点部署中将新客户与非默认网站关联时，应用程序显示以下错误： `The store view is not in the associated website`。
...
* 现在，您可以在需要出生日期时从店面创建新客户。 以前，即使您输入了有效的DOB值，也无法在需要此字段时创建新客户。 [GitHub-26700](https://github.com/magento/magento2/issues/26700)

<!-- MC-31945-->

* 现在，通过管理员创建客户时，会按预期采用默认客户组设置。

<!-- MC-31435-->

* 在客户注销再重新登录后，`PHPSessionId`现在会按预期更改。

<!-- MC-32152-->

* 现在，在多网站部署中，客户购物车的“管理员”视图显示了从多个网站添加到购物车的所有产品。 以前，当客户将产品从非默认网站添加到购物车时，即使&#x200B;**共享客户帐户**&#x200B;设置为&#x200B;**全局**，管理员也不会显示该产品。
...
* 现在，当您尝试将数量为0或负数的产品添加到愿望清单时，应用程序会显示以下信息性消息： `Please enter a valid number in this field`。 以前，应用程序显示以下错误： `We can't update your Wish List right now`。 _Pawe Tylek在拉取请求[中提交的修复25641](https://github.com/magento/magento2/pull/25641)_。 [GitHub-25032](https://github.com/magento/magento2/issues/25032)

<!-- ENGCOM-7071-->

* 应用程序不再支持从`Magento\Framework\Api\AbstractExtensibleObject`类继承DTO类。 缺少PHP批注`@api`  已添加到`AbstractExtensibleModel`，缺少`@deprecated`  已将批注添加到`AbstractExtensibleObject`。  _Alexander Taranovsky在拉取请求[中提交的修复22011](https://github.com/magento/magento2/pull/22011)_。 [GitHub-22010](https://github.com/magento/magento2/issues/22010)
...
* 管理员可从订单选项卡访问图形订单图表，该图表现在可准确反映订单数量。
...
* 现在，在“创建新文件夹”弹出窗口中输入新文件夹名称时，按下ESC键可按预期关闭弹出窗口，而不创建文件夹。  _由Shankar Konar在拉取请求[25349](https://github.com/magento/magento2/pull/25349)_&#x200B;中提交的修复。 [GitHub-572](https://github.com/magento/magento2/issues/572)
...
* 删除了对`addToolbarBlock`的重复调用，该调用在`ORDER BY`中导致了两个相似的情况。  _由Vasilii Burlacu在拉取请求[27263](https://github.com/magento/magento2/pull/27263)_&#x200B;中提交的修复。 [GitHub-26708](https://github.com/magento/magento2/issues/26708)

<!-- ENGCOM-6014-->

* 商家现在可以对管理员用户帐户设置过期日期。 _由Laura Folco在拉取请求[中提交的修复22837](https://github.com/magento/magento2/pull/22837)_。 [GitHub-22833](https://github.com/magento/magento2/issues/22833)

<!-- ENGCOM-7071-->

* 我们已将`@api` PHP批注添加到`AbstractExtensibleModel`，将`@deprecated` PHP批注添加到`AbstractExtensibleObject`，将`@see`添加到`AbstractExtensibleModel`。  _Alexander Taranovsky在拉取请求[中提交的修复22011](https://github.com/magento/magento2/pull/22011)_。 [GitHub-22010](https://github.com/magento/magento2/issues/22010)
...
* 删除了当之前添加的产品缺货时，使用GraphQL将产品添加到购物车时导致错误的代码。 _AleksLi在拉取请求[中提交的修复27015](https://github.com/magento/magento2/pull/27015)_。 [GitHub-26683](https://github.com/magento/magento2/issues/26683)
...
* 现在，保存在启用了`onthefly`索引器和平面表的商店中的产品会遵循网站分配。 以前，如果您从某个产品中删除了所有网站分配，然后保存了该产品，则无论指定的网站分配如何，所有平面表都包含该产品。  _由Abel Truong在拉取请求[中提交的修复27365](https://github.com/magento/magento2/pull/27365)_。 [GitHub-16315](https://github.com/magento/magento2/issues/16315)
...
* JavaScript mixin模块已重构，以改进高级捆绑模块mixin的加载和应用。 以前，应用程序不会为高级捆绑模块加载或应用mixin。  _由Mateusz Krzeszowiak在拉取请求[中提交的修复25587](https://github.com/magento/magento2/pull/25587)_。 [GitHub-25586](https://github.com/magento/magento2/issues/25586)

<!-- ENGCOM-6344-->

* `grunt clean`命令现在会按预期清除生成的代码和元数据文件夹。 _由Andrii Beziazychnyi在拉取请求[中提交的修复25765](https://github.com/magento/magento2/pull/25765)_。 [GitHub-25739](https://github.com/magento/magento2/issues/25739)
...
* 当网站显示设置指定应排除的税时，订单汇总小计不再显示排除的税。 以前，当使用不同的显示设置配置多个网站时，尽管进行了商店级别配置，但在服务器变量中定义的设置还是会用于商店。

### 支付方式

<!-- MC -->

* 已删除将第三方支付方法Authorize.Net、eWay、CyberSource、Braintree和Worldpay集成到核心代码中。 商家应该迁移到Commerce Marketplace上提供的官方扩展。
...
* 切换账单地址不再导致JavaScript函数文本显示在店面结账工作流程的付款部分中。 _Chris Pook在拉取请求[中提交的修复26378](https://github.com/magento/magento2/pull/26378)_。 [GitHub-26375](https://github.com/magento/magento2/issues/26375)
...
* 现在，您可以将PayPal Express结帐与任何受支持的信用卡结合使用。 以前，在使用PayPal Express结帐时单击信用卡按钮来完成订单时，应用程序挂起，您无法输入任何信用卡信息。

<!-- MC-33117-->

* 现在，当触发欺诈筛选器时，在PayPal Payflow Pro中下达的订单将设置为`Suspected Fraud`状态。 以前，在申请端进行付款审批之前，未验证PayPal上的付款交易状态。
...
* 现在，当Stock数量为1时，您就可以从Admin处成功将产品添加到购物车中。 以前，应用程序没有将产品添加到购物车并显示以下消息： `The requested qty is not available`。 _由Serhii Petrychenko在拉取请求[中提交的修复26650](https://github.com/magento/magento2/pull/26650)。_ [GitHub-25675](https://github.com/magento/magento2/issues/25675)
...
* `getCustomer`方法现在返回在`phpdoc`块中定义的customer对象。 以前，此方法返回`NULL`，`phpdoc`块未突出显示此方法可以返回`NULL`。 _Fanis Strezos在拉取请求[中提交的修复26423](https://github.com/magento/magento2/pull/26423)_。 [GitHub-25268](https://github.com/magento/magento2/issues/25268)
...
* Elasticsearch7.6.x现在是Adobe Commerce和Magento Open Source的默认目录搜索引擎。 如果不安装Elasticsearch7.6.x，则无法安装或升级到版本2.4.0。已删除Elasticsearch版本2.x代码。 Elasticsearch版本5.x和6.x已弃用，不再受支持。 请参阅[搜索引擎先决条件](../../../installation/prerequisites/search-engine/overview.md)。
...
* 当客户在多网站部署中使用店面快速搜索时，该应用程序不会再引发异常。

<!-- ENGCOM-6842-->

* 当商家通过单击&#x200B;**营销** > **SEO和搜索** > **搜索条款**&#x200B;页脚上的搜索条款选项打开搜索条款页面时，应用程序不再显示警告。  _修复由vishal-webkul在拉取请求[中提交25246](https://github.com/magento/magento2/pull/25246)_。 [GitHub-25245](https://github.com/magento/magento2/issues/25245)
...
* 实施自定义`di.xml`的`Magento\Ups\Model\Carrier`插件现在可在开发人员模式下正常工作。 以前，应用程序引发此错误： `1 exception(s): Exception #0 (BadMethodCallException): Missing required argument $data of Magento\Ups\Model\Carrier`。 _由Bartomiej Szubert在拉取请求[26130](https://github.com/magento/magento2/pull/26130)_&#x200B;中提交的修复。 [GitHub-25390](https://github.com/magento/magento2/issues/25390)

### Sitemap

<!-- MC-29362-->

* 现在，在生成站点地图时，应用程序会按预期使用项目基本URL。
...
* 应用程序现在会按预期从`sale_sequence_meta`和`sales_sequence_profile`表中删除与已删除的存储区视图相关的记录。 _由Bartomiej Szubert在拉取请求[22296](https://github.com/magento/magento2/pull/22296)_&#x200B;中提交的修复。 [GitHub-14958](https://github.com/magento/magento2/issues/14958)

<!-- ENGCOM-6869-->

* 现在，在具有不同基本URL的商店之间切换时，商店切换器可按预期工作。 以前，当客户使用商店切换器在各商店之间切换时，如果目标商店的基础URL与当前商店不同，则请求失败。 _Tobias Nilsson在拉取请求[中提交的修复26548](https://github.com/magento/magento2/pull/26548)_。 [GitHub-23755](https://github.com/magento/magento2/issues/23755)，[GitHub-26384](https://github.com/magento/magento2/issues/26384)
...
* 测试现在包括记录与CMS页面相关的管理员操作的任务。 _Lukasz Bajsarowicz在专用存储库合作伙伴magento2ee的拉取请求172中提交的修复_。 [GitHub-171](https://github.com/magento/magento2/issues/171)

<!-- ENGCOM-7040 7051-->

* 无效的功能测试名称已修改为遵循最佳实践。 _修复程序由Lukasz Bajsarowicz在提取请求[27118](https://github.com/magento/magento2/pull/27118)中提交，在专用存储库合作伙伴magento2ee中提交了151个提取请求。 [GitHub-27117](https://github.com/magento/magento2/issues/27117)_

<!-- ENGCOM-7026-->

* Magento Open Source`cron`现在用于在创建测试数据后重新编制索引，在适用的情况下，`cache:flush config`将替换`cache:flush`。 _由Lukasz Bajsarowicz在拉取请求[26990](https://github.com/magento/magento2/pull/26990)_&#x200B;中提交的修复。 [GitHub-26989](https://github.com/magento/magento2/issues/26989)
...
* 您现在无需连接到Internet即可运行`\Magento\Downloadable\Test\Unit\Helper\DownloadTest`单元测试。  _由aleromano89在拉取请求[26264](https://github.com/magento/magento2/pull/26264)_&#x200B;中提交的修复。 [GitHub-23521](https://github.com/magento/magento2/issues/23521)
...
* 已将`<amOnPage>`的所有不正确使用替换为适用于测试中的管理员注销的适当`<actionGroup>`。 _Lukasz Bajsarowicz在专用存储库合作伙伴magento2ee的拉取请求116中提交的修复_。 [GitHub-26331](https://github.com/magento/magento2/issues/26331)
...
* 当设置&#x200B;**管理员** > **商店** > **配置** > **高级** > **开发人员** > **翻译内联** > **已启用店面**&#x200B;时，内联翻译现在可在店面上按预期工作。

<!-- MC-31663-->

* 现在，为店面启用内联翻译后，可按预期工作。

<!-- ENGCOM-7064-->

* 产品URL密钥现已按预期转译。 _由DanieliMi在拉取请求[26506](https://github.com/magento/magento2/pull/26506)_&#x200B;中提交的修复。 [GitHub-26499](https://github.com/magento/magento2/issues/26499)
...
* **排序依据**&#x200B;标签不再与移动视图中类别页面上的&#x200B;**排序依据**&#x200B;按钮重叠。 _由Andrii Kartavtsev在拉取请求[中提交的修复26381](https://github.com/magento/magento2/pull/26381)_。 [GitHub-25300](https://github.com/magento/magento2/issues/25300)

<!-- ENGCOM-7137-->

* 您现在可以通过启用&#x200B;**使用默认值**&#x200B;复选框来更新可下载产品的示例和链接标题的值。 _由Abel Truong在拉取请求[中提交的修复27295](https://github.com/magento/magento2/pull/27295)_。 [GitHub-27169](https://github.com/magento/magento2/issues/27169)

<!-- ENGCOM-6532-->

* 更正了微型购物车上的&#x200B;**更新**&#x200B;按钮和&#x200B;**数量**&#x200B;字段之间的间距。  _Hitesh在拉取请求[中提交的修复26234](https://github.com/magento/magento2/pull/26234)_。 [GitHub-26227](https://github.com/magento/magento2/issues/26227)
...
* WYSIWYG图像上传程序现在使用`pub/media/wysiwyg`作为存储根。 以前，已上传的内容使用`pub/media`作为其存储根目录，导致在最近更改图像上传程序代码后，无法访问这些已上传的图像。 _Pieter Hoste在拉取请求[中提交的修复24878](https://github.com/magento/magento2/pull/24878)_。 [GitHub-22609](https://github.com/magento/magento2/issues/22609)

<!-- ENGCOM-7045-->

* Chrome浏览器上的TinyMCE4现在可按预期处理双字节字符。 以前，TinyMCE4无法在Chrome上正确处理双字节字符（如中文字符）。 _西博和在拉取请求[25454](https://github.com/magento/magento2/pull/25454)_&#x200B;中提交的修复。 [GitHub-24637](https://github.com/magento/magento2/issues/24637)

## 已知问题 — 常规

**问题**：安装了PHP 7.4.2的部署中出现店面错误消息异常。 在PHP 7.4.2中部署2.4.0时，店面错误消息中的空格符号将替换为加号(+)字符。  此错误是PHP 7.4.2固有的错误，无法通过Magento Open Source进行更正。 **解决方法**：我们建议使用其他版本的PHP 7.4.x。请参阅[店面上显示的原始消息数据](https://support.magento.com/hc/en-us/articles/360045804332)知识库文章。<!-- MC-34170-->
...
**问题**：商家无法从管理员创建新订单，因为&#x200B;**按SKU添加产品**&#x200B;和&#x200B;**添加产品**  启用JavaScript捆绑时，订单创建页面中缺少按钮。  **解决方法**：为您的部署禁用JavaScript绑定。<!-- MC-36044-->

**问题**：当客户在签出发送到多个地址的订单时尝试删除奖励点时，应用程序出现`404 not found`错误。<!-- MC-35955-->

**问题**：从客户的愿望列表中编辑可配置产品会导致以下意外行为：“配置产品”页面上显示意外字段，单击&#x200B;**确定**&#x200B;后，“配置产品”页面不会消失。 应用程序还显示以下消息： `Please load Wish List item`。 **解决方法**：重新加载“配置产品”页面。<!-- MC-35617-->
...
**问题**：“管理客户活动”页面的“上次订购的项目”、“比较列表中的产品”和“最近比较的产品”部分中的&#x200B;**刷新**&#x200B;按钮无法按预期工作。 目前，应用程序在每次单击&#x200B;**刷新**&#x200B;按钮时都会滚动页面，并且不显示产品名称。 请参阅[刷新客户活动不起作用](https://magento.zendesk.com/hc/en-us/articles/360046091332)知识库文章。<!-- MC-35296-->
...
您可以使用书写器安装Magento Open Source2.4.0。
