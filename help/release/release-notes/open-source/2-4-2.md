...
在此版本中，将安装配置为使用拆分数据库的功能已被弃用。 当前使用拆分数据库的商家应该开始计划恢复或迁移到单个数据库，或者使用替代方法。 有关此问题的概述，请参阅Magento Open Source](https://community.magento.com/t5/Magento-DevBlog/Deprecation-of-Split-Database-in-Magento-Commerce/ba-p/465187) DevBlog文章中的[弃用拆分数据库功能。  有关迁移说明，请参阅[从拆分数据库还原到单个数据库](../../../configuration/storage/revert-split-database.md)。
...
此版本为以下功能添加了GraphQL覆盖范围：

* 已添加对[比较列表](https://developer.adobe.com/commerce/webapi/graphql/queries/compare-list.html)的支持。 购物者可以[创建](https://developer.adobe.com/commerce/webapi/graphql/mutations/create-compare-list.html)和[删除](https://developer.adobe.com/commerce/webapi/graphql/mutations/delete-compare-list.html)比较列表，以及[添加](https://developer.adobe.com/commerce/webapi/graphql/mutations/add-products-to-compare-list.html)和[删除](https://developer.adobe.com/commerce/webapi/graphql/mutations/remove-products-from-compare-list.html)项到比较列表中。 此外，以访客身份创建比较列表的购物者可以客户身份登录并[保留](https://developer.adobe.com/commerce/webapi/graphql/mutations/assign-compare-list-to-customer.html)他们的比较列表。
...
* 整数类型对象ID已被弃用，取而代之的是类型ID的`uid`个属性。<!-- MC-36346-->
...
* 在此版本中，将安装配置为使用拆分数据库的功能已被弃用。 请参阅Magento Open Source](https://community.magento.com/t5/Magento-DevBlog/Deprecation-of-Split-Database-in-Magento-Commerce/ba-p/465187) DevBlog文章中的[弃用拆分数据库功能。
...
* 已在`EntityAbstract.php`中将`@param string $attribute`更改为`@param AbstractAttribute|string[]|string $attribute`。 [GitHub-30191](https://github.com/magento/magento2/issues/30191)
...
* 当虚拟产品的状态从启用更改为禁用（反之亦然）时，虚拟产品不再更改类型。 以前，当产品状态发生变化时，应用程序会将其类型从虚拟更改为简单。
...
* 店面产品页面现在可以按预期显示层级价格，无需缴税。 [GitHub-12225](https://github.com/magento/magento2/issues/12225)
...
* 系统消息列表现在可准确地反映`cron`运行后产品更新，并且应用程序已按计划更新产品。

<!-- ENGCOM-8164 -->

* 已将基于`schedule_id`和`status`的索引添加到`cron_schedule`表中。 [GitHub-29601](https://github.com/magento/magento2/issues/29601)
...
* 自定义地址多选属性现在可在来宾结帐时以及管理员和我的帐户订单详细信息页面上正确显示。
...
* 已将缺少的`order_data`数组添加到`EmailSender`类。 这使这些类与Magento的建议保持一致，即使用标量变量而不是电子邮件模板中的对象。 [GitHub-29604](https://github.com/magento/magento2/issues/29604)
...
* 向`editorconfig`添加了规则，以支持将JSON和YAML文件缩进两个空间。 [GitHub-30066](https://github.com/magento/magento2/issues/30066)，[GitHub-30063](https://github.com/magento/magento2/issues/30063)

<!-- ENGCOM-8079 -->

* `FlushCacheByTags`类已更新为使用后增效工具，而不是围绕增效工具。 [GitHub-29558](https://github.com/magento/magento2/issues/29558)
...
* 在保存全局范围的类别URL重写时，URL重写生成器现在将类别对象`url_key`、`url_path`和`store_id`设置为默认存储值。 以前，它使用上次处理的存储ID的值保存`url_key`、`url_path`和`store_id`。 [GitHub-29585](https://github.com/magento/magento2/issues/29585)###个分组产品

<!-- MC-35361-->

* 现在， products查询返回分组产品的所有预期数据。 以前，`product_links`是空数组。
...
* 不再为`image_with_border.phtml`设置布尔值。 已从`.product-image-photo`中删除多余的`max-width`和`height`设置。 （这些值已通过重置样式在图像上全局设置。） [GitHub-30186](https://github.com/magento/magento2/issues/30186)

### 导入/导出

<!-- MC-35081-->

* 当您使用已删除某些字段值的CSV文件（`entity type = "customer address"`和`import behavior = "add/update"`）导入客户地址时，不再为客户地址`region_id`分配`NULL`值。
...
* 产品导出CSV文件现在会按预期显示在导出CSV文件的管理员列表中。 以前，运行以下命令时，应用程序引发错误： `bin/magento queue:consumers:start exportProcessor --single-thread --max-messages=10000`。
...
* 我们改进了部分索引器的性能。 应用程序现在会清除每个1000-ID批次迭代更改的高速缓存ID。 缓存上下文不再累积ID。 以前，缓存上下文为每个1000-ID批处理迭代累积缓存ID，并在每个迭代上清除相同的ID集。

<!-- ENGCOM-7929 -->

* 修正了部分再索引系统和完整再索引系统之间的不一致问题。 现在，两个索引系统都使用相同的代码，这样可以正常工作。 以前，两个索引系统对于共享索引器使用不同的逻辑，这可能导致索引器在每次运行部分重新索引系统之后失效。 [GitHub-29478](https://github.com/magento/magento2/issues/29478)，[GitHub-29297](https://github.com/magento/magento2/issues/29297)
...
* 我们已经解决了手动索引器的问题，该问题导致类别页面为空或类别页面上的产品计数减少。 出现以下情况时，出现类别页面不完整或产品计数问题：
   ...
   * 在`catalog_category_product`或`catalog_product_category`索引上执行了部分索引器，同时对其中一个索引执行了完全重新索引。 这两个索引过程可能会发生冲突，从而导致产品从类别页中丢失。

   * 已在大型目录上执行`bin/magento indexer:reindex inventory`。 在`catalogsearch_fulltext`部分索引器完成执行之前，类别页中缺少产品。
...
* 从UTC转换日期对象时，应用程序不再引发严重错误。 以前，在某些情况下，`convertConfigTimeToUtc`方法引发异常。 [GitHub-29525](https://github.com/magento/magento2/issues/29525)

<!-- ENGCOM-7987 -->

* `DataObject`已替换为`\Magento\MediaGalleryCatalogUi\Ui\Component\Listing\Columns\Thumbnail::prepareDataSource`中的产品模型。[GitHub-1711](https://github.com/magento/adobe-stock-integration/issues/1711)
...
* 更正了媒体集的CMS页面的MFTF测试中跳过的问题。 [GitHub-30164](https://github.com/magento/magento2/issues/30164)

#### 新测试

<!-- ENGCOM-7804 -->

* 添加了一个测试，用于启用向管理员中的朋友发送电子邮件。 [GitHub-29145](https://github.com/magento/magento2/issues/29145)
...
* 启用永久购物车后，购物者在其会话Cookie过期后无法再下订单。 相反，应用程序会显示登录页面，以便购物者可以登录。 以前，应用程序会下达订单。 在`PHPSESSID`过期时，应用程序未正确验证永久客户，因此从永久信息重新创建了`customerSession`。
...
* 客户会话过期后，应用程序不再显示客户购物车的内容。 以前，当来宾客户尝试在启用永久购物车时签出时，应用程序会显示此错误： `No such entity with cartid = 0`。

### 定价

<!-- MC-34394-->

* 连续的异步价格更新不再相互干扰，并且将为每项操作分配正确的状态。 以前，如果异步价格更新失败，则后续所有异步价格更新将失败，直到重新启动相应的队列使用者。 [GitHub-27865](https://github.com/magento/magento2/issues/27865)
...
* `dev/tests/integration/testsuite/Magento/Customer/Controller/AccountTest`已改进。 （`assert`现在基于`XPath`选择器，而不是`assertStringContainsString`。） [GitHub-29700](https://github.com/magento/magento2/issues/29700)
...
* 现已在`ApiFunctional TestFramework`中正确解析包含URL的标头值。 [GitHub-26425](https://github.com/magento/magento2/issues/26425)
...
* 为移动视图中的空白主题添加了可折叠导航功能。 [GitHub-30237](https://github.com/magento/magento2/issues/30237)
...
* 购物者输入无效数据时，“创建新帐户”页面上的&#x200B;**创建帐户**&#x200B;按钮将保持活动状态。 以前，此按钮已禁用，这样可防止购物者在出错后重新尝试创建帐户。 这是2.4.1中的一个已知问题。[GitHub-30513](https://github.com/magento/magento2/issues/30513)

<!-- MC-38698-->

* 改进了“管理订单编辑”页面上的装运网格加载。
...
**问题**：`[magento_root]/index.php`文件已被删除，对于Apache配置，应用程序现在默认从`/pub`中运行。 从子文件夹提供的存储将无法按预期工作，并可能会显示404错误。 **解决方法**：使用符号链接将安装模拟到子文件夹中。 以下示例使用`https://shop01.com/shop/`说明如何使用符号链接来模拟子文件夹中的安装。
...
**问题**：当购物者单击&#x200B;**使用Venmo**&#x200B;支付时，应用程序会按预期在Braintree中创建订单，但不会在Admin中创建该订单。 请参阅[2.4.2已知问题：BraintreeVenmo付款不起作用](https://support.magento.com/hc/en-us/articles/360054662652/)知识库文章。 <!-- BUNDLE-2894-->
...
您可以使用[编辑器](../../../installation/composer.md)安装Magento Open Source2.4.2。
