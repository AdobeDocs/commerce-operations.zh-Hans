...
DHL已引入架构版本6.2，并且将在不久的将来弃用架构版本6.0。 支持DHL集成的Adobe Commerce 2.4.4及更早版本仅支持版本6.0。部署这些版本的商家应尽早应用`AC-3022.patch`，以继续将DHL作为装运承运人提供。 [有关下载和安装修补程序的信息，请参阅应用修补程序以继续提供 DHL 作为承运商](https://support.magento.com/hc/en-us/articles/7707818131597-Apply-a-patch-to-continue-offering-DHL-as-shipping-carrier)知识库文章。

## 其他发布信息

尽管这些功能的代码与Magento开源核心代码的季度版本捆绑在一起，但其中一些项目也是独立发布的。 这些项目的 Bug 修复记录在每个项目的文档中提供的单独的、特定于项目的发布信息中。

>[!NOTE]
>
>除 Braintree 之外，所有供应商捆绑扩展均已从 Magento Open Source 2.4.4 中删除。

>[!NOTE]
>
>Composer 2.2 引入了一项安全功能，要求商家在执行插件之前识别其 `composer.json` 文件中的受信任插件。 当前，`composer.json`文件中包含但未标记为受信任的插件会自动安装。 但是，从2022年7月起，Composer将不加载插件，除非它们被明确允许。 有关如何修改`composer.json`文件以明确允许加载插件的说明，请参阅升级到Adobe Commerce 2.4.4](https://support.magento.com/hc/en-us/articles/6215997614093-Composer-plugins-issues-when-upgrading-to-Adobe-Commerce-2-4-4)知识库文章时的[Composer插件问题。

### 此版本中包含的修补程序

Magento Open Source2.4.4包括解决所有问题，这些问题通过为Adobe Commerce和Magento Open Source2.4.3、2.4.3-p1和2.3.7-p2提供的以下修补程序得以解决：

* 已在此版本中解决`MDVA-43395_EE_2.4.3-p1_COMPOSER_v1.patch`和`MDVA-43443_EE_2.4.3-p1_COMPOSER_v1.patch`解决的漏洞。
...
* 在2.3.4中，作为降低安全风险的一部分，为了支持更严格的变量语法，已弃用电子邮件变量用法。 此版本中完全删除了此旧版行为，以继续降低安全风险。
  ...
  因此，在升级到Adobe Commerce 2.4.4或Magento Open Source2.4.4后，可在以前版本的Magento Open Source中使用的电子邮件或新闻稿模板可能无法正常工作。受影响的模板包括自定义模块或第三方扩展中的管理员覆盖、主题、子主题和模板。 即使在使用[升级兼容性工具](https://experienceleague.adobe.com/docs/commerce-operations/upgrade-guide/upgrade-compatibility-tool/overview.html?lang=en)修复已弃用的使用之后，您的部署仍可能会受到影响。 请参阅[迁移自定义电子邮件模板](https://developer.adobe.com/commerce/frontend-core/guide/templates/email-migration/)，以了解有关迁移受影响模板的潜在影响和准则的信息。

* 集成令牌无法再用于API持有者令牌身份验证。 以前，集成令牌可用作基于令牌的身份验证的独立密钥。 但是，由于从未过期的访问令牌的安全影响，默认情况下，此行为已被禁用。 上一个行为可以通过命令行或管理员启用。 请参阅[基于令牌的身份验证](https://developer.adobe.com/commerce/webapi/get-started/authentication/gs-authentication-token/)。<!-- AC-1619 -->

* 会话ID不再存储在数据库中。<!-- AC-522-->

* 现在，在数据库中存储OAuth访问令牌和密码重置令牌时会对其进行加密。<!-- AC-520 1323-->
...
* Magento Open Source店面现在默认启用HTTPS。 默认情况下启用&#x200B;**在店面上使用安全URL**&#x200B;和&#x200B;**在管理员中使用安全URL**&#x200B;设置，并且所有内置Cookie现在都设置为安全。 <!-- AC-1173-->

* 现在所有Magento开源安装都需要依赖混淆插件。 以前，只有基于 Composer 的安装才需要此插件。 它现在允许受信任的版本。 商家可以绕过此插件中设置的禁止某些 Composer 安装组合的约束。 对于受信任的版本，可以绕过约束，Magento Open Source现在会在继续安装之前显示警告。  <!-- AC-501 970-->

* 开发人员现在可以基于每个端点配置Magento开源RESTful端点接受的数组大小限制。 请参阅 [API 安全性](https://developer.adobe.com/commerce/webapi/get-started/api-security/)。 <!-- AC-465-->

* 添加了用于限制用户可在系统范围内通过 Web API 请求的资源大小和数量的机制，以及用于覆盖单个模块上的默认值的机制。 这样就解决了 解决 `MC-43048__set_rate_limits__2.4.3.patch`的问题。 查看[API安全性](https://developer.adobe.com/commerce/webapi/get-started/api-security/)。<!-- AC-1120-->

### 平台增强

Magento Open Source 2.4.4 现在支持 PHP 8.1。所有项目库和依赖项都已更新，以便与 PHP 8.1 兼容。其他平台增强功能包括：

* Magento Open Source 2.4.4 提供了对 Elasticsearch 7.16 和 OpenSearch 1.2 的支持。具有本地托管部署的应用程序开源商家可以使用任一解决方案。 但是，OpenSearch现在是托管在云中的Magento开源2.4.4部署的默认搜索引擎。 所有具有云部署的客户在升级到版本 2.4.4 时都必须使用 OpenSearch。 请参阅[在Cloud 2.4.4](https://support.magento.com/hc/en-us/articles/4419942355725-Switching-to-OpenSearch-for-Adobe-Commerce-on-Cloud-2-4-4)上切换到OpenSearch for Adobe Commerce

* `JQuery`库已升级到版本3.6。`jquery-ui`库已升级到版本1.13.0。其他几个JavaScript库已更新至最新版本。

* 现在支持TinyMCE 5.8.1。 TinyMCE4已从代码库中移除。<!-- AC-41-->

* `RequireJS`库已升级到最新版本(v2.3.6)。 [GitHub-33672](https://github.com/magento/magento2/issues/33672) <!-- AC-40 422-->

* PHPUnit 已升级到最新版本 （9.5.x）。 测试和测试框架已更新为与新版本兼容。 <!-- AC-404-->

* 大多数 Laminas 依赖项已升级到与 PHP 8.1 兼容的最新版本。从代码库中删除了三个 Laminas 依赖项，以减少依赖项的数量。
...
* **性能改进**：
   ...
   * GraphQL购物车操作的性能已得到改进。 现在，在GraphQL请求期间只调用一次`collectQuoteTotals()`方法，这缩短了响应时间。<!-- PWA-2110-->

   * 通过更改GraphQL请求的缓存方式，店面性能得到了改进。 Fastly和Varnish现在缓存随`auth`令牌发送的GraphQL请求。<!-- PWA-1817-->

* **新店面相关的管理员配置设置**。 `storeConfig`查询现在返回“零小计结帐”和“支票汇票”付款方法的配置设置。<!-- PWA-1576-->

* **已更新核心GraphQL库**。 使核心GraphQL能够运行的`webonyx`库已升级到版本^14.9。<!-- PWA-2137 2184 -->

* **修复了包含多站点和多语言存储的GraphQL中的翻译问题**。 GraphQL解析器现在会按预期根据存储范围返回翻译后的字符串。<!-- PWA-1946-->
...
除[Braintree](https://experienceleague.adobe.com/en/docs/commerce-admin/stores-sales/payments/braintree)之外，所有供应商捆绑的扩展都已从Magento Open Source2.4.4代码库中移除。 商家应该迁移到官方扩展，这些扩展在[Commerce Marketplace](https://marketplace.magento.com/extensions.html)上可用。
...
商家现在可以向图像（图像、横幅、幻灯片）添加替换文本(`alt_text`)以增强内容可访问性。 [GitHub-746](https://github.com/magento/magento2-page-builder/issues/746) <!-- PB-1193-->
...
* 提高了整个店面的图像删除和移动图标按钮的对比度，从而提高了低视力用户的可读性。

<!-- AC-1279-->

* 在商家编辑“搜索引擎优化”折叠面板的输入时出现的铅笔图标中添加了替代文本。

<!-- AC-1272-->

* 用于在整个产品界面中执行搜索的放大镜图标已被分配了一个可访问的名称和文本替代项。

<!-- AC-1286-->

* 产品页面 **新视图** 文本输入字段现在具有辅助功能名称。
...
* 最低广告价格&#x200B;**这是什么？购物车页面上的**&#x200B;链接现在可以单击，并按预期打开信息弹出窗口。 以前，单击此链接不起作用。
...
* 访客购物者现在可以使用包含多种类型的自定义客户地址属性的客户地址成功结账。 以前，在签出期间显示时，自定义属性的值的格式不正确。 [GitHub-30290](https://github.com/magento/magento2/issues/30290)

<!-- MC-24379-->

* Magento开源现在可以在启用表费率时将免费送货阈值正确应用于订单。 [GitHub-21832](https://github.com/magento/magento2/issues/21832)

<!-- AC-271-->

* 现在 `POST /V1/guest-carts/:cartId/items` ，您可以使用 将具有不同选项的简单产品添加到购物车。 以前，此调用会更改您添加的第一个简单产品的数量，而不是添加具有指定选项的第二个简单产品。 [GitHub-32302](https://github.com/magento/magento2/issues/32302)

<!-- AC-1089-->

* 现在，在为`Poland`国家/地区和`PLN`货币配置部署时，结账页面的“订单摘要”部分会显示正确的货币和金额。 以前，购物车和结帐页面显示金额 **为 0** 兹罗提。

<!-- MC-32805-->

* Magento开源不再在购物者在迷你购物车中更改产品数量后单击 **更新购物车** 时引发错误。 以前，应用程序开源显示此错误：  `The quote item isn't found. Verify the item and try again`。

<!-- MC-43176-->

* 对特定商店中可配置产品的 GraphQL `products` 查询现在仅返回指定商店中产品的价格范围。 以前，请求返回非指定商店中简单产品的价格范围。
...
* Magento开源现在可以正确计算折扣，当两个购物车规则应用于满足这些条件的订单时：一个购物车规则应用于优惠券代码和 **整个购物车**  条件的固定价格，第二个购物车价格规则没有优惠券和 **产品价格折扣** 条件的百分比。 以前，当以前的购物车规则已对某些产品应用折扣时，相关算法无法在购物车产品之间按比例正确拆分折扣。
...
* 当您使用`addConfigurableProductsToCart`突变将可配置产品的禁用子项添加到购物车时，Magento Open Source不再引发错误。 以前，applicationOpen Source引发完整性约束违规错误。
...
* 当为Magento Open Source配置了&#x200B;**整个购物车的固定金额折扣**&#x200B;购物车价格规则时，价格为0的产品现在可以成功添加到购物车中。 以前，applicationOpen Source显示此错误： `report.ERROR: Warning: Division by zero in /home/mer33515/public_html/ee24develop/app/code/Magento/SalesRule/Helper/CartFixedDiscount.php on line 85`。
...
* 现在，订单电子邮件包含的数据已按照预期使用购物者的区域设置进行了本地化。 以前，订购电子邮件使用管理员区域设置。

<!-- ACP2E-107-->

* 现在，单击任何电子邮件模板的预览，会按照预期在单独的窗口中打开模板预览。  以前，applicationOpen Source同时从“电子邮件模板”网格和弹出窗口中打开模板预览。
...
* 在复制产品时，Magento Open Source现在会根据其定义的范围复制所有产品字段。 以前，**name**&#x200B;和&#x200B;**description**&#x200B;字段已重置为`global`范围。

<!-- MC-42612-->

* 当产品价格为0时，Magento Open Source不再将增量舍入应用于折扣。 以前，applicationOpen Source计算的是负折扣金额。
...
* `magento2/app/code/Magento/Bundle/Pricing/Price/ConfiguredPrice.php`中的类`BundleDiscountPrice`现在已声明为预期类。 以前，applicationOpen Source引发此错误： `Class Magento\Bundle\Pricing\Price\BundleDiscountPrice not found`。 [GitHub-33334](https://github.com/magento/magento2/issues/33334)
...
* `setBillingAddressToCart`突变现在正确使用`same_as_shipping`参数来设置帐单地址，以与预期的送货地址匹配。 以前，applicationOpen Source显示此错误： `The shipping method is missing. Select the shipping method and try again`。 [GitHub-30924](https://github.com/magento/magento2/issues/30924)
...
* 当&#x200B;**Show Telephone**&#x200B;管理员配置设置设为可选时，`setShippingAddressesOnCart`突变现在支持在电话号码值为空的购物车上设置送货地址。 以前，applicationOpen Source引发此错误： `Field CartAddressInput.telephone of required type String! was not provided`。
...
* 不支持的`str_contains`方法已替换为支持的函数。 这是Magento Open Source2.4.3的一个已知问题。[GitHub-33680](https://github.com/magento/magento2/issues/33680)
...
* `gift-card Cms`构件数据已从`Magento/WidgetSampleData` (Magento Open Source)移动到`Magento/GiftCardSampleData` (Adobe Commerce)。 这解决了在PHP8.1上使用示例数据安装(Magento Open Source)的问题。
...
* 改进了`JwsManager`类中的JWK检查，以处理在标头中直接设置算法时的情况。 以前，此检查忽略此有效方案。 [GitHub-32636](https://github.com/magento/magento2/issues/32636)
...
* 已删除未使用的`jquery.hoverIntent` JavaScript库。 [GitHub-33732](https://github.com/magento/magento2/issues/33732) <!-- AC-97-->

* `lib/internal/Magento/Framework/Filter/Money.php`已弃用。 它包含在PHP 8.x中删除的`money_format()`函数。[GitHub-33870](https://github.com/magento/magento2/issues/33870)<!-- magento/magento2/pull/34366)-->

* 删除已弃用、不支持的`grunt-autoprefixer`包。 [GitHub-34037](https://github.com/magento/magento2/issues/34037) <!-- magento/magento2/pull/34032-->
...
* 从Magento Open Source2.4.2-p1升级时，`bin/magento setup:upgrade`现在可按预期运行。 以前，applicationOpen Source引发此错误： `Unable to apply data patch Magento\Paypal\Setup\Patch\Data\UpdateBmltoPayLater for module Magento_Paypal`。 [GitHub-33678](https://github.com/magento/magento2/issues/33678)

<!-- MC-42830-->

* 现在，在管理员下达订单期间，可使用正确的商店ID来检索保存的信用卡。 现在，在根据网站范围配置设置下达订单的过程中，会显示存储的信用卡。 以前，当管理员尝试从管理员为客户创建订单并选择“Stored Credit Card”（存储的信用卡）方法时，没有适用于存储的信用卡的选项。

<!-- AC-344-->

* 解决了礼品卡金额的PayPal信用卡显示异常问题。 以前，启用PayPal信用卡并配置多个礼品卡金额时，如果购物者更改了礼品卡金额的值，店面不会更新分期付款的金额。 这是Magento Open Source2.4.3中的已知问题。
...
* 默认情况下，Magento Open Source在产品保存期间同步调整图像大小。 商家现在可以通过对`di.xml`文件进行细微编辑来作为后台异步流程调整图像大小。
...
* Magento Open Source现在会按预期从店面&#x200B;**HTML** > **我的订单** > **查看订单**&#x200B;页面中删除帐户标记。 以前，applicationOpen Source在storefront客户订单注释部分显示HTML标记。
...
* 价格范围的分层导航选项现在可以按预期方式使用自定义价格属性。 应用程序开源在过滤自定义价格属性时使用价格导航步骤的配置。 以前，应用程序开源使用手动步骤配置。

<!-- MC-42545-->

* 店面现在在分层导航中显示当前类别的所有子类别，而不考虑目录中可用的类别数量。

<!-- MC-41706-->

* Elasticsearch 目录搜索现在不区分变音符号。 以前，搜索没有重音符的字词与搜索带重音符的同一字词会产生不同的结果。

<!-- ACP2E-8-->

* 现在可以对包含特殊字符的 SKU 属性正确设置搜索权重。
...
* 的 `call_user_func_array`测试覆盖率已更新。 [GitHub-34301](https://github.com/magento/magento2/issues/34301)

<!-- AC-1978-->

* 在使用 AWS Elasticsearch 运行时， `Magento\GraphQl\CatalogGraphQl\ProductSearchTest` 测试不再引发此错误： `Magento\GraphQl\CatalogGraphQl\ProductSearchTest::testSearchSuggestions Failed asserting that an array is not empty. /var/www/html/dev/tests/api-functional/testsuite/Magento/GraphQl/CatalogGraphQl/ProductSearchTest.php:94 /var/www/html/dev/tests/api-functional/framework/Magento/TestFramework/TestCase/GraphQlAbstract.php:257`。

<!-- magento/magento2/pull/33699-->

* 通过重命名所有模拟变量以包含 `Mock` 后缀并删除对已弃用的对象管理器类的引用，修复了许多非关键单元测试警告。 [GitHub-33695](https://github.com/magento/magento2/issues/33695)

* 该 `Magento\Framework\MessageQueue\TopologyTest` 测试现在与适用于服务的 AWS MQ [!DNL RabbitMQ] 兼容。 以前，此测试失败，并显示以下消息： `Invalid exchange configuration: magento-topic-based-exchange1 Failed asserting that two arrays are equal`。 <!-- AC-1248-->

<!-- magento/magento2/pull/34454-->

* 单元测试现在与 PHP 8.1 兼容。 [GitHub-34441](https://github.com/magento/magento2/issues/34441)

#### 单元测试

以下单元测试已被重构为使用 `PHPUnit` ，而不是 `AspectMock`：
...
* Magento Open Source现在可以从“联系我们”表单电子邮件输入字段中修剪不间断空格字符。 以前，提交表单时不会从电子邮件输入中删除不间断空格字符（如果给定），这会导致日志文件出错。 生成的发送给商店管理员的联系人电子邮件中也缺少&#x200B;**回复**&#x200B;字段。

<!-- MC-42793-->

* 现在，Magento Open Source会根据相关产品在管理员中的位置来显示相关产品、追加销售产品和交叉销售产品。

<!-- MC-42502-->

* 在模态中使用`Multiselect`时，`.action-close`按钮现在可按预期工作。 以前，`action-close`按钮不起作用，因为它继承了模态的`.action-close`按钮的CSS。 [GitHub-27240](https://github.com/magento/magento2/issues/27240)

<!-- AC-1217-->

* 优惠内容片段现在显示在“产品”视图的主价格字段中。 以前，在产品视图中的相关产品块中的此字段缺少此代码片段，这会导致Google搜索结果显示错误的价格。 [GitHub-34063](https://github.com/magento/magento2/issues/34063)
...
* 向REST API添加新反序列化程序时，不再删除其他反序列化程序。 以前，REST API 接受新的内容类型，但 CORE 为 API 定义的内容类型不再起作用，并返回了 400 错误。 [GitHub-26433](https://github.com/magento/magento2/issues/26433)

<!-- AC-698-->

* 您现在可以删除文件中定义的 `webapi.xml`参数的覆盖。 [GitHub-33843](https://github.com/magento/magento2/issues/33843)

<!-- AC-1212-->

* Magento Open Source 现在可以在 `customertoken` 多次连续失败的登录尝试后按预期生成 by GraphQL 或 REST API 请求。 以前，应用程序开源不会检查 in `oauth_token_request_log` 的值`lock_expires_at`是否大于当前日期和时间，并且始终返回失败的尝试次数，这会阻止客户登录。[GitHub-34067](https://github.com/magento/magento2/issues/34067)
...
* `/rest/all/V1/categories/<above_created_category_id>`  PUT调用现在遵循`available_sort_by`指定的值。  以前，在“类别”页面上选择了“可用产品列表排序依据”下拉列表以及所有可能的选项，并选中了&#x200B;**全用**&#x200B;复选框。 [GitHub-32596](https://github.com/magento/magento2/issues/32596)
...
* 更正了将商店代码添加到 URLS **设置为**“是&#x200B;**”且 REST 调用不包含商店代码时**&#x200B;发生的错误。尽管有效负载有效，但调用返回了以下错误： `Fatal error: Allowed memory size of ********** bytes exhausted (tried to allocate **** bytes) in vendor/magento/module-store/Model/StoreRepository.php on line 75`。
...
**问题**：当在管理员&#x200B;**商店** > **配置** > **常规** > **国家/地区选项** > **允许国家/地区**&#x200B;中未选择任何选项时，购物者无法将产品添加到购物车。 应用程序打开Source改为显示以下控制台错误： `Failed to load resource: the server responded with a status of 400 (Bad Request)`。<!-- AC-2564-->
...
社区工程团队[Magento参与者](https://magento.biterg.io/app/kibana)按月、季度、年维护了主要参与人员和合作伙伴的列表。 从该投稿人页面，您可以点击链接访问GitHub上其合并的PR。

### 合作伙伴的贡献

下表重点说明各伙伴的贡献。 此表列出了提供提取请求的合作伙伴、外部提取请求编号以及与请求关联的GitHub问题编号（如果可用）。
...
您可以使用Composer](../../../installation/composer.md)安装[Magento开源2.4.4。

## 迁移工具包

数据迁移工具有助于将现有的Magento 1.x存储数据传输到Magento 2.x。此命令行界面包括验证、进度跟踪、日志记录和测试功能。 有关安装说明，请参阅 [安装数据迁移工具](../../../tools/data-migration-tool/install.md)。 考虑探索或参与 [数据迁移存储库](https://github.com/magento/data-migration-tool)。

[代码迁移工具包](https://github.com/magento-commerce/code-migration)有助于将现有的Magento 1.x商店扩展和自定义转移到Magento 2.x。命令行界面包括用于转换Magento 1.x模块和布局的脚本。
