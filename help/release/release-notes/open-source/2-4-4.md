...
Magento Open Source2.4.4引入了对PHP 8.1的支持。所有项目库和依赖项都已更新，以便与PHP 8.1兼容。核心编辑器依赖项和第三方库也已升级到与PHP 8.1兼容的最新版本。此版本还支持OpenSearch 1.2。
...
Magento Open Source2.4.4包括针对Adobe Commerce和Magento Open Source2.4.3、2.4.3-p1和2.3.7-p2提供的以下修补程序解决的所有问题：
...
此版本包括一项安全修复和平台安全改进。 此安全修复程序已反向移植到Magento Open Source2.4.3-p2和Magento Open Source2.3.7-p3。
...
* Magento Open Source店面现在默认启用HTTPS。 此 **在店面上使用安全URL** 和 **在管理员中使用安全URL** 默认情况下启用设置，并且所有内置Cookie现在都设置为安全。  <!-- AC-1173-->

* 现在，所有Magento Open Source安装都需要依赖关系混淆插件。 以前，只有基于编辑器的安装才需要此插件。 它现在允许使用受信任的版本。 商家可以绕过此插件中设置的限制，这些限制禁止对Composer安装进行某些组合。 对于受信任的版本，可以绕过约束，现在Magento Open Source会在继续安装之前显示警告。  <!-- AC-501 970-->

* 开发人员现在可以基于每个端点配置Magento Open SourceRESTful端点接受的阵列大小限制。 参见 [API安全](https://developer.adobe.com/commerce/webapi/get-started/api-security/). <!-- AC-465-->
...
Magento Open Source2.4.4现在支持PHP 8.1。所有项目库和依赖项都已更新，以便与PHP 8.1兼容。其他平台增强功能包括：

* Magento Open Source2.4.4支持Elasticsearch7.16和OpenSearch 1.2。部署托管在本地的应用程序开源商家可以使用任一解决方案。 但是，OpenSearch现在是云中托管的Magento Open Source2.4.4部署的默认搜索引擎。 所有使用云部署的客户如果升级到版本2.4.4，则必须使用OpenSearch。 参见 [在Cloud 2.4.4上切换到OpenSearch for Adobe Commerce](https://support.magento.com/hc/en-us/articles/4419942355725-Switching-to-OpenSearch-for-Adobe-Commerce-on-Cloud-2-4-4)
...
但以下各项除外 [Braintree](https://docs.magento.com/user-guide/payment/braintree.html)，则所有供应商捆绑的扩展都已从Magento Open Source2.4.4代码库中移除。 商家应迁移到官方扩展，该扩展可在 [Commerce Marketplace](https://marketplace.magento.com/extensions.html).
...
此版本增强了对标准无障碍准则的符合性。 它包括改进的工具提示、可访问的屏幕元素命名和标记，以及重新设计的图标和按钮。 超过80%的此类修复有助于改善视力缺佳或视力缺佳用户的购物体验。
...
* 当多存储层次结构发生更改时，Magento Open Source不再重定向到所有存储视图范围。 以前，在保存层级后，购物者被重定向到不正确的商店URL，并在多商店部署中选择了所有商店查看范围。

### 捆绑产品

<!-- MC-42885-->

* 购物者现在可以为其购物车添加一个包含固定产品税和两个选项的捆绑产品。 以前，applicationOpen Source未添加产品并显示以下错误： `We can't add this item to your shopping cart right now`.
...
* `cron` 现在会按预期清除产品类别缓存，该过程持续于 `indexer_update_all_views` 执行。 以前，在对类别中的许多产品进行批量更新后，applicationOpen Source在类别页上显示不正确的产品计数。

<!-- MC-42811-->

* 当捆绑产品重新出现在库存中时，Magento Open Source现在会按预期更新类别产品缓存。
...
* 在启用了永久购物车的部署中，如果会话超时，购物者现在可以在订购订单时没有出现错误。 以前，在会话超时后，applicationOpen Source显示付款错误。
...
* 如果更新数量无效，Magento Open Source现在会将购物车产品数量还原为上一个值。
...
* 购物者点击时，Magento Open Source不再引发错误 **更新购物车** 更改迷你购物车中的产品数量后。 以前，applicationOpen Source显示以下错误：  `The quote item isn't found. Verify the item and try again`.
...
* 在从中删除迷你购物车的商店中结帐时，Magento Open Source不再引发控制台错误。 [GitHub-34513](https://github.com/magento/magento2/issues/34513)
...
* 现在，当两个购物车规则应用于满足以下条件的订单时，Magento Open Source可以正确计算折扣：一个购物车规则与优惠券代码一起应用，另一个购物车规则与 **整个购物车的固定价格**  条件和第二个购物车价格规则没有优惠券和 **产品价格折扣百分比** 条件。 以前，当之前的购物车规则已对某些产品应用折扣时，相关算法无法在购物车产品之间正确按比例分摊折扣。
...
* 使用后，Magento Open Source不再引发错误 `addConfigurableProductsToCart` 进行更改，以便在更改商店视图后直接将可配置产品添加到购物车。

<!-- ACP2E-176-->

* 当您使用时，Magento Open Source不再引发错误 `addConfigurableProductsToCart` 变体，用于将可配置产品的已禁用子项添加到购物车。 以前，applicationOpen Source引发完整性约束违规错误。
...
* 已将验证添加到签出工作流中，以将客户的街道地址与默认值进行比较。 如果验证过程标识了更改，则会设置 `customer_address_id` 到 `null` 将其视为新的客户地址。 以前，如果客户在结帐时保存地址而没有进行验证，并且在对结帐字段应用验证后，同一客户尝试使用同一地址下订单，则应用程序开源不接受编辑的地址并引发异常。

### 购物车价格规则

<!-- MC-42288-->

* 现在，在相关的购物车价格规则与一起保存之前，优惠券生成会被阻止 `auto` 选项已启用。 如果您尝试保存此规则但未启用 `auto` 选项，应用程序开源会显示以下消息： `Rule is not saved with auto generate option enabled. Please save the rule and try again`.
...
* 现在，当使用配置Magento Open Source时，可以将价格为0的产品成功添加到购物车 **整个购物车的固定金额折扣** 购物车价格规则。 以前，applicationOpen Source显示以下错误：  `report.ERROR: Warning: Division by zero in /home/mer33515/public_html/ee24develop/app/code/Magento/SalesRule/Helper/CartFixedDiscount.php on line 85`.
...
* 当产品具有，则在产品网格上执行批量属性更新操作时，Magento Open Source不再引发异常 `datetime` 属性。
...
* 添加了对路由器无法实例化从URL收集的操作类时的验证。 应用程序开源现在显示标准错误消息。 以前，applicationOpen Source显示以下错误：  `PHP Error: Cannot instantiate abstract class Magento\Catalog\Controller\Product\Compare`.
...
* 在部分索引过程中，更新子产品不再删除从其父可配置产品继承的目录规则折扣。 以前，当我们将可配置产品分配给某个类别，但未分配给其关联的简单产品，然后使用该类别创建目录规则时，应用程序Open Source在部分索引过程中删除了分配给简单产品的目录规则。
...
* 在部分重新索引期间，Magento Open Source不再删除可配置产品的目录价格规则。 以前，只有子产品在重新编制可配置产品的索引时才会重新编制索引。
...
* 可配置的产品选项标签名称现在基于管理员订单创建期间选定的商店视图。 以前，applicationOpen Source使用默认存储视图中的标签名称。
...
* 尝试将可配置产品的已禁用子项添加到购物车时，Magento Open Source不再引发内部错误。

### 客户

<!-- MC-24204-->

* Magento Open Source现在会在订单创建期间考虑管理员区域设置的网站范围。 以前，订单详细信息（如客户地址属性）只能在多站点部署中在一个网站上正常使用。 [GitHub-23254](https://github.com/magento/magento2/issues/23254)
...
* 当从管理员和添加发货评论时，Magento Open Source的发货详细信息电子邮件中不再包含错误消息 **通知客户** 复选框处于激活状态。 以前，送货确认电子邮件包含此消息： `We're sorry, an error has occurred while generating this content`.
...
* Magento Open Source现在包括 `Content-Disposition: inline` 电子邮件中的标头（预期）。 [GitHub-29258](https://github.com/magento/magento2/issues/29258)
...
* 助手 `Magento\Payment\Helper\Data` 不再在构造函数中创建新布局。 以前，当在自定义命令中使用此帮助程序但未指定区号时，Magento Open Source引发错误。 [GitHub-33908](https://github.com/magento/magento2/issues/33908)
...
* 新的必需选项已添加到 `.htaccess` 和 `nginx.conf` 通过Magento Open Source分发以支持PHP 8.x的文件。 过时的选项也已删除。 [GitHub-34358](https://github.com/magento/magento2/issues/34358)

<!-- MC-42448-->

* 商家现在可以上传多个产品的视频。 现在，仅当上传的文件名包含扩展名时，文件生成过程才会在文件名后附加扩展名。 以前，当商家尝试上传并为两个不同的产品保存相同的视频时，应用程序开源会引发此错误： `Notice: Undefined index: extension in /app/7ha7zds7wvqys_stg/vendor/magento/framework/File/Uploader.php on line 699`. 出现此情况是因为Magento Open Source尝试将文件扩展名附加到名称，而不管上传的文件名是否具有扩展名。

<!-- magento/magento2/pull/34106-->

* 已更正的问题 `sprintf(__())` 和 `sprintf(Magento\Framework\Phrase())` 在整个代码库中构建与PHP 8.x的兼容性。 以前，applicationOpen Source抛出以下类型的错误： `Expected parameter of type 'string', 'Magento\Framework\Phrase' provided`. [GitHub-34085](https://github.com/magento/magento2/issues/34085)
...
* 正在执行 `bin/magento support:backup:code` 现在可按预期创建备份。 以前，applicationOpen Source会创建一个备份，但会引发错误。
...
* 具有的产品 **设置为新** 现在可以成功保存分配了空开始日期和过期结束日期的属性。 以前，当您使用以下设置保存产品时，applicationOpen Source会引发此错误： `Make sure the To Date is later than or the same as the From Date`.
...
* 此 `setBillingAddressToCart` 变异现在正确使用 `same_as_shipping` 参数，用于设置帐单地址以按预期匹配运送地址。 以前，applicationOpen Source显示以下错误： `The shipping method is missing. Select the shipping method and try again`. [GitHub-30924](https://github.com/magento/magento2/issues/30924)
...
* 此 `setShippingAddressesOnCart` 突变现在支持在以下情况下为电话号码值为空的购物车设置送货地址： **显示电话** 管理员配置设置已设置为可选。 以前，applicationOpen Source引发此错误： `Field CartAddressInput.telephone of required type String! was not provided`.
...
* 在店面同时加载图像时，刷新图像缓存时，Magento Open Source不再引发错误。
...
* Magento Open Source现在在重新索引后按预期在店面上显示产品。 以前，当被重新索引的前500个产品有库存，而后500个产品无库存，店面没有显示任何其他产品。
...
* 不支持的 `str_contains` 方法已被替换为支持的函数。 这是Magento Open Source2.4.3的一个已知问题。 [GitHub-33680](https://github.com/magento/magento2/issues/33680)
...
* 当管理员具有只读权限时，Magento Open Source现在显示信息性错误 `pubs/media` 尝试访问包含图像的产品的产品详细信息页面。 以前，applicationOpen Source抛出PHP错误。 [GitHub-32819](https://github.com/magento/magento2/issues/32819)
...
* 助手 `Magento\Payment\Helper\Data` 不再在构造函数中创建新布局。 以前，当在自定义命令中使用此帮助程序但未指定区码时，应用程序Open Source会引发错误。 [GitHub-33908](https://github.com/magento/magento2/issues/33908)
...
* 以下库已被删除，因为Magento Open Source2.4.x支持的所有浏览器都具有对此功能的内置支持：
...
* 现在，管理员订单详细信息页面会按预期加载。 以前，当加载含特定税的订单的订单详细信息页面时，applicationOpen Source会引发以下错误： `Call to a member function getId() on array`.

<!-- MC-38521-->

* 具有包括订单和送货权限的受限制权限的管理员现在可以按预期查看订单页面。 以前，当具有这些权限的管理员尝试查看订单时，applicationOpen Source会引发错误。 [GitHub-14633](https://github.com/magento/magento2/issues/14633)
...
* 已解决PayPal信用卡显示礼品卡金额的异常。 以前，在启用PayPal信用卡并配置多个礼品卡金额时，如果购物者更改了礼品卡价值的金额，店面不会更新分期付款的金额。 这是Magento Open Source2.4.3中的一个已知问题。

### PayPal

<!-- MC-42154-->

* 使用PayPal Express付款方式结帐时，Magento Open Source不再显示错误。 以前，尽管结帐过程已完成，但应用程序开源会显示以下错误： `Something went wrong`.

<!-- AC-722-->

* PayPal Express现在可从购物车中按预期工作。 以前，当您单击 **PayPal** 按钮启动从购物车快速结帐，应用程序Open Source引发此错误：  `To check out, please sign in with your email address`. [GitHub-33445](https://github.com/magento/magento2/issues/33445)
...
* 默认情况下，Magento Open Source在产品保存期间同步调整图像大小。 商家现在可以通过对进行细微的编辑，将图像大小调整为后台异步流程 `di.xml` 文件。
...
* 新 `product_alert` consumer通过创建队列消息、运行使用者并缩短执行时间，改进了客户警报的发送。 以前，当发送超过100,000个产品警报时， applicationOpen Source会引发内存不足异常。 应用程序开源也用了20多个小时来发送所有警报。
...
* Magento Open Source现在启用 **保存** 添加视频时输入不完整的URL时的button和autocompletes字段（管理员） **目录**  > **产品** > **添加视频**)。 以前， **保存** 按钮已禁用，并且字段未填充。
...
* Magento Open Source现在会按预期从店面删除HTML标签 **帐户** > **我的订单** > **查看订单** 页面。 以前，applicationOpen Source在storefront客户订单注释部分显示HTML标签。
...
* 价格范围的分层导航选项现在与自定义价格属性一起按预期工作。 在筛选自定义价格属性时，应用程序Open Source会使用价格导航步骤的配置。 以前，applicationOpen Source使用手动步骤配置。
...
* Magento Open Source现在将正确的价格字段添加到Elasticsearch价格指数映射中。 以前，使用商店ID而非网站ID向索引中添加了不正确的价格字段。
...
* 现在，当购物者在多送货模式下从购物车中删除产品，然后切换到单个送货地址时，Magento Open Source会成功更新迷你购物车。 以前，产品价格未按预期更新。
...
* 对在线货运公司的费率请求已从多个请求减少为一个。 现在，可以正确计算每个项目/包的包保险和处理价格。 以前，applicationOpen Source显示的FedEx配送方式运费不正确。
...
* 现在，在部署中包含位于不同URL上的多个商店时，“以客户身份登录”功能可按预期工作。 以前，即使访问正确的存储域，applicationOpen Source也不会加载正确的存储。

### 税金

<!-- MC-41924-->

* 现在，当客户选择发运到多个地址后删除项目时，Magento Open Source可以正确计算迷你购物车小计。

<!-- MC-41945-->

* 现在，当购物车仅包含虚拟产品时，Magento Open Source会在所有结帐页面上显示相同的税率。
...
* 此 `UserExpiration` 验证器不再失败 `de_DE` 和 `uk_UA` 区域设置。 以前，当管理员尝试在区域设置设置为的情况下从管理员创建新用户时设置到期日期时，applicationOpen Source会引发错误 `de_DE` 或 `uk_UA`. [GitHub-32497](https://github.com/magento/magento2/issues/32497)
...
* Magento Open Source现在会修剪“联系我们”表单电子邮件输入字段中的不间断空格字符。 以前，提交表单时没有从电子邮件输入中删除不间断空格字符（如果给定），这会导致日志文件出错。 此 **回复** 生成的发送给商店管理员的联系人电子邮件中也缺少字段。

<!-- MC-42793-->

* Magento Open Source现在会根据相关产品、追加销售产品和交叉销售产品在管理员中的位置显示这些产品。
...
* 管理员客户网格现在显示所有客户数据，包括新添加的数据 `date` 自定义属性（预期）。 以前，在以下情况下， applicationOpen Source会引发错误，并且无法正确显示管理员客户网格： `date` 属性集被设置为列。
...
* 购物者点击时，Magento Open Source不再显示重复的地址 **更改地址** 在部署中的Review and Payments页面上 **地址搜索** 已启用配置设置。
...
* 现在，各Magento Open Source中的视图断点是一致的。
...
* 使用页面生成器添加的图像在浏览器窗口宽度为768像素时不再在店面上重复。 以前，使用此浏览器窗口宽度时，应用程序开源会同时显示移动版和桌面版的图像，从而复制店面上的图像。
...
* 您现在可以使用的子类创建对象 `\Magento\Framework\Api\AbstractSimpleObjectBuilder` 位于PHP 7.3。以前， `preg_match` 当Magento Open Source在Redhat上使用PHP 7.3托管时，引发此警告消息： `Warning: preg_match(): Compilation failed: unrecognized character follows...`.
...
* Magento Open Source现在生成 `customertoken` 多次连续失败的登录尝试后，按GraphQL或REST API请求执行。 以前，applicationOpen Source不检查 `lock_expires_at` 在 `oauth_token_request_log` 大于当前日期和时间，并且始终返回失败尝试次数，这会阻止客户登录。 [GitHub-34067](https://github.com/magento/magento2/issues/34067)
...
* 当满足以下条件时，Magento Open Source不再在类别侧栏中呈现愿望清单 **在侧栏中显示** 已禁用愿望清单选项。 以前，applicationOpen Source忽略此选项。
...
**问题**：在“管理员”中没有选择选项时，购物者无法将产品添加到购物车 **存储** >  **配置** > **常规** > **国家/地区选项** > **允许国家/地区**. 应用程序开源改为显示以下控制台错误： `Failed to load resource: the server responded with a status of 400 (Bad Request)`. <!-- AC-2564-->
