...
* Magento Open Source现在生成 `customertoken` 按照预期在连续多次失败的登录尝试后发送GraphQL或REST API请求。 以前， applicationOpen Source不检查 `lock_expires_at` in `oauth_token_request_log` 大于当前日期和时间，并始终返回失败的尝试次数，这会阻止客户永远登录。 [GitHub-34067](https://github.com/magento/magento2/issues/34067)
