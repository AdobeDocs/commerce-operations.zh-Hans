...
此版本添加了共享路由的GraphQL覆盖范围。 [路由查询](https://devdocs.magento.com/guides/v2.4/graphql/queries/route.html)和[RoutableInterface](https://devdocs.magento.com/guides/v2.4/graphql/interfaces/routable-interface.html)支持产品、类别和CMS页面上的路由请求。 `urlResolver`查询已弃用，其功能已被`route`查询取代。
...
* 当此区域文件的`di.xml`不包含已注册的插件时，不再为新的自定义区域类型触发全局Magento Open Source插件（例如`webapi_rest`和`graphql`）。
