...
description: 了解2.4.3Magento Open Source版的新增功能、增强功能、错误修复和已知问题。
...
* Laminas库依赖项已升级到PHP 8.x兼容版本。 已从`composer.json`文件中删除一些多余的依赖项。 **Magento Open Source2.4.3使用Laminas 3.4.0**。<!-- MC-39513-->
...
此版本添加了共享路由的GraphQL覆盖范围。 [路由查询](https://developer.adobe.com/commerce/webapi/graphql/queries/route.html)和[RoutableInterface](https://developer.adobe.com/commerce/webapi/graphql/interfaces/routable-interface.html)支持产品、类别和CMS页面上的路由请求。 `urlResolver`查询已弃用，其功能已被`route`查询取代。
...
[升级兼容性工具](https://experienceleague.adobe.com/docs/commerce-operations/upgrade-guide/upgrade-compatibility-tool/overview.html)的范围已根据社区反馈进行了扩展。 加入我们的[#upgrade-compatibility-tool](https://magentocommeng.slack.com/archives/C019Y143U9F)Slack渠道，获得Adobe产品团队和社区的支持，并帮助指导该工具的未来方向。
...
* [卡拉纳](https://marketplace.magento.com/klarna-m2-klarna.html)
...
* 具有受限访问权限（例如，仅被分配对一个网站的访问权限的管理员）的管理员无法再编辑设置为全局范围的类别。

<!-- MC-39037-->

* 生成的系统报告（**系统** > **支持** > **系统报告**）现在已正确呈现。 以前，报告内容未对齐。

<!-- MC-30152 ENGCOM-8625-->

* 在创建捆绑产品期间启用&#x200B;**动态价格**&#x200B;设置时，应用程序现在会按预期关闭&#x200B;**价格**&#x200B;字段上的验证。 以前，在启用&#x200B;**动态价格**&#x200B;设置的情况下，当您从&#x200B;**价格**&#x200B;字段中移除值时，应用程序引发验证错误。 [GitHub-26214](https://github.com/magento/magento2/issues/26214)
...
* 已将 `itemResolvers` 参数添加到目录 `di.xml` 文件。 因此，如果禁用可配置和分组产品模块，则签出不再中断。 [GitHub-30860](https://github.com/magento/magento2/issues/30860)

<!-- ENGCOM-8944-->

* 现在，在管理员重新订购工作流程期间，该应用程序会在“付款和运输信息”**部分中显示单选按钮**。[GitHub-30257](https://github.com/magento/magento2/issues/30257)

<!-- MC-39158 ENGCOM-8493-->

* 现在，当购物车包含具有多个选项的捆绑产品时，应用程序会正确应用具有购物车级别固定折扣的购物车价格规则。 以前，购物车价格规则并不完全应用于订单。 [GitHub-30952](https://github.com/magento/magento2/issues/30952)
...
* 现在，在创建新构件期间，应用程序会显示版面更新中的所有子类别（锚点类别和非锚点类别）。

<!-- MC-39481-->

* 仅为一个商店视图更新产品名称时，`product`查询不再覆盖多商店部署中所有商店视图的默认值。 [GitHub-31083](https://github.com/magento/magento2/issues/31083)
...
* 现在在`\Magento\CatalogRule\Model\Indexer\IndexBuilder::reindexById`和`\Magento\CatalogRule\Model\Indexer\IndexBuilder::reindexByIds`中以相同的方式应用时区。 [GitHub-29549](https://github.com/magento/magento2/issues/29549)
...
* 现在，在并非所有网站都启用异步电子邮件发送的多站点部署中，应用程序会按预期发送电子邮件。 以前，如果至少有一个网站禁用了此设置，那么即使启用了此设置，也不会从任何网站发送电子邮件。 发票、发运和贷项通知单电子邮件中存在类似问题。 但是，已成功发送订单注释、发票注释、装运注释和贷项通知单注释电子邮件。 [GitHub-31950](https://github.com/magento/magento2/issues/31950)

<!-- MC-30127 magento/magento2#31455-->

* 现在，捆绑商品的发票和发票 PDF 包含的价格与预期的价格相同。 以前，发票 PDF 包含捆绑产品的价格不正确。 [GitHub-12856](https://github.com/magento/magento2/issues/12856)

<!-- ENGCOM-9021-->

* 已将字符串强制转换添加到电子邮件模板筛选器方法中，以确保返回值为 `string`. 以前，当在未处于开发人员模式下捕获异常时，应用程序会返回一个 `phrase` 对象。 这反过来又引发了致命的 `Uncaught TypeError`. [GitHub-32671](https://github.com/magento/magento2/issues/32671)
...
* 已改进`ProcessManager`分叉的子进程的异常处理。 现在发生异常时，主进程会退出，并且仅显示一次错误消息。 以前，会记录多个索引器故障，并显示多个消息。 [GitHub-30622](https://github.com/magento/magento2/issues/30622)

<!-- MC-39132-->

* 当此区域文件的`di.xml`不包含已注册的增效工具时，将不再为新的自定义区域类型触发全局Magento Open Source增效工具（例如，`webapi_rest`和`graphql` ）。
...
* 分组产品库存状态现在将按预期更新为缺货，当所有子产品缺货时。 [GitHub-32647](https://github.com/magento/magento2/issues/32647)
...
* 现在，应用程序会将产品税等级 `None` 设置为如果导入产品时带有 `tax_class_name` 值 `None` 或 `0`. 以前，如果`product tax_class_name`在CSV文件中为`None`，则应用程序会创建一个新的税类`None`，该税类与现有的税类重复。 如果CSV文件中的`product tax_class_name`为`0`，则应用程序忽略该值，并且产品税分类在导入后未发生更改。
...
* 当子进程失败时，进程管理器现在退出，并返回错误。 以前，如果传递给进程管理器的函数数（例如，索引器尺寸）低于`MAGE_INDEXER_THREADS_COUNT`环境变量的值，则始终会成功退出进程管理器。 [GitHub-30964](https://github.com/magento/magento2/issues/30964)

<!-- MC-39272-->

* 现在，当索引器模式设置为&#x200B;**保存时更新**&#x200B;时，使用RESTPUT`/V1/products/:sku/links`请求链接产品时，产品在店面搜索结果中可按预期方式使用。
...
* 应用程序不再发送AJAX请求以重新加载不受请求影响的客户数据部分(`Magento_Customer/js/section-config`)。 [GitHub-31948](https://github.com/magento/magento2/issues/31948)

<!-- MC-39569-->

* 在类别中添加、删除或重新排序产品后，以下索引器不再失效： `catalog_category_product`和`catalogsearch_fulltext`（及其依赖项）。 以前，这些意外移除会触发对站点进行完全的重新索引。 现在，当未启用平面目录时，会在这些情况下阻止完全重新索引。
...
* 已从新闻稿增效工具中删除缓存订阅状态。 [GitHub-19345](https://github.com/magento/magento2/issues/19345)
...
* 该应用程序现在在`frontend`上呈现付款块，而不考虑电子邮件的发送区域。 （当前区域现在在渲染付款块之前模拟为`frontend`。） 以前，付款块显示在发送电子邮件的地区。 因此，无论销售电子邮件是从管理员触发还是由REST API触发，资源的URL都会尝试从错误的区域（`webapi_rest`或`adminhtml`）加载它们。

<!-- MC-40810-->

* 现在，该应用程序会将可下载产品的链接发送到在结帐期间指定的电子邮件地址。 以前，当来宾购物者使用PayPal Express结帐并输入不同的电子邮件地址来提交订单和结帐时，应用程序会将可下载的产品链接发送到第一个地址。
...
* 现在，当启用数量递增且小数库存小于1时，会按预期将层价格应用于产品。 以前，最低层级价格数量设置为1。
...
* `/V1/guest-carts/examplecartid/items`调用现在返回请求的存储视图。 以前，它返回存储区中的第一个存储区视图，而不是请求的视图。
...
* 购物者现在可以使用“返回”浏览器按钮返回到“选择配送方式”页面，同时结帐具有多个地址的订单。 以前，应用程序显示的“选择传送方法”页面已损坏。 [GitHub-30268](https://github.com/magento/magento2/issues/30268)

<!-- MC-40021 ENGCOM-8983-->

* 现在，在购物者更改“收货地址”多地址页上的数量并单击浏览器的“返回”按钮后，产品数量将按预期保持不变。
...
* 您可以在一个网站上启用了`United States`，而在其他网站上启用了`Disable all countries`作为默认作用域的多站点部署中成功向管理员下达订单。 以前，应用程序未下订单并显示此错误： `Please check the shipping address information. "regionId" is required. Enter and try again`。

### 存储

<!-- MC-40538-->

* 在显示商店主页时，应用程序不再将`0`字符串视为空值。 以前，应用程序将请求路径开头的整数值视为存储ID，这会对SEO产生意外影响。

<!-- MC-40313-->

* `lib/internal/Magento/Framework/App/Action/Plugin`下的`\Magento\Framework\App\ActionInterface`的插件已被删除，以遵循应该使用插件来自定义另一个模块的一个模块行为的准则。 [GitHub-28050](https://github.com/magento/magento2/issues/28050)
...
* 启用`customer/create_account/vat_frontend_visibility`后，应用程序现在会在客户注册页面上显示&#x200B;**增值税号**&#x200B;字段。
...
* 配置内联转换时，添加到字符串中的括号不再进行转义。 以前，`escapeHtmlAttr`方法会将括号转换为HTML实体代码。

<!-- ENGCOM-8694-->

* 管理员中无法翻译的短语现在可以翻译。 （此提取请求有助于不断努力将所有管理字符串本地化。） [GitHub-11175](https://github.com/magento/magento2/issues/11175)
...
* 现在，在您更改 CMS 页面布局后，应用程序会将正确的主题应用于该页面。 以前，在您保存布局更改后，无论将哪个主题分配给页面，应用程序都会将分配的主题更改为 Luma。
...
下表重点说明各伙伴的贡献。 此表列出了提出拉取请求的合作伙伴、外部拉取请求以及与其关联的GitHub问题编号（如果可用）。
...
您可以使用[编辑器](../../../installation/composer.md)安装Magento Open Source2.4.3。
