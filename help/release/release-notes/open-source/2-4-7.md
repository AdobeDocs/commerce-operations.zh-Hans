...
虽然这些功能的代码与Magento Open Source核心代码版本捆绑在一起，但其中几个项目也单独发布。 这些项目的错误修复记录在单独的特定于项目的版本信息中，这些信息可在每个项目的文档中找到。
...
* **自动生成的优惠券代码数量的限制**。 Magento Open Source现在限制自动生成的优惠券代码数量。 默认最大值为250,000。 商家可以使用新的&#x200B;**[!UICONTROL Code Quantity Limit]**&#x200B;配置选项(**[!UICONTROL Stores]** > **[!UICONTROL Settings:Configuration]** > **[!UICONTROL Customers]** > **[!UICONTROL Promotions]**)，以防止使用许多优惠券使系统不堪重负。<!-- AC-8753 -->
...
* **添加了Subresource Integrity (SRI)支持**&#x200B;以符合PCI 4.0要求验证支付页面上的脚本完整性。 子资源完整性(SRI)支持为驻留在本地文件系统中的所有JavaScript资源提供完整性哈希。 默认SRI功能仅在管理员和店面区域的支付页面上实施。 但是，商家可以将默认配置扩展到其他页面。 请参阅&#x200B;_Commerce PHP开发人员指南_.<!--AC-11513-->中的[子资源完整性](https://developer.adobe.com/commerce/php/development/security/subresource-integrity/)
...
Magento Open Source 2.4.7仍与PHP 8.1兼容，仅用于升级目的。 不支持且不建议使用PHP 8.1。 Magento Open Source 2.4.7核心代码、所有捆绑的扩展以及所有Adobe拥有的扩展和SaaS服务都与PHP 8.3兼容。<!-- AC-9668 -->

* **RabbitMQ 3.13支持**。 此版本与最新版本的RabbitMQ 3.13兼容。兼容性仍与RabbitMQ 3.11和3.12兼容，这两个版本分别支持到2024年8月和2024年12月，但Adobe建议仅将Magento Open Source 2.4.7与RabbitMQ 3.13一起使用。<!-- AC-9659 -->

* **编辑器2.7.x**。 与Composer 2.2.x的兼容性得以保留。<!-- AC-9663 -->

* **清漆缓存7.4支持**。 此版本与最新版本的Varnish Cache 7.4兼容。与6.0.x和7.2.x版本的兼容性保持不变，但我们建议仅将Magento Open Source 2.4.7与Varnish Cache版本7.4或版本6.0 LTS一起使用。<!-- AC-9662 -->
...
* Commerce UPS XML API网关已迁移到新的[Commerce UPS REST API](https://experienceleague.adobe.com/zh-hans/docs/commerce-admin/stores-sales/delivery/shipping-carriers/ups)，以支持UPS正在对其[API安全模型](https://developer.ups.com/oauth-developer-guide)进行的更新。 (UPS正在为所有API实施OAuth 2.0安全模型（持有令牌）。) 所有以前的Commerce UPS XML API已从Magento Open Source2.4.7代码库中删除。<!-- AC-8918 -->

* Magento Open Source[与FedEx的集成](https://experienceleague.adobe.com/zh-hans/docs/commerce-admin/stores-sales/delivery/shipping-carriers/fedex)已从旧版FedEx WSDL Web服务迁移到最新的FedEx RESTful API。 FedEx Web服务跟踪、地址验证和验证邮政编码WSDLS将于2024年5月停用。<!-- AC-1470 -->
...
* 已从核心Magento Open Source代码库中删除了Temando送货模块。 此功能在Magento Open Source2.4.4中已弃用。<!-- AC-8130 -->
...
* **销售规则性能改进**。 通过许多（大约100,000个）有效的销售规则提高了企业部署的性能。 大量实施促销的企业部署通常会部署许多活动的购物车规则。 在结账操作期间，这些类型的运行Magento Open Source 2.4.7的企业部署将不会看到与配置的购物车价格规则数量有关的性能下降。<!-- ACPT-1198 -->
...
* 帐户区域&#x200B;**中的**&#x200B;[!DNL Vaulting]&#x200B;付款 — 已登录的客户现在可以在客户帐户区域[保管库/商店](https://experienceleague.adobe.com/zh-hans/docs/commerce-admin/stores-sales/payments/braintree#step-3-complete-the-advanced-settings)新信用卡/借记卡和[!DNL PayPal]帐户。 以前，客户在结账时保存他们的付款以供以后使用时只能进行保险库/存储，现在他们可以在无需创建新交易的情况下保险库新的信用卡/借记卡和PayPal帐户。
...
* **支持订单取消**。 [cancelOrder](https://developer.adobe.com/commerce/webapi/graphql/schema/orders/mutations/cancel-order/)变异允许客户取消订单，传递其标识符和取消原因。<!-- LYNX-197 -->
...
* **增强了对自定义属性的支持**。 通过丰富API数据以支持所有属性类型，增强了GraphQL自定义属性支持。 GraphQL EAV属性架构现在支持在管理员中扩展客户属性和客户地址对象，以及使用GraphQL检索它们。 特定的增强功能包括： <!-- LYNX-61 110-->

   * 扩展/添加了针对特定领域（如客户和客户地址）的自定义属性支持
   * 添加了自定义属性的缓存
   * 增强了对产品的现有自定义属性支持

* **增强的GraphQL缓存功能提高了页面加载速度**。 缓存功能已添加到这些查询中，提高了大多数PWA页的页面加载速度：
...
* **已改进GraphQL分析器性能**。 通过减少每个请求调用解析方法的次数，GraphQL解析器的性能已得到改进。 现在只调用一次。 以前，解析器至少被调用过三次。
...
* 删除`app/etc/config.php`后，`bin/magento setup:install`现在成功完成。 以前，在安装期间不会重新生成缺少的文件，Magento Open Source会引发错误。 [GitHub-37805](https://github.com/magento/magento2/issues/37805)
...
* 购物者现在可以编辑其店面购物车中捆绑产品的数量。 以前，在店面购物车中编辑产品并显示错误时，Adobe Commerce不会处理`ItemId`的null值。 [GitHub-37696](https://github.com/magento/magento2/issues/37696)

<!-- ACP2E-1801 -->

* 现在，使用包含斜杠 （`/`） 字符的 SKU 可以将捆绑产品成功添加到管理订单中。 以前，管理员用户无法使用此方法将产品添加到管理订单中，并且Magento开源抛出了JavaScript错误。
...
* 改进了为产品属性生成购物车规则过滤器文本。 不再多次加载单个产品。
...
* 对于拥有大地址簿的客户，结帐页面加载时间已缩短。 Magento Open Source现在仅处理&#x200B;**[!UICONTROL Customer Addresses Limit]**&#x200B;设置中指定的客户地址数。 以前，Magento Open Source会加载整个通讯簿。

<!-- ACP2E-2174 -->

* 现在，当一个物料缺货时，可以按预期更改购物车中库存物料的产品数量。 以前，当某个商品缺货时，顾客无法更改该商品的产品数量。

<!-- ACP2E-2269 -->

* `AdvancedSalesRule`产品属性过滤器现在可以正确处理十进制属性后端类型。 因此，`Coupon Type`设置为&#x200B;**[!UICONTROL No Coupon]**&#x200B;的Cart规则现在可按预期工作。
...
* 客户分区Cookie现在将执行Cookie的域设置。 以前，即使共享了会话，迷你购物车在包含子域的商店中也不会按预期更新。

<!-- AC-9340 -->

* 将订单发运至多个地址不再会在区域ID处理期间触发错误。

<!-- AC-8853 -->

* 在购物车级别应用的固定折扣金额现在是正确的。[GitHub-37496](https://github.com/magento/magento2/issues/37496)

<!-- AC-2044 -->

* 现在，当购物者在选择统一费率发运方法（将发运方法设置为条件）后应用优惠券时，优惠券会成功应用于订单。 以前，购物车页面显示“优惠券无效”错误，且未应用优惠券。 [GitHub-34866](https://github.com/magento/magento2/issues/34866)
...
* 现在，当从服务器中删除会话时，本地存储中的迷你购物车和客户数据会重置。 以前，此数据在会话文件被删除时仍然出现。

<!-- ACP2E-1763 -->

* 启用永久购物车后，不再将离散购物车错误地从管理员处合并。

<!-- ACP2E-1580 -->

* 购物车税和配送估算器现在可以准确地反映默认目标配置。 以前，当您配置默认纳税目标计算设置并指定“省/市/自治区”和“ZIP”时，购物车中仅显示&#x200B;**[!UICONTROL Estimate Tax and Shipping]**&#x200B;下的ZIP值。 这是由于Magento Open Source在调用条件语句（用于从管理员设置默认区域ID）之前将区域ID设置为undefined所致。
...
* 从购物车页面提供的配送估计值现在准确。 `collectShippingRates` 现在仅设置一次。 以前，运费估算是重复的。 [GitHub-36648](https://github.com/magento/magento2/issues/36648)

<!-- ACP2E-1417 -->

* 当指定购物车中的产品缺货时，`cart`查询不再返回错误。 以前，使用此查询进行购买的购物者在商品缺货时无法完成购买，并且其结账页面为空。
...
* 现在，只在需要身份验证弹出窗口时，而不是在启用来宾签出时，才初始化身份验证弹出窗口。 现在，当来宾购物者尝试继续结帐且禁用来宾结帐时，它会初始化。 [GitHub-30672](https://github.com/magento/magento2/issues/30672)
...
* 产品缺货状态现在可以在店面上正确显示。 以前，可供发货的产品被错误地标识为缺货。
...
* `cataloginventory_stock`部分重新索引过程的问题已得到解决，索引器的性能已得到提高。 以前，库存和产品类别无法准确更新。

<!-- AC-8226 -->

* 现在，当产品所属的类别已移至新的父级时，Magento Open Source会为该产品生成301重定向。 [GitHub-37039](https://github.com/magento/magento2/issues/37039)
...
* 商户现在可以在配置可配置产品时选择&#x200B;**[!UICONTROL Skip quantity at this time]**&#x200B;和&#x200B;**[!UICONTROL Skip image uploading at this]**，而不会影响关联的简单产品的来源。 以前，选择&#x200B;**[!UICONTROL Skip quantity at this time]**&#x200B;导致产品源消失。
...
* `aggregate_sales_report_bestsellers_data` cron作业不再创建非常大的临时MySQL表，现在已成功完成。 此cron作业现在为每个存储插入数据，而不是同时为所有存储插入数据。 以前，此cron作业可能导致`No space left on device`个错误。

<!-- ACP2E-2427 -->

* 通过阻止cron作业尝试获取不必要的cron作业锁，`cron_schedule`表中的挂起记录数已减少。 以前，cron作业会在运行该作业时太晚时尝试获取锁。 未将`cron_schedule`中的Cron作业标记为错过，即使它们已经过了计划时间。 Magento Open Source还在错误日志中重复显示此错误： `report.WARNING: Could not acquire lock for cron job: indexer_update_all_views`。
...
* Magento Open Source现在会根据配置设置在“编辑客户”页面上显示&#x200B;**[!UICONTROL Date of Birth]**、**[!UICONTROL Tax/VAT Number]**、**[!UICONTROL Gender]**、**[!UICONTROL Telephone]**、**[!UICONTROL Company]**&#x200B;和&#x200B;**[!UICONTROL Fax]**&#x200B;字段。 [GitHub-36196](https://github.com/magento/magento2/issues/36196)
...
* 客户电子邮件地址现在可以包含变音符号。 以前，前端验证禁止在其电子邮件地址中创建带有变音符的用户。 [GitHub-12075](https://github.com/magento/magento2/issues/12075)
...
* `varnish:vcl:generate`命令现在包含`input-file argument`。 这支持将自定义VCL添加到Git存储库(相对于Commerce根)。 [GitHub-37363](https://github.com/magento/magento2/issues/37363)

<!-- AC-10739 -->

* 已从代码库中移除对Varnish 4.x和5.x的支持。 [GitHub-38304](https://github.com/magento/magento2/issues/38304)
...
* `app/etc/env.php`中每个主题的队列连接配置现在可以正常工作。 以前，使用自定义交换创建新消息队列时，产品警报队列无法正确生成。 如果使用自定义交换配置 `app/etc/env.php` 了主题的队列连接，则会为具有该自定义交换的所有主题创建一个新绑定。 商家无法处理产品警报。
...
* 使用`$_ENV`变量的`bin/magento config:show`和`set`命令现在支持使用包含驼峰式大小写或大写字符的网站和商店代码。

<!-- AC-8923 -->

* 添加了在Tailwind 3中使用的具有`!important`的类，以支持CSS布局。 [GitHub-37568](https://github.com/magento/magento2/issues/37568)
...
* 修订了由无效XML配置触发的错误消息，可提供更多信息。 [GitHub-37788](https://github.com/magento/magento2/issues/37788)

<!-- AC-9335 8817-->

* 已重构的已弃用代码，与在整个代码库中创建动态属性有关。

<!-- AC-7009 -->

* 对于产品数据库非常活跃的大型商店，索引器的性能已得到提高。 现在，索引器对每个索引对最新的产品信息编制一次索引，而不是多次。 以前，产品会从网站上消失，并且网站上的产品数据可能会过期。 [GitHub-30012](https://github.com/magento/magento2/issues/30012)
...
* 已添加`AddDataForCostaRica.php`以提供哥斯达黎加的状态信息。 [GitHub-37382](https://github.com/magento/magento2/issues/37382)。
...
* `populateWithArray`函数现在可将对象属性正确转换为蛇形大小写，使其与`AbstractModel` getter和setter兼容。 以前，当数据属性名称一行包含多个大写字母时，Snake大小写格式不正确。
...
* Magento Open Source尝试打印传送标签时，不再在日志文件中引发异常。 以前，如果扩展修改包含附件的响应标头，系统会在日志文件中引发异常。

<!-- AC-7986 -->

* 已向`elements.xsd`添加了更多HTML类支持。 [GitHub-36891](https://github.com/magento/magento2/issues/36891)
...
* 将包含表情符号的CMS内容块保存到数据库后，该表情符号将不再被截断。 以前，由于默认数据库配置不支持四字节字符，因此在emoji之后会截断内容。

<!-- AC-8823 -->

* 登录页面上的&#x200B;**[!UICONTROL Show Password]**&#x200B;复选框处于活动状态时，密码现在会按预期显示。
...
* 启用&#x200B;**[!UICONTROL RSS Feed Top Level Category]**&#x200B;后，在请求类别的RSS源期间不再发生错误。 以前，当启用&#x200B;**[!UICONTROL RSS Feed Top Level Category]**&#x200B;时，浏览RSS源类别页会导致日志文件出现Elasticsearch`CRITICAL`错误。
...
* 现在，当动态定价设置为“否”且应用折扣时，`products`查询返回所有相关店面定价详细信息。 以前，不返回产品的价格范围。 [GitHub-35649](https://github.com/magento/magento2/issues/35649)
...
* 现在，`customer`查询在按存储ID进行筛选时，仅按预期返回与特定存储视图相关的客户评论。
...
* 现在，`categoryList`查询会按预期返回捆绑产品子产品的`options`块中已填充的`product`节。 以前，不返回有关捆绑产品子产品的信息。

<!-- AC-7114 -->

* 现在，`route`查询会按预期返回类别和产品的路由，而不会出现内部服务器错误。 [GitHub-36544](https://github.com/magento/magento2/issues/36544)
...
* `OptionValueProvider`类`get()`方法现在会按预期返回基于给定`option_id`的属性选项值。 以前，它返回错误。 [GitHub-35910](https://github.com/magento/magento2/issues/35910)

<!-- AC-6745 -->

* `products`查询现在返回所有可用的聚合（筛选器）及其正确转换的标签。 以前，价格和类别聚合标签无法按预期进行转换。 [GitHub-36140](https://github.com/magento/magento2/issues/36140)
...
* 已将`category_url_path`添加到`ProductAttributeFilterInput`，这支持在单个查询中请求某个类别的所有产品。 [GitHub-32460](https://github.com/magento/magento2/issues/32460)
...
* 产品导入过程现在使用正确的分隔符解析值。 以前，导入过程使用管道(“|”)运算符来分析多个选择值，直到确定的值分隔符不等于默认值为止。 导入过程随后失败。

<!-- ACP2E-2490 -->

* 现在，当通过CSV导入更新`URL Key`值时，会自动创建301个重定向。

<!-- ACP2E-2129 -->

* 现在，装运表费率会按预期在表费率导入CSV文件中更新。 表速率上载文件将临时存储，直到异步配置处理器cron作业运行。 此cron作业会选取文件的新位置并相应地对其进行处理。 以前，表速率没有按预期更新，因为异步处理器在错误位置查找文件。
...
* Magento Open Source现在在导入客户记录期间会考虑&#x200B;**[!UICONTROL Disable Automatic Group Change Based on VAT ID]**&#x200B;设置。 以前，忽略导入文件中的`disable_auto_group_change`值。 [GitHub-36409](https://github.com/magento/magento2/issues/36409)
...
* 商家现在可以指定显示导入数据的区域设置以及数据验证器应如何解析此数据。 如果未指定区域设置值，Magento Open Source将使用默认配置区域设置（而不是默认的存储视图）来解析数据。 在管理中，导入和导出过程继续使用管理用户界面区域设置，分别导入、解析和设置数据格式。
...
* 在计划的导入操作期间，如果图像未成功导入，Magento Open Source现在会在计划的导入网格中显示信息性错误消息。 以前，不显示错误消息。
...
* 改进了未设置目录规则时`catalogrule_product`索引器的性能。 [GitHub-34784](https://github.com/magento/magento2/issues/34784)

<!-- ACP2E-2204 -->

* 价格索引器性能问题已得到解决。 索引器的性能不再逐渐下降。 索引器已重构，通过更改临时表的创建方式来提高索引器处理速度。 这些临时表现在由更耗时的DELETE语句重新创建。
...
* 在索引器设置为&#x200B;**[!UICONTROL Update on schedule]**&#x200B;时创建自定义产品类型不会再导致索引更新`cron`任务失败。 [GitHub-36471](https://github.com/magento/magento2/issues/36471)

<!-- AC-6816 -->

* 当客户网格包括新创建的自定义客户属性时，`bin/magento indexer:reindex customer_grid`不再因MySQL错误而失败。 [GitHub-36233](https://github.com/magento/magento2/issues/36233)
...
* 向管理员使用启用控制器添加了缺少的`return`语句。 以前，没有`return`语句，并且`FrontendController`类中的控制器处理不正确。 [GitHub-31374](https://github.com/magento/magento2/issues/31374)

<!-- AC-9390 -->

* 已将MySQL `CREATE TEMPORARY TABLE ... LIKE`替换为已重构的`CREATE TEMPORARY TABLE`，以符合MySQL 8.x。[GitHub-37926](https://github.com/magento/magento2/issues/37926)

<!-- ACP2E-1462 -->

* Magento Open Source不再包含向服务器发送格式错误的HTTP请求时显示的错误消息中指向服务器根目录的完整路径。 现在，只有字符串作为第二个参数传递给`explode`方法。
...
* 现在，当`V1/inventory/source-items`更新子产品时，店面上的可配置产品Stock状态会按预期更新。
...
* 商家现在可以为订单创建贷项通知单，其中删除了与可配置产品关联的简单产品。 以前，商家无法创建贷项通知单，Magento Open Source抛出`TypeError`异常。
...
* 当购物者从订单中扣除的产品数量在选择配送源时位于可用产品范围之内时，Magento Open Source在重新加载页面后不再引发错误。

<!-- ACP2E-1259 -->

* 现在，当批量库存更新更新可配置产品及其子产品`Stock Availability`值时，可配置产品库存状态会正确更新。 以前，无法通过批量更新将可配置产品及其子产品退回到Stock中。
...
* 现在，在处理从店面订购的非默认来源以进行发运时，默认和非默认来源都会显示产品可销售数量的正确信息。 以前，Magento Open Source显示的产品来源库存状态不正确。
...
* 现在，可以将文本从一个页面生成器文本编辑器复制并粘贴到另一个页面生成器文本编辑器，操作方式符合预期。 以前，您尝试粘贴文本的页面无法保存，Magento Open Source显示此控制台错误： `[ERROR] Page Builder was rendering for 5 seconds without releasing locks.`。
...
* Magento Open Source现在可以通过阻止在出现此错误时创建订单来正确处理来自PayPal的502错误。 以前，尽管PayPal发生了502错误，导致Magento Open Source记录与PayPal记录不一致，但仍会创建订单。
...
* 现在，在部署中，商家在其Braintree帐户中拥有两个相同货币的不同商家帐户ID，可在正确的帐户中按预期捕获部分开票的订单。

<!-- BUNDLE-3250 -->

* 启用&#x200B;**[!UICONTROL Enable Vault for Card Payments]**&#x200B;后，Magento Open Source在为按Braintree支付的订单创建第二张部分发票时不再引发错误。 以前，当商家尝试创建第二张发票时，Magento Open Source引发此错误： `Your payment could not be taken. Please try again or use a different payment method. Payment method token is invalid`。

<!-- ACP2E-1229 -->

* 当购物者更新其账单地址时，结账工作流程中不再自动取消选择BraintreeVault付款方式选项。 以前，当购物者取消选择“**[!UICONTROL My billing and shipping address are the same]**”按钮时，将取消选择付款方式。
...
* 现在，电话号码会按预期呈现，并且具有Google支付模式弹出窗口送货地址字段中的地址。 以前，电话号码丢失。

<!-- BUNDLE-3228 -->

* 现在，当Braintree响应包含`threeDSecureInfo/eciFlag`的空值时，可使用Braintree成功下达订单。 以前，当该值为空时，在签出期间发生PHP错误。

<!-- BUNDLE-3243 -->

* 当购物者从微型购物车中用Google Pay结帐时，若在订单查看页面上应用优惠券，Magento Open Source不会再引发错误。 以前，购物者会被重定向到404页。

<!-- BUNDLE-3245 -->

* 现在，购物者可以使用Braintree支付和Magento Open Source2.4.6完成结账，并且使用不可见的ReCAPTCHA v2或v3。 [GitHub-37241](https://github.com/magento/magento2/issues/37241)
...
* 当购物者导航到触发301重定向的页面时，除非页面的Cache-Control标头包含`private `，否则仅缓存`200`或`404`响应。 以前，Cache-Control标头中具有`private`的HTTP状态200会被缓存，而HTTP状态301会被缓存，除非其Cache-Control标头中包含`private`。 [GitHub-36492](https://github.com/magento/magento2/issues/36492)
...
* 优化了从选项卡将 **[!UICONTROL Products in Category]** 产品分配到类别的过程。 现在，在网格中完全加载产品之前，**[!UICONTROL Add Product by SKU]**&#x200B;选项卡中的&#x200B;**[!UICONTROL Assign]**&#x200B;和&#x200B;**[!UICONTROL Remove]**&#x200B;按钮处于禁用状态。 以前，管理员用户无法按SKU在包含数百万个已注册产品的目录中添加产品。

<!-- ACP2E-2098 -->

* 使用许多自定义产品属性执行时， `addProductToCart` 突变的性能有所提高。 以前，当使用许多产品属性（大约1000）执行此突变时，Magento开源尝试加载所有产品属性。
...
* 在为订单创建部分发运后，Magento Open Source现在会按预期转移到新的发运页面。 以前，在创建部分发运后，Magento Open Source不会为订单的其余产品创建完整的发运页面。 [GitHub-38063](https://github.com/magento/magento2/issues/38063)
...
* 当管理员用户使用登录作为客户功能时，Magento Open Source不再在使用多个运送地址进行签出时引发异常。
...
* 税费现在已按预期包含在配置设置指定的结帐工作流程运费中。

<!-- AC-9758 -->

* `cart`查询`applied_taxes`响应值现在按预期包含所有单个税收规则。以前，如果配置了多个税则，它会返回总税值。
...
* 街道字段客户帐户编辑地址表单中的街道字段现已正确保存。 现在，这些字段已按表单中的给定键正确排序。 [GitHub-37973](https://github.com/magento/magento2/issues/37973)
...
* 改进了模板渲染期间出现的错误消息传递。 Magento Open Source现在会显示一条信息性消息。 以前，当发生PHP错误时，Magento Open Source显示HTML泄露。
...
* 现在，当包含多选客户属性时，可按预期加载客户注册表单。 以前，Magento Open Source引发此异常： `[2023-03-05T05:57:40.032602+00:00] report.CRITICAL: Exception: Deprecated Functionality: explode(): Passing null to parameter #2 ($string) of type string is deprecated in /magento2ee/app/code/Magento/CustomAttributeManagement/Block/Form/Renderer/Multiselect.php on line 53 in /lib/internal/Magento/Framework/App/ErrorHandler.php:62`。 [GitHub-37189](https://github.com/magento/magento2/issues/37189)

<!-- ACP2E-1602 -->

* 现在，包含单引号的欢迎消息在店面中正确呈现，并且没有控制台错误。 以前，如果欢迎消息包含单引号，则在店面呈现期间不会加载欢迎消息。 Magento Open Source还显示控制台错误。

<!-- ACP2E-1369 -->

* 当您尝试在“管理员顺序”页面中的评论历史记录和返回之间快速更改视图时，Magento Open Source不再引发错误。 以前，Magento Open Source显示此常规错误： `A technical problem with the server created an error`。

<!-- ACP2E-1374 -->

* 权限仅限于一个网站的管理员用户现在可以更新设置为一定 `all stores view` 范围的网站特定产品属性。
...
* 现在可以在发票、装运和贷项通知单 PDF 文档中正确呈现多行可自定义选项。 以前，多行文本前面会出现意外的分页符，如果文本超出页面数，文本将被截断。

<!-- AC-7402 -->

* 现在，当为商店ID与网站ID不匹配的另一个网站创建客户时，管理员用户可以向管理员添加新客户地址。 以前，Magento Open Source显示此弹出消息： `Something went wrong`。 [GitHub-36582](https://github.com/magento/magento2/issues/36582)
...
* 为一个字段选择日期后，日历图标现在按预期显示在管理员&#x200B;**[!UICONTROL Marketing]** > **[!UICONTROL Cart Price Rules]**&#x200B;的日期范围字段中。 以前，为一个字段选择日期后，日历图标不再显示在其他字段中，并且Magento Open Source会显示占位符文本。 [GitHub-36499](https://github.com/magento/magento2/issues/36499)
...
* 现在，在产品库全屏模式下单击视频缩略图即可按预期显示视频。 以前，不显示视频，并且浏览器控制台显示JavaScript错误。 [GitHub-36456](https://github.com/magento/magento2/issues/36456)
...
* 在为自定义存储生成缺失的URL重写时，Magento Open Source不再为根类别和多存储设置中设置为&#x200B;**[!UICONTROL Not Visible Individually]**&#x200B;的产品生成产品URL重写。
...
* REST `V1/carts/mine/estimate-shipping-methods`方法现在将验证所有级别的有效负载字段数据，并为无效数据返回400 HTTP代码。 以前，仅验证第一级API有效负载，如果在有效负载的非验证级别中返回了无效数据，则Magento Open Source会返回500状态代码。

<!-- ACP2E-2053 -->

* 现在，用于更新具有层价格和不同属性集的产品的异步批量REST API操作已成功完成。 以前，无法以这种方式更新具有不同属性集的产品，Magento Open Source显示此错误： `The product was unable to be saved. Please try again`。

<!-- AC-7002 -->

* 现在，当在层价格创建过程中使用无效的客户组名称时，`POST /V1/products/tier-prices`端点会返回一条信息更丰富的错误消息。 以前，Magento Open Source返回类似以下内容的错误： `Notice: Undefined index: XXXXX in ~/public_html/vendor/magento/module-catalog/Model/Product/Price/Validation/TierPriceValidator.php on line 474"`。 [GitHub-36431](https://github.com/magento/magento2/issues/36431)
...
您可以使用[编辑器](https://experienceleague.adobe.com/docs/commerce-operations/installation-guide/composer.html?lang=zh-Hans)安装Magento Open Source 2.4.7。
