...
* 管理配置&#x200B;**中的**[!DNL Braintree]&#x200B;发行说明和支持链接 — 商家现在可以直接从Commerce管理链接到在线Braintree支持和发行说明。
...
* 在导入客户记录期间，Magento Open Source现在会考虑&#x200B;**[!UICONTROL Disable Automatic Group Change Based on VAT ID]**&#x200B;设置。 以前，忽略导入文件中的`disable_auto_group_change`值。 [GitHub-36409](https://github.com/magento/magento2/issues/36409)
...
* 禁用`Magento_AsyncConfig`模块后，电子邮件模板中现在会按预期显示HTML转换的内容。 以前，电子邮件模板仅包含原始HTML。
...
* 当您编辑超链接，然后保存HTML时，Page Builder不再将`tabindex`模板属性添加到超链接标记`a`。
...
* 已优化从&#x200B;**[!UICONTROL Products in Category]**&#x200B;选项卡将产品分配给类别的流程。 现在，在网格中完全加载产品之前，**[!UICONTROL Add Product by SKU]**&#x200B;选项卡中的&#x200B;**[!UICONTROL Assign]**&#x200B;和&#x200B;**[!UICONTROL Remove]**&#x200B;按钮处于禁用状态。 以前，管理员用户无法按SKU在包含数百万个已注册产品的目录中添加产品。

<!-- ACP2E-2098 -->

* 使用许多自定义产品属性执行时，`addProductToCart`突变的性能已得到改进。 以前，当使用许多产品属性（大约1000）执行此突变时，Magento Open Source尝试加载所有产品属性。
...
* REST `V1/carts/mine/estimate-shipping-methods`方法现在将验证所有级别的有效负载字段数据，并为无效数据返回400 HTTP代码。 以前，仅验证第一级API有效负载，如果在有效负载的非验证级别中返回无效数据，则Magento Open Source返回500状态代码。

<!-- ACP2E-2053 -->

* 现在，用于更新具有层价格和不同属性集的产品的异步批量REST API操作已成功完成。 以前，无法以这种方式更新具有不同属性集的产品，并且Magento Open Source显示以下错误： `The product was unable to be saved. Please try again`。
