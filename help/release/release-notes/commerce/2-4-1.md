...
Adobe Commerce 2.4.1引入了性能和安全性的增强功能，以及B2B功能集的显着增加。 安全增强包括对 `SameSite` 属性，并为支付相关和订单相关API端点以及下单店面页面添加了CAPTCHA保护。 B2B改进主要针对订单批准流程、B2B送货方法、扩展了管理员操作的日志记录，以及增强了店面安全性。
...
* **增强的安全扫描工具**. Adobe与 [乐观的安全性](https://sansec.io/)是防止数字窃取的领头羊，将其包含8700个威胁签名的数据库集成到安全扫描工具中。 这种合作关系将允许商家通过主动检测恶意软件和减少误报来实时分析其网站的安全状态。 商户可通过访问 `https://account.magento.com/scanner`. 有关更多信息，请参阅 [使用增强的安全扫描工具保护店面](https://magento.com/blog/magento-news/secure-your-storefront-enhanced-magento-security-scan-tool) 博客文章。

{{cve-notice}}

### 基础架构改进

此版本包含对核心质量的增强，这改进了框架的质量以及这些功能领域：客户帐户、目录、CMS、OMS、导入/导出、促销和定位、购物车和结账、B2B以及暂存和预览。

* **站点范围的分析工具与管理员集成**. 的 [工具](https://docs.magento.com/user-guide/reports/site-wide-analysis-tool.html) 通过提供24/7实时性能监控、报告和自助服务建议，为云基础架构安装Adobe提供系统分析和工具。 商户可以使用新管理员 [角色资源](https://docs.magento.com/user-guide/system/permissions-role-resources.html) 以通过管理员安全地访问其客户详细信息页面。 请参阅 [常见问题解答](https://support.magento.com/hc/en-us/articles/360048646671) ，以查看概述。 <!-- SWAT-807-->
...
页面生成器现在支持全屏模式，该模式支持更轻松地编辑内容，并在整个管理员中提供一致的体验编辑内容。 请参阅 [工作区](https://docs.magento.com/user-guide/cms/page-builder-workspace.html). <!-- PB-543-->

### GraphQL

此版本增加了GraphQL对以下功能的覆盖：
...
* **礼品选项**. 所有客人和客人都可以在订单上添加礼品。 在Adobe Commerce安装中，他们还可以向订单中添加礼品包装、礼品收据和打印卡。 请参阅 [`setGiftOptionsOnCart` 突变](https://devdocs.magento.com/guides/v2.4/graphql/mutations/set-gift-options.html) 和 [`updateCartItems` 突变](https://devdocs.magento.com/guides/v2.4/graphql/mutations/update-cart-items.html) <!-- MC-32345-->

* **奖励积分**. 客户可以对购物车应用或删除奖励点。 他们还可以查看自己的奖励点历史。 请参阅 [`applyRewardPointsToCart`](https://devdocs.magento.com/guides/v2.4/graphql/mutations/apply-reward-points.html) 和 [`removeRewardPointsFromCart`](https://devdocs.magento.com/guides/v2.4/graphql/mutations/remove-reward-points.html) 讨论如何管理购物车中的奖励点数。<!-- MC-23366-->
...
* **存储的付款方法**. 已登录的客户现在可以将付款详细信息(包括Braintree信用卡和PayPalBraintree)存储在我的帐户中。 <!-- MC-32348 35945 35946-->
...
* **克拉娜·GraphQL**. 在 [克拉纳的支付方式](https://devdocs.magento.com/guides/v2.4/graphql/payment-methods/klarna.html) 和 [`createKlarnaPaymentsSession`](https://devdocs.magento.com/guides/v2.4/graphql/mutations/create-klarna-payments-session.html)
...
* 对维尼亚风格指南的更新，该指南适用于设计令牌、排版规则、颜色、核心组件和页面布局 <!-- PWA-519 419-->
...
版本2.4.1引入了B2B v1.3.0。此版本包括对订单批准、发运方法、购物车和管理员操作日志记录的改进。

#### 订单批准改进

B2B订单批准已得到增强，以提高可用性并允许对采购订单执行批量操作。
对订单批准和拒绝的改进包括：

* **为没有编辑权限的用户新建查看规则页面**. B2B购买者现在可以在没有编辑权限的情况下，在新的查看规则页面上查看适用于其公司的规则。 <!-- BUNDLE-104 -->

* **需要我的批准选项卡上的计数警报图标**. “我的采购订单”视图中的“需要我的审批”选项卡现在显示一个计数器，用于指示待定审批活动的数量。 <!-- BUNDLE-102 -->

* **批量订单批准和拒绝**. B2B经理和公司管理员现在可以批量拒绝和批准采购订单。 这些更改允许批准者在一个操作中批准或拒绝多个采购订单。 <!-- BUNDLE-140 128 111 153 -->

* 商户现在可以搜索 **适用于** 和 **需要获得** “我的采购订单”视图的字段，并可在规则创建期间选择多个用户角色。 <!-- BUNDLE-105 106-->

* 有关如何配置订单批准规则的示例，请参阅规则配置页面。 <!-- BUNDLE-103 -->

请参阅 [批准规则](https://docs.magento.com/user-guide/customers/account-dashboard-approval-rules.html)

#### B2B送货方法增强功能

B2B商户现在可以控制提供给每个公司的送货方法。 商户可以从管理员配置以下内容：<!-- BUNDLE-160 161 162 -->

* B2B公司帐户的一组特定发送方法
* 对每个公司帐户使用All或B2B特定发运方法
* 每个公司帐户的B2B运送方法的特定列表

#### 购物车改进

* 商户现在可以允许用户通过单个操作清除其购物车的内容，并且可以在每个网站上独立配置此功能。 <!-- BUNDLE-108 -->

* B2B购买者现在可以直接将单个项目或其购物车的全部内容添加到申请列表。 <!-- BUNDLE-145 144-->

#### 新增管理功能

* B2B商户可以代表客户使用“按帐户付款”作为付款方法从管理员创建订单。 <!-- BUNDLE-166 178-->
* 商家现在可以直接从客户详细信息页面查看与用户关联的所有报价。 <!-- BUNDLE-139 -->
* 商家现在可以按公司筛选“客户现在在线”网格。 <!-- BUNDLE-137 -->
* 管理员现在可以按销售代表在管理员中筛选客户。 <!-- BUNDLE-110 -->

请参阅 [B2B功能](https://docs.magento.com/user-guide/configuration/general/b2b-features.html).

#### 增强了店面的安全性

为了减少创建欺诈性或垃圾邮件帐户的次数，商户现在可以在店面的“新公司请求”表单上启用Google reCAPTCHA。 请参阅 [reCAPTCHA](https://docs.magento.com/user-guide/configuration/security/google-recaptcha-storefront.html).<!-- BUNDLE-154 -->

#### 扩展了管理员操作的日志记录

现在，在公司模块中执行的管理操作将记录在管理操作日志中。 操作会从所有相关的公司模块中记录： `Company`, `NegotiableQuote`, `CompanyCredit`, `SharedCatalog`.
<!-- BUNDLE-180 181 182 183 -->

此版本还包含多个错误修复。 请参阅 [B2B发行说明](https://experienceleague.adobe.com/docs/commerce-admin/b2b/release-notes.html).
...
* 现在，当运行后未部署某些主题时，应用程序会显示一则信息性错误消息 `bin/magento setup:static-content:deploy`. 以前，当部署成功完成但并非部署了所有包时，应用程序不会显示错误。 如果在启用并行处理的情况下执行此命令，并且每个主题需要更多时间来部署，然后达到指定的最大执行时间，则此命令可以成功完成，尽管主题未部署。
...
* 您现在可以使用 `bin/magento sampledata:deploy` 使用编辑器安装Adobe Commerce后，按预期部署示例数据。 以前，应用程序会引发以下错误： `Git installations must deploy sample data from GitHub; see https://devdocs.magento.com/guides/v2.3/install-gde/install/sample-data-after-clone.html for more information`. _由Andrii Beziazychnyi在拉取请求中提交的修复 [27481](https://github.com/magento/magento2/pull/27481)_. [GitHub-19481](https://github.com/magento/magento2/issues/19481)
...
* 正在执行 `bin/magento setup:upgrade` 现在按预期完成。 以前，应用程序显示用于缓存的打印阵列内容。 _修复了由Sathish Subramanian在拉取请求中提交的问题 [27567](https://github.com/magento/magento2/pull/27567)_. [GitHub-27091](https://github.com/magento/magento2/issues/27091)
...
* 应用程序不再显示 **添加属性** 按钮(**商店** > **属性** 或 **添加属性集** 按钮(**商店** > **属性** > **客户**)时，登录管理员缺少创建这些实体的相应权限。 以前，当没有相应权限的网站管理员尝试创建 **属性集** 或 **客户** 属性。

<!-- MC-36230-->

* 当对特定网站具有受限角色的管理员尝试从管理员创建子类别时，应用程序不再引发错误。
...
* 的 **在** Adobe Stock图库图像详细信息页面的部分现在可准确识别图像是否与产品关联。 _由Nazar Klovanych在拉取请求中提交的修复 [28798](https://github.com/magento/magento2/pull/28798)_. [GitHub-1474](https://github.com/magento/adobe-stock-integration/issues/1474)
...
* 单击 **在** “媒体集”中图像的部分现在会打开按图像过滤的实体网格。 以前，图像标题不会显示在网格的已应用过滤器部分中。 _由Nazar Klovanych在拉取请求中提交的修复 [29429](https://github.com/magento/magento2/pull/29429)_. [GitHub-1694](https://github.com/magento/adobe-stock-integration/issues/1694)
...
* 拥有正确权限的管理员现在可以访问高级报表和区段报表。

<!-- MC-34352-->

* Adobe Commerce成功生成了预先报告数据文件，并在具有拆分数据库的部署中按预期将其发送到清单。 以前，应用程序不会生成或发送 `quotes.csv` 文件到库存，因此库存未生成预期报表。

### Braintree

* 在结帐页面上应用促销代码后，Braintree现在会向PayPal发送正确的金额。

* 在结帐页面上启用了条款和条件后，Apple Pay现在可以按预期工作。

* 在从管理员中启用维姆诺后，在桌面设备上结帐期间不再出现浏览器错误。

* 在 **名称** 结帐工作流的字段。 以前，身份验证失败，因为Braintree3DS API不支持非ASCII字符。

* 使用PayPal下订单时，应用程序现在在结帐工作流的发运部分显示正确的收件人名称。

* 当购物者在结帐期间多次更改送货方法时，应用程序现在会按预期更新“订单审核”页面。
...
* GraphQL现在支持订购选项类型的捆绑产品 `radio` 和 `dropdown` 有多种选择。 以前，应用程序会显示一条消息，说明 `BundleItem.type: radio/dropdown`. _由Michał Derlatka在拉取请求中提交的修复 [29256](https://github.com/magento/magento2/pull/29256)_. [GitHub-26110](https://github.com/magento/magento2/issues/26110)
...
* 更新了Adobe Commerce徽标，并从README文件中删除了无关空格。 _拉取请求中由Rafael Corêa Gomesr̻提交的修复 [28891](https://github.com/magento/magento2/pull/28891)_. [GitHub-29056](https://github.com/magento/magento2/issues/29056)
...
* 当商家使用与“公司结构”页面相同的URL创建CMS页面时，应用程序现在会引发错误。 以前，应用程序会显示CMS页面，而不是公司结构页面。
...
* 消息队列用户配置已扩展，新参数有助于控制用户并保存服务器资源，并且可能会降低用户队列CPU消耗。 请参阅 [配置消息队列](https://developer.adobe.com/commerce/php/development/components/message-queues/configuration/) 对 `maxIdleTime`, `sleep`和 `onlySpawnWhenMessageAvailable` 参数。
...
* 当新客户单击 **创建帐户** 按钮。 以前，应用程序未创建客户帐户，并在客户单击按钮时显示错误。

<!-- MC-34024-->

* 当客户未输入任何数据时，结帐工作流不再显示自定义客户地址属性值。
...
* 对批量操作进行了重构以异步保存和刷新客户区段，这改进了包含许多客户（超过3,000,000位）的部署中这些任务的性能。

<!-- MC-36224-->

* 现在，客户区段条件在拆分数据库部署中可以按预期工作。 以前，当您尝试通过添加条件来编辑客户区段时，应用程序会引发错误： `SQLSTATE[42S02]: Base table or view not found`.
...
* 现在，当您创建动态块并添加相关目录价格规则时，表标题与数据表（按预期）匹配。
...
* 发送给分配给公司的销售代表的通知电子邮件现在包括分配的公司徽标。 以前，通知电子邮件包含默认的LUMA徽标，而不是上传的公司徽标电子邮件。
...
* 现在，当“订单”列表跨多个页面时，购物者可以更改每页显示的订单数。 以前，当您导航到订单的最后一页并尝试更改每页显示的订单数时，应用程序会显示此消息： `You have placed no orders`. 这是2.4.0的已知问题。
...
* 如果将CMS页面分配给多个存储视图，则应用程序不会再在站点层次结构中多次显示该页面。
...
* 礼品卡帐户现在可以按预期捕获订单编号。 以前， **更多信息** “历史记录”选项卡中选定礼品帐户的字段未显示订单ID。

<!-- MC-34519-->

* 现在，使用逗号作为小数分隔符可按预期工作。 以前，逗号分隔符会忽略小数值。
...
* 贷项通知单现在可以正确反映涉及折扣产品的订单的总计，以及通过礼品卡和商店贷项组合支付的订单总计。

### Google标签管理器

<!-- MC-33729-->

* 当 **Cookie限制模式** 设置和Google标签管理器。

### GraphQL

<!-- ENGCOM-7661 7662 7559 MC-35646 32345-->

* 购物者在结帐期间可以选择礼品消息和包装选项。 GraphQL现在包含不同类型购物车项目的礼品消息选项。 请参阅 [`setGiftOptionsOnCart` 突变](https://devdocs.magento.com/guides/v2.4/graphql/mutations/set-gift-options.html) 和 [`updateCartItems` 突变](https://devdocs.magento.com/guides/v2.4/graphql/mutations/update-cart-items.html). _修复了Oleh Usik在拉取请求中提交的问题 [28519](https://github.com/magento/magento2/pull/28105), [27956](https://github.com/magento/magento2/pull/27956), [28072](https://github.com/magento/magento2/pull/28072), [28072](https://github.com/magento/magento2/pull/28072)、和246（在专用repo partners-magento2ee中）_. [GitHub-253](https://github.com/magento/magento2/issues/28519)
...
* 客户可以对购物车应用或删除奖励点。 他们还可以查看自己的奖励点历史。 请参阅 [`applyRewardPointsToCart`](https://devdocs.magento.com/guides/v2.4/graphql/mutations/apply-reward-points.html) 和 [`removeRewardPointsFromCart`](https://devdocs.magento.com/guides/v2.4/graphql/mutations/remove-reward-points.html) 讨论如何管理购物车中的奖励点数。 _由Petkovski Marjan在拉取请求285中提交的修复，在私有存储库合作伙伴 — magento2ee和Dmitriy Gallyamov的拉取请求284和281中提交到私有存储库合作伙伴 — magento2ee_. [GitHub-28835](https://github.com/magento/magento2/issues/28835), [GitHub-28833](https://github.com/magento/magento2/issues/28833)
...
* 现在，对于相关产品的GraphQL查询将返回从target规则创建的相关产品的值。 以前，仅当从产品设置中添加了相关产品时，才会查询相关产品（向上销售和交叉销售）返回值。 _修复了Ulzii在private repo partners-magento2ee的拉取请求288中提交的问题_. [GitHub-28566](https://github.com/magento/magento2/issues/28566)
...
* GraphQL现在只使用授权令牌来检索类型的用户及其ID。 以前，GraphQL在授权令牌为空时使用活动客户的Cookie来检索此信息。 _由Alexander Taranovsky在拉取请求中提交的修复 [27373](https://github.com/magento/magento2/pull/27373)_. [GitHub-28040](https://github.com/magento/magento2/issues/28040)
...
* `updateCustomer` 不再允许您设置无效 `INT` 值 `gender` 参数。 _由Alexander Taranovsky在拉取请求中提交的修复 [28487](https://github.com/magento/magento2/pull/28487)_. [GitHub-28481](https://github.com/magento/magento2/issues/28481)
...
* 从中删除了不必要的代码注释 `app/code/Magento/CatalogInventory/Model/StockState.php`. _修复了由Vitaliy Prokopov在拉取请求中提交的问题 [27758](https://github.com/magento/magento2/pull/27758)_. [GitHub-26702](https://github.com/magento/magento2/issues/26702)
...
* 色板分层导航过滤器现在与整个Adobe Commerce中使用的标准过滤器一致。 _由Bartłomiej Szubert在拉取请求中提交的修复 [28015](https://github.com/magento/magento2/pull/28015)_. [GitHub-28011](https://github.com/magento/magento2/issues/28011)
...
* 添加了测试以涵盖 **在中使用** 媒体库中图像的链接。 _由Nazar Klovanych在拉取请求中提交的修复 [29392](https://github.com/magento/magento2/pull/29392)_. [GitHub-1963](https://github.com/magento/adobe-stock-integration/issues/1693)
...
* Newsletter订阅电子邮件现在使用与其他Newsletter相关电子邮件相同的HTML标记。 _由Paweł Tylek在拉取请求中提交的修复 [27357](https://github.com/magento/magento2/pull/27357)_. [GitHub-28166](https://github.com/magento/magento2/issues/28166)

<!-- ENGCOM-7788-->

* 添加测试，以将新闻稿订阅者删除为管理员用户。 _修复了由Dmitry Tsymbal在拉取请求中提交的问题 [28972](https://github.com/magento/magento2/pull/28972)_. [GitHub-29032](https://github.com/magento/magento2/issues/29032)
...
* 的 `CustomerAddressI` 现在，新创建客户的值将用引号进行验证。 _由Andrii Kalinich在拉取请求中提交的修复 [29139](https://github.com/magento/magento2/pull/29139)_. [GitHub-28793](https://github.com/magento/magento2/issues/28793)
...
* 的 `description` 和 `short_description` 已重新排序产品属性，以更好地适应页面生成器内容暂存的计划更改。 _修复了由Matt Walters在拉取请求中提交的问题 [29238](https://github.com/magento/magento2/pull/29238)_. [GitHub-543](https://github.com/magento/magento2-page-builder/issues/543)
...
* 现在，在您为退货授权(RMA)创建送货标签后，“退货”页面会按预期工作。 以前，商家在为RMA创建送货标签后，无法与“退货”页面上的任何页面元素进行交互。 这是2.4.0的已知问题，以及 `MC-35984-2.4.0-CE-composer.patch` 已在2.4.0中解决此问题。

<!-- MC-35826-->

* 现在，当具有受限权限的管理员尝试为不再位于所分配网站范围内的产品创建返回时，应用程序会显示一条信息性错误消息或所有可用产品。 以前，应用程序会显示一个空白页面。
...
* 应用程序不再分配状态 `Complete` 开具发票后，需要零付款的订单。
...
* Elasticsearch结果现在包含每个存储视图的属性选项的正确值。 如果“下拉列表”或“多选”属性在非默认存储视图中与默认存储视图中具有不同的选项值，则Elasticsearch现在会为该值编制索引或在结果中返回具有该值的产品。 以前，Elasticsearch不会将该值编入索引，也不会在结果中返回具有该值的产品。
...
* 现在，客户在使用多个地址签出时，可以在结帐工作流的订单审核页面上成功删除奖励点。 以前，应用程序会抛出 `404 Not Found` 客户单击 **删除** 按钮，删除任何地址的点。 这是2.4.0中的已知问题。
...
* 的 **将产品设置为新** 现在，属性标签会按预期在产品的详细信息和计划更新页面上显示。

<!-- MC-34484-->

* 您现在可以成功预览CMS块的暂存计划。 以前，应用程序会引发致命错误。

<!-- MC-33789-->

* 现在，在编辑包含计划更新的CMS页面时，您可以访问“CMS预览”页面，并将其从默认存储重新分配到其他存储。 以前，当您单击预览链接时，应用程序会引发404错误。

<!-- MC-33572-->

* 您现在可以使用POST `/V1/products/special-price` 更新产品的特殊价格，而不指定 `price_to` 参数值。 以前，会尝试在 `price_to` 指定的参数失败。
...
* 应用程序现在检查区域(前端或 `adminhtml`)。 以前，对 `/rest/V1/integration/admin/token` 启用内联翻译时，凭据不正确，导致在API响应中错误消息周围包含内联翻译标记。 _由Zach Nanninga在拉取请求中提交的修复 [28856](https://github.com/magento/magento2/pull/28856)_. [GitHub-28656](https://github.com/magento/magento2/issues/28656)
...
* 删除了未使用的 `AdminAnalytics` 测试(`TrackingScriptTest`)。 _Lukasz Bajsarowicz在拉取请求中提交的修复 [28605](https://github.com/magento/magento2/pull/28605)_. [GitHub-28850](https://github.com/magento/magento2/issues/28850)
...
* 在“可视化促销商”中对产品进行排序并启用“库存”后，应用产品现在会显示源库存而不是默认的产品库存。

### Web API框架

<!-- MC-36084-->

* 现在，使用REST创建的发票包括礼品卡信息，与在“管理员”中创建的发票类似。 以前，使用POST `/V1/order/:id/invoice` 要开票，订单未显示已核销的礼品卡代码或礼品卡金额。

<!-- MC-35838-->

* 具有多个网站的商户现在可以使用REST API创建和更新产品，同时保留图像和图像角色继承。 以前，当商家使用REST API创建和更新产品，并更新产品以用于商店视图，并且为该商店视图加载和保存默认图像角色时。 因此，在更新后，存储视图图像角色不再继承默认范围。
...
* 启用网站限制不再阻止以来宾身份登录的客户的清漆ESI请求。 以前，这些来宾无法访问主页主菜单。

<!-- MC-32369-->

* 现在，在启用网站限制时，应用程序会按预期加载产品主页。

### 愿望列表

<!-- MC-35810-->

* 现在，您可以在启用了支持多个愿望列表的部署中使用愿望列表搜索功能在公共愿望列表中查找产品。 以前，客户使用希望列表搜索来查找产品，选择该产品，然后单击 **添加到购物车**，应用程序未将产品添加到购物车并显示此错误： `Invalid Form Key. Please refresh the page`.
...
* 店面类别页面现在可按预期显示希望列表搜索小组件。

<!-- ENGCOM-7580 7285-->

* 用于的填充 `Map`, `WeakMap`, `FormData`和 `MutationObserver` 现在只会根据需要加载。 _修复了由Dmitry Tsymbal在拉取请求中提交的问题 [28330](https://github.com/magento/magento2/pull/28330) 和马特乌什·克尔泽索维亚克的拉车请求 [27618](https://github.com/magento/magento2/pull/27618)_. [GitHub-28377](https://github.com/magento/magento2/issues/28377), [GitHub-28382](https://github.com/magento/magento2/issues/28382)
...
* 现在，应用程序可正确验证购物者可以创建的最大愿望列表数。 _由Eduard Chitoraga在专用存储库合作伙伴 — magento2ee的拉取请求247中提交的修复_.
