...
* `braintree-disabled-partial-capture-for-applepay-googlepay.patch`.  对于通过Braintree以Apple Pay支付的订单，商家无法提交部分退款。 当商家尝试从订单发票创建部分退款的贷项通知单时， **要退款的数量** 字段不可编辑。 此修补程序解决了该问题。 请参阅 [Adobe Commerce 2.4.4：无法创建部分发票](https://support.magento.com/hc/en-us/articles/4487952754957-Adobe-Commerce-2-4-4-Unable-to-create-partial-invoices) 知识库文章。  <!-- BUNDLE-3088-->
...
* Adobe Commerce店面现在默认启用HTTPS。 此 **在店面上使用安全URL** 和 **在管理员中使用安全URL** 默认启用设置，并且所有内置Cookie现在都设置为安全。  <!-- AC-1173-->

* 现在，所有Adobe Commerce安装都需要依赖项混淆插件。 以前，仅基于编辑器的安装需要此插件。 它现在允许使用可信版本。 商家可以绕过此插件中设置的限制，这些限制禁止对Composer安装进行某些组合。 对于受信任的版本，可以绕过约束，Adobe Commerce现在会在继续安装之前显示警告。  <!-- AC-501 970-->
...
* 新 **在购物车加载时启用库存检查** 配置选项在购物车中加载产品时提供可切换的库存检查。 默认情况下处于启用状态。 禁用此选项后，Adobe Commerce会在加载报价时跳过库存检查，从而加快结帐速度，尤其是对于包含许多项目的购物车。
...
* 通过延迟总计算来优化结账时的销售规则处理。 通常，每当购物者将产品添加到购物车、更新产品数量或单击  **下单** 按钮。 商家可以通过设置 `checkout/deferred_total_calculating` 中的变量 `env.php` 文件。 或者，您可以运行 `bin/magento setup:config:set --deferred-total-calculating 1|0`.  <!-- MCP-573-->

* 改进了异步下单期间受购物车价格规则影响的订单的验证流程。 <!-- MCP-304-->
...
* **适用于可转让报价的完整GraphQL覆盖范围**.  B2B公司用户现在可以使用GraphQL完成与可协商报价相关的所有任务。 此API的早期版本支持协商流程，但不支持签出。 <!-- PWA-2101-->
...
但以下各项除外 [Braintree](https://docs.magento.com/user-guide/payment/braintree.html)，则所有供应商捆绑的扩展都已从Adobe Commerce 2.4.4代码库中移除。 商家应该迁移到官方扩展，该扩展可在 [Commerce Marketplace](https://marketplace.magento.com/extensions.html). <!-- AC-1165-->
...
PWA Studio v.12.3.0与Adobe Commerce 2.4.4兼容。它包括reCaptcha、Page Builder内容优化和个性化内容支持。 有关增强功能和错误修复的信息，请参阅 [PWA Studio版本](https://github.com/magento/pwa-studio/releases). 请参阅 [版本兼容性](https://developer.adobe.com/commerce/pwa-studio/integrations/adobe-commerce/version-compatibility/) 有关PWA Studio版本及其兼容的Adobe Commerce核心版本的列表。
...
此版本提高了对标准无障碍指南的合规性。 这些增强功能可改善视力障碍或语言、认知和学习能力受限的用户体验。  屏幕Reader工具的性能也得到了提高。
...
* 现在可以从命令行成功更改Sitemap生成器配置。 以前，当您尝试在管理员之外更改Sitemap配置时，Adobe Commerce显示此错误： `Import failed: Notice: Trying to access array offset on value of type null in app/code/Magento/Cron/Model/Config/Backend/Sitemap.php on line 78`.  [GitHub-31428](https://github.com/magento/magento2/issues/31428)
...
* 管理员现在可以在配置了自定义管理员路径且启用了密钥的部署中登录到管理员。 以前，Adobe Commerce会显示此错误： `Invalid security or form key. Please refresh the page`.
...
* 购物者现在可以为其购物车添加捆绑产品（具有固定产品税和两个选项）。 以前，Adobe Commerce不添加产品并显示此错误： `We can't add this item to your shopping cart right now`.
...
* `cron` 现在，会按预期清除产品类别缓存，该过程持续于 `indexer_update_all_views` 执行。 以前，在对类别中的许多产品进行批量更新后，Adobe Commerce会在类别页面上显示不正确的产品计数。
...
* 当购物者单击时，Adobe Commerce不再引发错误 **更新购物车** 更改迷你购物车中的产品数量后。 以前，Adobe Commerce会显示此错误：  `The quote item isn't found. Verify the item and try again`.
...
* 在从已删除mini cart的商店中结帐时，Adobe Commerce不再引发控制台错误。 [GitHub-34513](https://github.com/magento/magento2/issues/34513)
...
* 使用后，Adobe Commerce不再引发错误 `addConfigurableProductsToCart` 进行突变，在更改商店视图后直接将可配置产品添加到购物车。

<!-- ACP2E-176-->

* 当您使用时，Adobe Commerce不再引发错误 `addConfigurableProductsToCart` 突变，将可配置产品的已禁用子项添加到购物车。 以前，Adobe Commerce引发完整性约束违规错误。
...
* 验证已添加到结账工作流中，以将客户的街道地址与默认值进行比较。 如果验证过程标识了更改，则会设置 `customer_address_id` 到 `null` 将其视为新的客户地址。 以前，如果客户在结账时未验证即保存了地址，并且在将验证应用于结账字段后同一客户尝试使用同一地址下订单，则Adobe Commerce不接受编辑后的地址并引发异常。

### 购物车价格规则

<!-- MC-42288-->

* 现在，在保存相关的购物车价格规则之前，禁止生成优惠券 `auto` 选项已启用。 如果您尝试保存此规则而不启用 `auto` 选项，Adobe Commerce会显示以下消息： `Rule is not saved with auto generate option enabled. Please save the rule and try again`.
...
* 现在，当为Adobe Commerce配置时，可以将价格为0的产品成功添加到购物车 **整个购物车的固定金额折扣** 购物车价格规则。 以前，Adobe Commerce会显示此错误： `report.ERROR: Warning: Division by zero in /home/mer33515/public_html/ee24develop/app/code/Magento/SalesRule/Helper/CartFixedDiscount.php on line 85`.
...
* Adobe Commerce当产品具有 `datetime` 属性。
...
* 添加了对路由器无法实例化从URL中收集的动作类的验证。 Adobe Commerce现在显示标准错误消息。 以前，Adobe Commerce会显示此错误：  `PHP Error: Cannot instantiate abstract class Magento\Catalog\Controller\Product\Compare`.
...
* 当从管理员和添加发货注释时，Adobe Commerce的发货详细信息电子邮件中不再包含错误消息 **通知客户** 复选框处于激活状态。 以前，送货确认电子邮件包含此消息： `We're sorry, an error has occurred while generating this content`.
...
* Adobe Commerce现在包括 `Content-Disposition: inline` 电子邮件中的标头（按预期）。 [GitHub-29258](https://github.com/magento/magento2/issues/29258)
...
* 帮助程序 `Magento\Payment\Helper\Data` 不再在构造函数中创建新布局。 以前，当在自定义命令中使用此帮助程序但未指定区号时，Adobe Commerce会引发错误。 [GitHub-33908](https://github.com/magento/magento2/issues/33908)
...
* 已更正的问题 `sprintf(__())` 和 `sprintf(Magento\Framework\Phrase())` 在整个代码库中构建与PHP 8.x的兼容性。 以前，Adobe Commerce会引发以下类型的错误： `Expected parameter of type 'string', 'Magento\Framework\Phrase' provided`. [GitHub-34085](https://github.com/magento/magento2/issues/34085)
...
* 正在执行 `bin/magento support:backup:code` 现在会按预期创建备份。 以前，Adobe Commerce会创建一个备份，但会引发错误。
...
* 此 `staging_synchronize_entities_period` `cron` 作业现在按预期运行。 以前，已成功应用第一个暂存更新，但Adobe Commerce在后续更新中抛出此错误： `report.ERROR: Cron Job staging_synchronize_entities_period has an error: The active update can't be deleted`.
...
* 复制产品时，Adobe Commerce现在会根据其定义的范围复制所有产品字段。 以前， **name** 和 **描述** 字段已重置为 `global` 范围。
...
* 产品具有 **设置为新** 现在，可以成功保存分配了空开始日期和过期结束日期的属性。 以前，当您使用以下设置保存产品时，Adobe Commerce会引发此错误： `Make sure the To Date is later than or the same as the From Date`.
...
* 此 `setBillingAddressToCart` 突变现在正确使用 `same_as_shipping` 用于设置帐单地址以按预期匹配运送地址的参数。 以前，Adobe Commerce会显示此错误： `The shipping method is missing. Select the shipping method and try again`. [GitHub-30924](https://github.com/magento/magento2/issues/30924)
...
* 此 `setShippingAddressesOnCart` 现在，当满足以下条件时，突变支持在电话号码值为空的购物车上设置送货地址： **显示电话** 管理员配置设置设为可选。 以前，Adobe Commerce引发此错误： `Field CartAddressInput.telephone of required type String! was not provided`.
...
* 不受支持的 `str_contains` 方法已被替换为支持的函数。 这是Adobe Commerce 2.4.3存在的已知问题。 [GitHub-33680](https://github.com/magento/magento2/issues/33680)
...
* Adobe Commerce现在会在管理员处于只读状态时显示信息性错误 `pubs/media` 尝试访问包含图像的产品的产品详细信息页面。 以前，Adobe Commerce引发PHP错误。 [GitHub-32819](https://github.com/magento/magento2/issues/32819)
...
* 为与PHP 8.1兼容，向返回类型不完整的内部方法添加了返回类型声明。 [GitHub-34498](https://github.com/magento/magento2/issues/34498) <!-- AC-1606-->

<!-- AC-1147-->

* 已更新根 `composer.json` 隐喻和  `composer.json` 每个模块的文件转换为PHP 8.1。 [GitHub-34009](https://github.com/magento/magento2/issues/34009)
...
* 管理员订单详细信息页面现在会按预期加载。 以前，当加载含特定税订单的订单详细信息页面时，Adobe Commerce会引发以下错误： `Call to a member function getId() on array`.
...
* `bin/magento setup:upgrade` 从Adobe Commerce 2.4.2-p1升级时，现在可按预期运行。 以前，Adobe Commerce引发此错误： `Unable to apply data patch Magento\Paypal\Setup\Patch\Data\UpdateBmltoPayLater for module Magento_Paypal`. [GitHub-33678](https://github.com/magento/magento2/issues/33678)
...
* PayPal Express现在可从购物车中按预期工作。 以前，当您单击 **PayPal** 按钮启动从购物车快速结帐，Adobe Commerce引发此错误：  `To check out, please sign in with your email address`. [GitHub-33445](https://github.com/magento/magento2/issues/33445)
...
* 已添加索引至 `magento_giftcardaccount.code` 以提高性能。
...
* 默认情况下，Adobe Commerce会在产品保存期间同步调整图像大小。 商家现在可以通过对进行细微的编辑来作为后台异步流程调整图像大小 `di.xml` 文件。
...
* 新 `product_alert` consumer通过创建队列消息、运行使用者并缩短执行时间来改进客户警报的发送。 以前，在发送超过100,000个产品警报时，Adobe Commerce会引发内存不足异常。 Adobe Commerce还需要20多个小时才能发送所有警报。

### 产品视频

<!-- MC-42105-->

* Adobe Commerce现在支持 **保存** 添加视频时输入不完整URL时的button和autocompletes字段（管理员） **目录**  > **产品** > **添加视频**)。 以前， **保存** 按钮已禁用，并且未填充字段。

### 退货授权(RMA)

<!-- MC-42987-->

* “管理员创建退货产品”网格现在显示税，包括仅配置为随价格显示税的产品价格。 以前，Adobe Commerce不会检查上的配置显示设置， `tax/calculation/price_includes_tax` 标志。
...
* 过期的奖励积分现在标记为 **已过期** 在店面奖励点历史网格的“原因”部分。
...
* Adobe Commerce现在会按预期从店面删除HTML标签 **帐户** > **我的订单** > **查看订单** 页面。 以前，Adobe Commerce在店面客户订单注释部分中显示HTML标记。
...
* `category_name` 已添加到系统保留的产品属性代码。 如果用户尝试使用属性代码创建产品属性，Adobe Commerce现在会显示错误 `category_name`. 以前，产品未列在预期类别中，并记录了Elasticsearch错误。
...
* 使用S3存储适配器时，现在可按预期导入表速率。 以前，Adobe Commerce会显示此错误： `File "https://[bucket].s3.eu-central-1.amazonaws.com/[prefix]/tmp/phpLjGmHf" not found`. [GitHub-33072](https://github.com/magento/magento2/issues/33072)
...
* 此 `UserExpiration` 验证器不再失败 `de_DE` 和 `uk_UA` 区域设置。 以前，当管理员尝试在语言环境设置为的情况下从管理员创建新用户时设置过期日期时，Adobe Commerce会引发错误 `de_DE` 或 `uk_UA`. [GitHub-32497](https://github.com/magento/magento2/issues/32497)
...
* Adobe Commerce现在会修剪来自联系我们的表单电子邮件输入字段中的不间断空格字符。 以前，提交表单时不会删除电子邮件输入中的不间断空格字符（如果给定），这会导致日志文件出现错误。 此 **回复** 生成的发送给商店管理员的联系人电子邮件中缺少字段。
...
* 管理员客户网格现在显示所有客户数据，包括新添加的数据 `date` 自定义属性（按预期）。 Adobe Commerce以前，当 `date` 属性集设置为列。
...
* 禁用Cookie限制模式后，升级后Adobe Commerce不再显示此消息： `The store will not work correctly in the case when cookies are disabled`. [GitHub-33811](https://github.com/magento/magento2/issues/33811)
...
* 当购物者单击时，Adobe Commerce不再显示重复的地址 **更改地址** 在部署中的Review and Payments页面上， **地址搜索** 已启用配置设置。
...
* 可视推销 **从缺货到缺货** 现在，当所有简单产品缺货时，可配置产品可按预期自动排序选项正常工作。
...
* 现在，可以使用的子类创建对象 `\Magento\Framework\Api\AbstractSimpleObjectBuilder` PHP 7.3上的。以前， `preg_match` 当Adobe Commerce在Redhat上使用PHP 7.3托管时，引发此警告消息： `Warning: preg_match(): Compilation failed: unrecognized character follows...`.
...
* Adobe Commerce现在生成 `customertoken` 多次连续失败的登录尝试后，按预期执行GraphQL或REST API请求。 以前，Adobe Commerce不检查的 `lock_expires_at` 在 `oauth_token_request_log` 大于当前日期和时间，并且始终返回尝试失败的次数，这会阻止客户登录。 [GitHub-34067](https://github.com/magento/magento2/issues/34067)
...
* 当满足以下条件时，Adobe Commerce不再在类别侧边栏中呈现愿望清单 **在侧栏中显示** 已禁用愿望清单选项。 以前，Adobe Commerce会忽略此选项。
...
**问题**：商家无法针对通过Braintree以Apple Pay支付的订单提交部分退款。 当商家尝试从订单发票创建部分退款的贷项通知单时， **要退款的数量** 字段不可编辑。  **解决方法**：应用修补程序 `braintree-disabled-partial-capture-for-applepay-googlepay.patch`. 请参阅 [Adobe Commerce 2.4.4：无法创建部分发票](https://support.magento.com/hc/en-us/articles/4487952754957-Adobe-Commerce-2-4-4-Unable-to-create-partial-invoices) 知识库文章。  <!-- BUNDLE-3088-->

**问题**：在Admin中未选择任何选项时，购物者无法将产品添加到购物车 **存储** >  **配置** > **常规** > **国家/地区选项** > **允许国家/地区**. Adobe Commerce改为显示以下控制台错误： `Failed to load resource: the server responded with a status of 400 (Bad Request)`. <!-- AC-2564-->
