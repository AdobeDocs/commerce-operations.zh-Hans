...
Adobe Commerce 2.4.4引入了对PHP 8.1的支持。为了与PHP 8.1兼容，所有项目库和依赖项都已更新。核心编辑器依赖项和第三方库也已升级到与PHP 8.1兼容的最新版本。此版本还支持OpenSearch 1.2。
...
>[!NOTE]
>
>除Braintree之外，所有与供应商捆绑的扩展都已从Adobe Commerce 2.4.4中删除。
...
Adobe Commerce 2.4.4包括解决以下修补程序解决的所有问题，这些修补程序为Adobe Commerce和Magento Open Source2.4.3、2.4.3-p1和2.3.7-p2提供：
...
* `braintree-disabled-partial-capture-for-applepay-googlepay.patch`.  商户无法通过Braintree提交通过Apple支付支付的订单的部分退款。 当商家尝试从订单发票为部分退款创建贷项通知单时， **退款数量** 字段，此时将无法编辑。 此修补程序可解决此问题。 请参阅 [Adobe Commerce 2.4.4:无法创建部分发票](https://support.magento.com/hc/en-us/articles/4487952754957-Adobe-Commerce-2-4-4-Unable-to-create-partial-invoices) 知识库文章。  <!-- BUNDLE-3088-->
...
此版本包括一项安全修复和平台安全改进。 此安全修复已支持到Adobe Commerce 2.4.3-p2和Adobe Commerce 2.3.7-p3。
...
* 现在，Adobe Commerce店面默认启用HTTPS。 的 **在店面上使用安全URL** 和 **在管理员中使用安全URL** 设置默认处于启用状态，所有内置cookie现在都设置为安全。  <!-- AC-1173-->

* 现在，所有Adobe Commerce安装都需要使用依赖项混淆插件。 以前，仅基于编辑器的安装需要使用此插件。 现在，它允许使用可信的版本。 商家可以绕过此插件中设置的限制，这些限制会禁止某些编辑器安装组合。 对于受信任的版本，可以绕过这些限制，现在，在继续安装之前，Adobe Commerce会显示一条警告消息。  <!-- AC-501 970-->
...
Adobe Commerce 2.4.4现在支持PHP 8.1。为与PHP 8.1兼容，已更新所有项目库和依赖项。其他平台增强功能包括：

* Adobe Commerce 2.4.4支持Elasticsearch7.16和OpenSearch 1.2。托管部署在本地的Adobe Commerce商户可以使用任一解决方案。 但是，对于在云中托管的Adobe Commerce 2.4.4部署，OpenSearch现在是默认的搜索引擎。 升级到版本2.4.4的所有具有云部署的客户都必须使用OpenSearch。 请参阅 [在Cloud 2.4.4中切换到OpenSearch for Adobe Commerce](https://support.magento.com/hc/en-us/articles/4419942355725-Switching-to-OpenSearch-for-Adobe-Commerce-on-Cloud-2-4-4)
...
Adobe Commerce性能增强提高了高吞吐量订单处理和消息队列优化。 此版本中引入的异步订单功能支持创建大约60,000个订单/小时。 早期版本的Adobe Commerce支持每小时处理大约10,000个订单，这对闪电销售构成了潜在的瓶颈。 新的多用户功能支持在单个云实例上缩放消息队列用户的数量，并增加每小时处理的订单数。

此版本中的性能增强功能：

* AsyncOrder功能支持比同步执行提供的更快的订单放置。 启用AsyncOrder后，在后台执行订单投放，而购物者在店面完成其他任务。

* 新 **在购物车加载时启用库存检查** 配置选项可在购物车中加载产品时提供可切换的库存检查。 默认情况下处于启用状态。 禁用此选项时，Adobe Commerce会在报价加载时跳过库存检查，这将加快结帐速度，特别是对于包含许多项目的购物车。

* 新 `multiple_processes` 配置选项支持在多个进程中运行并行使用者。 以前， `cron` 需要时运行单个用户。 启动多个用户以并行运行进程可以提高任务执行速度。 要启用此功能，请添加 `multiple_processes` 到 `app/etc/env.php` 文件。

* 通过提高 `max_input_vars` 在 `php.ini` 文件来支持输入变量卷。

* 通过延迟总计计算来优化结帐期间的销售规则处理。 通常，每当购物者将产品添加到购物车、更新产品数量或单击  **下单** 按钮。 商户可以通过设置 `checkout/deferred_total_calculating` 变量 `env.php` 文件。 或者，您也可以运行 `bin/magento setup:config:set --deferred-total-calculating 1|0`.  <!-- MCP-573-->

* 改进了在异步订单放置期间受购物车价格规则影响的订单的验证流程。 <!-- MCP-304-->
...
* **可转让报价的完整GraphQL覆盖**.  B2B公司用户现在可以使用GraphQL完成与可转让报价有关的所有任务。 此API的早期版本支持协商流程，但不支持结账。 <!-- PWA-2101-->

#### 新的突变

* [`assignCustomerToGuestCart`](https://devdocs.magento.com/guides/v2.4/graphql/mutations/assign-customer-to-guest-cart.html) 突变

* [`placeNegotiableQuoteOrder`](https://devdocs.magento.com/guides/v2.4/graphql/mutations/place-negotiable-quote-order.html) 突变

* [`setNegotiableQuoteBillingAddress`](https://devdocs.magento.com/guides/v2.4/graphql/mutations/set-negotiable-quote-billing-address.html) 突变

* [`setNegotiableQuotePaymentMethod`](https://devdocs.magento.com/guides/v2.4/graphql/mutations/set-negotiable-quote-payment-method.html) 突变<!-- PWA-2114-->

* [`setNegotiableQuoteShippingMethods`](https://devdocs.magento.com/guides/v2.4/graphql/mutations/set-negotiable-quote-shipping-methods.html) 突变

* [`setNegotiableQuoteShippingAddress`](https://devdocs.magento.com/guides/v2.4/graphql/mutations/set-negotiable-quote-shipping-address.html) 突变

* **性能改进**:

   * GraphQL购物车操作的性能已得到改进。 的 `collectQuoteTotals()` 现在，在GraphQL请求期间只调用一次方法，这可缩短响应时间。 <!-- PWA-2110-->

   * 对GraphQL请求的缓存方式进行了更改，从而改进了Storefront性能。 现在， Fastly and Quist可缓存随 `auth` 令牌。 <!-- PWA-1817-->
...
* **修复了GraphQL中具有多站点和多语言存储的翻译问题**. GraphQL解析程序现在可根据预期的存储范围返回翻译后的字符串。 <!-- PWA-1946-->

* GraphQL现在为新旧项目提供描述性事务名称，这有助于进行调试。 [GitHub-30915](https://github.com/magento/magento2/issues/30915) <!-- PWA-1311-->
...
除 [Braintree](https://docs.magento.com/user-guide/payment/braintree.html)，则所有供应商捆绑的扩展都已从Adobe Commerce 2.4.4代码库中删除。 商户应迁移到官方扩展，这些扩展在 [Commerce Marketplace](https://marketplace.magento.com/extensions.html). <!-- AC-1165-->
...
PWA Studiov.12.3.0与Adobe Commerce 2.4.4兼容。它包含对reCaptcha、页面生成器内容优化和个性化内容的支持。 有关增强功能和错误修复的信息，请参阅 [PWA Studio版本](https://github.com/magento/pwa-studio/releases). 请参阅 [版本兼容性](https://developer.adobe.com/commerce/pwa-studio/integrations/adobe-commerce/version-compatibility/) 以获取PWA Studio版本及其兼容的Adobe Commerce核心版本列表。
...
新 `Magento_ElasticsearchCatalogPermissionsGraphQl` 在更新的实时搜索期间，模块被禁用 [安装](https://experienceleague.adobe.com/docs/commerce-merchant-services/live-search/onboard/install.html). 必须将现有的Live Search安装升级到Live Search 2.0.0，才能利用这些功能。 有关更多信息，请参阅实时搜索 [发行说明](https://experienceleague.adobe.com/docs/commerce-merchant-services/live-search/release-notes.html).

### 辅助功能更新

此版本提高了对标准无障碍准则的遵守率。 这些增强功能可改善视力有限或语言、认知和学习能力有限的用户的体验。  屏幕Reader工具性能也得到改进。

辅助功能增强功能包括：

* 包含文本的工具提示链接以及用于输入的准确、以任务为中心的可见标签
* 编辑按钮现在具有唯一的文本。
* 管理员按钮现在具有独特、可访问且描述性的用途，以符合AA标准
* 传达含义的图标图像现在提供了文本替代内容。
* 增强了“管理员”按钮和表单字段的对比度，使其与相邻颜色的对比度至少为3:1。
...
* Adobe Commerce现在会按照预期在生成日志文件中记录静态内容部署错误。
...
* 现在，可以从命令行成功更改站点地图生成器配置。 以前，当您尝试在管理员之外更改站点地图配置时，Adobe Commerce会显示此错误： `Import failed: Notice: Trying to access array offset on value of type null in app/code/Magento/Cron/Model/Config/Backend/Sitemap.php on line 78`.  [GitHub-31428](https://github.com/magento/magento2/issues/31428)
...
* 现在，具有受限权限的管理员可以按预期创建返回。

### 后端

<!-- MC-43161-->

* 管理员现在可以在配置了自定义管理员路径且启用了密钥的部署中登录到管理员。 以前，Adobe Commerce会显示以下错误： `Invalid security or form key. Please refresh the page`.

<!-- MC-41949-->

* 当多商店层次结构发生更改时，Adobe Commerce不再重定向到“所有商店视图”范围。 以前，保存层级后，购物者会被重定向到错误的商店URL，并且在多商店部署中选择了“所有商店查看”范围。

### 捆绑产品

<!-- MC-42885-->

* 购物者现在可以向其购物车添加包含固定产品税和两个选项的捆绑产品。 以前，Adobe Commerce未添加产品并显示以下错误： `We can't add this item to your shopping cart right now`.
...
* `cron` 现在，按照 `indexer_update_all_views` 执行。 以前，在对类别中的许多产品进行批量更新后，Adobe Commerce在类别页面上显示错误的产品计数。

<!-- MC-42811-->

* 现在，当捆绑产品重新出现在库存中时，Adobe Commerce会按预期更新类别产品缓存。
...
* 现在，在启用永久性购物车的部署中，在会话超时后，购物者可以无错误下单。 以前，Adobe Commerce会在会话超时后显示付款错误。
...
* 如果更新数量无效，Adobe Commerce现在会将购物车产品数量还原为上一个值。
...
* 现在，在启用表费率的情况下，Adobe Commerce可正确将免费发运阈值应用于订单。 [GitHub-21832](https://github.com/magento/magento2/issues/21832)
...
* Adobe Commerce在购物者单击时不再引发错误 **更新购物车** 在迷你购物车中更改产品数量后。 以前，Adobe Commerce会显示以下错误：  `The quote item isn't found. Verify the item and try again`.
...
* Adobe Commerce在购物者添加的帐单地址缺少street字段时不再引发此错误： `Uncaught TypeError: Unable to process binding "text: function(){return currentBillingAddress().street.join(', ') }"`. [GitHub-33826](https://github.com/magento/magento2/issues/33826)

<!-- AC-1625-->

* Adobe Commerce在从中删除迷你购物车的商店结账期间不再引发控制台错误。 [GitHub-34513](https://github.com/magento/magento2/issues/34513)
...
* Adobe Commerce在您使用 `addConfigurableProductsToCart` 突变，在更改商店视图后直接将可配置产品添加到购物车。

<!-- ACP2E-176-->

* Adobe Commerce在您使用 `addConfigurableProductsToCart` 突变，将可配置产品的禁用子项添加到购物车。 以前，Adobe Commerce会抛出完整性约束违规错误。

<!-- ACP2E-206-->

* 当 **显示购物车中的交叉销售项目** 配置设置被禁用。
...
* 已在结帐工作流中添加验证，以比较客户的街道地址与默认值。 如果验证过程识别了更改，则会设置 `customer_address_id` to `null` 将其视为新客户地址。 以前，如果客户在结帐时未进行验证就保存了地址，而同一客户在对结帐字段应用验证后尝试使用同一地址下订单，则Adobe Commerce不接受编辑的地址并引发异常。

### 购物车价格规则

<!-- MC-42288-->

* 现在，在将相关的购物车价格规则与 `auto` 选项。 如果您尝试在未启用 `auto` 选项，Adobe Commerce将显示以下消息： `Rule is not saved with auto generate option enabled. Please save the rule and try again`.
...
* 现在，在为Adobe Commerce配置了 **整个购物车的固定金额折扣** 购物车价格规则。 以前，Adobe Commerce会显示以下错误： `report.ERROR: Warning: Division by zero in /home/mer33515/public_html/ee24develop/app/code/Magento/SalesRule/Helper/CartFixedDiscount.php on line 85`.
...
* GraphQL查询现在会根据子产品的可见性过滤器设置返回可配置产品的子产品的结果。

<!-- MC-41853-->

* 当可配置产品属性发生更改时，Adobe Commerce将不再修改相关产品价格。 以前，可配置产品的最低广告价格(MAP)会覆盖商店前相关产品的价格。

<!-- MC-42659-->

* 管理员现在可以重新分配类别中剩余的最后一个产品，并保存空类别。
...
* 添加了对路由器无法实例化从URL收集的操作类的情况的验证。 Adobe Commerce现在显示标准错误消息。 以前，Adobe Commerce会显示以下错误：  `PHP Error: Cannot instantiate abstract class Magento\Catalog\Controller\Product\Compare`.

<!-- ACP2E-10-->

* 现在，当两个购物车规则应用于满足以下条件的订单时，Adobe Commerce会正确计算折扣：一个购物车规则与优惠券代码和 **整个购物车的固定价格**  条件和第二个购物车价格规则没有优惠券和 **产品价格折扣百分比** 条件。 以前，当前购物车规则已对某些产品应用了折扣时，相关算法无法按比例在购物车产品中正确拆分折扣。
...
* 在部分索引过程中，更新子产品不再删除从其父可配置产品继承的目录规则折扣。 以前，当我们将一个可配置产品分配给某个类别，但没有分配到其关联的简单产品，然后使用该类别创建目录规则时，Adobe Commerce会在部分索引过程中删除分配给该简单产品的目录规则。
...
* Adobe Commerce在部分重新索引期间不再删除可配置产品上的目录价格规则。 以前，在重新编入可配置产品的索引时，只会重新编入子产品的索引。
...
* Adobe Commerce在您尝试将可配置产品的禁用子产品添加到购物车时不再引发内部错误。

### 客户

<!-- MC-24204-->

* Adobe Commerce现在在订单创建过程中考虑网站范围以了解管理员区域设置。 以前，订单详细信息（如客户地址属性）仅在一个网站上的多站点部署中正常工作。 [GitHub-23254](https://github.com/magento/magento2/issues/23254)

### 客户区段

<!-- MC-42241-->

* 现在，在您创建公司帐户时，新客户区段的匹配客户计数会自动更新。 以前，您必须刷新区段数据才能获得准确的计数。
...
* 从管理员和 **通知客户** 复选框。 以前，发送确认电子邮件包含以下消息： `We're sorry, an error has occurred while generating this content`.
...
* Adobe Commerce现在包含 `Content-Disposition: inline` 电子邮件中的标头（如预期）。 [GitHub-29258](https://github.com/magento/magento2/issues/29258)
...
* 现在，单击任何电子邮件模板的预览可在单独的窗口中按预期打开模板预览。  以前，Adobe Commerce会同时从“电子邮件模板”网格和弹出窗口中打开模板预览。
...
* 帮手 `Magento\Payment\Helper\Data` 不再在构造函数中创建新布局。 以前，当在自定义命令中使用此帮助程序而不指定区域代码时，Adobe Commerce会抛出错误。 [GitHub-33908](https://github.com/magento/magento2/issues/33908)
...
* 现在，当bootstrap文件中发生异常时，Adobe Commerce会返回500响应代码。 以前，Adobe Commerce会返回一个200 OK状态代码。 [GitHub-22196](https://github.com/magento/magento2/issues/22196)
...
* 向 `.htaccess` 和 `nginx.conf` 用于支持PHP 8.x的随Adobe Commerce分发的文件。 已删除过时的选项。 [GitHub-34358](https://github.com/magento/magento2/issues/34358)

<!-- MC-42448-->

* 商家现在可以上传多个产品的视频。 现在，仅当上传的文件名包含扩展名时，文件生成过程才会向文件名附加扩展名。 以前，当商家尝试上传，然后为两个不同的产品保存同一视频时，Adobe Commerce抛出了以下错误： `Notice: Undefined index: extension in /app/7ha7zds7wvqys_stg/vendor/magento/framework/File/Uploader.php on line 699`. 发生此情况的原因是，无论上传的文件名是否具有扩展名，Adobe Commerce都尝试将文件扩展名附加到该名称。

<!-- magento/magento2/pull/34106-->

* 更正了 `sprintf(__())` 和 `sprintf(Magento\Framework\Phrase())` 构建以在整个代码库中与PHP 8.x兼容。 以前，Adobe Commerce会抛出以下类型的错误： `Expected parameter of type 'string', 'Magento\Framework\Phrase' provided`. [GitHub-34085](https://github.com/magento/magento2/issues/34085)
...
* 正在执行 `bin/magento support:backup:code` 现在可按预期创建备份。 以前，Adobe Commerce创建了备份，但抛出了错误。
...
* 的 `staging_synchronize_entities_period` `cron` 工作现在按预期工作。 以前，成功应用了第一个暂存更新，但Adobe Commerce在后续更新中抛出了此错误： `report.ERROR: Cron Job staging_synchronize_entities_period has an error: The active update can't be deleted`.
...
* 现在，Adobe Commerce在复制产品时，会根据其定义的范围复制所有产品字段。 以前， **name** 和 **描述** 字段已重置为 `global` 范围。

<!-- MC-42612-->

* 当产品价格为0时，Adobe Commerce不再对折扣应用增量舍入。 以前，Adobe Commerce会计算负折扣金额。

<!-- MC-42215-->

* 具有 **设置为新** 现在可以成功保存分配了空开始日期和过期结束日期的属性。 以前，Adobe Commerce在您保存具有以下设置的产品时会抛出此错误： `Make sure the To Date is later than or the same as the From Date`.

<!-- MC-41803-->

* 当您更改其SEO URL标识符时，Adobe Commerce不再复制CMS层次结构中的现有CMS页面。 以前，Adobe Commerce会在CMS层次结构中创建一个新节点，并复制现有节点。
...
* 类 `BundleDiscountPrice` in `magento2/app/code/Magento/Bundle/Pricing/Price/ConfiguredPrice.php` 现在已按预期声明。 以前，Adobe Commerce会抛出以下错误： `Class Magento\Bundle\Pricing\Price\BundleDiscountPrice not found`. [GitHub-33334](https://github.com/magento/magento2/issues/33334)
...
* 现在，当您在存储视图级别上从网格内编辑CMS页面时，Adobe Commerce会成功保存该页面 **使用父节点层次结构** 配置设置被禁用。 以前，Adobe Commerce显示此错误，并且未保存CMS页面编辑： `A technical problem with the server created an error. Try again to continue what you were doing. If the problem persists, try again later`.

<!-- ACP2E-171-->

* 现在，对于包含同一页面的节点，可以正确显示痕迹导航。 以前，Adobe Commerce会为属于多个节点的页面显示相同的节点名称。 即使每个页面存在多个节点，Adobe Commerce也仅检索页面的第一个节点。
...
* 的 `mage-messages` 如果没有新消息，则不会重置cookie。

### 礼品卡

<!-- MC-42327-->

* 在购物者输入无效礼品卡时添加了信息更丰富的错误消息。
...
* 的 `setBillingAddressToCart` 突变现在正确使用 `same_as_shipping` 参数来设置帐单地址以匹配预期的送货地址。 以前，Adobe Commerce会显示以下错误： `The shipping method is missing. Select the shipping method and try again`. [GitHub-30924](https://github.com/magento/magento2/issues/30924)
...
* GraphQL现在为新旧项目提供描述性事务名称，这有助于进行调试。 [GitHub-30915](https://github.com/magento/magento2/issues/30915)

<!-- MC-42903-->

* 的 `setShippingAddressesOnCart` 变异现在支持在购物车上设置具有空电话号码值的送货地址(当 **显示电话** 管理员配置设置设置为可选。 以前，Adobe Commerce会抛出以下错误： `Field CartAddressInput.telephone of required type String! was not provided`.
...
* MySQL查询已针对GraphQL进行优化 `products` 使用搜索参数优化搜索结果的查询。
...
* Adobe Commerce在店面上同时加载图像时，刷新图像缓存不再引发错误。
...
* 现在，Adobe Commerce会在计划导出后，将导出文件名中的时间戳转换为用户的时区。 以前，这些值不会进行转换，Adobe Commerce会显示UTC时间戳。

<!-- MC-42330-->

* 当多次导入同一CSV文件时，Adobe Commerce将不再在远程存储中创建重复的图像。
...
* 现在，重新索引后，Adobe Commerce会在店面上按预期显示产品。 以前，当前重新编入索引的前500个产品有现货，而后500个产品无现货时，店面没有显示任何其他产品。
...
* 不支持的 `str_contains` 方法已被替换为支持的函数。 这是Adobe Commerce 2.4.3的已知问题。 [GitHub-33680](https://github.com/magento/magento2/issues/33680)
...
* 现在，当管理员具有只读属性时，Adobe Commerce会显示一个信息性错误 `pubs/media` 会尝试访问包含图像的产品的产品详细信息页面。 以前，Adobe Commerce抛出PHP错误。 [GitHub-32819](https://github.com/magento/magento2/issues/32819)
...
* PHP 8.1中已弃用的函数已从代码库中删除。 [GitHub-34497](https://github.com/magento/magento2/issues/34497)

* 为了与PHP 8.1兼容，向返回类型不完整的内部方法添加了返回类型声明。 [GitHub-34498](https://github.com/magento/magento2/issues/34498) <!-- AC-1606-->

<!-- AC-1147-->

* 更新了根 `composer.json` 元包  `composer.json` 文件添加到PHP 8.1。 [GitHub-34009](https://github.com/magento/magento2/issues/34009)
...
* 以下库已被删除，因为Adobe Commerce 2.4.x支持的所有浏览器都对此功能具有内置支持：
...
* 现在，购物者可以从店面和管理员中成功重新订购包含产品的现有订单，这些订单包含自定义选项类型为“file”（文件类型）和类型下拉列表的组合。 以前，Adobe Commerce抛出错误且未处理重新排序。

<!-- MC-42746-->

* 现在，通过商店贷项支付并应用了固定产品税(FPT)的订单的发票包括正确的总计。

<!-- MC-42332-->

* 管理员订单详细信息页面现在按预期加载。 以前，Adobe Commerce在为含特定税的订单加载订单详细信息页面时抛出以下错误： `Call to a member function getId() on array`.

<!-- MC-38521-->

* 具有限制权限（包括订单和发运权限）的管理员现在可以按预期查看订单页面。 以前，Adobe Commerce在具有这些权限的管理员尝试查看订单时抛出错误。 [GitHub-14633](https://github.com/magento/magento2/issues/14633)
...
* `bin/magento setup:upgrade` 现在，从Adobe Commerce 2.4.2-p1升级时可以按预期运行。 以前，Adobe Commerce会抛出以下错误： `Unable to apply data patch Magento\Paypal\Setup\Patch\Data\UpdateBmltoPayLater for module Magento_Paypal`. [GitHub-33678](https://github.com/magento/magento2/issues/33678)
...
* 已解决PayPal信用卡金额显示异常问题。 以前，启用PayPal信用并配置多个礼品卡金额时，如果购物者更改了礼品卡值的金额，则店面不会更新分期付款的金额。 这是Adobe Commerce 2.4.3中的已知问题。

#### PayPal

<!-- MC-42154-->

* Adobe Commerce在使用PayPal Express付款方法结账期间不再显示错误。 以前，尽管结帐过程已完成，但Adobe Commerce仍显示以下错误： `Something went wrong`.

<!-- AC-722-->

* PayPal Express现在可以按预期从购物车运行。 以前，当您单击 **PayPal** 按钮以从购物车开始快速结帐，Adobe Commerce抛出了以下错误：  `To check out, please sign in with your email address`. [GitHub-33445](https://github.com/magento/magento2/issues/33445)
...
* 在 `magento_giftcardaccount.code` 以提高性能。
...
* 默认情况下，Adobe Commerce会在产品保存期间同步调整图像大小。 商家现在可以通过对 `di.xml` 文件。
...
* 已优化包含相关产品的产品详细信息页面的加载。 相关产品条件查询也已优化。
...
* 新 `product_alert` 消费者通过创建队列消息、运行消费者并缩短执行时间，改进了客户警报的发送。 以前，Adobe Commerce在发送超过100,000个产品警报时会引发内存不足异常。 Adobe Commerce还花费了20多小时才发送所有警报。

### 产品视频

<!-- MC-42105-->

* Adobe Commerce现在启用 **保存** 添加视频时输入不完整的URL时，按钮和自动结束字段(管理员 **目录**  > **产品** > **添加视频**)。 以前， **保存** 按钮被禁用，且未填充字段。

### 退货授权(RMA)

<!-- MC-42987-->

* 现在，“管理员创建退货”网格可显示税，包括仅针对配置为以价格显示税的产品的价格。 以前，Adobe Commerce不会检查 `tax/calculation/price_includes_tax` 标记。
...
* 奖励点平衡和奖励点历史记录现在显示相同的平衡。 现在可正确扣除过期分数。 以前，“奖励分数平衡”未准确反映过期分数。

<!-- ACP2E-158-->

* 过期的奖励积分现在标记为 **过期** 在“奖励点历史记录”网格的“原因”部分。
...
* Adobe Commerce现在会按照预期从店面删除HTML标记 **帐户** > **我的命令** > **查看顺序** 页面。 以前，Adobe Commerce在店面客户订单评论部分显示HTML标记。
...
* 价格范围的分层导航选项现在可在自定义价格属性中按预期使用。 Adobe Commerce会在筛选自定义价格属性时使用价格导航步骤的配置。 以前，Adobe Commerce使用手动步骤配置。
...
* `category_name` 已添加到系统保留的产品属性代码中。 现在，如果用户尝试使用属性代码创建产品属性，则Adobe Commerce会显示错误 `category_name`. 以前，产品未列在预期类别中，并且记录了Elasticsearch错误。

<!-- ACP2E-68-->

* 热门搜索词现在在每个GraphQL搜索查询后更新。

<!-- ACP2E-249-->

* Adobe Commerce现在为Elasticsearch价格指数映射添加了正确的价格字段。 以前，在使用商店ID而不是网站ID的索引中添加了不正确的价格字段。
...
* 现在，当购物者在多次送货模式下从购物车中删除产品，然后切换到单次送货地址时，Adobe Commerce会成功更新迷你购物车。 以前，产品价格未按预期更新。

<!-- AC-267-->

* 现在，使用S3存储适配器时，表速率的导入可按预期工作。 以前，Adobe Commerce会显示以下错误： `File "https://[bucket].s3.eu-central-1.amazonaws.com/[prefix]/tmp/phpLjGmHf" not found`. [GitHub-33072](https://github.com/magento/magento2/issues/33072)
...
* Adobe Commerce现在在“管理员”和“店面发票”页面合计中显示免费装运成本(0)。 以前，当订单的发运为零时，Adobe Commerce不会在发票页发运合计中显示发运金额合计。

<!-- ACP2E-120-->

* 对在线运输承运人的费率请求已从多个请求减少到一个。 现在，可以正确计算每个项目/包的包保险和处理价格。 以前，Adobe Commerce对FedEx发运方法显示错误的运费率。

<!-- ACP2E-174-->

* Adobe Commerce现在在为从波多黎各发货创建送货标签时显示UPS费率。 以前，Adobe Commerce不显示这些UPS费率。
...
* 产品计划的更新图像和视频现已正确保存。 以前，在创建新的计划更新后，会删除视频、URL、标题和描述。

<!-- MC-42501-->

* Adobe Commerce现在在“计划新更新”窗体中为启用了属性的商店视图显示“最低公告价格”属性。

<!-- MC-42590-->

* Adobe Commerce现在在产品计划更新表单中仅显示一个“图像”选项卡（如预期）。 以前，Adobe Commerce会复制此选项卡。

<!-- MC-23994-->

* 现在，在计划更新预览中可以正确显示产品库存状态。 以前，在创建计划更新期间启用之前禁用的产品时，产品状态显示为缺货。

### 存储

<!-- MC-42884-->

* 现在，在包含不同URL上的多个商店的部署中，“以客户身份登录”功能可按预期工作。 以前，即使访问正确的存储域，Adobe Commerce也无法加载正确的存储。

### 站点范围分析工具

<!-- ACP2E-240-->

* 商家现在可以按预期访问网站范围分析工具。 以前，破坏的键对会阻止某些商家访问该工具。

### 税

<!-- MC-41924-->

* 现在，当客户在选择送货到多个地址后删除某个项目时，Adobe Commerce会正确计算小型购物车小计。

<!-- MC-41945-->

* 现在，当购物车仅包含虚拟产品时，Adobe Commerce在所有结帐页面上显示相同的税率。
...
* 集成测试内存消耗已降低，提高了测试性能并减少了测试完成所需的时间。
...
* 更正了 `StorefrontPOWorkflowVerifyApprovalFlowTabTest`. [GitHub-34378](https://github.com/magento/magento2/issues/34378)
...
* 的 `Magento\Framework\MessageQueue\TopologyTest` 现在，测试与AWS MQ for RabbitMQ服务兼容。 以前，此测试失败，并显示以下消息： `Invalid exchange configuration: magento-topic-based-exchange1 Failed asserting that two arrays are equal`. <!-- AC-1248-->
...
* 现在，当Adobe Commerce在其基本URL中配置了子路径时，可以正确显示通知消息。
...
* Adobe Commerce在加载具有预先存在的自定义主题的商店主页时不再显示错误消息。 以前，Adobe Commerce会显示以下消息：  `The store will not work correctly in the case when cookies are disabled`.

### 翻译和区域设置

<!-- AC-285 MC-41400-->

* 的 `UserExpiration` 验证器不再失败 `de_DE` 和 `uk_UA` 区域设置。 以前，当管理员尝试从将设置为的管理员区域设置创建新用户时，Adobe Commerce会抛出错误 `de_DE` 或 `uk_UA`. [GitHub-32497](https://github.com/magento/magento2/issues/32497)
...
* 现在，在设置为葡萄牙语（葡萄牙）区域设置的商店中创建新帐户时，您可以成功输入客户的出生日期。 以前，Adobe Commerce在您尝试创建帐户时抛出错误。 [GitHub-34130](https://github.com/magento/magento2/issues/34130)
...
* Adobe Commerce现在从“联系我们”表单电子邮件输入字段中裁切非中断空格字符。 以前，提交表单时不会从电子邮件输入中删除非中断空格字符（如果提供），这会导致日志文件中出现错误。 的 **回复** 在发送给存储管理员的“联系人”电子邮件消息中也缺少字段。

<!-- MC-42793-->

* Adobe Commerce现在可根据其在管理员中的位置显示相关产品、向上销售产品和交叉销售产品。
...
* 管理员客户网格现在可显示所有客户数据，包括新添加的数据 `date` 自定义属性（如预期）。 以前，Adobe Commerce会抛出错误，并且在 `date` 属性集被设置为列。
...
* 在禁用Cookie限制模式时，Adobe Commerce在升级后不再显示此消息： `The store will not work correctly in the case when cookies are disabled`. [GitHub-33811](https://github.com/magento/magento2/issues/33811)
...
* Adobe Commerce在购物者点击时不再显示重复的地址 **更改地址** 在部署中， **地址搜索** 配置设置已启用。
...
* 现在，视图断点在Adobe Commerce中保持一致。

<!-- ACP2E-199-->

* Adobe Commerce现在可以在“管理员客户”页面上正确显示自定义客户日期属性和自定义区域设置。

<!-- MC-41760-->

* 使用页面生成器添加的图像在使用浏览器窗口宽度768像素显示时，不会再在店面上重复显示。 以前，Adobe Commerce会使用此浏览器窗口宽度在店面上通过显示移动版和桌面版图像来复制图像。
...
* 视觉推销员 **从库存到底** 当所有简单产品都无现货时，自动排序选项现在可以按预期对可配置产品起作用。

<!-- ACP2E-61-->

* 未添加任何条件时，将不再保存可视化促销商规则。
...
* 您现在可以使用的子类创建对象 `\Magento\Framework\Api\AbstractSimpleObjectBuilder` 7.3菲律宾比索。以前， `preg_match` 在使用PHP 7.3将Adobe Commerce托管在Redhat上时，抛出此警告消息： `Warning: preg_match(): Compilation failed: unrecognized character follows...`.
...
* Adobe Commerce现在生成 `customertoken` 按照预期在连续多次失败的登录尝试后发送GraphQL或REST API请求。 以前，Adobe Commerce不会检查 `lock_expires_at` in `oauth_token_request_log` 大于当前日期和时间，并始终返回失败的尝试次数，这会阻止客户永远登录。 [GitHub-34067](https://github.com/magento/magento2/issues/34067)
...
* Adobe Commerce在 **在侧栏中显示** “希望列表”选项已禁用。 以前，Adobe Commerce忽略此选项。
...
**问题**:商户无法通过Braintree提交通过Apple支付支付的订单的部分退款。 当商家尝试从订单发票为部分退款创建贷项通知单时， **退款数量** 字段，此时将无法编辑。  **解决方法**:应用修补程序 `braintree-disabled-partial-capture-for-applepay-googlepay.patch`. 请参阅 [Adobe Commerce 2.4.4:无法创建部分发票](https://support.magento.com/hc/en-us/articles/4487952754957-Adobe-Commerce-2-4-4-Unable-to-create-partial-invoices) 知识库文章。  <!-- BUNDLE-3088-->

**问题**:在“管理员”中未选择任何选项时，购物者无法将产品添加到购物车 **存储** >  **配置** > **常规** > **国家/地区选项** > **允许国家/地区**. Adobe Commerce而是会显示以下控制台错误： `Failed to load resource: the server responded with a status of 400 (Bad Request)`. <!-- AC-2564-->
