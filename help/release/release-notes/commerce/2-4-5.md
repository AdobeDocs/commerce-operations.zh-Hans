...
* `AC-2655_2.4.4.patch.zip`。 此修补程序解决了未选中“允许国家/地区”选项时，购物者无法将产品添加到购物车的问题。 如果不允许国家/地区[&#128279;](https://support.magento.com/hc/en-us/articles/5377681213325-Users-not-able-to-add-product-to-cart-if-nothing-selected-in-Allow-Countries)知识库文章中未选择任何内容，则查看用户无法将产品添加到购物车。
...
具有Adobe ID并希望简化登录Adobe Commerce和Adobe业务产品的Adobe Commerce商家可以将Commerce身份验证与Adobe IMS身份验证工作流集成。 为您的Commerce商店启用此集成后，每个管理员用户必须使用其Adobe凭据(而不是其Commerce凭据)登录。  请参阅[AdobeIdentity Management服务(IMS)集成概述](https://experienceleague.adobe.com/docs/commerce-admin/start/admin/ims/adobe-ims-integration-overview.html?lang=zh-Hans)。
...
* 当产品缺货时，`cart`查询不再返回null响应。 已将包含错误消息的新`errors`元素引入响应。 以前，当您运行包含缺货产品的查询时，Adobe Commerce在响应中的`items`部分下显示了`null`值。 查看[购物车查询](https://developer.adobe.com/commerce/webapi/graphql/schema/cart/queries/cart/)。
...
* 当`int`配置为可搜索的后端`type`属性时，Elasticsearch查询现在按预期工作。 以前，Adobe Commerce引发`Elasticsearch\Common\Exceptions\BadRequest400Exception`异常。

<!-- ACP2E-99-->

* 您现在可以在Elasticsearch查询中将Search同义词与&#x200B;**要匹配的最少搜索词**&#x200B;参数一起使用。 以前，如果在设置中指定此参数并为特定关键字添加了搜索词，则搜索不会返回任何结果。
...
**问题**： Adobe Commerce 2.4.5的全新安装（非升级）后，[!DNL RabbitMQ]基础结构元素（交换、队列）可能未初始化。[!DNL RabbitMQ]的这种错误配置可能会导致在执行异步操作或生成性能配置文件时发生致命错误。 云部署不受影响。 **解决方法**：执行Adobe Commerce 2.4.5本地全新安装的商家应在使用`bin/magento setup:config:set`配置[!DNL RabbitMQ]或将[!DNL RabbitMQ]配置选项传递到`setup:install`命令以安装Adobe Commerce后运行`bin/magento setup:upgrade --keep-generated`。 请参阅[[!DNL RabbitMQ]](../../../installation/prerequisites/rabbitmq.md)。
