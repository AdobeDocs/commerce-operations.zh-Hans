...
* `AC-2655_2.4.4.patch.zip`. 此修补程序解决了未选中“允许国家/地区”选项时，购物者无法将产品添加到购物车的问题。 请参阅 [如果未在允许国家/地区中选择任何内容，则用户无法将产品添加到购物车](https://support.magento.com/hc/en-us/articles/5377681213325-Users-not-able-to-add-product-to-cart-if-nothing-selected-in-Allow-Countries) 知识库文章。
...
* **Adobe Commerce云托管的部署**：Adobe Commerce 2.4.5云托管的Elasticsearch不支持AEM 7.11或更高版本。 OpenSearch是Adobe Commerce 2.4.5云部署的默认搜索引擎。
...
通过将的主索引数量从3个减少到1个，主索引的性能得到了改进。 `catalog_product_index_price_tmp` 表格。 这一改进减少了因共享目录导致的eSKU乘数，从而减少了价格指数中创建的记录数。 索引时间已显着减少。 <!-- MCP-653 MC-43342-->
...
启用新的 **启用共享目录直接产品价格分配** 配置选项还可以提高产品价格索引器的性能。 <!-- MCP-946-->
...
* 此 `bin/magento config:set graphql/session/disable 1` 命令允许商家完全禁止为所有GraphQL操作创建会话Cookie。 默认情况下，Adobe Commerce会创建这些Cookie并依赖它们进行授权，这会影响性能。 将来，我们建议使用令牌作为授权GraphQL请求的唯一形式。 我们不建议单独使用会话Cookie或将其与授权令牌结合使用。 请参阅 [GraphQL Authorization](https://devdocs.magento.com/guides/v2.4/graphql/authorization-tokens.html). <!-- B2B-2204 -->
...
有关增强功能和错误修复的信息，请参阅 [PWA Studio版本](https://github.com/magento/pwa-studio/releases). 请参阅 [版本兼容性](https://developer.adobe.com/commerce/pwa-studio/integrations/adobe-commerce/version-compatibility/) 有关PWA Studio版本及其兼容的Adobe Commerce核心版本的列表。
...
* DI配置验证现在验证对中已移除或弃用的非API核心类的引用 `di.xml` 以及核心类和接口的首选项。  <!-- AC-438-->
...
* 现在，您可以重命名数据修补程序，并将旧类名作为别名添加到 `patch_list` 数据库表。 现在，Adobe Commerce在应用修补程序之前，会检查数据库中是否已存在数据修补程序别名。 以前，Adobe Commerce在这些情况下会引发错误。
...
* 刷新Adobe Commerce缓存后，Admin中不再显示已禁用全页缓存，并且 `use_stale_cache` 已启用。
...
* 现在可处理由SKU添加的受限产品的权限异常。 购物者现在可以在店面看到相应的消息，并且错误表中的数量字段已禁用。 以前，Adobe Commerce抛出如下异常： `There has been an error processing your request`.
...
* 此 `cart` 当产品缺货时，查询不再返回空响应。 新 `errors` 响应中引入了包含错误消息的元素。 以前，当您使用缺货产品运行查询时，Adobe Commerce显示 `null` 下的值 `items` 部分作为响应。 请参阅 [购物车查询](https://devdocs.magento.com/guides/v2.4/graphql/queries/cart.html).
...
* 新 `ConfigurableWishlistItem.configured_variant` 字段已替换 `ConfigurableWishlistItem.child_sku` 字段。 当客户的愿望清单中包含未配置的可配置产品时，后一个字段触发了内部错误。
...
* 当 `Synchronize widget products with backend storage` 设置已启用，Adobe Commerce会将最近查看的产品数据添加到中 `catalog_product_frontend_action` 数据库表。 在添加记录时，它包括客户或访客ID。 此 `recently_viewed_product` 如果客户ID和访客ID为空，则响应中的部分现在为空。 因此，当 `customer/section/load` 在发送Ajax请求后，Adobe Commerce可以根据客户或访客ID正确筛选最近查看过的产品。 以前，响应包含 `catalog_product_frontend_action` 数据库表，因为没有检查空的客户或访客ID。

<!-- ACP2E-28-->

* 具有的类别规则 `Quantity` 现在，在实施暂存后，可配置产品的属性可正确运行。 以前，当产品的 `row_id` 和 `entity_id` 在 `catalog_product_entity` 表由于暂存更新，可视促销器 **按规则匹配产品** 功能未正确筛选产品。
...
* 在以下情况下，您现在可以更改类别中显示的每页产品限制 **记住类别分页**  已启用。 以前，Cookie `form_key` 和UI `form_key` 差异，Adobe Commerce会显示此错误：  `Invalid Form Key. Please refresh the page`.
...
* Adobe Commerce现在会在创建全局的过程中按预期显示可配置属性 `select` 通过修补程序脚本访问属性。 以前，符合条件的全局属性会被隐藏。
...
* 现在，当可配置产品的某个子产品的可用性因计划的更新而发生更改时，Adobe Commerce可正确显示可配置产品的库存状态、可用配置和显示的价格。 以前，当可配置产品具有两个子产品，并且计划更新禁用了一个子产品的可用性时，父产品的库存状态错误地显示为 **缺货** 当更新处于活动状态时，更新完成后，其配置的显示价格和可用性计算错误。
...
* 现在，当购物者单击后输入无效的电子邮件格式时，Adobe Commerce会在“配送”页面上显示错误消息 **下单**. 以前，错误消息显示在“付款”页面上。 [GitHub-33590](https://github.com/magento/magento2/issues/33590
...
* 此 `updateCartCurrency` 函数现在设置string而不是cart对象内的对象。 以前，Adobe Commerce不会使用加载报价 `getQuote` 因为 `updateCartCurrency` 函数设置一个对象，而不是在cart对象内设置字符串。 [GitHub-34199](https://github.com/magento/magento2/issues/34199)
...
* Adobe Commerce不再显示 `preg_replace()` 管理员错误。 第三个参数(`$subject`)现在属于类型 `array|string` 而不是 `bool`.
...
* Adobe Commerce现在，当您设置无效的Cookie域(**存储**  > **配置**  >  **Web**  >  **默认Cookie设置Cookie域**)。 以前，网站崩溃。 [GitHub-35048](https://github.com/magento/magento2/issues/35048)
...
* 激活Adobe Commerce后，不会再引发错误  **选中此处将RSS源链接到您的愿望清单** 复选框 **共享愿望清单**. [GitHub-34998](https://github.com/magento/magento2/issues/34998)
...
* `products` 现在，查询可以正确返回包含带以下内容的礼品卡产品的产品数据： `gift-card` URL键。
...
* 此 `categories` 在获取一个类别的列表时，查询不再引发异常，因为其中某个类别包含无法在文件系统上找到的图像。 以前，Adobe Commerce引发此异常： `Category image not found`. [GitHub-34266](https://github.com/magento/magento2/issues/34266)
...
* 发出Adobe Commerce请求后，GraphQL会话不再结束。 以前， `ClearCustomerSessionAfterRequest` 插件将购物者注销。 [GitHub-34550](https://github.com/magento/magento2/issues/34550)

<!-- AC-818-->

* 此 `products` 查询不再返回 `price_range` 受禁用的变体影响的可配置产品的值。 [GitHub-33629](https://github.com/magento/magento2/issues/33629)
...
* Adobe Commerce现在在“贷项通知单网格”页面上显示从商店视图（其名称前面有数字）创建的订单的贷项通知单。 以前，Adobe Commerce显示以下错误： `The store that was requested wasn't found. Verify the store and try again. Exception in /var/www/html/vendor/magento/module-store/Model/StoreRepository.php:75`. [GitHub-35122](https://github.com/magento/magento2/issues/35122)
...
* 管理员现在可以使用PayPal PayflowPro支付方式向管理员下单。 以前，Adobe Commerce会显示此错误： `No such entity with cartId = 0`.
...
* 此 **使用时间** 当客人客户下带有折扣的订单并随后注册账户时，折扣值不再增加两次。
...
* 此 `GET /V1/returnsAttributeMetadata` 现在，端点在以下情况下可正常工作： `rma_item` 实体默认属性集ID与默认安装ID不同。 以前，如果这些ID不同，此端点会返回空结果。

<!-- ACP2E-247-->

* 此 **使用默认值** 复选框（用于在产品编辑页面上启用RMA）现在可按预期用于“默认商店”。 以前，保存产品后会立即清除复选框。
...
* 在应用奖励点后，结账性能得到了改进。 时间 `entity` 为空，则不会执行数据库查询，因为查询 `magento_reward_history` 表格在这种情况下不是强制性的。

<!-- ACP2E-674-->

* 来宾订单的贷项通知单不再检查 `customer_id`，跳过查询 `magento_reward_history` 奖励分数的数据库表。 此更改提高了性能。

### 角色

* 此 **存储** >  **配置** > **服务** 页面现在会在资源访问设置为时，按预期显示Adobe Commerce Web API信息  **自定义** 在“角色资源”选项卡上。 [GitHub-35506](https://github.com/magento/magento2/issues/35506)
...
* 在以下情况下，Elasticsearch查询现在可以按预期工作 `int` 配置为可搜索的后端 `type` 属性。 此前，Adobe Commerce抛出 `Elasticsearch\Common\Exceptions\BadRequest400Exception` 例外。
...
* Adobe Commerce现在会在搜索建议所在的部署店面中显示准确的搜索结果建议计数， **显示每个建议的结果计数** 设置已启用。 以前，关键字旁边显示的计数为零。
...
* 此 `custom_design_to` 现在，在计划更新发生更改时，属性值会按预期更新。 以前，将两个没有结束时间的单独类别设计更新视为一个更新。 当第一个计划的更新运行期间被删除时， `custom_design_from` 时间值变得大于 `custom_design_to` 下次计划更新中的时间值。
...
* 当管理员尝试在管理员中保存街道行过多的地址时，Adobe Commerce现在显示信息性错误消息 **存储** > **属性**  > **客户地址**.  管理员现在可以删除额外的地址信息并成功保存该地址。 以前，Adobe Commerce会提交额外的行，但不会保存数据。
...
* 收藏夹图标上传表单现在支持 `.ico` 文件类型。 以前，当您尝试在管理员中上传具有此扩展类型的favicon文件时，Adobe Commerce会显示此错误： `Warning: imagecreatefromstring(): one parameter to a memory allocation multiplication is negative or zero, failing operation gracefully in /var/www/html/vendor/magento/module-media-storage/Model/File/Validator/Image.php on line 64`. [GitHub-34858](https://github.com/magento/magento2/issues/34858)
...
* 现在，您可以将Vimeo视频设置为在 `banner` CSP设置为时的元素 `restrict mode`. 以前，Adobe Commerce引发JavaScript错误。
...
**问题**： [!DNL RabbitMQ] 对Adobe Commerce 2.4.5执行全新安装（非升级）后，基础架构元素（交换、队列）可能未初始化。此配置不正确 [!DNL RabbitMQ] 可能导致在执行异步操作或生成性能配置文件时产生致命错误。 云部署不受影响。 **解决方法**：在本地执行全新安装的Adobe Commerce 2.4.5的商家应该运行 `bin/magento setup:upgrade --keep-generated` 使用后 `bin/magento setup:config:set` 配置 [!DNL RabbitMQ] 或通过传递以下代码安装Adobe Commerce [!DNL RabbitMQ] 配置选项 `setup:install` 命令。 请参阅 [[!DNL RabbitMQ]](../../../installation/prerequisites/rabbitmq.md).
