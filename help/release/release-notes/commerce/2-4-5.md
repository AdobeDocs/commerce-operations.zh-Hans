...
* `bin/magento config:set graphql/session/disable 1`命令允许商家完全禁止为所有GraphQL操作创建会话Cookie。 默认情况下，Adobe Commerce会创建这些Cookie并依赖它们进行授权，这会影响性能。 将来，我们建议使用令牌作为授权GraphQL请求的唯一形式。 我们不建议单独使用会话Cookie或将其与授权令牌结合使用。 查看[GraphQL授权](https://devdocs.magento.com/guides/v2.4/graphql/authorization-tokens.html)。<!-- B2B-2204 -->
...
* 当产品缺货时，`cart`查询不再返回null响应。 已将包含错误消息的新`errors`元素引入响应。 以前，当您运行包含缺货产品的查询时，Adobe Commerce在响应中的`items`部分下显示了`null`值。 查看[购物车查询](https://devdocs.magento.com/guides/v2.4/graphql/queries/cart.html)。
...
* 现在，CMS页面的活动计划更新在CMS页面的计划更改部分中按预期显示。
