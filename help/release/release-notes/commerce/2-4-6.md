...
虽然这些功能的代码与Adobe Commerce核心代码的发行版本捆绑在一起，但其中几个项目(例如B2B、Page Builder和Progressive Web Applications (PWA) Studio)也单独发行。 每个项目的文档中都提供了特定于项目的单独发行信息，其中记录了这些项目的错误修复。

## Adobe Commerce 2.4.6 亮点
...
到目前为止，尚未发生与这些问题相关的已确认攻击。 但是，可能会利用某些漏洞访问客户信息或接管管理员会话。 这些问题中的大多数要求攻击者首先获取对管理员的访问权限。 因此，我们提醒您采取一切必要措施来保护您的管理员，包括但不限于以下措施：

* IP允许列表
* [双因素身份验证](https://developer.adobe.com/commerce/testing/functional-testing-framework/two-factor-authentication/)
* 使用 VPN
* 使用唯一的位置，而不是 `/admin`
* 良好的密码卫生

有关这些已修复问题的最新讨论，请参阅[Adobe安全公告](https://helpx.adobe.com/security/products/magento/apsb23-17.html)。
...
Adobe Commerce 2.4.6 现在支持：

* Composer 2.2.x. Composer 1.x 已被移除。 <!-- AC-2839 -->

* 瑞迪斯 7.0.x。尽管与 Redis 6.2 仍然兼容，但我们建议将此版本与 Redis 7.0.x 一起部署，因为 Redis 6.2 预计将在 2024 年结束生命周期。 <!-- AC-6606 -->

* 现在支持 OpenSearch 作为 Adobe Commerce 本地和云部署的默认搜索引擎。 它支持作为Magento Open Source的独立搜索引擎。 此版本支持OpenSearch v2.x，并已经过OpenSearch 2.5测试。尽管与OpenSearch 1.x的兼容性仍然存在，但我们建议将此版本与OpenSearch 2.x一起使用。您可以在云和本地[搜索引擎先决条件](https://experienceleague.adobe.com/docs/commerce-operations/installation-guide/prerequisites/search-engine/overview.html)上安装并运行带有OpenSearch 2.x的Adobe Commerce和Magento Open Source 2.4.6。<!-- AC-449 3555 -->
...
* 已从代码库中移除已到达生命周期结束的Zend框架(ZF1)组件。 查看[与向后不兼容的更改高亮显示](https://developer.adobe.com/commerce/php/development/backward-incompatible-changes/highlights/).<!-- AC-1512 -->
...
* **提高了影响许多客户区段的操作的性能**。 商家可以通过启用新的配置设置来提高涉及100多个客户区段的运营性能。 可以根据需要禁用&#x200B;**[!UICONTROL Real-time Check if Customer is Matched by Segment]**&#x200B;设置，以通过减少针对客户的每个客户区段的验证来缩短处理时间。 查看[客户区段验证](https://experienceleague.adobe.com/docs/commerce-operations/performance-best-practices/configuration.html#customer-segments-validation)。<!-- AC-6832 ACPT-835-->

* **新的系统配置设置，用于限制产品网格中显示的产品数**。 商家可以使用新的&#x200B;**[!UICONTROL Limit Number of Products in Grid]**&#x200B;配置设置来提高产品网格性能。 此限制配置为默认20000用，仅影响UI组件使用的产品集合。 限制显示的产品数量，可以提高具有200,000多种产品的网格的运行性能。 查看网格[限制产品数量](https://experienceleague.adobe.com/docs/commerce-operations/performance-best-practices/configuration.html#limit-number-of-products-in-grid)<!-- ACPT-42 -->

* **改进的导入性能（每分钟最多100,000条记录）**。 商家可以使用新的`POST /rest/<store_view_code>/V1/import/csv` REST API端点将数据导入Adobe Commerce。 此端点提供与管理员导入功能相同的功能，并支持使用CSV文件创建、更新和删除产品、产品定价和客户实体。 查看[导入数据](https://developer.adobe.com/commerce/webapi/rest/modules/import/)。<!-- ACPT-6 -->

* **通过负载平衡提高了高吞吐量订单处理性能**。 云基础架构上具有高并发订单量（每分钟1000个订单）的Adobe Commerce 2.4.6现在可以通过平衡不同节点上的数据库负载来提高订单处理性能。 商家可以通过为MySQL数据库和Redis实例启用辅助连接来重新平衡此负载。 查看[高吞吐量订单处理](https://experienceleague.adobe.com/docs/commerce-operations/performance-best-practices/high-throughput-order-processing.html#load-balancing) <!-- ACPT-495 -->


### Adobe Commerce扩展中继包

此版本引入了Adobe Commerce扩展metapackage v1.1.0，它自动将所选Adobe Commerce扩展与此核心版本捆绑在一起。 运行`composer update`时将安装此中继包中包含的这些扩展版本，从而简化升级到最新核心版本时升级这些扩展的过程。 这些扩展仍将保持独立的发布计划。
...
* 适用于Adobe Commerce的[Adobe I/O Events](https://developer.adobe.com/commerce/events/get-started/)

此扩展metapackage的未来版本将包含其他扩展。

### 适用于Adobe Commerce的Adobe I/O Events

适用于Adobe Commerce的Adobe I/O Events捕获Commerce事件并将数据从Adobe Commerce传输到其他Adobe系统。 开发人员可以在Adobe Developer App Builder中构建云原生应用程序，这些应用程序可订阅Commerce事件，例如库存更新、订单状态更改和价格更新。 开发人员可以使用此功能交换数据，以在其任务关键型系统中编排整个购买者历程。
...
新的`adobe-ims-metapackage` v2.2.0与Adobe Commerce 2.4.6捆绑在一起。此版本中未更改任何功能。<!-- CABPI-505-->
...
* 采购订单GraphQL架构涵盖B2B公司用户购买Order Management功能，如创建、编辑和删除采购订单。 查看[采购订单(B2B)](https://developer.adobe.com/commerce/webapi/graphql/schema/b2b/purchase-order/)。
...
页面生成器 v.1.7.3 与 Adobe Commerce 2.4.6 兼容。


### PWA Studio

PWA Studio v.13.0.x与Adobe Commerce 2.4.6兼容。它包括多项增强功能，可改善辅助功能。 有关错误修复的信息，请参阅[PWA Studio发行版](https://github.com/magento/pwa-studio/releases)。 有关PWA Studio版本及其兼容的Adobe Commerce核心版本的列表，请参阅[版本兼容性](https://developer.adobe.com/commerce/pwa-studio/integrations/adobe-commerce/version-compatibility/)。


### 已知问题

**问题**： New Relic当前与PHP 8.2不兼容。运行New Relic的部署可以在PHP 8.1上运行Adobe Commerce 2.4.6，但不能在PHP 8.2上运行。**解决方法**：在PHP 8.1上运行Adobe Commerce 2.4.6，直到发布支持PHP 8.2的新版本的New Relic。此版本预计将于2023年夏季发布。<!-- AC-7769-->
...
* 在运行静态内容部署(`bin/magento set:static-content:deploy -f --strategy compact`)时，压缩策略不再导致对外部域的无效引用。

<!-- AC-5994-->

* 执行`bin/magento setup:di:compile -vvv`（或使用`-v`或`-vv`选项）现在会在失败时返回信息性消息。 [GitHub-35391](https://github.com/magento/magento2/issues/35391)

<!-- AC-5973-->

* 在已运行数据迁移工具的部署中运行`bin/magento setup:upgrade`命令时，Adobe Commerce不再引发以下异常： `PHP Fatal error:  Declaration of Migration\Logger\Logger::addRecord(int $level, string $message, array $context = []): bool must be compatible with Monolog\Logger::addRecord(int $level, string $message, array $context = [], ?Monolog\DateTimeImmutable $datetime = null): bool in /var/www/html/vendor/magento/data-migration-tool/src/Migration/Logger/Logger.php on line 34`。 [GitHub-35604](https://github.com/magento/magento2/issues/35604)
...
* 在 Alpine Linux 上升级后，运行`bin/magento sampledata:remove`以删除示例数据不再失败，并显示此错误：。 `Undefined constant "MagentoHackathon\Composer\Magento\Deploystrategy\GLOB_BRACE`[GitHub-36062](https://github.com/magento/magento2/issues/36062)
...
* 按预期在搜索结果中找到的项目总数 <!--- AC-3522-->
* 用户使用键盘控件成功登录时登录的用户名 <!--- AC-6477-->
* 产品搜索结果页眉现在可以读取元素的正确 `<title>` 值 <!--- AC-2489-->
* 输入字段是否为必填字段 <!--- AC-2494-->
...
* 使用荷兰语言环境时，现在可以从管理员&#x200B;**[!UICONTROL Catalog]** > **[!UICONTROL Products]**&#x200B;成功筛选捆绑产品。 以前，当您尝试筛选`Type: Bundle Product`时Adobe Commerce显示此错误： `Something went wrong with processing the default view and we have restored the filter to its original state`。 [GitHub-35704](https://github.com/magento/magento2/issues/35704)

<!-- ACP2E-1038-->

* 当捆绑产品的特殊价格超过1000时，产品网格不再引发异常。 以前，当您按SKU搜索产品或导航到&#x200B;**[!UICONTROL Catalog]** > **[!UICONTROL Products]**&#x200B;时，Adobe Commerce引发此错误： `Non Numeric value encountered`。 [GitHub-36284](https://github.com/magento/magento2/issues/36284))
...
* 现在，在管理员减少购物车中的产品数量后，购物者可以从购物车中减少该产品的数量。 以前，当购物者尝试减少产品数量时，Adobe Commerce引发以下错误： `The requested qty is not available`。 [GitHub-35780](https://github.com/magento/magento2/issues/35780)

<!-- AC-6744-->

* 在多配送结帐期间，当产品数量超过配置的最大值时，Adobe Commerce不再在JavaScript控制台中引发错误。 Adobe Commerce现在改为显示信息性错误消息。 [GitHub-36184](https://github.com/magento/magento2/issues/36184)
...
* 现在，在将产品添加到购物车时权限不足时，如果`addProductsToCart`突变失败，Adobe Commerce会返回一条信息性消息。
...
* `setGiftOptionsOnCart`突变现在需要客户授权才能进行客户购物车操作。
...
* 现在，当顾客从“我的购物车”页面修改可配置商品时，会在商品详情页面中预先选择色板选项颜色和尺寸属性。 （jQuery 3.x 版本现在支持 `swatchRenderer` Widget 名称，现在是驼峰大小写。以前，不会预先选择色板选项。
...
* 当管理员用户手动将产品添加到已分配了许多简单产品的可配置产品时，Adobe Commerce不再引发错误。 以前，Adobe Commerce显示此错误： `Something went wrong with processing the default view and we have restored the filter to its original state`。
...
* 管理优惠券代码工作流程中添加了验证逻辑，以便在生成优惠券时强制执行优惠券代码设置。 以前，当&#x200B;**优惠券数量**&#x200B;值大于2500时，优惠券代码生成会忽略&#x200B;**[!UICONTROL Code Length]**&#x200B;设置。

<!-- ACP2E-1029-->

* 现在，在使用PayPal Express付款方式结账期间应用奖励券时，会按预期将奖励积分添加到客户的帐户中。
...
* 您现在可以按多个购物车价格规则筛选优惠券报告。 [GitHub-36272](https://github.com/magento/magento2/issues/36272)

<!-- AC-2812-->

* 现在，当管理员用户在配置具有自定义选项的产品时按&#x200B;**[!UICONTROL Enter]**&#x200B;时，将应用并保存自定义选项。 以前，您可以选择选项并将产品添加到订单，但选项未保存，并且提示管理员用户重新选择产品选项。 [GitHub-35284](https://github.com/magento/magento2/issues/35284)
...
* `catalog_index_refresh_price` cron作业现在已成功执行。 以前，Adobe Commerce在cron执行期间记录此错误： `report.ERROR: Cron Job catalog_index_refresh_price has an error: SQLSTATE[HY093]: Invalid parameter number: number of bound variables does not match number of tokens`。
...
* 当&#x200B;**欧盟国家/地区**&#x200B;设置中没有国家/地区(**[!UICONTROL Stores]** > **[!UICONTROL Configuration]** > **[!UICONTROL General]** > **[!UICONTROL Country options]** > **[!UICONTROL European Union Countries]**)时，管理员用户现在可以成功打开管理员&#x200B;**[!UICONTROL Store]** > **[!UICONTROL Configuration]**&#x200B;页面。 以前，当管理员用户尝试打开此页面时，Adobe Commerce引发此错误： `TypeError: explode(): Argument #2 ($string) must be of type string, null given in /var/www/php/folder/vendor/magento/module-config/ViewModel/CountryFilter.php:46`。 [GitHub-35901](https://github.com/magento/magento2/issues/35901)

<!-- AC-6361-->

* `jquery.cookie` 依赖项现在可按预期方式处理 JavaScript 依赖项。 以前，不能对 使用 `jquery/jquery.cookie`别名。 [GitHub-35913](https://github.com/magento/magento2/issues/35913)

<!-- AC-3226-->

* 该 `bin/magento i18n:collect-phrases` 命令现在可在导出的 CSV 文件中包含 `<title>` 文本。 [GitHub-35462](https://github.com/magento/magento2/issues/35462)
...
* 在运行Adobe Commerce 2.4.3-p2的部署中，现在可以按预期重新授权OAuth集成。 以前，重新授权失败，错误为： `The attempt to post data to consumer failed due to an unexpected error. Please try again later`。 Adobe Commerce还尝试解密未加密的值。 [GitHub-35313](https://github.com/magento/magento2/issues/35313)
...
* 由于运行PHP 8.1的部署上弃用的`trim()`函数，AMQP使用者不再失败。以前，当AMQP使用者在安装后启动时，Adobe Commerce引发此错误： `Deprecated Functionality: trim(): Passing null to parameter #1 ($string) of type string is deprecated in .../vendor/magento/framework-amqp/Config.php on line 143`。 [GitHub-35380](https://github.com/magento/magento2/issues/35380)

<!-- AC-2882-->

* `getFilteredTimerIds`方法现在按预期工作。  类`Magento\Framework\Profiler\Driver\Standard\Stat`现在与PHP 8.1兼容。以前，调用此方法时Adobe Commerce会引发异常。 [GitHub-35307](https://github.com/magento/magento2/issues/35307)
...
* 当管理员用户从管理员统计数据仪表板选择&#x200B;**[!UICONTROL Last 24 Hours]**&#x200B;选项时，Adobe Commerce现在会显示准确的时间范围。 以前，Adobe Commerce显示的时间范围不正确。 发生这种情况是因为数据库的时区保持不变。
...
* 查询现在根据 `customer` 范围筛选订单。 现在，您可以指定查询应按商店、网站还是全局筛选订单。

<!-- ACP2E-1172-->

* 该 `categoryList` 查询现在返回定位和非定位父类别的准确产品计数。 新 `is_anchor` 属性已添加到类别集合选定列列表中。 以前，请求返回的产品计数为 0。 [GitHub-36300](https://github.com/magento/magento2/issues/36300)
...
* 购物车重新加载现在仅在购物车由 GraphQL 突变初始化时按预期触发一次。 以前，购物车加载会针对库存触发不必要的购物车重新验证，每次加载购物车时都会消耗大量资源。
...
* 现在，当CSV文件中的库存数据列缺失时，可正确导入产品状态。 以前，在产品导入后，默认来源会自动分配给产品，其数量设置为零，并且当CSV文件中的库存数据列缺失时，其状态列为脱销。

<!-- ACP2E-841-->

* 现在，如果导入包含100条以上的记录，则导入高级定价值将为新记录和更新的记录提供正确的计数。 以前，导入成功消息提供的计数错误。
...
* `Fixer.io`导入服务现在可按预期工作。 现在可以成功导入汇率。 [GitHub-36010](https://github.com/magento/partners-magento2ee/issues/36010)
...
* `catalogpermissions_category`索引的索引器`batch_size`现在已按预期更改为`env.php`文件中提供的值。 以前，索引器`batch_size`覆盖的路径中的拼写错误会阻止将批次大小更改为`env.php`中提供的值。 [GitHub-652](https://github.com/magento/partners-magento2ee/issues/652)
...
* 为支持Adobe Commerce，在数据库转储中添加了`DATE`和`DATETIME`值的清理。 以前，数据库备份表中客户/报价/订单表的日期时间类型字段不会经过清理。

<!-- ACP2E-804-->

* `escaper.js` `escapeHtml`函数现在根据`InnerHtml`规则将&amp;特殊字符转换为`"$amp;"`。 此通用解决方案解决了标题输入框中的特殊字符和字符引用转换问题，并将插入的确切值保存下来。 以前，在内容层级的节点名称中，&amp;字符编码不正确。
...
* 旧版媒体集现在默认处于禁用状态。 以前，**[!UICONTROL Enable old Media gallery]**&#x200B;设置为&#x200B;**[!UICONTROL Yes]**。
...
* 现在，Admin sales order credit memo（管理员销售订单贷项通知单）页面会显示使用Store Credit（商店积分）和多张发票创建的订单的正确总计。 以前，总计计算不正确，用户无法生成贷项通知单。

<!-- ACP2E-998-->

* 现在，安装脚本中传递的订单前缀会正确添加到订单ID中。

<!-- ACP2E-790-->

* 现在，当订单包含具有长自定义文本值的产品时，可成功加载订单详细信息页面。 以前，未加载订单详细信息页面，并且如果产品包含文本自定义选项的长值，则`sales_order_item.product_options`列中的值未正确存储。
...
* 对于来宾用户创建的订单，客户组现在准确表示为&#x200B;**未登录**。 以前，来宾订单摘要中的客户组为空，这影响了确认电子邮件和订单导出。 [GitHub-35440](https://github.com/magento/magento2/issues/35440)

<!-- AC-2811-->

* 在发出贷项通知单并退还客户后，现在将按预期为虚拟订单分配&#x200B;**[!UICONTROL Closed]**&#x200B;状态。 以前，从管理员添加注释时，虚拟产品在“状态”下拉列表中显示为&#x200B;**[!UICONTROL Complete]**，但在订单注释的页脚中显示为&#x200B;**[!UICONTROL Closed]**。 [GitHub-35283](https://github.com/magento/magento2/issues/35283)

<!-- AC-6641-->

* 来宾现在可以订购数量小于1的产品，并且产品数量现在严格键入为`float`。 以前，由于产品数量被严格键入为`int`，因此产品数量少于一个的订单失败，并显示以下消息： `Some of the products are disabled`。 [GitHub-36060](https://github.com/magento/magento2/issues/36060)
...
* 现在，在启用异步索引的情况下为免费产品下达自动开票的订单可按预期运行。 没有向这些订单的`sales_order_grid or sales_invoice_grid`表添加任何条目。 以前，Adobe Commerce引发异常，因为应用程序尝试在放置订单期间为网格编制索引，而不是依赖`cron`。 [GitHub-36334](https://github.com/magento/magento2/issues/36334)
...
* 页面生成器产品属性列表中现在有`Dropdown`、`Text area`和`Text field`属性。 以前，只有`Dropdown`和`Text area`属性可用。
...
* 当商家单击&#x200B;**[!UICONTROL Reports]** > **[!UICONTROL Braintree Settlement]**&#x200B;时，Adobe Commerce不再引发以下错误： `Error: Undefined constant Braintree\PaymentInstrumentType::ANDROID_PAY_CARD in /var/www/html/vendor/paypal/module-braintree-core/Ui/Component/Report/Listing/Column/PaymentType.php:49`。 [GitHub-35434](https://github.com/magento/magento2/issues/35434)
...
* 现在，管理员用户可以在启用 Braintree 付款方式后创建订单或重新排序。 以前，Adobe Commerce 会引发此错误： `report.CRITICAL: Error: Call to a member function getMethodInstance() on null in /app/vendor/paypal/module-braintree-core/Block/Form.php:174`。

<!-- BUNDLE-3139-->

* 现在，启用并 **[!UICONTROL Enable Card Payments]** 禁用 Braintree PayPal后，使用 Braintree 付款方式结账可以成功完成。 以前，进度条永远不会解析，并且 Adobe Commerce 显示以下控制台错误： `adapter.js:66 Uncaught TypeError: Cannot read properties of undefined (reading 'clientToken')at Object.getClientToken (adapter.js:66:66)`。 [GitHub-35961](https://github.com/magento/magento2/issues/35961)

<!-- BUNDLE-3140 3141-->

* 已实施欺诈保护、ACH 支付和本地支付方式网络挂钩。 欺诈保护 Web 挂钩仅在 Braintree 中做出风险决策时触发。 只有在 ACH 付款更新到 `settled` 或 `settlement decline`后，才会触发 ACH 网络挂钩。
...
* 购物者现在可以在已禁用&#x200B;**[!UICONTROL Enable Card Payments]**&#x200B;的店面中使用Braintree付款方法。 以前，Adobe Commerce引发此JavaScript错误： `_adapter.js:66 Uncaught TypeError: Cannot read properties of undefined (reading 'clientToken')at Object.getClientToken (adapter.js:66:66).`
...
* 更新类别规则时，管理日志报告现在显示准确的&#x200B;**[!UICONTROL Value before change]**&#x200B;值。
...
* RMA注释字段(**[!UICONTROL Orders]** > **[!UICONTROL Return]**)不再包含多余的空格。 (`<textarea>`和`<?php`标记现在位于一行中。

<!-- ACP2E-1279-->

* 现在，在保存产品时，**[!UICONTROL Enable RMA]**&#x200B;字段的&#x200B;**[!UICONTROL Use Config Settings]**&#x200B;复选框会按预期保存其值。
...
* 启用&#x200B;**[!UICONTROL Minimum Terms to Match]**&#x200B;配置设置后，在引号中搜索字符串现在可以按预期工作。 以前，Adobe Commerce引发此错误： `{"error":{"root_cause":[{"type":"parsing_exception","reason":"[match_phrase] query does not support [minimum_should_match]' ...`。
...
* 在显示送货地址或客户地址时，Adobe Commerce现在遵循管理员客户&#x200B;**[!UICONTROL Show Company]**&#x200B;配置设置。 禁用此设置后，发运公司将不再包含在发运、客户地址或数据库订单记录中。 以前，尽管设置被禁用，仍会显示该信息。 [GitHub-33729](https://github.com/magento/magento2/issues/33729)
...
* Adobe Commerce现在可以在运行PHP 8.1的部署中的storefront category页面上显示已分组的含税产品。以前，Adobe Commerce不显示产品列表，并引发此异常： `main.CRITICAL: Exception: Deprecated Functionality: ucfirst(): Passing null to parameter #1 ($string) of type string is deprecated in /var/www/m24/vendor/magento/module-tax/Pricing/Render/Adjustment.php on line 188 in /var/www/m24/vendor/magento/framework/App/ErrorHandler.php:61`。 [GitHub-35500](https://github.com/magento/magento2/issues/35500)
...
* 当没有与客户地址关联的增值税ID时，现在可以自动分配客户组。 如果地址不含增值税ID，则被视为属于非增值税注册客户，Adobe Commerce会将该客户组更改为非增值税组。 以前，启用&#x200B;**[!UICONTROL Enable Automatic Assignment to Customer Group]**&#x200B;后，当从客户帐户中删除VAT ID时，客户帐户不会自动分配给`customerGroup`。
...
* 现在，在使用GraphQL添加或更新来宾购物者的地址时，您可以指定`vat_id`。

### 测试

<!-- ACP2E-1171-->

* 修复了不同版本的`ICU`和`libxml`库单元测试错误。
...
* 现在，内联翻译可正确使用特殊字符（包括位于SCRIPT标签中的短语中的扩展Unicode字符）。 以前，使用内联翻译时，日语字符在编辑后的脚本中渲染不正确。
...
* 运行`bin/magento i18n:collect-phrases`的输出现在按预期在`depend`构造中包含邮件模板转换。 [GitHub-35449](https://github.com/magento/magento2/issues/35449)
...
* 用户角色网格（管理员&#x200B;**[!UICONTROL System]** > **[!UICONTROL Permissions]** > **[!UICONTROL User roles]** > **[!UICONTROL Administrators]**）的筛选器函数现在按预期工作。 现在将单独处理数组值。 以前，当您尝试在此网格中输入查询时，Adobe Commerce引发此控制台错误，并且页面未加载： `Uncaught TypeError: Cannot read properties of null`。 [GitHub-35993](https://github.com/magento/magento2/issues/35993)

<!-- ACP2E-328-->

* 在通过REST API创建客户时，添加了属性值的验证。 现在，当提供无效的属性值时，创建过程将失败。 以前，没有性别验证，这不是必填字段。 在客户创建期间，为性别传递的值保存在`customer_entity`表中。
...
* 当产品按规则匹配时，您无法再手动更改视觉跟单员类别列表中的产品位置。

<!-- ACP2E-985-->

* 访问权限仅限于一个商店的管理员用户无法再在 Visual Merchantr 中拖放某个类别中的产品。 相反，他们现在可以使用该 **[!UICONTROL Position]** 列重新排列产品位置。 以前，这些用户只能通过拖放来更改类别中的产品位置。

<!-- ACP2E-953-->

* 类别编辑页面的“类别中的产品”部分现在显示正确数量的可配置产品。 现在，正确的表列用于检索可配置的产品数量。
...
* 商家现在可以使用REST API为总金额为零的订单创建贷项通知单。 以前，Adobe Commerce显示此错误： `The order does not allow a credit memo to be created. The credit memo's total must be positive. We can't create a credit memo for the invoice/order`。
...
* 现在，您可以使用 REST API `/rest/V1/bulk?searchCriteria[filter_groups]` 请求按搜索条件查询批量作。 以前，返回以下错误： `report.CRITICAL: Report ID: webapi-624bbb1db96c4; Message: Item (Magento\AsynchronousOperations\Model\Operation) with the same ID "0" already exists.`

<!-- magento/magento2/pull/36147-->

* 现在，使用管理基本价格 API，可在管理员和店面目录搜索页面上正确更新产品价格。 [GitHub-33767](https://github.com/magento/magento2/issues/33767)
...
1. 下载所需的修补程序。 请参阅从版本&lt; 2.4.5](https://experienceleague.adobe.com/docs/commerce-knowledge-base/kb/troubleshooting/known-issues-patches-attached/salesrule-label-issues-when-upgrade-from-versions-under-2.4.5.html)升级时的[salesRules标签问题。
...
**解决方法**：此问题的修补程序现已可用。 转到[Quality Patches Tool：搜索修补程序](https://experienceleague.adobe.com/tools/commerce-quality-patches/index.html)并搜索修补程序ACSD-50814。
...
您可以使用[编辑器](https://experienceleague.adobe.com/docs/commerce-operations/installation-guide/composer.html)安装Adobe Commerce 2.4.6。
