...
尽管这些功能的代码与Adobe Commerce核心代码的发布捆绑在一起，但其中一些项目(例如，B2B、Page Builder和Progressive Web Application(PWA)Studio)也是独立发布的。 这些项目的错误修复会在特定于项目的单独发行信息中进行记录，该信息可在每个项目的文档中找到。
...
Adobe Commerce 2.4.6引入了对PHP 8.2的支持。PHP 8.1仍完全受支持。 <!-- AC-6598 -->

此版本中移除了对PHP 7.4的支持。 不能在PHP 7.4上运行Adobe Commerce 2.4.6。
...
* **改进了影响许多客户区段的操作的性能**. 通过启用新的配置设置，商家可以提高涉及100多个客户区段的操作的性能。 的 **[!UICONTROL Real-time Check if Customer is Matched by Segment]** 可以根据需要禁用设置，以通过减少每个客户区段针对客户的验证来缩短处理时间。 请参阅 [客户区段验证](https://experienceleague.adobe.com/docs/commerce-operations/performance-best-practices/configuration.html#customer-segments-validation). <!-- AC-6832 ACPT-835-->

* **用于限制产品网格中显示的产品数量的新系统配置设置**. 商户可以使用 **[!UICONTROL Limit Number of Products in Grid]** 配置设置，以提高产品网格性能。 默认情况下，此限制配置为20000，仅影响UI组件使用的产品收藏集。 限制显示的产品数量可以提高网格中200,000多种产品的操作性能。 请参阅 [网格中的产品数量限制](https://experienceleague.adobe.com/docs/commerce-operations/performance-best-practices/configuration.html#limit-number-of-products-in-grid)<!-- ACPT-42 -->
...
* [螺栓快速结帐](https://experienceleague.adobe.com/docs/commerce-merchant-services/quick-checkout/overview.html)
* [Adobe I/OAdobe Commerce事件](https://developer.adobe.com/commerce/events/get-started/)

此扩展的未来版本将包含其他扩展。

### Adobe I/OAdobe Commerce事件
...
* 采购订单GraphQL架构涵盖B2B公司用户采购订单管理功能，例如创建、编辑和删除采购订单。 请参阅 [采购订单(B2B)](https://developer.adobe.com/commerce/webapi/graphql/schema/b2b/purchase-order/).
* 采购订单规则GraphQL架构提供创建、编辑和删除审批规则以及人工验证采购订单的功能。 请参阅 [采购订单批准规则(B2B)](https://developer.adobe.com/commerce/webapi/graphql/schema/b2b/purchase-order-rule/).
...
我们修复了Adobe Commerce 2.4.6核心代码中的数百个问题。 下面介绍了Adobe Commerce 2.4.6中包含的已修复问题的子集。
...
* Google reCAPTCHA现在可按预期用于店面订单。 以前，当购物者首次尝试下订单之前未先选择Google ReCAPTCHA时，Adobe Commerce会显示以下错误： `ReCaptcha validation failed, please try again` 和 `No such cart with id = ID`.
...
* Adobe Commerce现在会在 `addProductsToCart` 当将产品添加到购物车时，如果权限不足，突变会失败。
...
* 现在，旧版媒体集默认处于禁用状态。 以前， **[!UICONTROL Enable old Media gallery]** 设置为 **[!UICONTROL Yes]**.
...
* 现在，在启用BraintreePayPal并且 **[!UICONTROL Enable Card Payments]** 已禁用。 以前，进度栏从未解析，Adobe Commerce显示此控制台错误： `adapter.js:66 Uncaught TypeError: Cannot read properties of undefined (reading 'clientToken')at Object.getClientToken (adapter.js:66:66)`. [GitHub-35961](https://github.com/magento/magento2/issues/35961)
...
* 购物者现在可以在店面中使用Braintree付款方法， **[!UICONTROL Enable Card Payments]** 已被禁用。 以前，Adobe Commerce会抛出以下JavaScript错误： `_adapter.js:66 Uncaught TypeError: Cannot read properties of undefined (reading 'clientToken')at Object.getClientToken (adapter.js:66:66).`
...
* 管理员日志报表现在可以准确显示 **[!UICONTROL Value before change]** 值。
...
* 现在，在 **[!UICONTROL Minimum Terms to Match]** 配置设置已启用。 以前，Adobe Commerce会抛出以下错误： `{"error":{"root_cause":[{"type":"parsing_exception","reason":"[match_phrase] query does not support [minimum_should_match]' ...`.
...
* Adobe Commerce现在荣誉管理员客户 **[!UICONTROL Show Company]** 配置设置。 禁用此设置后，发运公司将不再包含在发运、客户地址或数据库订单记录中。 以前，尽管设置了禁用，仍会显示信息。 [GitHub-33729](https://github.com/magento/magento2/issues/33729)
...
* 现在，当没有与客户地址关联的增值税ID时，即可自动分配客户组。 如果地址不包含增值税ID，则它将被视为属于非增值税注册客户，而Adobe Commerce会将客户组更改为非增值税组。 以前， **[!UICONTROL Enable Automatic Assignment to Customer Group]** 启用，则客户帐户未自动分配给 `customerGroup` 从客户帐户中删除增值税ID时。
