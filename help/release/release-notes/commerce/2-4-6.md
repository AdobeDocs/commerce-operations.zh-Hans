...
* 现在，支持将OpenSearch用作Adobe Commerce内部部署和云部署的默认搜索引擎。 它支持作为Magento Open Source的独立搜索引擎。 此版本支持OpenSearch v2.x，并已经过OpenSearch 2.5测试。尽管与OpenSearch 1.x的兼容性仍然存在，但我们建议将此版本与OpenSearch 2.x一起使用。您可以在云和本地[搜索引擎先决条件](https://experienceleague.adobe.com/docs/commerce-operations/installation-guide/prerequisites/search-engine/overview.html)上安装并运行带有OpenSearch 2.x的Adobe Commerce和Magento Open Source 2.4.6。<!-- AC-449 3555 -->
...
* [Bolt Quick Checkout](https://help.bolt.com/platforms/adobe/adobe-commerce-setup-guide/m2-migrate/)
...
**客户查询现在支持订单排序**。 `orders`查询的`customer`输出对象现在包含排序字段。 您现在可以按订单编号或创建日期以及升序或降序对订单进行排序。<!-- AC-6903-->
...
* 在首次执行`bin/magento setup:upgrade`后，`composer install --no-dev`现在已成功执行。 [GitHub-36351](https://github.com/magento/magento2/issues/36351)
...
* 当未设置物料折扣数量或物料折扣数量大于或等于订单物料数量时，产品的`discount_percent`表的`sales_order_item`列现在按预期使用折扣百分比填充。 以前，在将此产品应用购物车价格规则时不会填充此列。
...
* 管理优惠券代码工作流中添加了验证逻辑，以便在生成优惠券时强制实施优惠券代码设置。 以前，当&#x200B;**[!UICONTROL Code Length]**&#x200B;优惠券数量&#x200B;**值大于2500时，优惠券代码生成会忽略**&#x200B;设置。
...
* 当&#x200B;**[!UICONTROL Store]**&#x200B;欧盟国家/地区&#x200B;**[!UICONTROL Configuration]**&#x200B;设置中没有国家/地区(**>** > **[!UICONTROL Stores]** > **[!UICONTROL Configuration]** > **[!UICONTROL General]**)时，管理员用户现在可以成功打开管理员&#x200B;**[!UICONTROL Country options]** > **[!UICONTROL European Union Countries]**&#x200B;页面。 以前，当管理员用户尝试打开此页面时，Adobe Commerce引发此错误： `TypeError: explode(): Argument #2 ($string) must be of type string, null given in /var/www/php/folder/vendor/magento/module-config/ViewModel/CountryFilter.php:46`。 [GitHub-35901](https://github.com/magento/magento2/issues/35901)
...
* 现在，在为具有子类别的类别保存类别`rowId`时，传递了正确的`urlKey`参数。 以前，在保存类别`categoryId`时传递了错误的`URLKey`。
...
* `recordLogin`方法已更新，以设置用户模型的登录值，并保留`logdate`表的`lognum`和`admin_user`字段中的正确值。 以前，不会记录更新后管理员用户的登录尝试。
...
* 禁用&#x200B;**[!UICONTROL Grid Filter Condition Type]** s **[!UICONTROL Use in Filter Option]设置后，客户和客户地址属性的**&#x200B;字段在属性编辑页面上不再可见。
...
* **[!UICONTROL Show my quotes]**&#x200B;页面上的&#x200B;**[!UICONTROL My Quotes]**&#x200B;筛选器现在按预期工作。 以前，筛选结果包括其他管理员用户的引号。
...
* 在GraphQL代码中实现多个接口的`Types`现在可以用`&`符号分隔，也可以按照GraphQL `,`规范使用`@doc`分隔。
...
* 现在，`bin/magento app:config:import`命令在包含某些`config.php`表达式时已成功导入`cron`文件的内容。 以前，导入失败，出现此错误： `Import failed: Notice: Trying to access array offset on value of type null in [..]/vendor/magento/module-config/Model/Config/Backend/Currency/Cron.php on line 66`。 [GitHub-31428](https://github.com/magento/magento2/issues/31428)
...
* `batch_size`索引的索引器`catalogpermissions_category`现在已按预期更改为`env.php`文件中提供的值。 以前，索引器`batch_size`覆盖的路径中的拼写错误会阻止将批次大小更改为`env.php`中提供的值。 [GitHub-652](https://github.com/magento/partners-magento2ee/issues/652)
...
* `escaper.js` `escapeHtml`函数现在根据`"$amp;"`规则将&amp;特殊字符转换为`InnerHtml`。 此通用解决方案解决了标题输入框中的特殊字符和字符引用转换问题，并将插入的确切值保存下来。 以前，在内容层级的节点名称中，&amp;字符编码不正确。
...
* 公司属性值现在按预期显示在结账工作流的帐单和送货地址部分。 以前，公司属性无法在订单详细信息页面上正确显示，因为在订单创建期间未正确设置商店。 仅当在多存储部署中将&#x200B;**主网站**&#x200B;的&#x200B;**[!UICONTROL optional]**&#x200B;显示公司&#x200B;**属性设置为**&#x200B;时，才能正确存储属性值。

<!-- ACP2E-1117-->

* 在选中`customer_address_id`复选框时，**[!UICONTROL My billing and shipping address are the same]**&#x200B;值现在为新帐单地址按预期设置。 在设置送货地址的客户地址ID之前，还引入了一个附加条件来标识帐单地址是否与送货地址相同。
...
* 已从管理员样式部分的所有&#x200B;**[!UICONTROL Tagline]**&#x200B;按钮中删除&#x200B;**[!UICONTROL Layout]**&#x200B;和&#x200B;**[!UICONTROL PayPal]**&#x200B;配置字段。
...
* `ANDROID_PAY_CARD`中的`Braintree/Ui/Component/Report/Listing/Column/PaymentType.php`常量已替换`GOOGLE_PAY_CARD`。 这可解决Braintree报表中的问题。
...
* 现在，在保存产品时，**[!UICONTROL Use Config Settings]**&#x200B;字段的&#x200B;**[!UICONTROL Enable RMA]**&#x200B;复选框会按预期保存其值。
...
* 启用&#x200B;**[!UICONTROL Display Out Of Stock Products]** GitHub-36055[后，搜索结果现在包含所有可用产品的缺货选项输出](https://github.com/magento/magento2/issues/36055)
...
* 现在，当没有与客户地址关联的增值税ID时，自动客户组分配将起作用。 如果地址不含增值税ID，则被视为属于非增值税注册客户，Adobe Commerce会将该客户组更改为非增值税组。 以前，启用&#x200B;**[!UICONTROL Enable Automatic Assignment to Customer Group]**&#x200B;后，当从客户帐户中删除VAT ID时，客户帐户不会自动分配给`customerGroup`。
...
* 在管理员`Search Weight`上的`Storefront Properties`属性的&#x200B;**[!UICONTROL Stores]**&#x200B;输入字段下添加了信息性文本  > [!UICONTROL Attributes] > **[!UICONTROL Product]**。
...
* 仅限一个商店访问的管理员用户无法再将Visual Merchandiser中的产品拖放到类别中。 他们现在可以使用&#x200B;**[!UICONTROL Position]**&#x200B;列重新排列产品位置。 以前，这些用户只能通过拖放来更改类别中的产品位置。
...
* 现在，当购物者尝试将具有产品`addProductsToWishlist`自定义选项无效值的项添加到愿望列表时，`select`查询会返回信息性错误消息。 以前，下拉自定义选项值的服务器验证缺失，导致数据库中的值无效。 因此，终结点返回此内部服务器错误： `GraphQL endpoint returns Internal server error with "Call`。
