...
Adobe Commerce 2.4.6引入了对PHP 8.2的支持。PHP 8.1仍得到完全支持，但对PHP 7.4的支持已被删除。 它包括显着的性能与可扩展性增强。 已优化用于批量购物车操作和类别树渲染响应的GraphQL操作。 此发行版本引入了Adobe Commerce扩展元包。
...
PHP 7.4支持已从此版本中删除。 无法在PHP 7.4上运行Adobe Commerce 2.4.6。
...
* 现在支持将OpenSearch作为Adobe Commerce内部部署和云部署的默认搜索引擎。 它支持作为独立的Magento Open Source搜索引擎。 此版本支持OpenSearch v2.x，并已通过OpenSearch 2.5测试。尽管与OpenSearch 1.x的兼容性仍然存在，但我们建议将此版本与OpenSearch 2.x一起使用。您可以在Cloud和内部部署上安装并运行Adobe Commerce和OpenSearch 2.xMagento Open Source2.4.6 [搜索引擎先决条件](https://experienceleague.adobe.com/docs/commerce-operations/installation-guide/prerequisites/search-engine/overview.html). <!-- AC-449 3555 -->
...
* **改进了影响许多客户区段的操作的性能**. 商户可以通过启用新的配置设置来提高涉及100多个客户区段的运营性能。 此 **[!UICONTROL Real-time Check if Customer is Matched by Segment]** 可以根据需要禁用设置，以通过减少针对客户的每个客户区段验证来缩短处理时间。 参见 [客户区段验证](https://experienceleague.adobe.com/docs/commerce-operations/performance-best-practices/configuration.html#customer-segments-validation). <!-- AC-6832 ACPT-835-->

* **用于限制产品网格中显示的产品数量的新系统配置设置**. 商家可以使用新的 **[!UICONTROL Limit Number of Products in Grid]** 配置设置以提高product grid性能。 此限制默认配置为20000，仅影响UI组件使用的产品集合。 限制显示的产品数量可以提高填充了超过200,000个产品的网格的运行性能。 参见 [限制网格中的产品数量](https://experienceleague.adobe.com/docs/commerce-operations/performance-best-practices/configuration.html#limit-number-of-products-in-grid)<!-- ACPT-42 -->

* **提高了导入性能（每分钟最多100,000条记录）**. 商家可以使用新的 `POST /rest/<store_view_code>/V1/import/csv` 用于将数据导入Adobe Commerce的REST API端点。 此端点提供与管理员导入功能相同的功能，并支持使用CSV文件创建、更新和删除产品、产品定价和客户实体。 <!-- ACPT-6 -->

* **通过负载平衡改进了高吞吐量订单处理性能**. 云基础架构上的Adobe Commerce 2.4.6如果客户的同时订单量很大（每分钟1000个订单），现在可以通过平衡不同节点之间数据库的负载来提高订单处理性能。 商家可以通过为MySQL数据库和Redis实例启用辅助连接来重新平衡此负载。 参见 [高吞吐量订单处理](https://experienceleague.adobe.com/docs/commerce-operations/performance-best-practices/high-throughput-order-processing.html#load-balancing) <!-- ACPT-495 -->


### Adobe Commerce扩展元包

此版本引入了Adobe Commerce扩展元包v1.1.0，它自动将所选Adobe Commerce扩展与此核心版本捆绑在一起。 在以下情况下，将安装此元包中包含的这些扩展的版本： `composer update` 运行，从而简化在升级到最新核心版本时升级这些扩展的过程。 这些扩展仍将保持独立的发布时间表。

Adobe Commerce 2.4.6的Adobe Commerce扩展元包中包含这两个扩展：
...
Adobe Commerce的Adobe I/O事件可捕获Commerce事件并将数据从Adobe Commerce传输到其他Adobe系统。 开发人员可以在Adobe Developer App Builder中构建云原生应用程序，这些应用程序可订阅Commerce事件，例如库存更新、订单状态更改和价格更新。 开发人员可以使用此功能交换数据，以协调其关键任务系统中的整个购买者历程。
...
新 `adobe-ims-metapackage` v2.2.0与Adobe Commerce 2.4.6捆绑在一起。此版本中未更改任何功能。<!-- CABPI-505-->


### B2B

GraphQL API现在完全公开了“公司采购订单”功能。 此版本引入了两个GraphQL架构：

* 采购订单GraphQL架构涵盖B2B公司用户采购订单管理功能，例如创建、编辑和删除采购订单。
* 采购订单规则GraphQL架构提供创建、编辑和删除审批规则以及手动验证采购订单的功能。

此版本包括多个错误修复。 本主题包含了B2B的预发行说明。 您将在中找到B2B的Adobe Commerce 2.4.6 GA发行说明 [B2B发行说明](https://experienceleague.adobe.com/docs/commerce-admin/b2b/release-notes.html).
...
PWA Studiov.13.0.x与Adobe Commerce 2.4.6兼容。它包括多项增强功能，可提高可访问性。 有关错误修复的信息，请参见 [PWA Studio版本](https://github.com/magento/pwa-studio/releases). 参见 [版本兼容性](https://developer.adobe.com/commerce/pwa-studio/integrations/adobe-commerce/version-compatibility/) 有关PWA Studio版本及其兼容的Adobe Commerce核心版本的列表。
...
此版本包括多个错误修复。 此处包含B2B的预发行说明。 您将在中找到B2B的Adobe Commerce 2.4.6 GA发行说明 [B2B发行说明](https://experienceleague.adobe.com/docs/commerce-admin/b2b/release-notes.html).

<!-- ACP2E-689-->

* 现在，在启用“采购订单”选项并选择使用PayPal付款选项创建的虚拟报价单后，Adobe Commerce会在付款期间显示正确的详细信息。 以前，在这些条件下，总数显示为零。
...
* “税务/增值税编号”客户属性现在可按预期与公司管理员帐户在管理员和店面一起使用。 创建公司帐户不再需要自定义税务/VAT属性。 以前，当商家创建具有自定义税务/VAT属性的公司帐户时，Adobe Commerce在店面和管理员上都抛出验证错误。

<!-- ACP2E-1236-->

* 现在，在特定范围上禁用共享目录功能可正常工作。 以前，当商家保存共享目录配置时，Adobe Commerce设置无效的范围。
...
* 我们已经解决了在分配了许多公司权限后，通过GraphQL提供的公司权限验证存在的性能问题。
...
* 性能 `SELECT` 公司权限操作已得到改进。

<!-- B2B-2456-->

* 现在，当没有对查询的类别明确设置类别权限时，类别查询会根据商店配置设置返回产品价格。

<!-- AC-6829-->

* 此 **[!UICONTROL Place Order]** 现在，使用批准的报价请求完成购买时，按钮按预期工作。 可协商报价的问题 `negotiableQuoteCheckoutSessionPlugin` 插件已解析。
...
* 现在，可以从管理员中成功筛选捆绑产品 **[!UICONTROL Catalog]** > **[!UICONTROL Products]** 使用荷兰语言环境时。 以前，当您尝试过滤时，Adobe Commerce显示此错误 `Type: Bundle Product`： `Something went wrong with processing the default view and we have restored the filter to its original state`. [GitHub-35704](https://github.com/magento/magento2/issues/35704)

<!-- ACP2E-1038-->

* 当捆绑产品的特殊价格超过1000时，产品网格不再抛出异常。 以前，当您按SKU搜索产品或导航到 **[!UICONTROL Catalog]** > **[!UICONTROL Products]**，Adobe Commerce引发此错误： `Non Numeric value encountered`. [GitHub-36284](https://github.com/magento/magento2/issues/36284))
...
* 在GraphQL中包含 `BundleProduct` 片段， `items.options.product` 对象按预期包含数据。 以前，对象在某些情况下可能为null。
...
* 现在，当购物者更改购物车页面上的产品数量而未更新购物车，然后尝试导航到下一页时，Adobe Commerce会在将购物者重定向到下一页之前按预期显示确认弹出窗口。

<!-- AC-5987-->

* 现在，购物者可以在管理员减少购物车中的产品数量后，减少购物车的产品数量。 以前，当购物者尝试减少产品数量时，Adobe Commerce会引发以下错误： `The requested qty is not available`. [GitHub-35780](https://github.com/magento/magento2/issues/35780)

<!-- AC-6744-->

* 当多配送结帐期间产品数量超过配置的最大值时，Adobe Commerce不再在JavaScript控制台中引发错误。 Adobe Commerce现在改为显示信息性错误消息。 [GitHub-36184](https://github.com/magento/magento2/issues/36184)
...
* 在更改购物者所属的客户组后，购物者无法再向购物车中添加现在不允许的类别中的产品。 当权限配置设置发生更改时，Adobe Commerce现在会检查特定类别的客户群组权限访问权限。
...
* 此 `addBundleProductsToCart` 如果所需的产品选项没有值，则突变不再将产品添加到报价中。 [GitHub-25676](https://github.com/magento/magento2/issues/25676)

<!-- B2B-2458-->

* Adobe Commerce现在在以下情况下返回信息性消息： `addProductsToCart` 将产品添加到购物车时权限不足，无法进行变异。
...
* 当您重新加载产品详细信息页面时，Adobe Commerce不再显示上一页的数据。 以前，在重新加载产品详细信息页面时，来自第一个选项卡的数据会与新加载的数据合并一秒。
...
* 现在，在类别权限允许的情况下，分配给批发组的客户可以将分组的产品添加到其购物车。 如果没有为客户组定义此权限，则优先使用配置设置。
...
* 现在，当由相关产品规则针对字母数字产品属性生成时，可在店面产品详细信息页面上成功获取相关产品。 [GitHub-36285](https://github.com/magento/magento2/issues/36285)
...
* Adobe Commerce不再错误地基于类别路径假定一个类别是另一个类别的父类别。 以前，当两个类别相邻，并且一个类别ID以另一个类别的类别ID开头时，Adobe Commerce会假定这些类别具有相同的父类别。 [GitHub-32464](https://github.com/magento/magento2/issues/32464)
...
* 在禁用延期交货的部署中，购物者不能再将可销售量为0的可配置产品添加到购物车中。 Adobe Commerce现在指示产品缺货。 [GitHub-35319](https://github.com/magento/magento2/issues/35319)
...
* 在创建新购物车价格规则期间，“管理客户区段”网格上的“是/否”过滤器现在可按预期工作。
...
* AMQP使用者不再因已弃用而失败 `trim()` 在运行PHP 8.1的部署中起作用。以前，当AMQP使用者在安装后启动时，Adobe Commerce引发此错误： `Deprecated Functionality: trim(): Passing null to parameter #1 ($string) of type string is deprecated in .../vendor/magento/framework-amqp/Config.php on line 143`. [GitHub-35380](https://github.com/magento/magento2/issues/35380)

<!-- AC-2882-->

* 此 `getFilteredTimerIds` 方法现在可按预期工作。  类 `Magento\Framework\Profiler\Driver\Standard\Stat` 现已与PHP 8.1兼容。以前，在调用此方法时，Adobe Commerce会引发异常。 [GitHub-35307](https://github.com/magento/magento2/issues/35307)
...
* Adobe Commerce现在，当管理员用户选择 **[!UICONTROL Last 24 Hours]** 选项。 以前，Adobe Commerce显示的时间范围不正确。 发生这种情况是因为数据库的时区保持不变。
...
* 现在，访问仅限一个网站的管理员用户可以使用“以客户身份登录”功能成功登录到管理员。
...
* 礼品卡产品金额现在与导入期间CSV文件中指定的金额相同。 已添加现有选项的检查，并且现在基于唯一键更新以前的值。 以前，当礼品卡多次导入时，礼品卡数量会增加一倍。
...
* 此 `categoryList` 现在，query可返回锚定和未锚定父类别的准确产品计数。 新 `is_anchor` 已将属性添加到类别集合选定列列表。 以前，请求返回0个产品计数。 [GitHub-36300](https://github.com/magento/magento2/issues/36300)
...
* 聚合数据以响应 `products` 查询现在包含正确的价格类型属性值 `attribute_code` 和 `label`. [GitHub-28878](https://github.com/magento/magento2/issues/28878)
...
* 导入与现有产品具有相同数字URL键的产品，但没有其终端字母字符，在以下情况下不再导致验证错误： **[!UICONTROL Product URL Suffix]** 在管理员中，被设置为空。
...
* 索引器 `batch_size` 对于 `catalogpermissions_category` 索引现在更改为中提供的值 `env.php` 文件按预期显示。 以前，索引器的路径中存在拼写错误 `batch_size` 覆盖防止将批次大小更改为中提供的值 `env.php`. [GitHub-652](https://github.com/magento/partners-magento2ee/issues/652)
...
* 已添加清理 `DATE` 和 `DATETIME` 数据库转储中的值以支持Adobe Commerce。 以前，数据库备份表中客户/报价/订单表的日期时间类型字段不会经过整理。

<!-- ACP2E-804-->

* 此 `escaper.js` `escapeHtml` 函数现在将&amp;特殊字符转换为 `"$amp;"` 按 `InnerHtml` 规则。 此通用解决方案解决了标题输入框中的特殊字符和字符引用转换问题，并在插入时保存确切的值。 以前，在内容层级的节点名称中，&amp;字符编码不正确。
...
此版本包括多个错误修复。 此处包含Inventory management的预发行发行说明。 您将在中找到Inventory management的Adobe Commerce 2.4.6 GA发行说明 [发行说明](https://experienceleague.adobe.com/docs/commerce-admin/inventory/release-notes.html).

<!-- ACP2E-1106-->

* 当已售出的子产品返回到库存中时，店面现在将复合产品（可配置、捆绑和分组）显示为库存中。 之前，店面显示复合产品在这些条件下缺货。

<!-- ACP2E-1148-->

* 现在，如果可配置产品选项是在数量设置为0和以下情况下创建的，则该选项在店面会按预期显示为无库存 **[!UICONTROL Display out-of-stock products]** 已启用。

<!-- ACP2E-118-->

* 如果产品仍然有库存，则库存数量更改时，类别页面缓存不再失效。 Adobe Commerce现在从缓存中加载页面，而不是在店面类别页面上的产品数量（没有任何其他变化）发生更改时重新生成页面。

<!-- ACP2E-1033-->

* 现在，在单一来源模式下将库存与类别列表产品计数一起使用时，类别列表产品计数是正确的 **[!UICONTROL Display Out-Of-Stock Products]** 设置已启用。 Adobe Commerce现在检查产品在计数时是否可销售。

<!-- ACP2E-1015-->

* 启用库存后，“店内交付”的购物车价格规则现在可按预期工作。 以前，在这些条件下，不应用购物车价格规则生成的折扣。

<!-- ACP2E-1003-->

* 在计划模式下更新产品库存不再清除所有缓存。 以前，清单索引器清除所有配置缓存。

<!-- ACP2E-992-->

* 的值 **[!UICONTROL Allow Multiple Boxes for Shipping]** 高级清单中产品的属性现在按预期保存。

<!-- ACP2E-979-->

* 现在，Adobe Commerce会在使用店内提货方式下订单的部分退款贷项通知单后发出准确的预订补偿。 以前，将错误的预订保存到 `inventory_reservation` 表时，管理员用户未选择 **[!UICONTROL Return to Stock]** 复选框。

<!-- ACP2E-952-->

* 在实施多来源库存的部署中，当其变体之一被手动退回到库存中时，Adobe Commerce不再将可配置产品显示为库存不足。

<!-- ACP2E-925-->

* 产品网格上的列位置(**[!UICONTROL Catalog]** > **[!UICONTROL Products]**)在配置了多个库存源的部署中重新加载页面后，不会再恢复到其之前的位置。

<!-- ACP2E-908-->

* 现在，为虚拟产品签发贷项通知单后，库存数量是正确的，因为 **[!UICONTROL Back to stock]** 未选中复选框。

<!-- ACP2E-859-->

* 您现在可以使用自动产品排序和分配给非默认库存的范围来保存类别。 以前，Adobe Commerce不保存类别并显示以下错误： `Something went wrong while saving the category`.

<!-- ACP2E-813-->

* 现在，当使用所有缺货的可配置变体创建产品时，可配置产品库存状态会按预期更新。

<!-- ACP2E-924-->

* 现在，预订不一致性分析器工具可以正确处理部分发运的订单，这些订单包含可配置、捆绑和分组的产品。 现在对复合产品类型进行了分析。 以前，只为父产品保存取消和退款，而不为可配置和一起发运的捆绑产品的子产品订单项目保存取消和退款。

<!-- ACP2E-1180-->

* 当管理员用户尝试将200个或更多库存来源分配给库存或产品时，Adobe Commerce不再显示错误。

<!-- ACP2E-1138-->

* Adobe Commerce现在支持为从中删除产品的订单创建贷项通知单。 以前，在创建发票后从订单中删除产品时，商家无法创建贷项通知单。 应用程序显示此错误： `Following products with requested skus were not found: s00001`.

<!-- ACP2E-1017 -->

* 现在，系统会根据单个商店ID和多个商店ID来筛选商店。 此 `event` 产品属性代码已添加到保留的属性代码列表中。 以前，当安装了Inventory模块时，低库存报表会引发异常。

<!-- ACP2E-748 -->

* 分层导航筛选器现在按预期工作，缺货产品现在已附加到店面类别产品列表。 新 `is_out_of_stock` sorting属性使用storefront产品集合的Elasticsearch动态字段映射器。

<!-- ACP2E-1209-->

* 当子产品库存状态由REST更改时，复合产品（捆绑、已分组和可配置）库存状态会按预期更新 `POST /rest/V1/inventory/source-items` 呼叫。
...
* 新媒体集已得到优化，以改善类别页面中目录树的加载。

### 日志记录

<!-- ACP2E-1312-->

* 您现在可以为0字节的日志文件生成系统报告。 以前，当一个或多个文件为空时，Adobe Commerce在为日志文件集合生成报告时引发错误。
...
* 将备注添加到已存档订单时，不会再将其返回到未存档订单网格，同时保留其已存档状态。
...
* 现在，在启用异步索引的情况下为免费产品下自动开具发票的订单可正常工作。 不会将任何条目添加到 `sales_order_grid or sales_invoice_grid` 这些订单的表。 以前，Adobe Commerce抛出异常，因为应用程序尝试在放置订单期间为网格编制索引，而不是依赖 `cron`. [GitHub-36334](https://github.com/magento/magento2/issues/36334)

### 页面生成器

<!-- ACP2E-956-->

* 您现在可以使用页面生成器拖放功能将图像批量导入图库。
...
* 启用Braintree付款方式后，管理员用户现在可以创建订单或重新排序。 以前，Adobe Commerce引发此错误： `report.CRITICAL: Error: Call to a member function getMethodInstance() on null in /app/vendor/paypal/module-braintree-core/Block/Form.php:174`.

<!-- BUNDLE-3139-->

* Braintree现在，在启用BraintreePayPal并且 **[!UICONTROL Enable Card Payments]** 已禁用。 以前，进度条从未解析，Adobe Commerce显示此控制台错误： `adapter.js:66 Uncaught TypeError: Cannot read properties of undefined (reading 'clientToken')at Object.getClientToken (adapter.js:66:66)`. [GitHub-35961](https://github.com/magento/magento2/issues/35961)
...
* 启用Braintree付款方式后，管理员用户现在可以为客户创建订单或重新排序。 以前，Adobe Commerce引发此错误： `report.CRITICAL: Error: Call to a member function getMethodInstance() on null in /app/vendor/paypal/module-braintree-core/Block/Form.php:174`

<!-- BUNDLE-3139-->

* 现在，购物者可以在店面使用Braintree支付方式，店面为 **[!UICONTROL Enable Card Payments]** 已被禁用。 以前，Adobe Commerce抛出此JavaScript错误： `_adapter.js:66 Uncaught TypeError: Cannot read properties of undefined (reading 'clientToken')at Object.getClientToken (adapter.js:66:66).`
...
* 此 **[!UICONTROL Use Config Settings]** 的复选框 **[!UICONTROL Enable RMA]** 现在，字段会在保存产品时按预期保存其值。
...
* 现在，当满足以下条件时，可在引号中搜索字符串： **[!UICONTROL Minimum Terms to Match]** 已启用配置设置。 以前，Adobe Commerce引发此错误： `{"error":{"root_cause":[{"type":"parsing_exception","reason":"[match_phrase] query does not support [minimum_should_match]' ...`.
...
* Adobe Commerce现在向管理员客户致敬 **[!UICONTROL Show Company]** 显示送货或客户地址时的配置设置。 禁用此设置后，送货公司将不再包含在送货、客户地址或数据库订单记录中。 以前，尽管设置被禁用，仍会显示该信息。 [GitHub-33729](https://github.com/magento/magento2/issues/33729)
...
* 对运输成本应用折扣时，Adobe Commerce不再引发最小订单金额错误。
...
* 重新计划暂存更新的结束日期不会在更新完成后导致错误。 以前，管理员无法打开产品页面和产品网格，Adobe Commerce引发此错误： `report.CRITICAL: Exception: Item (Magento\Catalog\Model\Product\Interceptor) with the same ID "1" already exists`.
...
* 现在可以在计划预览模式下将产品添加到购物车。 使用以下参数更新计划预览模式中的痕迹导航链接更正了此问题： `version`， `timestamp`、和 `signature`.
...
* 现在，当没有与客户地址关联的VAT ID时，自动客户组分配起作用。 如果地址不含增值税ID，则会将其视为属于非增值税注册客户，Adobe Commerce会将该客户组更改为非增值税组。 以前，何时 **[!UICONTROL Enable Automatic Assignment to Customer Group]** 启用，则不会自动将客户帐户分配给 `customerGroup` 从客户帐户中删除VAT ID的时间。
...
* 您现在可以指定 `vat_id` 使用GraphQL添加或更新来宾购物者的地址时。
...
* 用户角色网格（管理员）的过滤器功能 **[!UICONTROL System]** > **[!UICONTROL Permissions]** > **[!UICONTROL User roles]** > **[!UICONTROL Administrators]**)现在可按预期工作。 数组值现在单独处理。 以前，当您尝试在此网格中输入查询时，Adobe Commerce引发此控制台错误，并且页面未加载： `Uncaught TypeError: Cannot read properties of null`. [GitHub-35993](https://github.com/magento/magento2/issues/35993)
...
* 主题标题设置 `Welcome Text` 现在可以包含单引号字符。 以前，Adobe Commerce抛出此JavaScript错误： `Uncaught SyntaxError: Unable to process binding "ifnot: function(){return customer().fullname }"`.
...
* 当产品与规则匹配时，您无法再手动更改可视化促销类别列表中的产品位置。

<!-- ACP2E-985-->

* 仅限一个商店访问的管理员用户无法再将产品拖放到可视化促销器的某个类别中。 相反，他们现在可以使用 **[!UICONTROL Position]** 列来重新排列产品位置。 以前，这些用户只能通过拖放来更改类别中的产品位置。
...
* 修复器货币服务现在与通过ApiLayer生成的密钥一起按预期工作，并且正确导出汇率。 以前，当商家尝试通过管理员导入汇率时，Adobe Commerce会引发错误。
