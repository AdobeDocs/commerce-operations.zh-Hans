...
此版本包括八项安全修复和平台安全改进。 这些安全修复已支持到Adobe Commerce 2.4.5-p1和Adobe Commerce 2.4.4-p3。
...
* MariaDB 10.6（LST版本）。 此版本仍与MariaDB 10.4兼容，但我们建议升级到MariaDB 10.6。 <!-- AC-6921 -->
...
* 正在执行 `bin/magento set:di:com -vvv` (或 `-v` 或 `-vv` 选项)现在会在失败时返回一条信息性消息。 [GitHub-35391](https://github.com/magento/magento2/issues/35391)
...
* 的 `composer install` 主体不再覆盖 `.gitignore` 文件。 [GitHub-32888](https://github.com/magento/magento2/issues/32888)

<!-- AC-6671-->

* 正在运行 `php bin/magento staticdata:remove` 要在升级后删除示例数据，不再失败，并出现以下错误： `Undefined constant "MagentoHackathon\Composer\Magento\Deploystrategy\GLOB_BRACE`. [GitHub-36062](https://github.com/magento/magento2/issues/36062)
...
* 的 `bin/magento app:config:import`命令现在可成功导入 `config.php` 文件。 以前，导入失败，并出现以下错误： `Import failed: Notice: Trying to access array offset on value of type null in [..]/vendor/magento/module-config/Model/Config/Backend/Currency/Cron.php on line 66`. [GitHub-31428](https://github.com/magento/magento2/issues/31428)
...
* 启用reCAPTCHA后，使用Braintree下单时，购物者无法再绕过reCAPTCHA验证。

<!-- BUNDLE-3185-->

* 现在，在结帐期间，对于发往多个目标的订单，ReCAPTCHA验证可按预期工作。
...
* 现在，已为其产品生成SEO友好URL `url_key` 属性会在存储视图级别被覆盖。
