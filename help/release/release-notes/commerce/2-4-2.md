...
有关安全修补程序的一般信息，请参阅[安全修补程序发行说明](../security/overview.md)。 有关下载和应用安全修补程序（包括修补程序2.4.1-p1）的说明，请参阅[快速入门本地安装](../../../installation/composer.md)。 安全修补程序仅包括安全错误修复，而不包括完整修补程序中包含的其他安全增强功能。
...
* [Amazon支付](https://commercemarketplace.adobe.com//amzn-amazon-pay-magento-2-module.html)。 Amazon Pay在此版本中已弃用，将在Magento 2.5.0中删除。 版本2.4.3及更高版本将仅包含兼容性更新和主要错误的修复。
...
* 应用程序现在已成功重新创建图像，并且每当商家使用POST `rest/all/V1/products`发送媒体集数据时，这些图像都会按预期在管理员中可见。 以前，如果图像在`pub/media`中被删除，但是媒体中的记录仍然包含这些图像的记录，当商家通过POST `rest/all/V1/products`发送媒体集数据时，应用程序重新创建了图像，但是删除了这些图像。 因此，商家在管理员中只看到占位符，因为图像是在更新过程中创建和删除的。
...
* 已从&#x200B;**系统** > **导入**&#x200B;页面的“检查数据”消息区域中删除一个无关的HTML `<br>`标记。
...
* `DataObject`已在`\Magento\MediaGalleryCatalogUi\Ui\Component\Listing\Columns\Thumbnail::prepareDataSource`.[GitHub-1711](https://github.com/magento/adobe-stock-integration/issues/1711)中被替换为产品模型
...
* 在购物者登录到启用了永久购物车的商店后，应用程序现在会按预期显示欢迎消息。 以前，应用程序显示&#x200B;**不是您？** 登录后显示消息。

<!-- MC-38269-->

* 应用程序不再显示&#x200B;**不是您？** 在登录到启用了永久购物车的部署时，客户选择&#x200B;**记住我**&#x200B;选项时显示的文本。
...
* 提供`save_rewrites_history`自定义属性后，PUT `V1/categories/:id`端点现在会存储为类别URL密钥创建301重定向所需的数据。 [GitHub-29174](https://github.com/magento/magento2/issues/29174)，[GitHub-30240](https://github.com/magento/magento2/issues/30240)
