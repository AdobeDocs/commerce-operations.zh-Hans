...
Adobe Commerce 2.4.2引入了性能和安全性的增强功能以及显着的平台改进。 安全性增强包括扩展对 `SameSite` 属性。 B2B改进主要支持在线支付采购订单。 Elasticsearch7.9.x和Redis 6.x现在受支持。
...
* GraphQL架构已得到增强，可针对具有许多变体的可配置产品优化产品数据检索。 <!-- MC-36138-->
...
2.4.2引入了B2B v1.3.1。此版本包括对采购订单的在线支付以及多个错误修复的支持。

**现在可以使用在线付款方法完成采购订单**. B2B购买者被提示为每个购买者选择其首选付款方式 [采购订单](https://docs.magento.com/user-guide/stores/b2b-purchase-order-flow.html) 在初始结帐期间。 采购订单获得批准后，系统会提示买方输入付款详细信息，以将采购订单转换为最终订单。

为了支持接受在线支付的完整工作流，此功能还：

* 在初始结帐期间覆盖现有的付款方法模板以保持PCI合规性。

* 与第三方、自定义和社区开发的支付方法保持兼容。

* 当购买者需要向批准的采购订单添加付款详细信息时，通过电子邮件通知他们。

* 允许自定义管理员的电子邮件，以便商家使用与其品牌策略一致的模板。

* 引入 `Approved – Pending Payment` 状态，以明确显示何时需要执行操作。

* 在最终付款步骤中禁止添加或删除折扣代码，这可确保批准的采购订单的订单总额保持不变。

* 允许购买者在最终付款步骤期间更改付款方式，以保持灵活性并提高转化。
...
* 现在，开发人员可以在每个视区中以不同方式设置内容类型输出的样式，而无需使用 `!important` 指令。

* 迁移到页面生成器的内容不再填充默认的内联样式。

* 页面生成器不再要求将所有内容类型置于一行中。 的 [页面生成器阶段](https://docs.magento.com/user-guide/cms/page-builder-workspace.html#stage) 现在最初为空，并支持将以下内容类型直接添加到舞台：行、列、选项卡、HTML代码、块、动态块。
...
* 新移动设备 [视区切换器](https://docs.magento.com/user-guide/cms/page-builder-workspace.html#viewports) 通过表单字段值的视区范围，用户和开发人员可以执行以下操作：
   ...
   * 针对每个视区的不同内容类型优化最小高度字段参数。 (开箱即用地仅支持一个参数（最小高度）。 为其他内容参数启用移动优化所需的自定义开发。)

   * 向自定义字段和字段集添加字段范围（开发人员）。

### 交互式产品内指南

交互式产品内指南为商家提供了有关新功能公告、演练指南、入门信息和工具提示的使用提示和管理员内部的信息。  **如果未启用此功能，则管理员必须从管理员中选择加入才能接收产品指导**. 请参阅 [使用情况数据收集](https://docs.magento.com/user-guide/stores/admin.html) 和 [管理员使用情况](https://docs.magento.com/user-guide/configuration/advanced/admin.html#admin-usage).
...
所有模块(包括B2B、PageBuilder和AWS集成)都添加了对Adobe Stock S3的支持。
...
MCOM连接器与2.4.2完全兼容。 [Connector发行说明](https://omsdocs.magento.com/release-notes/connector/index.html). 此版本包括对OMS命令行界面的改进，现在，该界面提供了合作伙伴和支持团队可用来进行故障诊断的详细信息 `retry`, `queue_prune`, `order_sync`, `message process`和 `catalog export` 进程。

### 供应商开发的扩展

有关此版本的功能和更改的更新，请参阅以下文章：

* [Amazon Pay](https://docs.magento.com/user-guide/payment/amazon-pay.html). Amazon Pay已在此版本中弃用，并将在Magento2.5.0中删除。版本2.4.3及更高版本将仅包含兼容性更新和主要错误修复。
...
* 此版本中已弃用将安装配置为使用拆分数据库的功能。 请参阅 [弃用Adobe Commerce中的拆分数据库功能](https://community.magento.com/t5/Magento-DevBlog/Deprecation-of-Split-Database-in-Magento-Commerce/ba-p/465187) DevBlog文章。
...
* 的 **添加新评级** 按钮 **保存**, **删除**, **重置** 按钮在 **商店** > **属性** > **评级** 页面。

<!-- MC-35804-->

* 管理员现在仅显示登录管理员有权访问的网站中的数据。 以前，管理员会显示部署中所有网站的数据。

<!-- MC-35027-->

* 现在，所有选定的网站都存储在 `gws_websites` 无论网站ID的大小。 以前，网站ID会被截断，并且所选网站的列表不会存储在数据库中。 用户角色未正确保存，当商家尝试更改和保存角色时，应用程序显示此错误： `The "X" store ID is incorrect. Verify the store ID and try again`.

<!-- MC-37223-->

* 对于受限管理员用户，应用程序现在在管理网格中显示正确的项目计数。 以前，这些网格显示的计数反映了所有国家/地区的数据，而网格本身仅显示特定国家/地区的数据。 （此修复更正了“客户区段”、“目录价格规则”、“购物车价格规则”、“所有用户”、“锁定的用户”和“用户角色”网格中的问题。）

<!-- MC-34156-->

* 权限排除的管理员 `Magento_Catalog::edit_product_design` （编辑产品设计）现在可以通过在选定的商店视图中使用新名称保存现有产品来创建新产品。 以前，应用程序会显示以下错误：  `Not allowed to edit the product's design attributes`. [GitHub-28106 ](https://github.com/magento/magento2/issues/28106)

<!-- MC-36406-->

* 应用程序不再显示 **添加属性** 按钮 **商店** >属性。 以前，当没有相应权限的网站管理员尝试创建 **属性集** 或 **客户** 属性。

<!-- ENGCOM-8230 -->

* 当您尝试更改时，应用程序不再引发错误 `backend-frontname` 使用 `ssh` 容器。Adobe Commerce [GitHub-26762](https://github.com/magento/magento2/issues/26762)
...
* 管理员可以通过SKU从管理员将产品添加到购物车，而无需从管理员中重定向。 管理员不再被重定向到店面页面，并且应用程序会按预期在“管理员购物车项目”部分显示产品。

<!-- MC-36750-->

* 现在，当您上传CSV文件（该文件包含通过SKU将产品添加到购物车时的多种类型错误）时，应用程序会显示信息性错误消息。 以前，应用程序在您单击时会显示空白页面和一般错误消息 **选择文件** on **管理购物车** > **按SKU添加到购物车**.
...
* 配置目录权限的顺序不再阻止具有正确权限的管理员将产品添加到购物车。 以前，应用程序不会验证 `addToCart` 当前客户组的配置设置。 因此，当管理员尝试将产品添加到购物车时，应用程序抛出错误。
...
* 现在，使用GraphQL对优惠券应用价格规则时，如果 **整个购物车的固定金额折扣** 操作。
...
* 在对类别页面的重复访问期间，启用管理员的类别权限不再导致无法将类别页面缓存到完整页面缓存中。 以前，对页面的后续访问不会按预期从缓存中提供，并且性能会降低。
...
* 现在，自定义地址属性可在“我的帐户”区域和“管理员订单详细信息”页面中正确显示。
...
* 现在，应用程序可按预期为B2B用户保存自定义客户属性值。 以前，为添加或编辑公司用户而创建的自定义客户属性未与添加新用户表单绑定，因此属性值未保存。 此外，自定义日期属性是从数据库中作为字符串派生的，并未转换为正确的日期格式以便店面显示。
...
* 在可下载产品已部分退款后，应用程序不再在“我的下载产品”选项卡中列出可下载产品。 [GitHub-28388](https://github.com/magento/magento2/issues/28388)

### 动态块（以前称为横幅）

<!-- MC-23938-->

* 在您删除了与该块关联的客户区段后，应用程序将不再在店面上显示动态块。

<!-- MC-37322-->

* 现在，在确定是否显示动态块时，应用程序可正确考虑目录价格规则。
...
* 更改商店视图时，应用程序不再将客户重定向到主页。 的 `\Magento\VersionsCmsUrlRewrite\Model\StoreSwitcher::switch()` 方法现在仅在 `$urlPath` 在第二个存储视图中未找到。
...
* 的 `FlushCacheByTags` 类已更新为使用插件后处理而不是插件后处理。 [GitHub-29558](https://github.com/magento/magento2/issues/29558)

<!-- ENGCOM-8358 -->

* `MessageValidator` 不再对不包含的哈希阵列失败 `0` 元素。 [GitHub-30563](https://github.com/magento/magento2/issues/30563)
...
* URL重写生成器现在可设置类别对象 `url_key`, `url_path`和  `store_id` ，以在保存全局范围的类别URL时默认存储值。 以前，它会保存 `url_key`, `url_path`和 `store_id` 的值。 [GitHub-29585](https://github.com/magento/magento2/issues/29585)
...
* 现在，仅当成功保存订单后，才会发送包含礼品卡代码的电子邮件 **订购订单项目时创建礼品卡帐户** 配置设置已启用。 以前，如果在订购物品时生成礼品卡帐户，但在发送虚拟礼品卡后发生异常，则会重复使用礼品卡代码并将其发送给不同的客户。

<!-- MC-36496-->

* 应用程序不再复制礼品卡订单。 以前，当购物者订购了一张礼品卡时，应用程序会交付两个礼品代码。 之所以出现，是因为 `cron` 调用的作业 `\Magento\GiftCard\Observer\GenerateGiftCardAccountsInvoice::execute()`，并且此方法不会检查在发票创建期间是否已生成礼品卡代码。
...
* Google标签管理器 `productClick` 函数现在可按预期工作。 以前， `productClick` 未在列表页面上的产品上触发，且Google标签管理器无法捕获该事件。

<!-- MC-38781-->

* Google Tag Manager可配置产品选项现在在您编辑购物车中的可配置产品选项后，可在数据层中按预期进行更新。 以前， `removeFromCart` 当产品选项发生更改时，事件被错误重写，并且数量保持不变。
...
* 将产品添加到购物车的突变现在具有目录权限。 [GitHub-30179](https://github.com/magento/magento2/issues/30179)

<!-- MC-36946 -->

* GraphQL添加到购物车操作的响应时间已缩短。
...
* GraphQL现在具有目录权限。 应用程序可限制为 `products` 基于购物者客户群组的查询。
...
* 图像现在可在上传后按预期由目录事件传送小组件显示。 以前， `catalog_event_status_checker` 创建作业时已删除上传的图像。
...
* 现在，当CSV文件还包含错误条目时，计划产品导入流程会使用正确的数据字段更新产品。 以前，如果CSV文件在 **出错时：继续处理** 已启用。
...
* Adobe Commerce通过消除每个索引器实例多次刷新标记来改进缓存性能。 [GitHub-29890](https://github.com/magento/magento2/issues/29890)
...
* 在MFTF测试中，为 `Company`, `CompanyCredit`, `CompanyPayment`, `CompanyShipping`和 `ConfigurableSharedCatalog` 模块。 [GitHub-30164](https://github.com/magento/magento2/issues/30164)
...
* 应用程序不再显示 `Purchase Order number is a required field` 优惠券代码字段上方的消息，然后购物者在将采购订单配置为唯一可用付款选项的部署中输入任何信息。
...
* 现在，来宾购物者可以在 **自动分配给客户组** 启用。 付款在PayPal网关上处理，订单在Magento中创建，购物者会看到订单感谢页面。 以前，在PayPal网关上成功处理了订单，但订单未在Adobe Commerce中创建。 购物者而是被重定向到订单审核页面，应用程序在该页面中显示以下错误： `Failed address validation: %1" error`. [GitHub-25399](https://github.com/magento/magento2/issues/25399)
...
* 对于包含3,000,000个以上客户的客户区段，在编辑和保存操作期间的Adobe Commerce性能已得到改进。
...
* 现在，在您分解RMA时，所选选项适用于分解行项目。 以前，未保存“返回原因”、“项目条件”和“分辨率”的值。

<!-- MC-35360-->

* 现在，在启用JavaScript捆绑后，您可以删除RMA的跟踪和发运信息。

<!-- MC-36757-->

* 现在，对特定网站范围内的产品具有受限访问权限的管理员可以向返回添加项目，即使该项目不再位于其网站范围中也是如此。
...
* 现在，当部分订单完成时，将按照对应税订单的预期计算奖励点数。

<!-- MC-37244-->

* 商户现在可以成功为通过奖励积分购买的订单发出部分退款，并随后仍然退回剩余订单。 以前，商家无法退还剩余订单的奖励积分。
...
* 现在，当客户在支持多种货币的商店中使用非基本货币下订单时，可以正确计算奖励点数。
...
* 现在，在您检查主页计划更新时，编辑主页页面会按预期显示所有计划更新。 以前，应用程序只显示多个计划更新中的一个。

<!-- MC-34988 -->

* 使用发送0 `/rest/V1/products/special-price-information` 特殊价格端点现在可以保存价格并按预期返回计划。

<!-- MC-35362 -->

* 现在，当导入CSV文件包含字节顺序标记(BOM)时，计划导入按预期运行。

<!-- MC-38026 -->

* 当商家计划类别更新时，应用程序不再删除布局主体类。

<!-- MC-38234 -->

* 重新计划的产品更改现在在 `cron` 运行。

<!-- MC-38759-->

* 在您尝试计划类别更新时，应用程序不再引发错误。 以前，应用程序在您单击 **保存** 在“计划新更新”页上： `Something went wrong while saving the Magento\Catalog\Api\Data\CategoryInterface`.

<!-- MC-38455-->

* 的 `rest/V1/products/special-price` API现在可用于计划多个商店的产品特价。 以前，当商家为同一商品计划了多个商店的价格更新时，应用程序会引发错误 `from` 和 `to` 在新计划的开始时间可用回退的时间。
...
* 现在，当 `is one of` 用于要显示的产品功能的条件包含多个条件。 以前，当此条件包含多个值时，将不会进行分析。
...
* 更正了与GraphQL有关的问题 `testRequestCacheTagsForCategoryListOnMultipleIds` 测试。 [GitHub-29372](https://github.com/magento/magento2/issues/29372)
...
* 现在，将在中正确解析包含URL的标头值 `ApiFunctional TestFramewor`k. [GitHub-26425](https://github.com/magento/magento2/issues/26425)
...
* 调整了光标从中间到开始的位置 **消息** 字段 **我的帐户** > **礼品登记** > **共享礼品注册表**.
...
* 购物者现在可以成功地将产品从一个愿望清单移动到另一个愿望清单。
...
**问题**:的 `[magento_root]/index.php` 文件已被删除，应用程序现在从 `/pub` 默认情况下，使用Apache配置。 子文件夹中提供的存储区将无法按预期工作，并可能显示404错误。 **解决方法**:使用符号链接模拟子文件夹中的安装过程。 以下示例使用两个商店(`https://shop01.com/shop/` 和 `https://shop02.com/shop/`)以说明如何使用符号链接模拟子文件夹中的安装。
...
**问题**:商户必须从管理员中激活“快速强制TLS”功能，以便为所有存储页面启用全局HTTP到HTTPS重定向。 请参阅 [将云上所有页面的HTTP重定向到HTTPS（强制TLS）](https://support.magento.com/hc/en-us/articles/360006296953) 知识库文章。 <!-- MC-39988-->

**问题**:您不能使用 `addConfigurableProductToCart` GraphQL变异，在运行Inventory的多商店部署中，在非默认的商店视图中将可配置产品添加到购物车。 （Luma店面不会受到影响。） 应用程序显示以下错误： `Could not add item to cart. Please check required options and try again`. **解决方法**:使用 `addProductsToCart` 变异。 [GitHub-31660](https://github.com/magento/magento2/issues/31660)<!-- PWA-1298-->
...
**问题**:当购物者点击时，应用程序会按预期创建Braintree订单 **使用Venmo付款**，但不会在管理员中创建顺序。 **解决方法**:无。 请参阅 [2.4.2已知问题：Braintree维恩模付款不起作用](https://support.magento.com/hc/en-us/articles/360054662652/)  知识库文章。  <!-- BUNDLE-2894-->

### B2B已知问题

**问题**:B2B购买者可以使用在线支付方法绕过通常的采购订单流。 如果购买者可以将其整个结帐总额减少到0（例如，通过促销代码或礼品卡），然后删除代码或礼品卡，则会出现这种情况。 即使在这种情况下，申请仍根据所分配目录中物品的价格，为正确数量下订单。 **解决方法**:在启用在线付款方法以审批采购订单时，禁用礼品卡和优惠券代码。 <!-- B2B-1603-->

**问题**:当购买者尝试使用PayPal Express Checkout从采购订单下单时，当 **上下文模式** 已禁用。 <!-- B2B-1604-->

**问题**:当采购员创建采购订单并导航到结帐页面时，应用程序有时会显示404错误。 当购买者在导航到结帐页面而未完成上次购买之前，之前使用在线付款方法创建了其他采购订单时，会出现此错误。 采购员仍可以下订单。 **解决方法**:无。 <!-- B2B-1605-->

**问题**:在采购订单的结帐期间，即使买方在最终结帐期间更改其付款方式，特定付款方法的折扣仍会保持。 因此，客户可能会获得他们无权获得的折扣。 发生此情况的原因是，尽管付款方法发生了更改，但原始付款方法的购物车规则仍被应用。 **解决方法**:无。 请参阅 [2.4.2 B2B已知问题：在更改付款方法后，在线采购订单的折扣将保留](https://support.magento.com/hc/en-us/articles/360054667312) 知识库文章。 <!-- B2B-1012 -->

**问题**:的 `deleteRequisitionListOutput` 查询会返回有关已删除的申请列表（而不是剩余的申请列表）的详细信息。 <!-- MC-39894-->
