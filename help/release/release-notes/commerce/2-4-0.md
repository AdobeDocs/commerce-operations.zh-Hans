...
Adobe Commerce 2.4.0引入了对PHP 7.4、Elasticsearch7.6.x和MySQL 8.0的支持。重大的安全更改包括默认在管理员中启用双重身份验证。 B2B增强功能包括新的订单批准工作流程。

**在此版本中，Authorize.Net、eWay、CyberSource和Worldpay支付方法集成已从核心代码中删除。 商户应迁移到Commerce Marketplace上可用的官方扩展**.
...
此版本包括2.3.5-p1中包含的对核心质量的所有改进、对核心代码的100多项新修复以及30项安全增强功能。 其中包括由我们的社区成员解决的226个GitHub问题。 这些社区参与的内容包括对核心代码进行细微清理，以及在Inventory management和GraphQL中进行重大增强。
...
DHL已引入模式版本6.2，并将于2022年9月弃用模式版本6.0。 Adobe Commerce 2.4.4及更早版本（支持DHL集成）仅支持版本6.0。部署这些版本的商户应适用 `AC-3022.patch` 尽早将DHL作为航运公司继续提供。 请参阅 [应用补丁程序以继续提供DHL作为运输承运人](https://support.magento.com/hc/en-us/articles/7707818131597-Apply-a-patch-to-continue-offering-DHL-as-shipping-carrier) 有关下载和安装修补程序的信息，知识库文章。
...
尽管这些功能的代码与季度版本捆绑在一起，但其中一些项目(例如B2B、Page Builder、Inventory management和Progressive Web Application(PWA)Studio)也是独立发布的。 这些项目的错误修复会在特定于项目的单独发行信息中进行记录，该信息可在每个项目的文档中找到。
...
* **支持MariaDB 10.4**. 对MySQL 8.0的支持为商户提供了将MariaDB 10.4与Adobe Commerce一起部署的机会。 尽管商家仍可以将MariaDB 10.2与2.4.0结合使用，但我们建议升级到MariaDB 10.4以提高性能和可靠性。 不再支持MariaDB 10.0和10.1（由于在此版本中删除了对MySQL 5.6的支持）。
...
请参阅 [GraphQL开发人员指南](https://devdocs.magento.com/guides/v2.4/graphql/) 以了解有关此功能和其他增强功能的详细信息。 请参阅 [发行说明](https://devdocs.magento.com/guides/v2.4/graphql/release-notes.html) 有关GraphQL最近错误修复的详细讨论。
...
**适用于B2B的订单批准工作流程功能**. 订单审批允许采购组织的经理为其采购人配置审批规则。 请参阅 [批准规则](https://docs.magento.com/user-guide/customers/account-dashboard-approval-rules.html). 功能包括：

* 为每个公司帐户自助配置唯一批准规则
* 可基于订单金额、行项目数量或具有可配置阈值的发运成本的规则标准
* 功能所有方面的可自定义权限
* 需要审批的采购订单的快速店面视图
* 对每个采购订单执行的活动的完整历史记录日志
* 在审批流程的所有步骤中向相关方发送电子邮件通知。
...
* 现在，由任何输入类型（例如，下拉列表、多选）捕获的客户属性值会作为数据字段正确同步。
* 现在，即使购物车分析数据不包含任何项目，也会针对所有活动报价发送这些数据。 这允许商户在清空购物车时退出项目的联系人。
* 商家现在可以按单个数据字段同步网站名称、商店名称和商店视图名称。
* Wishlist、Review和Order同步现在只查找一次事务性数据同步限制。
* Client类的日志记录输出已得到改进，现在在API包装器中的所有方法中都保持一致。
* 可配置产品现在有一个库存数字，即其子产品的总和。
* 新插件会检测由第三方代码（在管理员之外）执行的库存更新。
* [数据映射](https://docs.magento.com/user-guide/configuration/dotdigital/data-mapping.html) 和 [事务型电子邮件](https://docs.magento.com/user-guide/configuration/dotdigital/transactional-emails.html) 配置信息已更新。
...
此捆绑的扩展取代了我们的核心Braintree集成，该集成已在此版本中删除。 请参阅 [Braintree付款](https://marketplace.magento.com/paypal-module-braintree.html).
...
* Adobe Commerce安装现已成功完成，在中预定义存储配置时，将按预期创建存储 `config.php`.
...
* 现在，具有有限角色范围的管理员可以修改替换文本，应用程序将保存这些值。 如果管理员不能修改替换文本，则替换文本框将呈灰显状态。
...
* 现在，在您更改商店的URL后，可以按预期使用商店的分析报表。 以前，当您尝试在URL发生更改后访问报表时，应用程序会显示404错误。

<!-- MC-32085-->

* 现在，在启用AR订阅时，将按照预期启用分析报表的数据同步。
...
* 现在，应用程序将禁用受限管理员更改产品数量属性的功能，并按预期禁用高级库存。 以前，只影响数量属性的可视显示，并且产品保存后，应用程序会更改数据库中的数量值。
...
* 对Catalog具有受限权限的管理员现在可以创建可下载的产品。 以前，管理员无法创建可下载的产品，并且应用程序引发错误。
...
* 现在，您可以使用“产品”网格中的“操作”下拉菜单，在从“管理员”更新多个产品属性时，指定产品是否具有重量。  _修复了由Bartomiej Szubert在拉取请求中提交的问题 [26075](https://github.com/magento/magento2/pull/26075)_. [GitHub-6310](https://github.com/magento/magento2/issues/6310)
...
* 目录价格规则现在可与“产品详细信息”页面上预期的自定义选项一起使用。 _Pavel Bystritsky在拉取请求中提交的修复 [22917](https://github.com/magento/magento2/pull/22917)_. [GitHub-22856](https://github.com/magento/magento2/issues/22856)
...
* Google标签管理器标记在客户在不接受Google标签管理器Cookie的情况下导航到新商店时不再触发。
...
* 现在，应用程序会在结帐工作流的地址部分按预期显示自定义客户地址属性值。 以前，应用程序会显示自定义客户地址属性代码而不是值，并且会触发JavaScript错误。
...
* 现在，应用程序会保存自定义客户地址属性，并按照预期在注册表单中实施这些属性。 以前，当您在从购物车创建帐户时创建新的自定义客户地址属性时，应用程序不会保存属性信息。
...
* 当您以分配给客户区段的用户身份登录时，如果该区段包含过去订单的条件，则此应用程序将不再从客户区段中删除您的客户帐户。 以前，您必须单击 **刷新区段数据** 登录后，应用程序会在“匹配的客户”列表中显示您的帐户。

<!-- MC-30466-->

* 当区段条件包括订单总数时，客户区段现在可以按预期工作。

<!-- MC-32409-->

* 现在，在将客户帐户设置为全局共享时，可按预期应用客户区段规则。
...
* 动态块现在可以按预期方式处理客户区段。 当应触发该块的条件未满足时，应用程序不再显示动态块。 以前，当购物车为空时，应用程序会为以来宾身份登录的用户显示一个动态块。

<!-- MC-32652-->

* 现在，动态块会根据客户区段和商店视图呈现在店面上。
...
* 的 `php bin/magento setup:cron:run` 命令现已删除。 此外，在升级时还应在crontabs中删除对此文件的引用。
...
* 现在，在安装了B2B的部署中，您可以将“类别”页面上的产品限制从24更改为36。 以前，当此产品限制增加到36时，应用程序会引发致命错误。
...
* 现在，在产品级别启用礼品包装后，可将其添加到购物车中。 以前，您可以在店面上向产品添加礼品包装，但应用程序不会在订单摘要中包含礼品包装。

<!-- MC-30543-->

* 当您将过期的礼品卡的到期日期更改为将来日期时，该礼品卡会按预期变为活动状态。 以前，礼品卡仍然过期。

<!-- MC-32551-->

* 现在，管理员创建订单页面在 **允许按订单级别发送礼品邮件** 设置和模板缩小已启用。 以前，当您尝试在启用这些设置时打开此页面时，应用程序会引发以下错误： `Warning: require(content.phtml): failed to open stream: No such file or directory`.

<!-- MC-31427-->

* 现在，应用产品会根据您订购礼品包装的产品数量正确计算礼品包装的成本。 以前，应用程序仅在订购中包含一个产品的礼品包装成本。

### Google标签管理器

<!-- MC-29878-->

* 商户现在可以延迟向Google Analytics发送数据，直到满足指定条件为止。

<!-- MC-30673-->

* 失踪者 `Magento_GoogleTagManager::checkout/set_checkout_option.phtml` 模板已还原。

### GraphQL

<!-- ENGCOM-7074-->

* 应用程序现在返回GraphQL消息 `The cart isn't active` 而不是 `Current user does not have an active cart` 在某些情况下，当购物车变为不活动并创建新购物车ID时。 _修复了Vadim Malesh在拉取请求中提交的问题 [27187](https://github.com/magento/magento2/pull/27187)_. [GitHub-26117](https://github.com/magento/magento2/issues/26117)

<!-- ENGCOM-7105-->

* 删除了在使用GraphQL将产品添加到购物车时导致错误的代码，此代码会在以前添加的产品无现货时导致该代码。 _由AleksLi在拉取请求中提交的修复 [27015](https://github.com/magento/magento2/pull/27015)_. [GitHub-26683](https://github.com/magento/magento2/issues/26683)
...
* GraphQL变异 `setShippingMethodsOnCart` 不再在  `available_shipping_methods` 属性。 _修复了Vadim Malesh在拉取请求中提交的问题 [27004](https://github.com/magento/magento2/pull/27004)_. [GitHub-26742](https://github.com/magento/magento2/issues/26742)
...
* 现在，使用页面生成器上传到CMS块的图像，在从基本URL与店面URL不同的管理员上传后，会按预期在预览中显示。
...
* 现在，所有导出的网格数据(包括Adobe Commerce和自定义数据)都具有选项标签，而不是所有类型列的选项值 `Select` 或 `Multiselect`. 以前，导出此数据时，导出的电子表格包含ID为的列，而不是网格中显示的值。 _修复了Maksym Novik在拉取请求中提交的问题 [26523](https://github.com/magento/magento2/pull/26523)_. [GitHub-25963](https://github.com/magento/magento2/issues/25963)
...
* 现在，订单状态更改将按照 **系统** > **操作日志** > **报表**.

<!-- MC-29637-->

* 应用程序现在会按预期在“管理操作”日志中记录销售装运操作。
...
* 现在，当 **选择要记录的所有操作** 在“管理操作日志记录”选项卡(**管理员** > **商店** > **配置** > **高级**)。

<!-- ENGCOM-7274-->

* 现在，CMS页面修改在操作日志中按预期记录。 MFTF现在涵盖此功能。  _Lukasz Bajsarowicz在拉取请求中提交的修复 [27597](https://github.com/magento/magento2/pull/27597)_. [GitHub-171](https://github.com/magento/magento2/issues/171)
...
* Adobe Commerce `cron`NewRelic APM Monitoring - Transactions中的Transaction Traces现在按预期包含作业名称。 这包括 `cron` 从命令行执行的作业以及标准调度程序执行的作业。  _Lukasz Bajsarowicz在拉取请求中提交的修复 [25957](https://github.com/magento/magento2/pull/25957)_. [GitHub-22047](https://github.com/magento/magento2/issues/22047)
...
* 客户数据区域失效逻辑已得到改进。 此版本引入了一种使所有客户部分数据失效的新方法，可避免自定义时本地存储出现已知问题 `sections.xml` 无效。 (以前，当您具有自定义内容时，无法正确填充专用内容（本地存储） *etc/frontend/sections.xml* （操作无效。） 请参阅 [专用内容](https://developer.adobe.com/commerce/php/development/cache/page/private-content/#invalidate-private-content).
...
* 当您从店面成功创建订单和RMA时，应用程序不再显示错误。 以前，应用程序按预期创建了RMA，但也显示了以下错误：  `We can't create a return right now. Please try again later`.

<!-- MC-30181-->

* 店面订单中包含的跟踪链接现在可按预期工作。 以前，当客户单击订单退货页面上的发运跟踪链接时，应用程序会显示以下错误： `PHP Fatal error: Uncaught Error: Call to a member function getTitle() on null in /app/z7kvt3uys6daq/var/view_preprocessed/pub/static/vendor/magento/module-shipping/view/frontend/templates/tracking/details.phtml:1`.

<!-- MC-30237-->

* 现在，在从管理员创建订单返回后，“返回”选项卡会按预期显示。

<!-- MC-34150-->

* 已从RMA电子邮件模板中删除了不必要的字符串(`app/code/Magento/Rma/view/frontend/email/new.html`)。
...
* 现在，当订单返回到在多商店环境中运行的商店时，应用程序会按预期退款奖励点，在该环境中，每个商店的奖励点费率各不相同。 以前，对于使用奖励积分购买的退回订单，申请没有按预期退款点数。

### 销售

<!-- MC-29426-->

* 现在，已完成的订单会同时显示在付款系统和Adobe Commerce中。 以前，订单出现在支付系统中，但不出现在Adobe Commerce。 [GitHub-25862](https://github.com/magento/magento2/issues/25862)
...
* Elasticsearch7.6.x现在是Adobe Commerce和Adobe Commerce的默认目录搜索引擎。 如果不同时安装Elasticsearch7.6.x，则无法安装或升级到版本2.4.0。Elasticsearch版本2.x代码已删除。 Elasticsearch版本5.x和6.x已弃用，不再受支持。 请参阅 [搜索引擎先决条件](../../../installation/prerequisites/search-engine/overview.md).
...
* 现在，仅在多站点部署中对一个网站具有受限访问权限的管理员可以预览该网站的计划更新。 以前，暂存预览默认使用默认存储区，如果用户无权访问默认存储区，则应用程序会引发异常。

<!-- MC-32736-->

* 编辑现有计划不再导致重复的计划。 以前，当您编辑现有计划时，应用程序会复制该计划，当您尝试打开重复的计划时，应用程序会引发错误。

<!-- MC-32432-->

* 现在，在为其中一个或多个产品创建计划更新后，交叉销售产品会按预期在查看和编辑购物车页面上显示。
...
* Adobe Commerce `cron` 现在用于在创建测试数据后重新索引，如果适用， `cache:flush config` 替换 `cache:flush`. _Lukasz Bajsarowicz在拉取请求中提交的修复 [26990](https://github.com/magento/magento2/pull/26990)_. [GitHub-26989](https://github.com/magento/magento2/issues/26989)
...
* 现在，当您将产品拖放到新位置时，编辑类别页面会按预期工作。 以前，在任何类别页面上拖放产品会将查看窗口重新定位到页面顶部。
...
* 现在，当您在包含多个愿望清单的部署中重命名一个愿望清单时，该应用程序可以正确保存并显示一个愿望清单的新名称。 以前，应用程序继续显示以前的愿望列表名称。

<!-- MC-31948-->

* 在运行Internet Explorer 11.x的部署中，您现在可以从购物车中导航和修改清单。以前，您无法在此环境中创建、编辑或移动愿望列表。
...
**问题**:在安装了PHP 7.4.2的部署中，会出现店面错误消息异常。 使用PHP 7.4.2部署2.4.0时，店面错误消息中的空间符号将替换为加号(+)字符。 此错误是PHP 7.4.2的本机错误，无法由Adobe Commerce更正。 **解决方法**:我们建议使用其他版本的PHP 7.4.x。请参阅 [在店面上显示原始消息数据](https://support.magento.com/hc/en-us/articles/360045804332)  知识库文章。 <!-- MC-34170-->
...
**问题**:在为退货授权(RMA)创建送货标签后，商家无法与退货页面上的任何页面元素进行交互。 请参阅 [创建送货标签时，“返回编辑”页面停止工作](https://support.magento.com/hc/en-us/articles/360046441312) 知识库文章。 的 **MC-35984-2.4.0-composer.patch** 此问题的修补程序现在可从 [版本](https://magento.com/tech-resources/download). <!-- MC-35984-->

**问题**:管理员无法将可通过SKU配置的产品添加到报价中。 当管理员单击 **添加到报价** “报价编辑”页面仍处于加载状态，管理员无法保存其更改。 **解决方法**:没有B2B Quote编辑的解决方法。  但是，您仍可以通过从产品列表中选择产品来订购产品，而不是通过SKU添加产品。 请参阅 [B2B管理员无法将可配置产品添加到报价](https://support.magento.com/hc/en-us/articles/360046801971) 知识库文章。 <!-- MC-35513-->
...
**问题**:如果折扣代码在采购订单(PO)获得批准之前过期，PO将继续显示折扣额，但在PO获得批准后，订单将以非折扣总额进行定位。 **解决方法**:安装 **B2B-709采购订单折扣补丁** 此问题的修补程序，现在可从 **我的帐户** > **下载** 部分 `magento.com`.

**问题**:在升级到B2B 1.2.0的多网站部署中，应用程序出现异常。 When `setup:upgrade` 运行时，在 `PurchaseOrder` 模块： `Module Magento_PurchaseOrder: Unable to apply data patch Magento\PurchaseOrder\Setup\Patch\Data\InitPurchaseOrderSalesSequence for moduleMagento_PurchaseOrder`. **解决方法**:安装 **B2B-716将NonTransactionableInterface接口添加到InitPurchaseOrderSalesSequence数据修补程序** 修补程序，该修补程序现在可从 **我的帐户** > **下载** 部分 `magento.com`.
...
* **问题**:集成测试失败，并出现以下错误： `PHP Fatal error: Declaration of Dotdigitalgroup\Email\Test\Integration\Model\Sync\Importer\ImporterFailedTest::setUp() must be compatible with PHPUnit\Framework\TestCase::setUp(): void in /var/www/vendor/dotmailer/dotmailer-magento2-extension/Test/Integration/Model/Sync/Importer/ImporterFailedTest.php on line 36`. **解决方法**:的 **BUNDLE-2684 Dotdigital Integration Tests补丁** 此问题的修补程序现在可从 [版本](https://magento.com/tech-resources/download). <!-- BUNDLE-2684-->
...
数据迁移工具可帮助将现有Magento1.x存储数据传输到Magento2.x。此命令行界面包括验证、进度跟踪、日志记录和测试功能。 有关安装说明，请参阅 [安装数据迁移工具](https://experienceleague.adobe.com/docs/commerce-operations/tools/data-migration/basics/install.html). 考虑探索或为 [数据迁移存储库](https://github.com/magento/data-migration-tool).
