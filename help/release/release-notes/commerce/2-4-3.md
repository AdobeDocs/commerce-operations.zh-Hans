...
由[Adobe AI](https://experienceleague.adobe.com/zh-hans/docs/commerce/live-search/overview)提供支持的[实时搜索](https://business.adobe.com/cn/ai.html)通过使用人工智能和机器学习算法执行对汇总的访客数据的深入分析，提供了直观的搜索体验。 请参阅[实时搜索发行说明](https://experienceleague.adobe.com/docs/commerce/live-search/release-notes.html?lang=zh-Hans)。
...
* **礼品登记簿**。 涵盖的任务包括： <!-- PWA-1290-->
   ...
   * [更新](https://developer.adobe.com/commerce/webapi/graphql/schema/gift-registry/mutations/update-items/)或从礼品注册表中删除[个项目。](https://developer.adobe.com/commerce/webapi/graphql/schema/gift-registry/mutations/remove-items/)
   ...
   * 从礼品注册表中添加[1&rbrace;、](https://developer.adobe.com/commerce/webapi/graphql/schema/gift-registry/mutations/add-registrants/)更新[或](https://developer.adobe.com/commerce/webapi/graphql/schema/gift-registry/mutations/update-registrants/)删除[注册者。](https://developer.adobe.com/commerce/webapi/graphql/schema/gift-registry/mutations/remove-registrants/)
...
* 管理员GWS现在为具有自定义权限的管理员在管理员集合中使用`int` SQL条件的`website_id`值。
...
* 使用`bin/magento setup:db-schema:split-sales`移动数据库表现在按预期工作。 现在可以正确创建表并填充数据。 以前，由于检查表存在性不正确，应用程序在您运行`SQLSTATE`时抛出`bin/magento setup:db-schema:split-sales`错误。
...
* 如果此区域文件的`webapi_rest`不包含已注册的插件，则不再为新自定义区域类型触发全局Adobe Commerce插件（例如`graphql`和`di.xml`）。
...
* 当属性字段值`resolution`设置为`required`no **时，RMA**&#x200B;属性不再显示为必需。
...
* 启用&#x200B;**后，应用程序现在会在客户注册页面上显示**&#x200B;增值税号`customer/create_account/vat_frontend_visibility`字段。 [GitHub-31326](https://github.com/magento/magento2/issues/31326)
