...
Adobe Commerce 2.4.3除了在平台方面进行了重大改进，还引入了性能和安全性增强功能。 安全增强功能包括扩展reCAPTCHA覆盖范围和包含内置速率限制。 核心编辑器依赖项和第三方库已升级到与PHP 8.x兼容的最新版本。

此版本包括核心代码的370多项新修复和33项安全增强。 Adobe Commerce 2.4.2发行说明中发现的所有已知问题都已在此版本中修复。
...
* 版本2.4.3已经过测试并被确认与Redis 6.0.12兼容。 （版本2.4.x仍与Redis 5.x兼容。）

* Laminas库依赖项已升级到与PHP 8.x兼容的版本。 已从删除一些冗余依赖项 `composer.json` 文件。 **Adobe Commerce 2.4.3使用Laminas 3.4.0**. <!-- MC-39513-->
...
[实时搜索](https://docs.magento.com/user-guide/live-search/overview.html) 提供者 [Adobe Sensei](https://www.adobe.com/sensei.html) 通过使用人工智能和机器学习算法来对汇总的访客数据进行深入分析，提供直观的搜索体验。 参见 [Live Search发行说明](https://experienceleague.adobe.com/docs/commerce-merchant-services/live-search/release-notes.html).
...
* **愿望清单**. 此 [addWishlistItemsToCart变异](https://devdocs.magento.com/guides/v2.4/graphql/mutations/add-wishlist-items-to-cart.html) 将项目从指定的愿望清单移至客户的购物车。 <!-- magento/partners-magento2ee#449-->

* **礼品登记处**. 涉及的任务包括： <!-- PWA-1290-->

   * 使用 [giftRegistry查询](https://devdocs.magento.com/guides/v2.4/graphql/queries/gift-registry.html) 以返回客户礼品登记处的内容。

   * 礼品注册搜索依据 [type](https://devdocs.magento.com/guides/v2.4/graphql/queries/gift-registry-type-search.html)， [电子邮件](https://devdocs.magento.com/guides/v2.4/graphql/queries/gift-registry-email-search.html)，或 [ID](https://devdocs.magento.com/guides/v2.4/graphql/queries/gift-registry-id-search.html).

   * [创建](https://devdocs.magento.com/guides/v2.4/graphql/mutations/create-gift-registry.html)， [更新](https://devdocs.magento.com/guides/v2.4/graphql/mutations/update-gift-registry.html)，或 [delete](https://devdocs.magento.com/guides/v2.4/graphql/mutations/remove-gift-registry.html) 礼品登记处。

   * [更新](https://devdocs.magento.com/guides/v2.4/graphql/mutations/update-gift-registry-items.html) 或 [移除](https://devdocs.magento.com/guides/v2.4/graphql/mutations/remove-gift-registry-items.html) 礼品注册处的商品。

   * [移动所有项目](https://devdocs.magento.com/guides/v2.4/graphql/mutations/move-cart-items-to-gift-registry.html) 从购物车到礼品注册处。 <!-- ENGCOM-9082 9087-->

   * [添加](https://devdocs.magento.com/guides/v2.4/graphql/mutations/add-gift-registry-registrants.html)， [更新](https://devdocs.magento.com/guides/v2.4/graphql/mutations/update-gift-registry-registrants.html)，或 [移除](https://devdocs.magento.com/guides/v2.4/graphql/mutations/remove-gift-registry-registrants.html) 礼品注册处的注册者。

   * [共享](https://devdocs.magento.com/guides/v2.4/graphql/mutations/share-gift-registry.html) 受邀者的礼物登记处。
...
* **共享路由**. 此 [路由查询](https://devdocs.magento.com/guides/v2.4/graphql/queries/route.html) 和 [可路由接口](https://devdocs.magento.com/guides/v2.4/graphql/interfaces/routable-interface.html) 在产品、类别和CMS页面上支持路由请求。 此 `urlResolver` 查询已弃用，其功能已被 `route` 查询。

请参阅 [GraphQL开发人员指南](https://devdocs.magento.com/guides/v2.4/graphql/) 以了解有关这些增强功能的详细信息。

### B2B

版本2.4.3引入了B2B v1.3.2。此版本包括多个错误修复。 参见 [B2B发行说明](https://experienceleague.adobe.com/docs/commerce-admin/b2b/release-notes.html).
...
此版本包括增强我们对Amazon Simple Storage Service (AWS S3)和Amazon Aurora云托管服务的支持。 它为AWS ElastiCache、AWSElasticsearch和AWS Managed Queues (Rabbit MQ)提供经认证的支持。 (我们测试了这些服务与Adobe Commerce的功能、性能和集成。)
...
* [Amazon Pay](https://docs.magento.com/user-guide/payment/amazon-pay.html). Amazon Pay已被弃用，将在以后的2.4.x版本中删除。 版本2.4.3及更高版本将仅包含针对兼容性的更新和针对主要错误的修复。
...
* 在将部署从Magento Open Source升级到Adobe Commerce后，以前创建的购物车价格规则现在显示在“内容暂存功能板”页面上。
...
* 此 `updateProductsInWishlist` 现在，变异可成功更新希望列表中属于捆绑产品的项目。 以前，此变异不会更新愿望清单项目，而是删除该项目并创建一个新项目，从而更改了项目ID。
...
* 以前缺失的捆绑产品数据现在包含在暂存过程中。 这解决了当购物者从产品列表页面购买捆绑产品而不是直接从产品页面添加捆绑产品时，产品行为不一致的问题。
...
* 订单不再忽略提供的具有送货地址的客户名称。 以前，忽略名称是因为 `same_as_billing` 标记未保存在数据库中。
...
* 使用者的所有队列消息 `quoteItemCleaner` 现在将其状态更改为 `complete` 在删除多个产品后按预期执行。 以前，此使用者只有一条消息将其状态更改为 `complete`，其余部分将状态更改为 `in progress`.
...
* 现在，在从“计划更新”预览中将捆绑包产品添加到购物车并单击购物车后，您会按预期重定向到结账页面。 [GitHub-447](https://github.com/magento/partners-magento2ee/issues/447)
...
* 现在，当购物者添加无效的产品数量并单击 **更新物料和数量** “管理购物车”页面上的按钮。 以前，应用程序升级了产品数量，并且未显示错误消息。 [GitHub-459](https://github.com/magento/partners-magento2ee/issues/459)
...
* 现在，您可以成功复制包含仅数字SKU的共享目录。 以前，当您尝试复制共享目录时，应用程序会引发错误，因为 `\Magento\Catalog\Model\ProductIdLocator` 类无法正确使用仅限数值的SKU。
...
* 现在，您可以从CMS编辑页的“层次”选项卡将新页分配给多个节点。 以前，在尝试将页面分配给节点时，会发生唯一约束冲突。 [GitHub-363](https://github.com/magento/partners-magento2ee/issues/363)
...
* 应用程序现在可以在店面和管理订单页面上正确显示自定义客户地址属性。 以前，地址信息部分中缺少下拉属性的选定选项，并且输入属性的值包含属性代码。 [GitHub-508](https://github.com/magento/partners-magento2ee/issues/508)
...
* 现在，当相应的客户区段同时应用于来宾和注册客户时，为其购物车中的注册客户显示动态块。 以前，应用程序只为来宾显示块。
...
* 应用程序现在会在购物车中显示相关客户区段中所有客户的动态块。 以前，即使客户区段同时包含来宾和注册客户，来宾也不会看到阻止。
...
* 为GraphQL请求添加了客户区段事件处理器。 以前，当在购物车价格规则条件中使用客户区段时，购物车价格规则折扣不会应用于购物车。 [GitHub-371](https://github.com/magento/partners-magento2ee/issues/371)
...
* 当您从管理员共享礼品注册时，应用程序发送的电子邮件现在包含指向注册位置的有效链接。 以前，使用此链接会导致404错误。 [GitHub-504](https://github.com/magento/partners-magento2ee/issues/504)
...
* 全局Adobe Commerce插件(例如， `webapi_rest` 和 `graphql` )时，将不再为新自定义区域类型触发 `di.xml` 此文件不包含已注册的插件。
...
* 商家现在可以通过商店视图在目录事件中指定或删除图像。 以前，当您在编辑目录事件时更改商店视图时， **删除图像** 复选框未处于活动状态，应用程序显示此错误： `Uncaught ReferenceError: toggleValueElements is not defined at HTMLInputElement.onclick`. [GitHub-364](https://github.com/magento/partners-magento2ee/issues/364)
...
* 应用程序现在以UTC时区保存目录事件日期。 以前，应用程序保存的目录事件日期以用户的时区表示。
...
* 现在，当您单击时，应用程序会显示一条准确、信息丰富的消息 **管理项目** 在“我的帐户礼品注册”页上，注册产品已缺货。 以前，应用程序引发此错误： `Error: Call to a member function getId() …`. [GitHub-490](https://github.com/magento/partners-magento2ee/issues/490)
...
* 取消礼品卡后，未决付款费用现在会按预期从礼品卡中删除。 以前，应用程序在以下情况下抛出错误： `cron` 在删除礼品卡后运行。

<!-- MC-37658-->

* 现在，购物者可以通过多个地址结帐时，从“查看订单”页面上的订单中删除礼品卡。 以前，当购物者选择礼品卡后单击时，应用程序不会将礼品卡从订单中移除 **移除**.

<!-- MC-34407-->

* 现在，当您在购物车中添加礼品卡后更改其值时，应用程序会在购物车中显示该礼品卡的新价格。
...
* 此 `setGiftOptionsOnCart` 变异现在可以正确地创造礼物信息。 [github-388](https://github.com/magento/partners-magento2ee/issues/388)

### 礼品注册表

<!-- MC-33057 magento/partners-magento2ee#467-->

* **活动日期** 现在，在创建或编辑礼品注册期间，店面和管理员上的值相同。 以前，每次编辑和保存礼品注册时，应用产品都会调整 **活动日期** 在店面和管理员面前都倒推一天。 [GitHub-466](https://github.com/magento/partners-magento2ee/issues/466)

<!-- MC-32587  magento/partners-magento2ee#511-->

* 该应用程序不再在礼品注册中显示禁用的产品。 以前，如果产品在添加到礼品注册表中后被禁用，当购物者访问注册表时，应用程序会显示以下消息： `Error: Call to a member function getId() …`. [GitHub-510](https://github.com/magento/partners-magento2ee/issues/510)

### 礼品包装

<!-- MC-38260-->

* 店铺浏览的礼品包装标签现在可以按预期翻译。

<!-- MC-36749-->

* 礼品包装现已按预期应用于发往多个地址的订单。

<!-- MC-34820-->

* 现在，当所有产品都移除时，应用程序会从购物车中移除礼品包装费用。
...
* 现在，您可以在计划导出中保存空值。 以前，应用程序会在创建或保存导出后更新空筛选器值。 两者 `no` 和 `not specified` 值在数据库中以零表示，其值为 `not selected` 被覆盖 `no`.
...
* 性能 `catalog_product_alert` `cron` 改进了在大表（数百万行）上运行的进程。 以前， `catalog_product_alert` 已加载所有产品警报，这会导致内存不足异常。
...
* 现在可以在Adobe Commerce on cloud infrastructure项目中查看和下载所有系统报告。 以前，您无法下载日志、数据、设计或商店的系统报告。
...
* 您现在可以从管理员中选择多个退货订单。 以前，应用程序在您单击时抛出错误 **提交** 选择多个返回结果并从“操作”下拉菜单中选择“关闭”后。
...
* 应用程序现在会根据管理员客户返回网格中的筛选值筛选记录。 以前，应用程序会将您定向到“管理所有客户”页面，然后单击 **重置筛选器** 按钮也重定向到此页面。 [GitHub-436](https://github.com/magento/partners-magento2ee/issues/436)
...
* 现在，当下订单的客户从系统中移除后，商家可以使用奖励积分取消全部或部分下订单的订单。

<!-- MC-40037-->

* 商家现在可以从购物车价格规则中删除奖励积分。 以前，当商家尝试设置 **奖励积分** 在创建购物车价格规则时设置为0，则保留奖励点数量的上一个值。

<!-- MC-39890-->

* 现在，当管理员批准多个审阅时，产品审阅的奖励积分会与相关网站关联 **营销** > **待处理审阅** 网格。 以前，奖励积分不会显示在店面上，也不会链接到管理员中的相关网站。
...
* 使用Safari的管理员现在可以从管理员成功将产品添加到订单。 以前，当管理员单击 **将所选产品添加到订单** 按钮时，应用程序显示旋转加载图标，页面挂起。 [GitHub-30265](https://github.com/magento/magento2/issues/30265)
...
* 开发人员现在可以更改Elasticsearch映射。 以前，发送到默认映射中Elasticsearch服务器的动态模板是硬编码的，这会阻止开发人员在Elasticsearch中索引与自定义模块关联的任何额外数据。 [GitHub-24363](https://github.com/magento/magento2/issues/24363)
...
* 仅限特定网站的管理员现在可以为同一网站上下达的订单创建装运。 以前，当缺少默认存储视图权限的管理员尝试发送置于管理员有权访问的存储视图中的订单时，应用程序会引发此异常： `Notice: Undefined offset: 1 in /app/code/Magento/Catalog/Model/Product/Attribute/Backend/GroupPrice/AbstractGroupPrice.php on line 293`.
...
* 应用程序不再取消选中 **附加注释** 购物者点击时的复选框 **获取配送方式和费率** 并从管理员创建订单时选择配送方式。 [GitHub-30256](https://github.com/magento/magento2/issues/30256)
...
* 在购物者更改“收货地址”多地址页上的数量并单击浏览器的“返回”按钮后，产品数量现在保持不变。 [GitHub-31956](https://github.com/magento/magento2/issues/31956)
...
* 在以下情况下，当商家尝试使用DHL发送订单时，应用程序不再引发错误 **创建配送标签** 复选框已启用，并且产品名称包含unicode字符。 以前，应用程序在请求创建标签时显示此错误： `The response is in wrong format`. [GitHub-31032](https://github.com/magento/magento2/issues/31032)
...
* 计划块更新功能现在在实施Fastly的部署中按预期工作。 以前，应用程序不更新块内容，因为Fastly缓存标记未失效，并且Fastly继续提供过时的内容。

<!-- MC-41142-->

* 现在，当购物车价格规则暂存预览分配给多个网站商店视图时，该预览会加载到相应的网站中。 以前，无论分配的网站是什么，购物车价格规则暂存预览都会加载到默认商店视图中。

<!-- MC-40916-->

* 在计划更新结束后，非默认商店视图中的产品特殊价格现在会按预期设置为默认值。
...
* 在预览类别的计划更新期间，应用程序不再执行包含所有产品ID的大型数据库查询。 相反，类别过滤器应用于产品收藏集，以创建一个临时表用于类别的暂存预览。 以前，应用程序将所有产品插入临时表中以进行暂存预览，这会降低性能。
...
* 现在，当您查看计划更新时，应用程序会显示包含正确数据的目录规则条件。 以前，如果您查看了使用条件创建的计划更新，则应用程序不会显示包含不正确数据的条件或显示的条件。
...
* 目录价格规则现在按预期应用于具有未定义属性值的产品。 以前，通过目录价格规则创建的折扣不会应用于具有未定义属性的产品。 [GitHub-461](https://github.com/magento/partners-magento2ee/issues/461)

<!-- MC-40283-->

* 现在，对于位于不同时区的商店，目录规则可在多网站部署中按预期工作。 以前，应用程序会一次性在所有网站上应用或停用目录规则。

<!-- MC-40054-->

* 为特定网站创建计划更新时，不再删除层价格。

<!-- MC-39784 magento/partners-magento2ee#475-->

* 在使用结束日期创建未来暂存更新后，您现在可以从可下载产品页面中保存产品。 以前，应用程序不会保存产品并显示以下消息： `The downloadable link isn't related to the product. Verify the link and try again`. [GitHub-474](https://github.com/magento/partners-magento2ee/issues/474)
...
* 应用程序现在显示 **增值税号** 在以下情况下，客户注册页面上的字段： `customer/create_account/vat_frontend_visibility` 已启用。 [GitHub-31326](https://github.com/magento/magento2/issues/31326)
...
* 添加了针对此方案的测试：当客户订阅了排队的新闻稿时，管理员用户可以编辑客户帐户。 [GitHub-30645](https://github.com/magento/magento2/issues/30645)

<!-- ENGCOM-8484-->

* 已删除 `cache:flush` 测试中的命令以缩短 `Catalog`， `CatalogUrlRewrite`、和 `LoginAsCustomer` 模块。 [GitHub-31056](https://github.com/magento/magento2/issues/31056)
...
* 配置内联翻译后，添加到字符串的括号将不再转义。 以前， `escapeHtmlAttr` 方法将括号转换为HTML实体代码。 [GitHub-32000](https://github.com/magento/magento2/issues/32000)
...
* 现在，从管理员发送的Order电子邮件使用商店区域设置，而不是与管理员帐户关联的区域设置。 [GitHub-26521](https://github.com/magento/magento2/issues/26521)
...
* 与管理员邀请页面上的选项卡存在对齐问题(管理员 **营销** > **私人销售** > **邀请**)已解决。 以前，客户已接受的邀请的选项卡显示未对齐。 [GitHub-333](https://github.com/magento/partners-magento2ee/issues/333)
...
* 当存在冗长的评级名称时，产品页面的“客户审核”部分中的评级星级和审核文本现在具有适当的间距。 [GitHub-31890](https://github.com/magento/magento2/issues/31890)
...
* 现在，在为新管理员生成电子邮件时，应用程序使用正确的自定义电子邮件模板。 以前，应用程序使用旧的默认模板，省略了管理员的名字和姓氏。
...
* 现在，具有受限角色的管理员可以成功添加、更新或复制他有权访问的分配给层次结构节点的页面。 以前，应用程序在这些情况下抛出致命错误。
...
* 通过可视化促销器加载“管理员”类别页面的性能已得到改进。 加载管理类别页面不再生成大型数据库查询。
...
* 现在，购物者会被重定向到他们试图进入的页面，然后才会被提示登录。 以前，在启用了网站限制并将限制模式设置为的部署中 **私人销售：仅登录**，购物者被重定向到商店主页。
...
* 现在，当TinyMCE中的CMS层次结构节点链接构件出现以下情况时，它将按预期呈现层次结构链接 **锚点自定义文本** 字段为空。
...
**问题**：GraphQL `category` 和 `categoryList` 查询忽略用于确定共享目录中的类别是隐藏还是公开的类别权限。 店面当前显示已分配给共享目录或未分配给共享目录的所有类别。 这会影响Adobe Commerce 2.4.3部署，这些部署实施了已启用B2B共享目录的PWA店面。 <!-- MC-42528-->
