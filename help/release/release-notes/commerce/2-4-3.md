...
此修补程序为Web API无法处理数组中包含20个以上项目的请求问题提供了解决方案。 此问题会影响运行Magento Open Source2.4.3、Adobe Commerce 2.4.3或2.3.7-p1的部署。 在这些版本中添加了内置速率限制功能，以防止拒绝服务(DoS)攻击，默认最大值为20。 此修补程序将默认限制还原为更高的值。 如果您怀疑您的商店正在遭受 DoS 攻击，Adobe 建议将默认输入限制降低到较低的值，以限制可以请求的资源数量。 请参阅“ [Web API 无法处理数组](https://support.magento.com/hc/en-us/articles/4406893342093) 知识库中超过 20 个项目的请求”一文。

## 应用 AC-384__Fix_Incompatible_PHP_Method__2.4.3_ce.patch 解决升级时出现的 PHP 致命错误

在升级到 Adobe Commerce 2.4.3 的过程中，可能会出现以下致命错误：

```
PHP Fatal error: Uncaught Error: Call to undefined function Magento\Framework\Filesystem\Directory\str_contains() in [...]/magento/vendor/magento/framework/Filesystem/Directory/DenyListPathValidator.php:74
```

此错误是由于使用`str_contains`函数导致的，该函数是一个PHP 8.x函数。 Adobe Commerce 2.4.3不支持PHP 8.x。此修补程序将此函数替换为支持的PHP 7.x函数。 请参阅[Adobe Commerce升级2.4.3、2.3.7-p1 PHP致命错误修补程序](https://support.magento.com/hc/en-us/articles/4408021533069-Adobe-Commerce-upgrade-2-4-3-2-3-7-p1-PHP-Fatal-error-Hotfix)知识库文章。
...
到目前为止，尚未发生与这些问题相关的已确认攻击。 但是，可能会利用某些漏洞访问客户信息或接管管理员会话。 这些问题大多要求攻击者首先获得管理员的访问权限。 列入允许列表因此，我们提醒您采取一切必要步骤来保护您的管理员，包括但不限于：IP身份验证、[双重身份验证](https://developer.adobe.com/commerce/testing/functional-testing-framework/two-factor-authentication/)、使用VPN、使用唯一位置而非`/admin`以及良好的密码卫生。 有关这些已修复问题的讨论，请参阅[Adobe安全公告](https://helpx.adobe.com/security/products/magento/apsb21-64.html)。

#### 其他安全增强功能

此版本的安全改进改进了与最新安全最佳实践的符合性，包括：

* **新Composer插件**&#x200B;有助于防止依赖关系混淆，并识别与公共包存储库上的内部包同名的恶意包。 请参阅[Adobe发布包含2.4.3版本](https://magento.com/blog/best-practices/adobe-releases-new-composer-plugin-magento-243-release)的新编辑器插件。
...
* **ReCAPTCHA覆盖范围已扩展**&#x200B;以包括：
   ...
   * ReCAPTCHA涵盖具有相应HTML页的Web API。 （这不包括通过集成访问的Web API。） ReCAPTCHA覆盖范围可保护端点免受垃圾邮件攻击。 当使用OAuth的第三方集成服务访问Web API时，ReCAPTCHA被禁用。<!-- MC-34472-->

   * 下单店面页面和与付款相关的Web API。 默认情况下，对这些页面的ReCAPTCHA保护处于禁用状态，并且可以从管理员将其启用。 此覆盖范围添加了一种反暴力机制，以保护商店免受分梳攻击。

{{cve-notice}}

### 基础架构改进

此版本包含一些增强功能，可改进框架和以下功能区域的质量：
...
* Laminas库依赖项已升级到PHP 8.x兼容版本。 已从`composer.json`文件中删除一些多余的依赖项。 **Adobe Commerce 2.4.3使用Laminas 3.4.0**。<!-- MC-39513-->
...
由[Adobe Sensei](https://www.adobe.com/sensei.html)提供支持的[实时搜索](https://experienceleague.adobe.com/en/docs/commerce-merchant-services/live-search/overview)通过使用人工智能和机器学习算法来对汇总的访客数据进行深入分析，提供了直观的搜索体验。 请参阅[实时搜索发行说明](https://experienceleague.adobe.com/docs/commerce-merchant-services/live-search/release-notes.html)。
...
* **愿望清单**。 [addWishlistItemsToCart突变](https://developer.adobe.com/commerce/webapi/graphql/mutations/add-wishlist-items-to-cart.html)将项目从指定的愿望清单移至客户的购物车。<!-- magento/partners-magento2ee#449-->

* **礼品登记簿**。 涵盖的任务包括： <!-- PWA-1290-->

   * 使用[giftRegistry查询](https://developer.adobe.com/commerce/webapi/graphql/queries/gift-registry.html)返回客户礼品注册的内容。

   * 按[类型](https://developer.adobe.com/commerce/webapi/graphql/queries/gift-registry-type-search.html)、[电子邮件](https://developer.adobe.com/commerce/webapi/graphql/queries/gift-registry-email-search.html)或[ID](https://developer.adobe.com/commerce/webapi/graphql/queries/gift-registry-id-search.html)搜索礼品注册表。

   * [创建](https://developer.adobe.com/commerce/webapi/graphql/mutations/create-gift-registry.html)、[更新](https://developer.adobe.com/commerce/webapi/graphql/mutations/update-gift-registry.html)或[删除](https://developer.adobe.com/commerce/webapi/graphql/mutations/remove-gift-registry.html)礼品注册表。

   * [更新](https://developer.adobe.com/commerce/webapi/graphql/mutations/update-gift-registry-items.html)或从礼品注册表中删除](https://developer.adobe.com/commerce/webapi/graphql/mutations/remove-gift-registry-items.html)个项目。[

   * [将所有商品](https://developer.adobe.com/commerce/webapi/graphql/mutations/move-cart-items-to-gift-registry.html)从购物车移至礼品注册表。<!-- ENGCOM-9082 9087-->

   * 从礼品注册表中添加[1}、[更新](https://developer.adobe.com/commerce/webapi/graphql/mutations/update-gift-registry-registrants.html)或[删除](https://developer.adobe.com/commerce/webapi/graphql/mutations/remove-gift-registry-registrants.html)注册者。](https://developer.adobe.com/commerce/webapi/graphql/mutations/add-gift-registry-registrants.html)

   * [与被邀请者共享](https://developer.adobe.com/commerce/webapi/graphql/mutations/share-gift-registry.html)礼品注册表。

* **可协商的引号**。 查看[`negotiableQuote`](https://developer.adobe.com/commerce/webapi/graphql/queries/negotiable-quote.html)和[`negotiableQuotes`](https://developer.adobe.com/commerce/webapi/graphql/queries/negotiable-quotes.html)查询。<!-- PWA-1292-->
...
* **共享路由**。 [路由查询](https://developer.adobe.com/commerce/webapi/graphql/queries/route.html)和[RoutableInterface](https://developer.adobe.com/commerce/webapi/graphql/interfaces/routable-interface.html)支持产品、类别和CMS页面上的路由请求。 `urlResolver`查询已弃用，其功能已被`route`查询取代。
...
此版本包括我们对Amazon简单存储服务(AWS S3)和Amazon Aurora云托管服务的支持的增强功能。 它为AWS ElastiCache、AWSElasticsearch和AWS Managed Queues (Rabbit MQ)提供认证支持。 (我们已测试这些服务与Adobe Commerce的功能、性能和集成。)
...
* [Amazon支付](https://marketplace.magento.com/amzn-amazon-pay-magento-2-module.html)。 Amazon Pay已被弃用，将在以后的2.4.x版本中删除。 版本2.4.3及更高版本将仅包含兼容性更新和主要错误的修复。
...
* [Yotpo产品审查](../../../upgrade/modules/upgrade.md)

## 已修复的问题

我们已经修复了2.4.3核心代码中的数百个问题。
...
* 当您运行`bin/magento setup:upgrade`从具有Redis的Magento Open Source部署升级到Adobe Commerce时，应用程序不再引发异常。
...
* 当应用程序安装有`—use-rewrites=0`选项或在`core_config_data_table`中将`web/seo/use_rewrites`设置为0时，管理员现在可以成功登录到部署。 [GitHub-32100](https://github.com/magento/magento2/issues/32100)

<!-- ENGCOM-8451 -->

* 已更新`AsyncCssPlugin`的`sortOrder`加载。 应用程序现在在`JsFooterPlugin`之前加载`AsyncCssPlugin`。 [GitHub-30882](https://github.com/magento/magento2/issues/30882)
...
* 使用 Grunt 或服务器端编译编译更少的文件现在会产生相同的结果。 以前。 `.abs- styles`，其中扩展了其他 `.abs- styles` `_extends.less`，在使用 Grunt 编译时未正确输出。 这导致了生产和开发部署之间的差异。 [GitHub-7231](https://github.com/magento/magento2/issues/7231)

### AdminGWS

<!-- MC-40703-->

* 管理员GWS现在为具有自定义权限的管理员在管理员集合中使用`website_id` SQL条件的`int`值。
...
* `updateProductsInWishlist`突变现在已成功更新属于希望列表中的捆绑产品的项。 以前，此变异不会更新愿望清单项，而是删除该项并创建一个新项，从而更改了项ID。

<!-- MC-38575-->

* 在使用`POST /V1/product/:sku`端点创建或更新捆绑产品时，您现在可以按预期设置`required_options`和`has_options`捆绑属性。 以前，这些自定义属性设置为0（零），尽管已尝试将其设置为1（一）。
...
* 订单不再忽略提供的具有送货地址的客户名称。 以前，由于数据库中未保存`same_as_billing`标志，因此省略了名称。

<!-- MC-40946-->

* 现在，当您在购物车中编辑产品时，指向礼品注册表的链接会按预期保留。 以前，单击&#x200B;**更新购物车**&#x200B;按钮时，这些链接会消失。

<!-- MC-40696-->

* 在删除多个产品后，消费者`quoteItemCleaner`的所有队列消息现在都会按预期将其状态更改为`complete`。 以前，此使用者只有一条消息将其状态更改为`complete`，其余消息将其状态更改为`in progress`。
...
* 当购物者添加无效的产品数量并单击“管理购物车”页面上的&#x200B;**更新项目和数量**&#x200B;按钮时，应用程序现在会显示信息性错误消息，并且不会更新产品数量。 以前，应用程序升级了产品数量并且不显示错误消息。 [GitHub-459](https://github.com/magento/partners-magento2ee/issues/459)
...
* 现在，您可以成功复制包含仅限数值的SKU的共享目录。 以前，当您尝试复制共享目录时，应用程序会引发错误，因为`\Magento\Catalog\Model\ProductIdLocator`类无法正确处理纯数字SKU。
...
* 现在，`cron`在`indexer_update_all_views`执行期间会按预期清除产品类别缓存。 以前，重新索引后“类别”页面上的产品计数不正确。
...
* 自定义类别布局更新文件现在会按预期应用到产品。 以前，更新文件句柄(`catalog_category_view_*`)与产品句柄不匹配。 [GitHub-27285](https://github.com/magento/magento2/issues/27285)

<!-- MC-40679-->

* 管理员产品列表的固定产品税(FPT)列已禁用排序。 以前，在对FPT列进行排序之后，无法重新加载“产品”页面。
...
* 使用GET`/rest/V1/products/?searchCriteria[filterGroups]`在产品搜索中按位置排序现在可按预期工作。 以前，产品集合没有要排序的字段`position`值。 [GitHub-31591](https://github.com/magento/magento2/issues/31591)
...
* 修复了CMS页面保存控制器的错误处理。 以前，当在`cms_page_prepare_save`事件中抛出`Error`对象时，应用程序将此对象传递给`addExceptionMessage`函数，从而违反其约定，因为此函数需要`Exception`。 通过使用`addErrorMessage`函数添加错误消息解决了此问题。 [GitHub-30149](https://github.com/magento/magento2/issues/30149)

<!-- MC-33490 magento/partners-magento2ee#435 -->

* 您现在可以从CMS的“编辑页面层次结构”选项卡将新页面分配给多个节点。 以前，当尝试将页面分配给节点时，会发生唯一约束冲突。 [GitHub-363](https://github.com/magento/partners-magento2ee/issues/363)
...
* 现在，该应用程序可为仅包含一个可配置产品的订单正确生成发票。 [GitHub-31143](https://github.com/magento/magento2/issues/31143)
...
* Cron清理查询已重构，以减少或消除以下性能问题：`cron`作业仍然卡在挂起状态，MySQL查询速度越来越慢，以及CPU使用率增加。 [GitHub-26507](https://github.com/magento/magento2/issues/26507)

<!-- ENGCOM-8571-->

* 状态为`running`且已超过24小时的`cronjobs`现在会自动更改为状态`error`。  因此，该作业的新实例可以再次运行，并且当作业错误地仍设置为状态`running`时，您无需手动更改作业状态。 以前，如果`cronjob`停滞在状态`running`中，应用程序会阻止启动同一作业的新实例，您必须手动更改作业状态。 [GitHub-8933](https://github.com/magento/magento2/issues/8933)
...
* 由于客户分段规则使用`Product was Ordered`条件而导致的数据库性能问题已得到解决。
...
* 客户地址属性日期值现在以四位数格式保存，而不是以两位数格式保存。

### 电子邮件

<!-- MC-39757-->

* 管理员重置密码页面上的密码重置链接现在可按预期工作。 以前，当自定义模板用于重置管理员密码页面时，当管理员单击电子邮件中的链接时，应用程序会显示此消息： `Your password reset link has expired`。 出现此问题是因为自定义电子邮件模板包含的用户ID变量不正确。

<!-- MC-41588-->

* 现在在“商店视图”级别启用时，从管理员发送客户电子邮件可以正常工作。 以前，在商店视图级别而不是全局级别启用电子邮件通知设置时，应用程序不会发送客户电子邮件。

<!-- MC-40950-->

* 现在，在多站点部署中，并非所有网站都启用了异步电子邮件发送，应用程序会按预期发送电子邮件。 以前，如果至少有一个网站禁用了此设置，则即使启用此设置，也不会从任何网站发送电子邮件。 “发票”、“发运”和“贷项通知单”电子邮件存在类似问题。 但是，已成功发送订单注释、发票注释、装运注释和贷项通知单注释电子邮件。 [GitHub-31950](https://github.com/magento/magento2/issues/31950)

<!-- MC-34323 magento/partners-magento2ee#505-->

* 当您从管理员处共享礼品注册表时，应用程序发送的电子邮件现在包含指向注册表位置的有效链接。 以前，获取此链接会导致404错误。 [GitHub-504](https://github.com/magento/partners-magento2ee/issues/504)
...
* 已在电子邮件模板筛选方法中添加字符串转换，以确保返回值为`string`。 以前，当未处于开发人员模式时捕获到异常时，应用程序返回`phrase`对象。 这进而触发了致命的`Uncaught TypeError`。 [GitHub-32671](https://github.com/magento/magento2/issues/32671)

<!-- ENGCOM-8807-->

* 在多站点部署中的一个网站上启用异步发送（**商店** > **配置** > **销售** > **销售电子邮件** > **常规设置** > **异步发送**）时，订单确认电子邮件现在会按预期发送。 [GitHub-31950](https://github.com/magento/magento2/issues/31950)
...
* 使用`bin/magento config:set system/backup/functionality_enabled 1`启用备份时，应用程序现在执行&#x200B;**从备份中排除媒体文件夹**&#x200B;设置。 以前，尽管进行了此设置，但会备份媒体文件夹，因为指向`/magento`的路径格式不正确，带有double //。
...
* 使用`bin/magento setup:db-schema:split-sales`移动数据库表现在按预期工作。 现在可以正确创建表并填充数据。 以前，由于检查表存在性不正确，应用程序在您运行`bin/magento setup:db-schema:split-sales`时抛出`SQLSTATE`错误。
...
* 如果此区域文件的`di.xml`不包含已注册的插件，则不再为新自定义区域类型触发全局Adobe Commerce插件（例如`webapi_rest`和`graphql`）。
...
* 商家现在可以通过商店视图在目录事件中指定或删除图像。 以前，当您在编辑目录事件时更改商店视图时，**删除图像**&#x200B;复选框处于非活动状态，应用程序显示此错误： `Uncaught ReferenceError: toggleValueElements is not defined at HTMLInputElement.onclick`。 [GitHub-364](https://github.com/magento/partners-magento2ee/issues/364)
...
* 现在，当您在“我的帐户礼品注册”页面上单击&#x200B;**管理项目**&#x200B;时，应用程序会显示一条准确、信息丰富的消息，并且注册产品已缺货。 以前，应用程序引发此错误： `Error: Call to a member function getId() …`。 [GitHub-490](https://github.com/magento/partners-magento2ee/issues/490)

<!-- MC-33667-->

* 您现在可以按预期从“添加新页面”页面将CMS页面添加到网站根层级。
...
* 您现在可以成功编辑使用属性集创建的产品，该属性集中的&#x200B;**设计组和计划设计更新**&#x200B;组已被删除。  以前，当您尝试编辑根据该属性集创建的产品时，应用程序会引发致命错误。 [GitHub-44](https://github.com/magento/partners-magento2ee/issues/44)

<!-- ENGCOM-8901-->

* `nowdoc`已替换`Magento_Backend`存储切换器中的`heredoc`。 [GitHub-32262](https://github.com/magento/magento2/issues/32262)
...
* 当由`user_id`筛选`Magento\Users\Model\ResourceModel\Users\Collection`时，应用程序不再抛出SQL异常。 [GitHub-31216](https://github.com/magento/magento2/issues/31216)

### 礼品卡

<!-- MC-40412-->

* 取消礼品卡后，未决付款费用现在会按预期从礼品卡中删除。 以前，删除礼品卡后运行`cron`时，应用程序会引发错误。
...
* 现在，当请求不包含`allow_open_amount`的值时，`addRequisitionListItemsToCart`查询会按预期返回自定义礼品卡的金额。
...
* `setGiftOptionsOnCart`突变现在可以正确创建礼品邮件。 [GitHub-388](https://github.com/magento/partners-magento2ee/issues/388)

### 礼品注册表

<!-- MC-33057 magento/partners-magento2ee#467-->

* 在创建或编辑礼品注册期间，店面和管理员中的&#x200B;**活动日期**&#x200B;值现在相同。 以前，每次编辑并保存礼品注册时，应用程序都会在店面和管理员上向后调整一天&#x200B;**活动日期**。 [GitHub-466](https://github.com/magento/partners-magento2ee/issues/466)

<!-- MC-32587  magento/partners-magento2ee#511-->

* 该应用程序不再在礼品注册中显示禁用产品。 以前，如果产品在添加到礼品注册表中后被禁用，则当购物者访问注册表`Error: Call to a member function getId() …`时，应用程序会显示此消息。 [GitHub-510](https://github.com/magento/partners-magento2ee/issues/510)

### 礼品包装

<!-- MC-38260-->

* 商店视图的礼品包装标签的翻译现在可以按预期工作。
...
* 启用Google Analytics后，当管理员尝试在店面加载页面时，应用程序不再引发错误。 已添加`connect-src`的CSP条目，以允许AJAX请求访问www.google-analytics.com。 [GitHub-30880](https://github.com/magento/magento2/issues/30880)
...
* GraphQL `products`查询现在返回属性选项，这些选项的排序顺序与属性编辑页面上使用的排序顺序相同。
...
* `CartItemPrices`对象现在包含新的GraphQL字段`fixed_product_taxes`，该字段返回应用于Cart项的固定产品税的数组。 以前，应用于购物车项目的固定产品税不包括在购物车查询中。

<!-- MC-40920-->

* 对GraphQL的空请求现在会引发响应代码200而不是500。 以前，GraphQL解析器在生成查询结果之前引发异常。

<!-- MC-40867 magento/partners-magento2ee#532 -->

* `applyRewardPointsToCart`突变现在更新了购物车的总计，以准确反映扣除的奖励积分的值。 [GitHub-486](https://github.com/magento/partners-magento2ee/issues/486)

<!-- MC-40379-->

* 当使用GraphQL链接追加销售产品时，GraphQL响应中不再包含禁用的产品。

<!-- MC-40262-->

* 在禁用来宾结帐功能后，商家现在可以使用GraphQL `setShippingAddressesOnCart`方法设置购物车的账单地址和运送地址。

<!-- MC-39508-->

* 当`categoryList`查询在`CategoryTree`对象上包含多个片段时，它不再引发异常。 [GitHub-31086](https://github.com/magento/magento2/issues/31086)
...
* 当product和category URL后缀包含null值时，应用程序不再在GraphQL查询期间引发类型错误。 [GitHub-30909](https://github.com/magento/magento2/issues/30909)

<!-- magento/magento2#31164-->

* `CustomizableDateValue`对象现在包含`type`属性。 其值是一个可以设置为DATE、DATE_TIME或TIME的枚举。

<!--  magento/partners-magento2ee#421-->

* [`dynamicBlocks`查询](https://developer.adobe.com/commerce/webapi/graphql/queries/dynamic-blocks.html)返回与指定筛选器匹配的动态块的内容。

<!-- ENGCOM-8603-->

* POST`V1/products/special-price-delete`请求现在只按预期删除具有指定`store_id`的价格。 之前，该调用已从所有商店中移除指定SKU的所有特殊价格。 [GitHub-25907](https://github.com/magento/magento2/issues/25907)

<!--  magento/partners-magento2ee#280 -->

* 修复了`createGiftRegistry`变异中的`country_code`属性的错误。

<!-- magento/partners-magento2ee#280 -->

* 修复了`updateGiftRegistryItems`变异的`giftRegistryUid`特性问题。

<!-- magento/partners-magento2ee#280 -->

* 添加了其他检查，以防止使用`updateGiftRegistryRegistrants`突变将随机人添加为礼品注册表。
...
* 现在，如果导入的产品具有`tax_class_name`值`None`或`0`，则应用程序会将产品税类设置为`None`。 以前，如果`product tax_class_name`在CSV文件中为`None`，则应用程序会创建一个新的税类`None`，该税类与现有的税类重复。 如果CSV文件中的`product tax_class_name`为`0`，则应用程序忽略该值，并且产品税分类在导入后未发生更改。

<!-- MC-40898-->

* 管理员现在可以从管理员成功更改捆绑产品的`bundle_values`的`name`。 以前，在您从管理员更改名称后，应用程序会在店面上将产品显示为缺货。 产品还与同一SKU合并到单个捆绑选项部分中。
...
* 当索引器模式设置为&#x200B;**保存时更新**&#x200B;时，使用RESTPUT`/V1/products/:sku/links`请求链接产品时，产品现在可在店面搜索结果中按预期提供。
...
* `composer.lock`文件已更新为编码标准的最新版本。 [GitHub-31152](https://github.com/magento/magento2/issues/31152)

<!-- ENGCOM-9015 -->

* 向`magento/module-jwt-framework-adapter`添加了`web-token/jwt-framework`程序包上缺少的依赖项。 [GitHub-32578](https://github.com/magento/magento2/issues/32578)
...
* 将插件添加到父类时，应用程序不再引发错误。 （`optionsProvider`参数现在在`Assest.php`中的`getContentIdentities`之后声明。） 以前，应用程序引发此错误： `Error: Cannot instantiate interface Magento\Framework\Data\OptionSourceInterface`。 [GitHub-31291](https://github.com/magento/magento2/issues/31291)
...
* 更正了 SameSite Cookie 参数的串联问题。 以前，不正确的串联将后缀`value`附加到 `lex` 、 `domain`和其他参数。[GitHub-26377](https://github.com/magento/magento2/issues/26377)、 [GitHub-32440](https://github.com/magento/magento2/issues/32440)
...
* 当客户尝试完成没有可用的装运承运人时的订单时，应用程序不再引发错误。 而是显示签出页面和以下消息： `Sorry, no quotes are available for this order at this time`。 以前，应用程序显示空白签出页面，并在异常日志中记录了以下消息： `array_keys() expects parameter 1 to be array, null given`。 [GitHub-30830](https://github.com/magento/magento2/issues/30830)
...
* 已从测试中删除`magento indexer:reindex`和`cache:flush`命令以及`AdminReindexAndFlushCache`操作组，以改善以下模块的执行： `Bundle`、`Catalog`、`CatalogRule`、`CatalogRuleConfigurable`、`CatalogUrlRewrite`、`Downloadable`、`Indexer`、`Paypal`和`Sales`。 [GitHub-31031](https://github.com/magento/magento2/issues/31031)
...
* 已使用`StorefrontCheckQuickSearchStringActionGroup`和`StorefrontAssertProductNameOnProductMainPageActionGroup`（现有操作组）重构测试。 [GitHub-31251](https://github.com/magento/magento2/issues/31251)
...
`AdminMassOrdersHoldOnPendingAndProcessingTest` （替换已弃用的`AdminMassOrdersHoldOnPendingAndProcessingTest`） <!-- ENGCOM-8669-->
...
`AssertAdminProductIsAssignedToCategoryActionGroup` (用SKU代替筛选产品网格，然后单击第一行（以减少测试执行时间） <!-- ENGCOM-8535 8534-->
...
* 已从`Catalog`、`CatalogUrlRewrite`、`Checkout`、`Config`、`ConfigurableProduct`、`Contact`、`Cookie`、`CurrencySymbol`、`Customer`、`Downloadable`、`Elasticsearch`、`Elasticsearch6`、`Fedex`、`Indexer`、`LayeredNavigation`、`LoginAsCustomer`、`Msrp`、`Multishipping`、`Sales`、`Swatches`、`Translation`、`UrlRewrite`、`Vault`、`Weee`和`Wishlist`模块中移除`CliCacheFlushActionGroup`。
...
* 在大表（数百万行）上运行时，`catalog_product_alert` `cron`进程的性能已得到改进。 以前，`catalog_product_alert`加载所有产品警报，这会导致内存不足异常。
...
* 现在，可以从Adobe Commerce on cloud infrastructure项目查看和下载所有系统报告。 以前，您无法下载日志、数据、设计或商店的系统报表。

<!-- MC-39737-->

* 管理员&#x200B;**报告** > **审阅** > **按产品**&#x200B;的上次审阅日期现在显示正确的审阅日期。 以前，应用程序显示产品创建日期，而不是审阅日期。

### 退货授权(RMA)

<!-- MC-41866-->

* 当属性字段值`required`设置为&#x200B;**no**&#x200B;时，RMA `resolution`属性不再显示为必需。
...
* 现在，应用程序会根据管理员客户返回网格中的筛选值来筛选记录。 以前，应用程序将您定向到“管理所有客户”页面，单击“重置筛选器”**按钮**&#x200B;也会重定向到此页面。 [GitHub-436](https://github.com/magento/partners-magento2ee/issues/436)
...
* 当管理员&#x200B;**营销** > **待处理审核**&#x200B;网格批准多个审核时，产品审核的奖励点现在与相关网站关联。 以前，奖励积分不会显示在店面上，也不会链接到管理员中的相关网站。
...
* 现在，在启用&#x200B;**允许延期交货**&#x200B;设置时，管理员用户可以为库存不足的项目下订单（**商店** > **配置** > **目录** > **库存** > **产品库存选项**）。 以前，应用程序引发错误。
...
* 现在，按日期过滤订单可返回准确的结果。 以前，当您按订单日期筛选订单时，应用程序不会返回在 00：00 UTC 之后下达的订单。

<!-- MC-40030-->

* 现在，当网站&#x200B;**范围用于**&#x200B;多商店部署中的 Price **属性时**，贷项通知单网格现在显示正确的货币符号。

<!-- MC-40013 ENGCOM-8781-->

* 重新加载“管理员创建新订单”页面后，结帐工作流程的“付款和运输信息”部分的付款方式单选按钮不再消失。 [GitHub-32106](https://github.com/magento/magento2/issues/32106)
...
* 店面订单详细信息页面现在显示正确的已发运产品数量。 以前，产品数量不正确，因为订单装运页的模板将&#x200B;**数量已发运**&#x200B;呈现为`int`。 这已更改为 `float`。

<!-- MC-39026-->

* 取消选中“ **通过电子邮件发送发票副本”按钮后，应用程序不再通过电子邮件发送订单** 发票副本。 以前，应用程序向&#x200B;**客户**&#x200B;发送电子邮件，向&#x200B;**发送发票电子邮件副本**。 [GitHub-28511](https://github.com/magento/magento2/issues/28511)
...
* 现在，当购物者从其帐户打印订单时，应用程序将使用&#x200B;**徽标中上传的徽标进行HTML打印视图**&#x200B;设置。 以前，应用程序显示LUMA徽标，而不是上传的徽标。
...
* 当购物者单击&#x200B;**获取配送方式和费率**&#x200B;并从管理员创建订单时选择配送方式时，应用程序不再取消选中&#x200B;**附加备注**&#x200B;复选框。 [GitHub-30256](https://github.com/magento/magento2/issues/30256)
...
* 启用&#x200B;**创建装运标签**&#x200B;复选框且产品名称包含Unicode字符时，当商家尝试使用DHL装运订单时，应用程序不再引发错误。 以前，应用程序在请求创建标签时显示此错误： `The response is in wrong format`。 [GitHub-31032](https://github.com/magento/magento2/issues/31032)

<!-- ENGCOM-8952 -->

* 您可以在多站点部署中成功向管理员下单，在该部署中，一个网站上启用了`United States`，而另一个网站上启用了`Disable all countries`作为默认范围。 以前，应用程序未下订单并显示此错误： `Please check the shipping address information. "regionId" is required. Enter and try again`。 [GitHub-30577](https://github.com/magento/magento2/issues/30577)
...
* 商户现在可以在创建包含结束日期的未来暂存更新后，从可下载产品页面成功保存包含链接示例的可下载产品。 以前，应用程序未保存产品并显示以下消息： `The downloadable sample isn't related to the product. Verify the link and try again`。 [GitHub-488](https://github.com/magento/partners-magento2ee/issues/488)
...
* 从2.3.x升级期间，`Magento_SalesRuleStaging`模块不再触发错误。
...
* 在更新前取消选中计划更新后，产品的&#x200B;**使用默认值**&#x200B;复选框保持选中状态。
...
* 现在，在创建包含结束日期的将来暂存更新后，您可以从可下载产品页面中保存产品。 以前，应用程序未保存产品并显示以下消息： `The downloadable link isn't related to the product. Verify the link and try again`。 [GitHub-474](https://github.com/magento/partners-magento2ee/issues/474)
...
* 已删除`lib/internal/Magento/Framework/App/Action/Plugin`下`\Magento\Framework\App\ActionInterface`的插件，以遵循以下准则：应使用插件来自定义另一个模块的一个模块的行为。 [GitHub-28050](https://github.com/magento/magento2/issues/28050)

<!-- MC-24772-->

* 现在，当您在网站创建期间禁用`Store View`时，尝试选择`Website`作为默认设置时，应用程序会显示此消息： `Please enable your Store View before using this Web Site as Default`。 以前，网站崩溃，应用程序不显示警报。
...
* 启用`customer/create_account/vat_frontend_visibility`后，应用程序现在会在客户注册页面上显示&#x200B;**增值税号**&#x200B;字段。 [GitHub-31326](https://github.com/magento/magento2/issues/31326)
...
* 已从集成测试中移除`CacheCleaner::cleanAll();`。 [GitHub-31074](https://github.com/magento/magento2/issues/31074)
...
* 已从测试中删除`cache:flush`命令以缩短`Catalog`、`CatalogUrlRewrite`和`LoginAsCustomer`模块的执行时间。 [GitHub-31056](https://github.com/magento/magento2/issues/31056)

<!-- ENGCOM-8499-->

* 已删除冗余参数，并将POST更改为`\Magento\Logging\Model\ProcessorTest::testLoggingProcessorLogsActionShipping`中所需的GET。
...
* 模板中用于管理存储的支付方式（**我的帐户** > **存储的支付方式** ）的文本字符串现在可翻译。 [GitHub-31211](https://github.com/magento/magento2/issues/31211)
...
* 已解决与管理员邀请页面（管理员&#x200B;**营销** > **私人销售** > **邀请**）上的选项卡的对齐问题。 以前，客户接受的邀请的选项卡显示未对齐。 [GitHub-333](https://github.com/magento/partners-magento2ee/issues/333)

<!-- MC-38226 magento/magento2#32008-->

* 使用PayPal Express签出时显示的订单审核页面现在已成功加载。 以前，模板包含未使用的&#x200B;**更新交付方法**&#x200B;按钮，该按钮在页面呈现期间仅被脚本部分隐藏。 [GitHub-32006](https://github.com/magento/magento2/issues/32006)
...
* 应用程序现在在&#x200B;**客户** > **登录**&#x200B;页面的登录区域上显示`DateRange`筛选器。 以前，应用程序显示`Text`筛选器。 [GitHub-30328](https://github.com/magento/magento2/issues/30328)

<!-- ENGCOM-9009 -->

* 已从模板文件中删除JavaScript，并将其移入不同的文件中，以减少管理页面上的渲染问题。 以前，在启用了HTML缩小的部署中，管理员页面无法正确渲染。 应用程序显示此错误： `An error has happened during application run. See exception log for details`。 [GitHub-32454](https://github.com/magento/magento2/issues/32454)

<!-- ENGCOM-9044 -->

* `dropdownDialog`小组件现在仅加载其需要的可拖动且可调整大小的`jquery-ui`块。 以前，它加载了大量不必要的代码，从而增加了加载时间并降低了性能。 [GitHub-32810](https://github.com/magento/magento2/issues/32810)
...
* 在`app/code/Magento/UrlRewrite/Model/ResourceModel/UrlRewriteCollection.php`集合的`addStoreFilter`函数中添加了对`store_id`的`main_table`引用。 以前，将`join`添加到还包含`store_id`列的表中的集合时出现问题。 [GitHub-31853](https://github.com/magento/magento2/issues/31853)
...
* 当&#x200B;**锚点自定义文本**&#x200B;字段为空时，TinyMCE中的CMS层次结构节点链接构件现在会按预期呈现层次结构链接。
...
**问题**：_礼品卡金额的PayPal信用卡显示异常_。 启用PayPal信用并配置多个礼品卡金额时，如果购物者更改礼品卡价值的金额，店面不会更新分期付款的金额。 Adobe Commerce 2.4.4和Magento Open Source2.4.4.<!-- MC-42499 AC-344-->将包含此问题的修复
...
我们感谢更广泛的Magento Open Source界，并感谢他们对这一释放的贡献。

社区工程团队[Magento投稿人](https://magento.biterg.io/app/kibana)按月、季度和年度维护最佳投稿人和合作伙伴的列表。 从该投稿人页面，您可以点击链接访问GitHub上其合并的PR。

### 合作伙伴的贡献

下表重点说明各伙伴的贡献。 此表列出了提供提取请求的合作伙伴、外部提取请求以及与请求关联的GitHub问题编号（如果可用）。

{{$include /help/_includes/release-notes/engcomm-2-4-3-partner.md}}

### 个人投稿人投稿

下表列出了我们的社区成员的意见。 此表列出了外部提取请求、与其关联的GitHub问题编号（如果可用）以及贡献提取请求的社区成员。
...
您可以使用[编辑器](../../../installation/composer.md)安装Adobe Commerce 2.4.3。
