...
迄今未发生任何已确认与这些问题有关的攻击。 但是，可能会利用某些漏洞访问客户信息或接管管理员会话。 这些问题大多要求攻击者首先获得管理员的访问权限。 因此，我们提醒您采取一切必要步骤来保护您的管理员，包括但不限于以下措施：IP允许列表、[双重身份验证](https://developer.adobe.com/commerce/testing/functional-testing-framework/two-factor-authentication/)、使用VPN、使用唯一位置而不是`/admin`以及良好的密码卫生。 有关这些已修复问题的讨论，请参阅[Adobe安全公告](https://helpx.adobe.com/cn/security/products/magento/apsb21-64.html)。
...
* 已弃用的TinyMCE v3库已被删除。 已从Adobe Commerce中删除与TinyMCE v3.x相关的`Magento_Tinymce3Banner`模块和MFTF测试。<!-- MC-42199 42170 -->

* 版本2.4.3已经过测试，并确认与Redis 6.0.12兼容。 （版本2.4.x仍与Redis 5.x兼容。）
...
由[Adobe Sensei](https://www.adobe.com/sensei.html)提供支持的[实时搜索](https://experienceleague.adobe.com/zh-hans/docs/commerce/live-search/overview)通过使用人工智能和机器学习算法来对汇总的访客数据进行深入分析，提供了直观的搜索体验。 请参阅[实时搜索发行说明](https://experienceleague.adobe.com/docs/commerce/live-search/release-notes.html?lang=zh-Hans)。
...
* **愿望清单**。 [addWishlistItemsToCart突变](https://developer.adobe.com/commerce/webapi/graphql/schema/wishlist/mutations/add-items-to-cart/)将项目从指定的愿望清单移至客户的购物车。<!-- magento/partners-magento2ee#449-->

* **礼品登记簿**。 涵盖的任务包括： <!-- PWA-1290-->

   * 使用[giftRegistry查询](https://developer.adobe.com/commerce/webapi/graphql/schema/gift-registry/queries/gift-registry/)返回客户礼品注册的内容。

   * 按[类型](https://developer.adobe.com/commerce/webapi/graphql/schema/gift-registry/queries/type-search/)、[电子邮件](https://developer.adobe.com/commerce/webapi/graphql/schema/gift-registry/queries/email-search/)或[ID](https://developer.adobe.com/commerce/webapi/graphql/schema/gift-registry/queries/id-search/)搜索礼品注册表。

   * [创建](https://developer.adobe.com/commerce/webapi/graphql/schema/gift-registry/mutations/create/)、[更新](https://developer.adobe.com/commerce/webapi/graphql/schema/gift-registry/mutations/update/)或[删除](https://developer.adobe.com/commerce/webapi/graphql/schema/gift-registry/mutations/remove/)礼品注册表。

   * [更新](https://developer.adobe.com/commerce/webapi/graphql/schema/gift-registry/mutations/update-items/)或从礼品注册表中删除[&#128279;](https://developer.adobe.com/commerce/webapi/graphql/schema/gift-registry/mutations/remove-items/)个项目。

   * [将所有商品](https://developer.adobe.com/commerce/webapi/graphql/schema/gift-registry/mutations/move-cart-items/)从购物车移至礼品注册表。<!-- ENGCOM-9082 9087-->

   * 从礼品注册表中添加[&#128279;](https://developer.adobe.com/commerce/webapi/graphql/schema/gift-registry/mutations/add-registrants/)1&rbrace;、[更新](https://developer.adobe.com/commerce/webapi/graphql/schema/gift-registry/mutations/update-registrants/)或[删除](https://developer.adobe.com/commerce/webapi/graphql/schema/gift-registry/mutations/remove-registrants/)注册者。

   * [与被邀请者共享](https://developer.adobe.com/commerce/webapi/graphql/schema/gift-registry/mutations/share/)礼品注册表。

* **可协商的引号**。 查看[`negotiableQuote`](https://developer.adobe.com/commerce/webapi/graphql/schema/b2b/negotiable-quote/queries/quote/)和[`negotiableQuotes`](https://developer.adobe.com/commerce/webapi/graphql/schema/b2b/negotiable-quote/queries/quotes/)查询。<!-- PWA-1292-->
...
* **共享路由**。 [路由查询](https://developer.adobe.com/commerce/webapi/graphql/schema/products/queries/route/)和[RoutableInterface](https://developer.adobe.com/commerce/webapi/graphql/schema/products/interfaces/routable/)支持产品、类别和CMS页面上的路由请求。 `urlResolver`查询已弃用，其功能已被`route`查询取代。

有关这些增强功能的详细信息，请参阅[GraphQL开发人员指南](https://developer.adobe.com/commerce/webapi/graphql/)。

### B2B

版本2.4.3引入了B2B v1.3.2。此版本包含多个错误修复。 请参阅[B2B发行说明](https://experienceleague.adobe.com/docs/commerce-admin/b2b/release-notes.html?lang=zh-Hans)。

### 页面生成器

Page Builder现在作为Magento Open Source中的捆绑扩展提供。 它现在是Adobe Commerce 2.4.3和Magento Open Source2.4.3的默认内容编辑工具。它可以用任何第三方模块替代WYSIWG编辑器。

“页面生成器”在下列管理区域中取代了TinyMCE编辑器：
...
此版本包括增强我们对Amazon Simple Storage Service (AWS S3)和Amazon Aurora云托管服务的支持。 它为AWS ElastiCache、AWS Elasticsearch和AWS Managed Queues (Rabbit MQ)提供经认证的支持。 (我们测试了这些服务的功能、性能，以及它们与Adobe Commerce的集成。)
...
* 在将部署从Magento Open Source升级到Adobe Commerce后，之前创建的购物车价格规则现在显示在“内容暂存仪表板”页面上。
...
* 现在，当应用程序安装在`core_config_data_table`中的`—use-rewrites=0`选项或`web/seo/use_rewrites`设置为0时，管理员可以成功登录到部署。 [GitHub-32100](https://github.com/magento/magento2/issues/32100)
...
* 使用Grunt或服务器端编译编译的Less文件现在会产生相同的结果。 以前。 使用Grunt编译时，扩展`_extends.less`中其他`.abs- styles`的`.abs- styles`未正确输出。 这导致了生产部署与开发部署之间的差异。 [GitHub-7231](https://github.com/magento/magento2/issues/7231)
...
* 现在，验证码在结账页面上可按预期工作。 以前，在购物者正确回答了CAPTCHA质询后，结账页面上的加载程序从未完成，并且应用程序显示以下错误： `captchaData[formId] is undefined`。 （仅当购物者使用他们之前从中访问运行2.3.5-p1的部署的相同浏览器时，才会发生此错误。）

<!-- ENGCOM-8630-->

* `_.isEmpty()`现在已成功完成`defaultCaptcha.js`文件的签入。 以前，这些检查没有完成，因此，升级后无法加载签出页面。 [GitHub-31641](https://github.com/magento/magento2/issues/31641)
...
* 现在，如果购物者未能单击“**更新**”按钮并返回到送货步骤，应用程序会放弃结账付款步骤上账单地址表单的更改。

<!-- MC-39517  magento/partners-magento2ee#460-->

* 现在，当购物者添加无效的产品数量并单击管理购物车页面上的&#x200B;**更新物料和数量**&#x200B;按钮时，应用程序会显示信息性错误消息，并且不会更新产品数量。 以前，应用程序升级了产品数量并且未显示错误消息。 [GitHub-459](https://github.com/magento/partners-magento2ee/issues/459)

<!-- MC-37807 ENGCOM-8530-->

* 现在，带有可自定义选项`(File)`的产品在多发结账过程中会按预期包括活动链接。 以前缺少此链接。 [GitHub-31095](https://github.com/magento/magento2/issues/31095)

<!-- MC-37689-->

* 现在，Admin购物车以正确的货币显示支持多种货币的商店的产品价格。 以前，价格会多次转换为指定的货币 — 先是从店面将产品添加到购物车时，然后再向管理员提交订单时。

<!-- MC-36703  magento/partners-magento2ee#439-->

* 现在，在管理员从店面上的购物者创建的管理员处完成订单后，应用程序会按预期清空购物车。 以前，当客户在管理员完成订单后重新登录时，店面购物车仍包含订单内容。 [GitHub-30262](https://github.com/magento/magento2/issues/30262)

<!-- MC-35640-->

* 购物者现在可以将产品添加到购物车中，其最低广告价格(MAP)超过其正常产品价格。
...
* 现在，类别列表视图上的&#x200B;**添加到购物车**&#x200B;按钮可按预期工作。 [GitHub-32232](https://github.com/magento/magento2/issues/32232)
...
* 您现在可以成功复制包含仅数字SKU的共享目录。 以前，当您尝试复制共享目录时，应用程序会引发错误，因为`\Magento\Catalog\Model\ProductIdLocator`类无法正确处理纯数字SKU。
...
* 使用GET`/rest/V1/products/?searchCriteria[filterGroups]`在产品搜索中按位置排序现在可按预期工作。 以前，产品收藏集没有用于排序的字段`position`值。 [GitHub-31591](https://github.com/magento/magento2/issues/31591)
...
* 现在，如果启用&#x200B;**启用前端调整大小**&#x200B;配置设置，则在上载期间大图像的大小会按照预期调整。

<!-- MC-40663 ENGCOM-8883-->

* 修复了CMS页面保存控制器的错误处理。 以前，在`cms_page_prepare_save`事件上抛出`Error`对象时，应用程序会将此对象传递给`addExceptionMessage`函数，从而破坏其约定，因为此函数需要`Exception`。 通过使用`addErrorMessage`函数添加错误消息解决了此问题。 [GitHub-30149](https://github.com/magento/magento2/issues/30149)
...
* 当您从期望列表中编辑可配置产品时，应用程序显示的配置弹出窗口现在会按预期在单击“**确定**”按钮时关闭。
...
* 现在，购物者可以从非默认商店视图中将可配置产品添加到购物车中。 以前，当非默认商店视图上的购物者尝试添加可配置产品时，应用程序显示以下错误： `Could not add item to cart. Please check required options and try again`。 [GitHub-31660](https://github.com/magento/magento2/issues/31660)
...
* Cron清理查询已重构，以减少或消除以下性能问题： `cron`个作业仍处于挂起状态、MySQL查询越来越慢以及CPU使用率增加。 [GitHub-26507](https://github.com/magento/magento2/issues/26507)
...
* 现在，该应用程序在购物车中为相关客户区段中的所有客户显示动态块。 以前，即使客户区段同时包含来宾和注册客户，来宾也不会看到阻止。

<!-- MC-39464-->

* 解决了使用`Product was Ordered`条件的客户分段规则导致的数据库性能问题。

<!-- MC-38895-->

* 已添加REST/SOAP API客户区段事件观察程序。 因此，当通过REST/SOAP API下达订单时，如果市场细分中定义的标准匹配，则客户市场细分现在会按预期自动更新。

<!-- MC-38346-->

* 已为GraphQL请求添加了一个客户区段事件处理器。 以前，在购物车价格规则条件中使用客户区段时，购物车价格规则折扣不会应用于购物车。 [GitHub-371](https://github.com/magento/partners-magento2ee/issues/371)
...
* 在启用Redis页面缓存的部署中停止Redis服务器时，应用程序不再引发严重错误。

<!-- MC-40739-->

* 使用`bin/magento config:set system/backup/functionality_enabled 1`启用备份时，应用程序现在会执行&#x200B;**从备份中排除媒体文件夹**&#x200B;设置。 以前，尽管进行了此设置，但会备份媒体文件夹，因为指向`/magento`的路径格式不正确，带有double //。
...
* 礼品包装现已按预期应用于要发运至多个地址的订单。

<!-- MC-34820-->

* 现在，在删除所有产品后，该应用程序将从购物车中移除礼品包装费。

<!-- MC-32259 magento/partners-magento2ee#500-->

* 当贷项通知单包含礼品包装和可配置产品费用时，贷项通知单合计现在是正确的。 以前，礼品包装税金额并不总是包括在贷项通知单合计中。 [GitHub-34](https://github.com/magento/partners-magento2ee/issues/34)
...
* 修复了`createGiftRegistry`突变中`country_code`属性的错误。

<!-- magento/partners-magento2ee#280 -->

* 修复了`updateGiftRegistryItems`突变的`giftRegistryUid`属性的问题。
...
* 更正了导致`products`查询返回有关没有层定价集的物料的价格层的错误信息的问题。 [GitHub-32279](https://github.com/magento/magento2/issues/32279)
...
* 当索引器模式设置为&#x200B;**保存时更新**&#x200B;时，使用REST PUT `/V1/products/:sku/links`请求链接产品时，产品现在可在店面搜索结果中按预期提供。
...
* 将插件添加到父类时，应用程序不再引发错误。 （`Assest.php`中的`getContentIdentities`之后现在声明了`optionsProvider`参数。）以前，应用程序引发此错误： `Error: Cannot instantiate interface Magento\Framework\Data\OptionSourceInterface`。 [GitHub-31291](https://github.com/magento/magento2/issues/31291)
...
* 在管理员中创建新发票时，**&#x200B;**&#x200B;的电子邮件副本复选框现在可按预期工作。 以前，如果在&#x200B;**销售电子邮件**&#x200B;中启用了发送发票电子邮件的全局设置，则忽略此复选框。 它现在是一致的，并且运行方式与装运和贷项通知单创建页面相同。 [GitHub-28511](https://github.com/magento/magento2/issues/28511)
...
* 现在，应用程序在保存退款的客户余额时，会按预期保存修改后的订单。 [GitHub-393](https://github.com/magento/partners-magento2ee/issues/393)
...
* 现在可配置发票发送。 以前，无法配置发票发送，应用程序始终会在发票创建后发送发票。 发票和订单电子邮件均在一个观察员的范围内发送。 现在，订单电子邮件和发票电子邮件的发送由单独的观察者来管理。 [GitHub-27656](https://github.com/magento/magento2/issues/27656)
...
* 现在，当购物者从其帐户打印订单时，该应用程序将使用&#x200B;**徽标中上传的徽标进行HTML打印视图**&#x200B;设置。 以前，应用程序显示LUMA徽标，而不是上传的徽标。

<!-- MC-38625-->

* 现在，当项目数超过20时，店面上的订单项目页码会按预期工作。 以前，寻呼机计入子产品，且总计数不正确。
...
* 开发人员现在可以更改Elasticsearch映射。 以前，通过默认映射发送到Elasticsearch服务器的动态模板进行硬编码，这会阻止开发人员在Elasticsearch中为与自定义模块关联的任何额外数据编制索引。 [GitHub-24363](https://github.com/magento/magento2/issues/24363)
...
* 现在，当购物车中的某些项目符合免费配送条件时，应用程序会显示正确的调整后配送价格。 以前，当购物车中的部分商品符合免运费条件时，应用产品不会调整发运价格，而是向购物者显示全部发运价格。

<!-- MC-41186-->

* 现在，仅限于特定网站的管理员可以为在同一网站上下达的订单创建装运。 以前，如果管理员无权访问默认商店视图，则当该管理员尝试发送放置在管理员有权访问的商店视图中的订单时，应用程序会引发此异常： `Notice: Undefined offset: 1 in /app/code/Magento/Catalog/Model/Product/Attribute/Backend/GroupPrice/AbstractGroupPrice.php on line 293`。

<!-- MC-41069-->

* 现在，该应用程序在确定订单是否符合免费DHL运输条件时考虑到了相关的购物车价格规则折扣。
...
* 在预览类别的计划更新期间，应用程序不再执行包括所有产品ID的大型数据库查询。 相反，类别过滤器将应用于产品收藏集，以创建一个临时表用于类别的暂存预览。 以前，应用程序将所有产品插入到临时表中以进行暂存预览，这会降低性能。

<!-- MC-40714-->

* 在从2.3.x升级的过程中，`Magento_SalesRuleStaging`模块不再触发错误。

<!-- MC-40697-->

* 现在，当您查看计划更新时，应用程序会显示包含正确数据的目录规则条件。 以前，如果您查看使用条件创建的计划更新，则应用程序不会显示条件或显示包含错误数据的条件。
...
* 如果在创建网站期间禁用了`Store View`，则在您尝试选择`Website`作为默认设置时，应用程序现在会显示此消息： `Please enable your Store View before using this Web Site as Default`。 以前，该网站崩溃，应用程序不显示警报。

### 税

<!-- MC-39571-->

* 现在，该应用程序在验证最小订单金额时会将隐藏税考虑在内。
...
* 已从测试中删除`cache:flush`命令以改进`Catalog`、`CatalogUrlRewrite`和`LoginAsCustomer`模块的执行时间。 [GitHub-31056](https://github.com/magento/magento2/issues/31056)

<!-- ENGCOM-8499-->

* 在`\Magento\Logging\Model\ProcessorTest::testLoggingProcessorLogsActionShipping`中已删除冗余参数并将POST更改为GET。
...
* 标题中的帐户链接现在符合WCAG标准。 以前，标头中的帐户链接包含重复的ID，这导致WCAG验证失败。

<!-- MC-40278-->

* 应用程序现在优先使用商店徽标图像的商店配置，而不是布局配置。 以前，徽标图像的大小是固定的，不会因商店而异。

<!-- MC-35227-->

* 该应用程序现在在使用标准Adobe Commerce主题的店面页面上显示一致的页面元素。 以前，并非所有样式都应用于空白主题，因此并非所有页面元素都显示在使用此主题的页面中。 （例如，“我的订单”页面搜索字段中不存在放大镜图标。）

### 翻译和区域设置

<!-- MC-23553 ENGCOM-8839-->

* 配置内联翻译后，添加到字符串的括号将不再转义。 以前，`escapeHtmlAttr`方法会将括号转换为HTML实体代码。 [GitHub-32000](https://github.com/magento/magento2/issues/32000)
...
* 删除了管理员在更改属性集后将产品添加到分组产品集时显示的网格单元格中的冗余标题。 [GitHub-30911](https://github.com/magento/magento2/issues/30911)
...
* 已从模板文件中删除JavaScript并将其移动到单独的文件中，以减少管理员页面上的渲染问题。 以前，在启用了HTML缩小的部署中，管理员页面无法正确渲染。 应用程序显示以下错误： `An error has happened during application run. See exception log for details`。 [GitHub-32454](https://github.com/magento/magento2/issues/32454)
...
* 现在，在通过批量更新将产品分配给网站后，将按预期生成多站点部署中特定网站的产品URL重写。

<!-- MC-40780 magento/magento2#32598-->

* 当为父类别启用URL键的&#x200B;**使用默认值**&#x200B;复选框时，应用程序现在可正确生成子类别的URL路径。 以前，在多存储视图部署中使用不同的URL键时，在层次结构中移动类别会导致`url_path`值不正确。 [GitHub-16202](https://github.com/magento/magento2/issues/16202)

<!-- MC-40371-->

* 现在，当您在类别层次结构中移动类别时，应用程序将更新分配给所有商店作用域的类别的`url_path`。 以前，在层次结构中移动类别会导致错误的`url_path`。

<!-- MC-39463-->

* 通过管理员使用自定义URL创建的URL重定向现在可以成功工作。 以前，GraphQL缓存来自GraphQL `urlResolver`查询的响应，并在URL重写更新后返回旧值。

<!-- MC-38931  magento/magento2#31106-->

* 现在，从网站删除产品时，将按预期删除产品URL重写。 [GitHub-24184](https://github.com/magento/magento2/issues/24184)

<!-- ENGCOM-8719 -->

* 在`app/code/Magento/UrlRewrite/Model/ResourceModel/UrlRewriteCollection.php`集合的`addStoreFilter`函数中添加了对`store_id`的`main_table`引用。 以前，将`join`添加到表中也包含`store_id`列的集合时会发生问题。 [GitHub-31853](https://github.com/magento/magento2/issues/31853)

<!-- ENGCOM-8894 -->

* `PUT /V1/products/:sku` REST终结点现在会按预期重新生成产品URL重写。 以前，终结点重新生成产品`url_key`值，但不重写URL。 [GitHub-30316](https://github.com/magento/magento2/issues/30316)
...
* 当您从期望列表中单击产品的&#x200B;**编辑项**&#x200B;按钮时，应用程序不再重置可配置产品的配置设置。 [GitHub-32119](https://github.com/magento/magento2/issues/32119)

<!-- MC-40417-->

* 现在，当您使用愿望列表中的可自定义文件选项更新产品时，应用程序会显示正确的产品价格。 以前，应用程序显示错误的产品价格，并且不显示指向上传文件的链接。

<!-- MC-39060-->

* 对于具有多个愿望列表的客户，其愿望列表中的产品总数现在与愿望列表中的项目数匹配。 以前，缺货产品包含在产品总数中。
...
**问题**： _&#x200B;**“添加到订单”**&#x200B;按钮不适用于通过SKU添加到购物车中的产品_。  按SKU将产品添加到订单后，当您单击&#x200B;**添加到订单**&#x200B;按钮时，应用程序会显示此错误消息： `An error has happened during application run`。 产品不会添加到购物车中。 **解决方法**：使用添加产品功能。<!-- AC-345-->

**问题**： GraphQL `category`和`categoryList`查询忽略类别权限，这些权限用于确定共享目录中的类别是隐藏还是公开。 店面当前显示已分配或未分配给共享目录的所有类别。 这会影响实施已启用B2B共享目录的PWA店面的Adobe Commerce 2.4.3部署。<!-- MC-42528-->
