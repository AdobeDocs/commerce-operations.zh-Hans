...
## 应用MC-__set_rate_limits__2.4.3.patch解决API速率限制问题
...
到目前为止，尚未发生与这些问题相关的已确认攻击。 但是，可能会利用某些漏洞访问客户信息或接管管理员会话。 这些问题中的大多数要求攻击者首先获得对管理员的访问权限。 因此，我们提醒您采取一切必要步骤来保护您的管理员，包括但不限于：IP身份验证、[双重身份验证](https://developer.adobe.com/commerce/testing/functional-testing-framework/two-factor-authentication)、使用VPN、使用唯一位置而非`/admin`以及良好的密码卫生。 有关这些已修复问题的讨论，请参阅[Adobe安全公告](https://helpx.adobe.com/security/products/magento/apsb21-64.html)。
...
由[Adobe AI](https://business.adobe.com/cn/ai.html)提供支持的[实时搜索](https://experienceleague.adobe.com/zh-hans/docs/commerce/live-search/overview)通过使用人工智能和机器学习算法来对汇总的访客数据进行深入分析，提供了直观的搜索体验。 请参阅[实时搜索发行说明](https://experienceleague.adobe.com/docs/commerce/live-search/release-notes.html?lang=zh-Hans)。
...
* [Amazon支付](https://commercemarketplace.adobe.com//amzn-amazon-pay-magento-2-module.html)。 Amazon Pay已被弃用，将在以后的2.4.x版本中删除。 版本2.4.3及更高版本将仅包含兼容性更新和主要错误的修复。
...
* 使用GET `/rest/V1/products/?searchCriteria[filterGroups]`的产品搜索中的按位置排序现在可按预期工作。 以前，产品收藏集没有用于排序的字段`position`值。 [GitHub-31591](https://github.com/magento/magento2/issues/31591)
...
* 现在，REST GET Cart API调用会返回帐单和送货地址的正确自定义属性值。 以前，自定义地址属性在“我的帐户前台”页面和“管理员”的订单详细信息页面中显示不正确。
...
* `Magento\Widget\Block\Adminhtml\Widget\Instance\Edit\Chooser\Container`中的页面布局不再进行硬编码。  因此，`getPageLayouts()`函数现在会按预期返回由不同模块声明的实际页面布局列表。 以前，只返回硬编码布局。 [GitHub-31168](https://github.com/magento/magento2/issues/31168)
...
* `ArrayIterator` PHP对象已更新为可与PHP 7.4正常配合使用。 [GitHub-32088](https://github.com/magento/magento2/issues/32088)
...
* 已删除冗余参数，并在`\Magento\Logging\Model\ProcessorTest::testLoggingProcessorLogsActionShipping`中根据需要将POST更改为GET。
