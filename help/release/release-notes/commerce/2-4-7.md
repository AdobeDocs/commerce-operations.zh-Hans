---
title: Adobe Commerce 2.4.7-beta3发行说明
description: 了解2.4.7-beta3 Adobe Commerce版本中的新增功能、增强功能、错误修复和已知问题。
...
# Adobe Commerce 2.4.7-beta3发行说明
...
Adobe Commerce 2.4.7-beta3引入了对PHP 8.3的支持。它引入了150多项质量修复和增强功能。 核心编辑器依赖项和第三方库已升级到最新可用版本。 此版本包括显着的性能改进，包括更快地加载具有多个选项的复杂产品的产品列表页面。
...
## Adobe Commerce 2.4.7-beta3测试环境

Adobe Commerce 2.4.7-beta3已针对以下组件版本进行了测试：
...
Adobe Commerce 2.4.7-beta3包括以下平台升级：
...
Adobe Commerce 2.4.7仍与PHP 8.1兼容，仅用于升级目的。 不支持PHP 8.1，因此不建议使用。 Adobe Commerce 2.4.7-beta3核心代码、所有捆绑的扩展以及所有Adobe拥有的扩展和SaaS服务都与PHP 8.3兼容。 <!-- AC-9668 -->

* **RabbitMQ 3.11支持**. 此版本与最新版本的RabbitMQ 3.11兼容。兼容性保持与RabbitMQ 3.9的兼容，该版本在2023年8月之前受支持，但我们建议仅将Adobe Commerce 2.4.7-beta3与RabbitMQ 3.11一起使用。

* **编辑器2.6.x**. 与Composer 2.2.x的兼容性得以保留。 <!-- AC-9663 -->

* **支持Varnish缓存7.4**. 此版本与最新版本的Varnish Cache 7.4兼容。与6.0.x和7.2.x版本的兼容性保持不变，但我们建议仅将Adobe Commerce 2.4.7-beta3与Varnish Cache版本7.4或版本6.0 LTS一起使用。 <!-- AC-9662 -->
...
* **每张订单支持多张优惠券**. 商家现在可以配置每张订单可使用新优惠券应用的最大优惠券数量 **[!UICONTROL Maximum number of coupons per order]** 配置选项。 默认情况下，此值设置为1。 此版本中添加了对此功能的REST支持，GraphQL支持将很快可用。

* Commerce UPS XML API网关已迁移到新的Commerce UPS REST API，以支持UPS对其所做的更新 [API安全模型](https://developer.ups.com/oauth-developer-guide). (UPS正在为所有API实施OAuth 2.0安全模型（持有者令牌）。) 所有以前的Commerce UPS XML API已从Adobe Commerce 2.4.7代码库中移除。 <!-- AC-8918 -->
...
Commerce 2.4.7-beta3包括以下对Commerce性能和可扩展性的增强：
...
* **针对具有许多商店的部署，加快存储级配置的保存操作**. 在具有500个以上存储的部署中保存配置设置可能非常耗时。 新的异步配置模块通过运行cron作业来启用异步配置保存操作，该作业使用使用者处理消息队列中的保存操作。 默认情况下，AsyncConfig处于禁用状态。
...
通过Application Server for GraphQL API，Adobe Commerce可以维护Commerce GraphQL API请求中的状态，并且无需引导，进而提高所有GraphQL API的吞吐量、降低延迟和高效资源使用。 通过在进程之间共享应用程序状态，GraphQL API请求可以显着提升效率，GraphQL API响应速度平均提高30%。 <!-- ACPT-27 -->

### Adobe Commerce扩展中继包

此版本引入了Adobe Commerce扩展metapackage v2.0.0-beta3，它自动将所选Adobe Commerce扩展与此核心版本捆绑在一起。 在以下情况下安装此中继包中包含的这些扩展的版本： `composer update` 运行，从而简化在升级到最新核心版本时升级这些扩展的过程。 这些扩展维护独立的发放计划。

适用于Adobe Commerce 2.4.7-beta3的Adobe Commerce扩展元包包含以下扩展：
...
* **卖方发起的报价**. B2B销售代表现在可以直接从Admin中的Quote和Customer网格启动报价。 请参阅 [询价](https://experienceleague.adobe.com/docs/commerce-admin/b2b/quotes/quote-request.html).
...
* REST和GraphQL的更新
   ...
   * 添加了端点，以便为本版本中引入的卖方启动的报价功能提供Web API覆盖范围： `POST /V1/negotiableQuote/draft`. 请参阅 [管理可转让报价](https://developer.adobe.com/commerce/webapi/rest/b2b/negotiable-manage/).

   * 共享目录的店面功能的增强Web API覆盖范围包括用于分配和重置自定义价格操作的新端点： `POST /V1/sharedCatalog/:sharedCatalogId/assignTierPrices` 和 `POST /V1/sharedCatalog/:sharedCatalogId/resetTierPrices`.
...
* 3DS的设备数据现在可以在Braintree控制面板上验证和显示。 此 `collectDeviceData` 参数现在作为 `true` 在 `verifyCard` 需要3DS。 <!-- BUNDLE-3327 -->

* Admin Payments配置页面现在包含一个指向发行说明和Braintree扩展页面的链接。 <!-- BUNDLE-3325 -->

* BraintreeSDK已更新至最新版本。 <!-- BUNDLE-3323 -->
...
* 为除Venmo之外的所有支付方法的核心操作添加了GraphQL支持。 有关支付服务的新GraphQL端点的介绍，请参见 [结帐](https://developer.adobe.com/commerce/webapi/graphql/schema/checkout/). <!-- BUNDLE-3326 -->

* 除Venmo之外，已增强所有支付方法的信用卡保险存储。 购物者现在可以保存或保存其ACH支付，并从其帐户页面查看或删除它们。 商家可以从管理员启用和禁用ACH保险存储付款。 <!-- BUNDLE-3324 -->

* 结账工作流现在包括快速付款部分，提供PayPal、Google Pay和Apple Pay **[!UICONTROL Express]** 按钮。 <!-- BUNDLE-3322 -->
...
PWA Studio v.13.3.x与Adobe Commerce 2.4.7-beta3兼容。 它包括多项增强功能，可改善辅助功能。 有关错误修复的信息，请参见 [PWA Studio版本](https://github.com/magento/pwa-studio/releases). 请参阅 [版本兼容性](https://developer.adobe.com/commerce/pwa-studio/integrations/adobe-commerce/version-compatibility/) 有关PWA Studio版本及其兼容的Adobe Commerce核心版本的列表。

### Web API框架

**使用每个购物车的多张优惠券**

新的REST端点支持查看、添加和删除与购物车关联的多个优惠券。 <!-- LYNX-208 -->
...
我们已在Adobe Commerce 2.4.7-beta3核心代码中修复了数百个问题。 此版本中包含的已修复问题的子集如下所述。 还介绍了以前的Commerce 2.4.7测试版中包含的修复。
...
* Commerce不再将产品描述文本插入到产品详细信息页面 **[!UICONTROL Meta Description]** 字段为空。 以前，当产品描述包含由PageBuilder或其他HTML编辑器生成的HTML代码时，HTML会显示在元中 **[!UICONTROL Description]** 字段，并且不能将此字段留空。
...
* 现在，精确的文件大小信息包含在PageBuilder中已上传图像的图像预览中。

<!-- ACP2E-2016 -->

* 现在，页面生成器页面元素在Chrome浏览器(v112)中以可接受的速度呈现。 以前，在Chrome浏览器中运行PageBuilder的情况下保存内容的速度较慢，并且Commerce会在浏览器控制台中记录此错误： `[ERROR] Page Builder was rendering for 5 seconds without releasing locks`.
...
* 当您尝试在以下情况下为产品URL创建301/302重定向时，Commerce不再引发错误 **生成类别/产品URL重写** 已启用。 创建产品并将其分配给类别后，将创建自动化URL重写：一个针对直接URL，一个针对通过类别路径的产品。 创建新产品URL重写时，将检查产品、类别和商店之间的关联。 现在，此检查基于关联的实体属性（类别和产品）。 以前，此检查是在预先存在的URL重写时执行的。
...
您可以使用安装Adobe Commerce 2.4.7-beta3 [Composer](https://experienceleague.adobe.com/docs/commerce-operations/installation-guide/composer.html).
