...
此版本的平台升级改进了与最新安全最佳实践的兼容性。 Adobe Commerce 2.4.7-beta1引入了：

* Composer 2.5.x.与Composer 2.2.x的兼容性得以保持。  <!-- AC-8129 -->

* **支持Varnish缓存7.3**. 此版本与最新版本的Varnish Cache 7.3兼容。与6.0.x和7.2.x版本保持兼容性，但我们建议仅将Adobe Commerce 2.4.7-beta1与Varnish Cache版本7.3或6.0版LTS一起使用。

* **RabbitMQ 3.11支持**. 此版本与最新版本的RabbitMQ 3.11兼容。虽然与RabbitMQ 3.9的兼容性保持不变（该版本在2023年8月之前受支持），但我们建议仅将Adobe Commerce 2.4.7-beta1与RabbitMQ 3.11一起使用。
...
* **针对具有许多商店的部署，加快商店级配置的保存操作**. 在具有500多个存储的部署中保存配置设置可能非常耗时。 新 [`Async Config`](../../../performance/configuration.md#asynchronous-configuration-save-badge-247-beta1typeinformative-urlhelpreleaserelease-notescommerce2-4-7md-tooltipavailable-in-247-beta1-only) 模块通过运行 `cron` 使用使用者处理消息队列中保存操作的作业。 `AsyncConfig` 是 **已禁用** 默认情况下。  <!-- ACPT-831 -->

* **更快生成大型配置的配置缓存**. 此 `bin/magento cache:clean config` 现在，命令会在启用配置缓存时对配置缓存进行预加温。 这减少了为大型配置生成配置缓存所需的停机时间。 配置保存操作不再清理 `config_scopes` 在将数据写入缓存之前进行缓存，这还可以减少在写入配置数据时锁定其他请求的时间。
<!-- ACPT-992 -->
...
此 `ApplicationServer` 模块实施PHP应用程序服务器，该服务器仅适用于内部部署。 希望实施此服务器的开发人员还必须安装和配置Swool和Nginx。

此应用程序服务器的此Beta版本仅适用于内部部署；它不支持基于云的部署。 它尚不支持B2B GraphQL功能。 当配置此版本的PHP应用程序服务器时，GraphQL请求在内部部署中可能无法按预期工作。 参见 [适用于GraphQL API的应用程序服务器](https://experienceleague.adobe.com/docs/commerce-operations/performance-best-practices/performance-best-practices/application-server.html). <!-- ACPT-27 -->

### Adobe Commerce扩展元包

此版本引入了Adobe Commerce扩展元包v2.0.0-beta1，它自动将精选Adobe Commerce扩展与此核心版本捆绑在一起。 在以下情况下，将安装此元包中包含的这些扩展的版本： `composer update` 运行，从而简化在升级到最新核心版本时升级这些扩展的过程。 这些扩展维护独立的发放计划。
...
此版本包括针对B2B议价报价、REST API更新以及多个错误修复的新功能和增强。

* **卖方启动的报价**. B2B销售代表现在可以直接从Admin中的Quote和Customer网格启动quote。 参见 [创建报价](https://experienceleague.adobe.com/docs/commerce-admin/b2b/quotes/quote-request.html)

* **行项目折扣**. 折扣现在可以应用为报价中特定行项目的折扣百分比、固定值或扩展定价。

* **报价中的备注交换**. 在协商过程中，B2B买方和卖方现在可以直接在报价中添加注释。 注释创建和更新包括在行项目和报价历史记录中，以便透明化。 参见 [协商报价](https://experienceleague.adobe.com/docs/commerce-admin/b2b/quotes/quote-price-negotiation.html)

* **改进了“报价详细信息”视图**. 在查看报价详细信息页面时，销售商和买方现在可以按SKU或从产品目录添加项目、添加或编辑注释、应用行项目折扣，以及直接从报价详细信息页面访问产品配置。

* REST API的更新

   * 添加了端点，以便为本版本中引入的卖方启动的报价功能提供Web API覆盖。 参见 [管理可转让报价](https://developer.adobe.com/commerce/webapi/rest/b2b/negotiable-manage/).
  ...
  新 `quickorder_active` 的字段 `storeConfig` 和 `availableStores` 查询。 此字段指示是否启用快速订购功能。

有关Commerce B2B的REST和GraphQL API覆盖率的详细信息，请参阅 [Commerce Web API](https://developer.adobe.com/commerce/webapi/) 开发人员文档。 参见 [B2B发行说明](https://experienceleague.adobe.com/docs/commerce-admin/b2b/release-notes.html) 有关错误修复的说明。

### GraphQL

此版本增加了增强的GraphQL缓存功能，并支持自定义属性的GraphQL架构，包括：
...
* 添加了 `quickorder_active` 字段到 [`storeConfig`](https://developer.adobe.com/commerce/webapi/graphql/schema/store/queries/store-config/) 和 [`availableStores`](https://developer.adobe.com/commerce/webapi/graphql/schema/store/queries/available-stores/) 查询。 此字段指示是否启用快速订购功能。 <!-- LYNX-58 -->
...
PWA Studiov.13.2.x与Adobe Commerce 2.4.7-beta1兼容。 它包括多项增强功能，可提高可访问性。 有关错误修复的信息，请参见 [PWA Studio版本](https://github.com/magento/pwa-studio/releases). 参见 [版本兼容性](https://developer.adobe.com/commerce/pwa-studio/integrations/adobe-commerce/version-compatibility/) 有关PWA Studio版本及其兼容的Adobe Commerce核心版本的列表。
...
* 现在，无需在产品选项中重复SKU，即可成功导入捆绑包产品。 以前，当您导入包含重复条目的捆绑产品时，Adobe Commerce会在产品选项中创建多个重复的SKU。
...
* 购物车税和运费估算器现在可以准确反映默认的目标配置。 以前，当您配置默认纳税目标计算设置并指定省/市/自治区和ZIP时，购物车下方仅显示ZIP值。 **[!UICONTROL Estimate Tax and Shipping]**. 这是因为Adobe Commerce在调用条件语句之前将区域ID设置为undefined，该语句从管理员设置默认区域ID。

<!-- ACP2E-1547 -->

* 启用永久购物车后，Adobe Commerce在结账期间不再引发此错误： `Invalid state change requested`. 以前，在引发此错误时，多个订单具有相同的 `quote_id` 偶尔会出现。
...
* 改进了依赖活动购物车的客户区段规则的性能，这减少了将产品添加到购物车所需的时间。
...
* Adobe Commerce现在返回自定义日期的有效结果 `01/01/1970`. 以前，自定义属性返回以下错误： `Invalid input datetime format of value '1/01/1970`. [GitHub-37274](https://github.com/magento/magento2/issues/37274)
...
* 当用户尝试打印送货标签时，Adobe Commerce不再在日志文件中引发异常。 以前，如果扩展修改包含附件的响应标头，则系统在日志文件中引发异常。
...
* 在为商店ID与网站ID不匹配的其他网站创建客户后，管理员用户现在可以从管理员中添加新的客户地址。 以前，Adobe Commerce显示此弹出消息： `Something went wrong`. [GitHub-36582](https://github.com/magento/magento2/issues/36582)
...
* 现在，在多个地址结账时可正确应用礼品卡。 以前，多地址装运的礼品卡金额应用不正确，导致礼品卡金额无效。
...
* 此 `addProductsToCart` query现在可成功将礼品卡添加到购物车。 以前，此查询需要礼品卡收件人姓名，但会返回发件人姓名。 Adobe Commerce引发此错误： `Please specify a recipient name`. [GitHub-36665](https://github.com/magento/magento2/issues/36665)
...
* Google Tag模块现在可将正确的数据推送到数据层。 以前，页面展示数据不会推送到数据层。
...
* 现在，在完全缓存清理后，基于规则的相关产品会按预期在GraphQL响应中加载。 以前，没有为请求的实体加载负责基于规则的匹配的product EAV属性，但在重新加载storefront产品页面后加载了这些属性。
...
* `product` 现在，查询仅返回分配给指定共享目录的产品聚合。 以前，未分配给共享目录的产品聚合数据会显示在分层导航中。
...
* 产品映像缺失时，产品导入过程不再中断。 以前，在使用CSV文件导入第三方服务期间缺少产品图像时，Adobe Commerce会显示以下错误： `Maximum error count has been reached or a system error is occurred!`.
...
* 当格式错误的HTTP请求被发送到服务器时，Adobe Commerce在显示的错误消息中不再包含指向服务器根的完整路径。 现在，只有字符串才会传递到 `explode` 方法作为第二个参数。
...
* 由数据源模型提供并使用设置数据补丁程序（通常来自第三方模块）安装的属性选项现在可用于客户区段规则。 以前，此组合不会返回与配置的区段匹配的客户列表。
...
* 购物者无法再在订单页面上保存不正确的订单状态。 发运项目后，订单状态不再从“完成”更改为“处理”。 以前，当购物者输入评论后，Adobe Commerce在保存订单状态之前不会检查当前订单状态。 [GitHub-36562](https://github.com/magento/magento2/issues/36562)

<!-- ACP2E-1387 -->

* 当正在返回的可配置产品被删除时，Adobe Commerce不再在storefront订单返回页面上引发错误。 以前，Adobe Commerce记录此错误： `report.CRITICAL: Error: Call to a member function getShipmentType() on null in magento2ee/app/code/Magento/Rma/view/frontend/templates/return/create.phtml:52`.
...
* 在启用了免运费且运费含税且计税是在折扣后应用的情况下，贷项通知单页面不再崩溃。 此前，Adobe Commerce曾抛出 `Division by zero` 例外。 [GitHub-36800](https://github.com/magento/magento2/issues/36800)
...
* 现在，当发票、发运和贷项通知单包含的ID与订单ID不同时，可以将它们连同其关联的订单一起成功地从存档中移回。 以前，如果将订单移至存档并还原，则如果ID号不匹配，则该订单中会缺少发票、发运、贷项通知单记录。 [GitHub-36847](https://github.com/magento/magento2/issues/36847)
...
* 现在，将文本从一个页面生成器文本编辑器复制并粘贴到另一个页面生成器文本编辑器可按预期工作。 以前，您尝试粘贴文本的页面无法保存，Adobe Commerce显示此控制台错误： `[ERROR] Page Builder was rendering for 5 seconds without releasing locks.`.
...
* 当购物者从迷你购物车结帐使用Adobe Commerce Pay时在订单审核页面上应用优惠券时，Google不再引发错误。 以前，购物者被重定向到404页。

<!-- BUNDLE-3245 -->

* 购物者现在可以使用Braintree支付和Adobe Commerce 2.4.6完成结账，并使用不可见的ReCAPTCHA v2或v3。 [GitHub-37241](https://github.com/magento/magento2/issues/37241)
...
* 在以下情况下，为Braintree支付的订单创建第二个部分发票时，Adobe Commerce不再引发错误： **[!UICONTROL Enable Vault for Card Payments]** 已启用。 以前，当商家尝试创建第二张发票时，Adobe Commerce引发此错误： `Your payment could not be taken. Please try again or use a different payment method. Payment method token is invalid`.
...
* 从Commerce管理员中禁用GraphQL后，New Relic交易名称现在可以正常显示。 [GitHub-36112](https://github.com/magento/magento2/issues/36112)

### 奖励

<!-- ACP2E-1473 -->

* 奖励积分余额更新电子邮件通知现在会考虑当前商店上下文。 以前，奖励积分会更新使用默认存储的配置设置的电子邮件。

<!-- ACP2E-1329 -->

* 现在，在创建贷项通知单后，奖励积分可以退款。 以前，商家在创建贷项通知单后无法退款奖励积分，并且订单状态保持不变 `Completed`，不是 `Closed`.

### RMA

<!-- ACP2E-1230 -->

* 此 **[!UICONTROL Enable RMA]** 现在，单个产品级别的属性遵循商店RMA设置配置。 以前，无论应用商店配置设置是什么，在添加产品时，此设置都会默认关闭。
...
* 启用Google Analytics内容实验后，当您添加或编辑类别的计划更新时，不再发生JavaScript错误。
...
* 现在，当应用计划更新时，可下载链接会按预期保留，而之前添加的产品会包含在购物车中。 以前，可下载链接从客户的“我的帐户”（“我的可下载产品”）页面和“管理员订单”视图页面中消失。

<!-- ACP2E-1458 -->

* 在应用支持产品的暂存更新后，产品现在可以按预期显示在店面上。 以前，当目录清单的索引器模式设置为“按计划更新”时，产品在管理员中启用，但在店面类别中不可见。 完全重新索引修复了此问题。

<!-- ACP2E-1647 -->

* 此 `staging.synchronize_entity_period` 中的使用者队列 `magento_bulk` 当没有要处理的暂存更新时，表不再增加。 以前，何时 `cron` 配置为每分钟运行一次，即有一条与 `staging.synchronize_entity_period` 每分钟都会将使用者添加到此表中。
...
* Target规则现在可以正确用于不同的存储视图。 以前，由于相关产品规则缓存存在问题，导致在多存储部署中无法显示推荐的产品。
...
* 现在，包含单引号的欢迎消息在店面中正确呈现，且没有控制台错误。 以前，如果欢迎消息包含单引号，则不会在店面渲染期间加载欢迎消息。 Adobe Commerce还显示控制台错误。

<!-- ACP2E-1369 -->

* 当您尝试在“管理员顺序”页面中的评论历史记录和返回之间快速更改视图时，Adobe Commerce不再引发错误。 以前，Adobe Commerce显示此一般错误： `A technical problem with the server created an error`.
...
* 现在，与提交订单详细信息页面上的评论相关的错误消息可提供更多信息。 以前，当购物者单击 **[!UICONTROL Submit comment]** 在不更改订单状态或在注释区域输入文本的情况下，Adobe Commerce会显示以下消息： `The comment is missing. Enter and try again`.

<!-- ACP2E-1659 -->

* 日历图标现在按预期显示在管理员的日期范围字段中 **[!UICONTROL Marketing]** > **[!UICONTROL Cart Price Rules]** 在为一个字段选择日期后。 以前，为一个字段选择日期后，日历图标不再显示在其他字段中，并且Adobe Commerce会显示占位符文本。 [GitHub-36499](https://github.com/magento/magento2/issues/36499)
...
* 新建的带有一个子节点的CMS页面现在包含在营销过滤器网格中(**[!UICONTROL Marketing]** > **[!UICONTROL URL rewrites]**)。 以前，不会为这些CMS页面创建URL重写。
...
* 您现在可以从管理员处打开状态为的订单 `Received` 时间 `AsyncOrder` 未启用。 以前，当订单状态被更改时，您无法从“管理员”中打开订单详细信息页面 `POST V1/orders`.
...
以下已知问题将在Adobe Commerce 2.4.7-beta2中修复：
...
**问题**：从产品中删除分配给某个产品的共享目录后，没有按预期从迷你购物车中删除该产品。 <!-- AC-8935 -->
