...
此版本包含与Adobe Commerce 2.4.6-p5、2.4.5-p7和2.4.4-p8中包含的相同的安全修复和平台安全改进。 请参阅 [Adobe安全公告](https://helpx.adobe.com/security/products/magento/apsb24-18.html) 以获取有关这些已修复问题的最新讨论。
...
* **RabbitMQ 3.13支持**. 此版本与最新版本的RabbitMQ 3.13兼容。兼容性仍与RabbitMQ 3.11和3.12兼容，这两个版本分别支持到2024年8月和2024年12月，但Adobe建议仅将Adobe Commerce 2.4.7与RabbitMQ 3.13结合使用。 <!-- AC-9659 -->

* **编辑器2.7.x**. 与Composer 2.2.x的兼容性得以保留。 <!-- AC-9663 -->

* **支持Varnish缓存7.4**. 此版本与最新版本的Varnish Cache 7.4兼容。兼容性保持与6.0.x和7.2.x版本，但我们建议仅将Adobe Commerce 2.4.7与Varnish Cache版本7.4或版本6.0 LTS一起使用。 <!-- AC-9662 -->
...
Adobe Commerce核心代码中的所有JavaScript库和NPM依赖项均已更新至最新可用版本。 所有Laminas库依赖项都已更新到与PHP 8.3兼容的最新版本。 <!-- AC-9667 9665 -->

#### 其他升级

* **每张订单支持多张优惠券**. 商家现在可以 [配置最大优惠券数量](https://experienceleague.adobe.com/en/docs/commerce-admin/config/sales/sales#multicoupon-settings) 可以按订单应用的新增功能 **[!UICONTROL Maximum number of coupons per order]** 配置选项。 默认情况下，此值设置为1。 您现在可以使用 [REST](https://developer.adobe.com/commerce/webapi/rest/modules/multicoupon/) 或 [GraphQL](https://developer.adobe.com/commerce/webapi/graphql/schema/cart/mutations/apply-coupons/) 将多个优惠券应用到购物车。

* Commerce UPS XML API网关已迁移到新的 [Commerce UPS REST API](https://experienceleague.adobe.com/en/docs/commerce-admin/stores-sales/delivery/shipping-carriers/ups) 支持UPS对其进行的更新 [API安全模型](https://developer.ups.com/oauth-developer-guide). (UPS正在为所有API实施OAuth 2.0安全模型（持有者令牌）。) 所有以前的Commerce UPS XML API已从Adobe Commerce 2.4.7代码库中移除。 <!-- AC-8918 -->

* Adobe Commerce [集成](https://experienceleague.adobe.com/en/docs/commerce-admin/stores-sales/delivery/shipping-carriers/fedex) with FedEx已从旧版FedEx WSDL Web Services迁移到最新的FedEx RESTful API。 FedEx Web服务跟踪、地址验证和验证邮政编码WSDLS将于2024年5月停用。 <!-- AC-1470 -->
...
* 企业商家现在最多可以配置 **100万条基于优惠券的活动购物车价格规则** 在Adobe Commerce中，购物车和结账操作的性能没有显着降低。 <!-- ACPT-1198 -->

* **增强索引器管理**. 新 `indexer:set-status` 命令支持索引器状态的动态管理。 管理员用户可以使用此命令将索引器状态更改为 `suspended`， `invalid`，或 `valid`. 此功能对于在大量批量操作（如产品导入或更新）期间管理系统性能特别有用，因为它允许控制索引器何时由系统的cron作业自动触发。 请参阅 [管理索引器](https://experienceleague.adobe.com/docs/commerce-operations/configuration-guide/cli/manage-indexers.html).

* **具有许多选项的复杂产品的产品列表页面**. 对于包含具有超过100个选项的复杂产品的产品列表页面，加载时间已缩短。 GraphQL按类别列出产品的请求性能也得到了改进。 <!-- ACPT-1683 -->

* **REST现在支持JSON格式 [导入API](https://developer.adobe.com/commerce/webapi/rest/modules/import/)**. 商家现在可以每分钟将100,000条记录以JSON格式导入Adobe Commerce。 <!-- ACPT-943 -->

* **销售规则性能改进**. 通过许多（大约100,000个）有效的销售规则提高了企业部署的性能。 大量实施促销的企业部署通常会部署许多活动的购物车规则。 在结账操作期间，这些类型的企业部署在运行Commerce 2.4.7时，不会看到与配置的购物车价格规则数量有关的任何性能下降。 <!-- ACPT-1198 -->

* **针对具有许多商店的部署，加快存储级配置的保存操作**. 在具有500个以上存储的部署中保存配置设置可能非常耗时。 新 [异步配置](../../../performance/configuration.md#asynchronous-configuration-save) 模块通过运行使用使用者处理消息队列中的保存操作的cron作业来启用异步配置保存操作。 默认情况下，AsyncConfig处于禁用状态。
...
[GraphQL应用程序服务器](../../../performance/application-server.md) 使Adobe Commerce能够维护Commerce GraphQL API请求中的状态，并且无需引导，从而提升所有GraphQL API的吞吐量、降低延迟并高效使用资源。 通过在进程之间共享应用程序状态，GraphQL API请求可以显着提升效率，GraphQL API响应速度平均提高30%。<!-- ACPT-27 -->
...
>[!NOTE]
>
>GraphQL应用程序服务器当前与不兼容 [[!DNL Amazon Simple Storage Service (AWS S3)]](https://aws.amazon.com/s3/). 当前使用的云基础架构上的Adobe Commerce [!DNL AWS S3] 对象 [远程存储](../../../configuration/remote-storage/cloud-support.md) 在Adobe于2024年晚些时候发布修补程序之前，无法使用GraphQL Application Server。<!-- ACPT-1840 -->

### Adobe Commerce扩展中继包

此版本引入了Adobe Commerce扩展metapackage v2.0.0，它自动将所选Adobe Commerce扩展与此核心版本捆绑在一起。 在以下情况下安装此中继包中包含的这些扩展的版本： `composer update` 运行，从而简化在升级到最新核心版本时升级这些扩展的过程。 这些扩展维护独立的发放计划。
...
利用Commerce Webhook (v1.2.0)，开发人员可以配置同步逻辑，以在Adobe Commerce事件触发时执行对外部系统的调用。 当Commerce必须立即使用第三方端点计算或验证订单总计、税额或付款等值，并将结果写回Adobe Commerce时，需要同步调用。

Commerce Webhook现在默认安装。 请参阅 [Adobe Commerce Webhooks概述](https://developer.adobe.com/commerce/extensibility/webhooks/)

### Adobe Stock

Adobe Stock包(`adobe-stock-integration`)现在打包在Commerce扩展元包中(`extensions-metapackage`)，以支持更频繁的更新。 <!-- CABPI-630 -->

### B2B

>[!NOTE]
>
>2.4.7版本与B2B 1.4.2兼容。但是，Adobe建议客户在升级之前等待即将发布的B2B 1.5.0 GA版本，该版本包含对PHP 8.3的支持。请参阅 [B2B发行说明](https://experienceleague.adobe.com/docs/commerce-admin/b2b/release-notes.html).

### Braintree

* **[!DNL Vaulted PayPal]和 [!DNL Pay Later] 更改** — 以前保存/存储了其PayPal帐户的登录客户可以选择按以下方式付款：
   * [!DNL Pay Now] (无需登录 [!DNL PayPal] 帐户，用户可以使用其默认卡进行支付)
   * 使用其他融资来源付款
   * 使用其他帐户付款
   * [!DNL PayPal Pay Later] 或 [!DNL PayPal Credit] 按钮

* **3DS支持[!DNL Google Pay]** — 包括的3DS验证支持 [!DNL Google Pay] 非令牌卡。 请参阅 [Braintree文档](https://developer.paypal.com/braintree/docs/guides/3d-secure/client-side/android/v3#using-3d-secure-with-google-pay) 以了解更多信息。

* **[!DNL Vault Apple Pay Payments]** — 允许已登录的客户保管库/存储其 [!DNL Apple Pay] 支付给其Commerce商店帐户以用于将来交易的款项。 这减少了结账的步骤数，并为回头的客户创造了更快的结账体验。

* **[!DNL Vault Google Pay Payments]** — 允许已登录的客户保管库/存储其 [!DNL Google Pay] 支付给其Commerce商店帐户以用于将来交易的款项。 这减少了结账的步骤数，并为回头的客户创造了更快的结账体验。

* **[!DNL Vault Venmo Payments]** — 允许已登录的客户保管库/存储其 [!DNL Venmo] 帐户到其Commerce商店帐户以用于未来交易。 这减少了结账的步骤数，并为回头的客户创造了更快的结账体验。

* **[!DNL Vault ACH Payments]** — 允许登录的客户将其ACH付款保存到/存储到其Commerce商店帐户，以便用于未来的交易。 这减少了结账的步骤数，并为回头的客户创造了更快的结账体验。

* **[!UICONTROL Express Payment]结帐顶部的按钮** — 为了提供更快的结账体验，我们引入了 [!DNL Express Payment] 选项中显示的复选框。 客户现在可以按以下方式付款： [!DNL PayPal]， [!DNL PayPal Pay Later]， [!DNL Apple Pay]、和 [!DNL Google Pay Express] 付款。

* **[!DNL Braintree]管理员配置中的发行说明和支持链接** — 商家现在可以直接从Commerce Admin链接到在线Braintree支持和发行说明。

* **GraphQL对所有人的支持 [!DNL Braintree] 支付方式除外[!DNL Venmo]**—GraphQL API中显示了更多配置。 这对于Headless应用程序特别有用。

* **[!DNL Vaulting]帐户区域中的付款** — 登录客户现在可以 [保险库/商店](https://experienceleague.adobe.com/en/docs/commerce-admin/stores-sales/payments/braintree#step-3-complete-the-advanced-settings) 新信用卡/借记卡及 [!DNL PayPal] 客户帐户区域中的帐户。 以前，客户在结账时保存他们的付款以供以后使用时只能进行保险库/存储，现在他们可以在无需创建新交易的情况下保险库新的信用卡/借记卡和PayPal帐户。

* **无摩擦交易** — 通过减少客户完成在线信用/借记卡交易的点击/步骤数，顺畅的交易可加快支付流程。 以前（启用3DS时），每个客户都会受到3DS的挑战。 有了新的无摩擦交易功能，客户只有在银行要求时才能获得3DS。 这会减少购物车放弃率，提高转化率，并带来更多销售。

* **争议Webhook** — 当客户对Braintree中的交易产生争议时，争议状态现在将传递给Commerce。 可在 **[!UICONTROL Sales]** > **[!UICONTROL Order]** 网格并附加到每个订单。

### Commerce与Adobe IMS集成

Adobe IMS集成包(`adobe-commerce/adobe-ims-metapackage`)现在打包在Commerce扩展元包中(`extensions-metapackage`)，以支持更频繁的更新。 <!-- CABPI-630 -->

### GraphQL

Commerce 2.4.7包括增强的GraphQL缓存功能、支持GraphQL架构的自定义属性、支持Headless订单取消以及改进的解析器缓存。
...
* [`cancelOrder`](https://developer.adobe.com/commerce/webapi/graphql/schema/orders/mutations/cancel-order/) 变异<!-- LYNX-172 -->
...
* [`applyCouponsToCart`](https://developer.adobe.com/commerce/webapi/graphql/schema/cart/mutations/apply-coupons/) 变异
* [`clearCart`](https://developer.adobe.com/commerce/webapi/graphql/schema/cart/mutations/clear-cart/) 变异
* [`confirmEmail`](https://developer.adobe.com/commerce/webapi/graphql/schema/customer/mutations/confirm-email/) 变异
* [`createGuestCart`](https://developer.adobe.com/commerce/webapi/graphql/schema/cart/mutations/create-guest-cart/) 变异
* [`estimateShippingMethods`](https://developer.adobe.com/commerce/webapi/graphql/schema/cart/mutations/estimate-shipping-methods/) 变异
* [`estimateTotals`](https://developer.adobe.com/commerce/webapi/graphql/schema/cart/mutations/estimate-totals/) 变异
* [`removeCouponsFromCart`](https://developer.adobe.com/commerce/webapi/graphql/schema/cart/mutations/remove-coupons/) 变异
...
* 为除Venmo外的所有支付方法的核心操作添加了GraphQL支持。 有关支付服务的新GraphQL端点的介绍，请参见 [结帐](https://developer.adobe.com/commerce/webapi/graphql/schema/checkout/). <!-- BUNDLE-3326 -->

* 除Venmo之外，已改进所有支付方法的信用卡保险存储。 购物者现在可以保存或保存其ACH支付，并从其帐户页面查看或删除它们。 商家可以从管理员启用和禁用ACH保险存储付款。 <!-- BUNDLE-3324 -->

* 结账工作流现在包括快速付款部分，它提供PayPal、Google Pay和Apple Pay **[!UICONTROL Express]** 按钮。 <!-- BUNDLE-3322 -->

[付款选项](https://experienceleague.adobe.com/docs/commerce-merchant-services/payment-services/payments-checkout/payments-options.html) 介绍支持的支付方式。

### PWA Studio

PWA Studiov14.0与Adobe Commerce 2.4.7兼容。它包括多项增强功能，可改善辅助功能。 有关错误修复的信息，请参见 [PWA Studio版本](https://github.com/magento/pwa-studio/releases). 请参阅 [版本兼容性](https://developer.adobe.com/commerce/pwa-studio/integrations/adobe-commerce/version-compatibility/) 有关PWA Studio版本及其兼容的Adobe Commerce核心版本的列表。
...
>[!NOTE]
>
>几个REST端点路由首次包含字符串 `V2` 而不是 `V1`. 例如， `GET /rest/default/V2/carts/{cartId}/coupons`. 对于Commerce商家，这些端点旨在取代 `V1` 个版本的端点。 这些端点仅在Adobe Commerce中可用。<!-- LYNX-208 -->
...
我们已在Adobe Commerce 2.4.7核心代码中修复了数百个问题。 此版本中包含的已修复问题的子集如下所述。 以前的Commerce 2.4.7-beta版本中包含的修复也进行了描述。

### 安装、升级、部署

<!-- AC-10396 -->

* 从设置过程中删除了不必要的缓存操作。 以前，Commerce在以下情况下将其配置写入磁盘是不必要的： `bin/magento setup:db-data:upgrade` 或 `bin/magento setup:upgrade` 运行，这会在安装期间导致某些模块出现问题。 [GitHub-38124](https://github.com/magento/magento2/issues/38124)
...
* 由于分隔符错误，数据库还原不再失败。 以前，Commerce在以下情况下引发此错误： `bin/magento setup:rollback --db` 已执行： `Syntax error or access violation: 1064 You have an error in your SQL syntax; check the manual that corresponds to your MariaDB server version for the right syntax to use near 'delimiter' at line 1, query was: delimiter ;;`.
...
* **[!UICONTROL Select]** 下拉菜单选项现在在“管理员”中可见 **[!UICONTROL Content]** > **[!UICONTROL Pages]** 在网格中选择多个页面时。 [GitHub-35386](https://github.com/magento/magento2/issues/35386)

<!-- AC-9209 -->

* 将路径更正为默认值 `system/dashboard/enable_charts` 在配置设置中。 现在，图表按预期显示在管理员中。 [GitHub-37668](https://github.com/magento/magento2/issues/37668)
...
* “管理网格操作”下拉菜单现在可以按预期悬停在选定链接上 **[!UICONTROL Content]** > **[!UICONTROL Pages]** （和B2B模块中的共享目录）。 以前，此菜单大部分是隐藏的，管理员用户必须滚动才能查看选项。 [GitHub-36710](https://github.com/magento/magento2/issues/36710)

### 分析

<!-- ACP2E-2266 -->

* 产品详细信息页面 `meta itemprop="image"` 和 `meta itemprop="description"` 属性现在可用于主产品图像。 [GitHub-37497](https://github.com/magento/magento2/issues/37497)

<!-- ACP2E-2314 -->

* 在以下情况下，购物车网格中的产品的“报表”部分现在显示正确的产品价格： **[!UICONTROL Catalog Price Scope]** 设置为 **[!UICONTROL Website]**. 以前，将产品分配给默认网站以外的网站时，产品价格显示为零。
...
* `price` 和 `quantity` Google Tag Manager数据层中的值现在会按预期发送给Google Analytics。 以前，这些值以字符串形式发送。
...
* 在以下情况下，类别产品计数现在可正确计入捆绑产品： **[!UICONTROL Display Out Of Stock*]** 已启用。
...
* 现在，将缓存设置为Fastly CDN (**[!UICONTROL Caching Application=Fastly CDN]**)。 这同时影响了客人购物者和注册客户。
...
* 现在，Commerce在启用永久购物车的情况下生成整页可缓存页面(例如，产品详细信息页面(PDP)和CMS页面)时，通过不加载特定于用户的报价数据来优化性能。 以前，系统会在生成这些页面期间不必要地加载特定于用户的会话和引用数据，从而影响性能。
...
* 现在，当触发添加到购物车操作时，Commerce会显示信息性消息。 以前， `The requested qty is not available` 并不总是按需要显示在店面。

<!-- ACP2E-2166 -->

* 对于拥有大地址簿的客户，结帐页面加载时间已缩短。 Commerce现在仅处理 **[!UICONTROL Customer Addresses Limit]** 设置。 以前，Commerce加载整个通讯簿。
...
* 现在，您可以使用以下成功保存新的购物车价格规则 `Magento_OfflineShipping` 扩展已禁用。 以前，Adobe Commerce引发此错误： `report.ERROR: Warning: Undefined array key "simple_free_shipping"`. [GitHub-37580](https://github.com/magento/magento2/issues/37580)
...
* 正在运行 `bin/magento cache:clean` 或 `bin/magento cache:flush` 除非另外指定，否则不再刷新整个内置或Varnish全页缓存。 [GitHub-38301](https://github.com/magento/magento2/issues/38301)

<!-- ACP2E-2257 -->

* 商家现在可以使用产品轮播创建CMS块，当 `catalog_product_price dimensions-mode` 设置为 `website`. 以前，Commerce不会保存块并引发此SLQ错误： `report.CRITICAL: PDOException: SQLSTATE[42S02]: Base table or view not found: 1146 Table`.
...
* Commerce不再将产品描述文本插入到产品详细信息页面 **[!UICONTROL Meta Description]** 字段为空。 以前，当产品描述包含由HTML生成器或其他HTML编辑器生成的页面代码时，该HTML会显示在元中 **[!UICONTROL Description]** 字段，并且不能将此字段留空。
...
* 其余的 `V1/products/<sku>/media` 端点现在可以处理多个媒体图像的同时上传。 以前，在同时处理多个请求时，会创建不一致的数据，并在将此数据保存到数据库时Commerce引发错误。
...
* 现在，多选属性会在以下情况下按预期保存： **[!UICONTROL Use Default Value]** 已选中store-view范围内的复选框。 以前，在编辑产品并保存默认值时取消选中此复选框。

<!-- ACP2E-1878 -->

* 具有网站限制访问权限的管理员用户现在可以将图像添加到其范围内的产品。 如果管理员用户无产品访问权限，Commerce会显示一条信息性消息。 以前，具有网站限制访问权限的管理员用户无法将图像添加到他们有权访问的产品。

<!-- ACP2E-1956 -->

* 用户定义 `special_from_date`， `special_to_date`， `news_from_date`、和 `news_to_date` 在存储视图范围内编辑产品时不会覆盖值。
...
* 商家现在可以更新可下载产品计划更改的将来开始日期或时间。 以前，Adobe Commerce在更新可下载产品的未来开始日期或时间时引发此错误： `The downloadable link isn't related to the product. Verify the link and try again`.
...
* 商家现在可以选择 **[!UICONTROL Skip quantity at this time]** 和 **[!UICONTROL Skip image uploading at this time]** 配置可配置产品而不影响关联的简单产品的来源时。 以前，选择  **[!UICONTROL Skip quantity at this time]** 导致产品来源消失。
...
* 中的待处理记录数 `cron_schedule` 通过阻止cron作业尝试获取不必要的cron作业锁，减少了表的数量。 以前，cron作业会在运行该作业时太晚时尝试获取锁。 Cron作业位于 `cron_schedule` 未标记为已错过，即使他们已错过计划时间。 Commerce还在错误日志中重复显示此错误： `report.WARNING: Could not acquire lock for cron job: indexer_update_all_views`.
...
* 包含变音符号或重音符号的电子邮件（即utf8电子邮件）现在受客户和公司支持，也受来宾和注册客户下订单的支持。 以前，Commerce不会保存包含变音符号并显示以下错误的客户记录： `Something went wrong while saving the customer`.
...
* 通过添加 `UNCACHEABLE` 使用Varnish输出HTTP标头时的值正在使用中。 以前，仅限 `HIT` 或 `MISS` 输出个值。 [GitHub-37912](https://github.com/magento/magento2/issues/37912)
...
* 此 `bin/magento varnish:vcl:generate` 命令现在包括 `input-file argument`. 这支持向Git存储库中添加与商务根相关的自定义VCL。 [GitHub-37363](https://github.com/magento/magento2/issues/37363)
...
* `phpcodesniffer-composer-installer` 在 `composer.json` 已升级到v1.0.0。 [GitHub-36913](https://github.com/magento/magento2/issues/36913)

<!-- AC-10858 -->

* 不符合的字符 [RFC3986](https://www.rfc-editor.org/rfc/rfc3986.html) 已从代码库中移除。 [GitHub-38355](https://github.com/magento/magento2/issues/38355)

<!-- AC-10692 -->

* 修复了中的PHP文档标记不正确 `Magento\Reports\Block\Adminhtml\Shopcart\Product\Grid` 类。 [GitHub-38186](https://github.com/magento/magento2/issues/38186)

<!-- AC-10865 -->

* 在整个代码库中重构了代码，该代码库是在编码实践要求包含在中的任何缓存区段时 `app/etc/env.php` 文件将包含前端部分。 此重构解决了升级Commerce 2.4.4之前版本部署时的潜在问题。 [GitHub-38363](https://github.com/magento/magento2/issues/38363)
...
* 此 `var/log/system.log` 文件不再包含 `main.INFO error` 消息。 菜单条目日志已从INFO级别移至DEBUG级别。 以前， `var/log/system.log` 文件被以下消息充满： `Add of item with id Magento_Theme::design_config was processed … `.
...
* 严重数据库读取失败时间 `synchronous_replication` 启用后，Adobe Commerce Cloud上的从属连接不再发生。
...
* 中的JSON序列化程序 `User` 模块已替换为更准确的新序列化程序(`JsonHexTag`)。 以前，在尝试使用由常规序列化器序列化的数据（包含未转义的元素）时，Commerce有时会崩溃。 [GitHub-31377](https://github.com/magento/magento2/issues/31377)
...
* 管理员用户现在可以在其Commerce实例连接到S3存储时生成发票。 以前，Commerce显示此错误： `Cannot create image resource. File not found`. [GitHub-35706](https://github.com/magento/magento2/issues/35706)
...
* 通过扩展Adobe Commerce中的TinyMCE编辑器以及2.4.6之前的Magento Open Source版本而创建的自定义样式格式现在可在Page Builder中按预期使用 `text` 内容类型。 以前，自定义样式格式会被忽略，并且只有在升级到Adobe Commerce 2.4.6-p1后才会加载默认格式。 [GitHub-36950](https://github.com/magento/magento2/issues/36950)
...
* 锁定提供程序的实例化现在完成 `\Magento\Framework\Lock\Proxy` 类。 现在，锁定提供程序配置有 `lock` 设置 `app/etc/env.php` 文件。
...
* 相关产品规则的 **为其中之一** 现在，当使用配置时，条件可正确运行 **要匹配的产品** 设置。 以前，此规则不标识任何产品。
...
* 客户实体(`rp_token`)验证功能在创建客户帐户期间已从属性验证中删除，并且变音符号验证仅包含在生成的客户电子邮件中。 以前，Commerce不会保存客户记录并显示此错误： `Something went wrong while saving the customer`.
...
* 管理员客户地址 **省/市/自治区** 字段现在会按预期保留上次保存的值。 以前，当您从此字段中移除值时，Commerce会继续在 **省/市/自治区** 字段。 [GitHub-36846](https://github.com/magento/magento2/issues/36846)
...
* 此 `setGiftOptionsOnCart` 突变现在会按预期从订单中删除礼品包装。 的值 `gift_wrapping_id` 现在取消在购物车级别设置。 以前， `gift_wrapping_id` 即使在购物车级别取消设置此值，订单下达后也会更新到其之前的值。
...
* 此 `setGiftOptionsOnCart` 现在，当从购物车中删除礼品消息时，突变会成功更新数据库。 以前，数据库中不会更新消息详细信息，并且不会从购物车中删除礼品消息。
...
* 此 `addProductsToCart` 变异现在成功地将礼品卡添加到购物车。 以前，此查询需要礼品卡收件人姓名，但会返回发件人姓名。 Adobe Commerce引发此错误： `Please specify a recipient name`. [GitHub-36665](https://github.com/magento/magento2/issues/36665)
...
* 此 `products` 现在，当存在多个商店视图时，查询会返回正确的标签。 [GitHub-29635](https://github.com/magento/magento2/issues/29635)
...
* 现在，验证检查在没有可导入的有效数据时按预期停止导入过程，Commerce现在显示此错误： `There are no valid rows to import`. 以前，验证在这些条件下通过，但导入过程失败，并显示以下消息： `entity values are mixed`. [GitHub-32905](https://github.com/magento/magento2/issues/32905)
...
* Commerce现在会考虑 **[!UICONTROL Disable Automatic Group Change Based on VAT ID]** 在导入客户记录期间设置。 以前， `disable_auto_group_change` 已忽略导入文件中的值。 [GitHub-36409](https://github.com/magento/magento2/issues/36409)
...
* 库存源数据现在由商店导入流程成功导入，Commerce现在会在导入流程完成时显示完整的通知消息。 以前，缺少通知消息。
...
* 改进了 `catalogrule_product` 未设置目录规则时的索引器。 [GitHub-34784](https://github.com/magento/magento2/issues/34784)
...
* 添加了一个缺失项 `return` 语句，用于启用管理员使用。 以前， `return` 语句和中的控制器 `FrontendController` 类处理不正确。 [GitHub-31374](https://github.com/magento/magento2/issues/31374)
...
* 修复了调用成员函数时可能发生的错误 `getId()` 日期 `int`，当在管理员中查看客户时，偶尔会触发异常。
...
* 商家现在可以通知购物者，订单已准备好提货。 **[!UICONTROL No Manage Stock]** 已为产品启用设置。 以前，启用此设置时，Commerce显示此错误消息： `Your order is not ready for pickup`.

<!-- ACP2E-2227 -->

* 商家现在可以为订单创建贷项通知单，其中删除了与可配置产品关联的简单产品。 以前，商家无法创建贷项通知单，Commerce掷回 `TypeError` 例外。
...
* 商家现在可以将同名不同大小写（混合使用大写和小写字符）的图像上传到媒体集。 以前，图像文件名在页面生成器中不区分大小写，并且名称与大小写不同的文件会覆盖在区分大小写的文件系统上托管的Commerce媒体存储中。

### 新闻稿

<!-- AC-10806 -->

* Commerce现在检查是否启用了新闻稿订阅功能，然后再接受新闻稿的新订阅者。 [GitHub-33040](https://github.com/magento/magento2/issues/33040)
...
* Commerce现在将正确的折扣金额应用于部分取消的订单。 以前，在以下位置计算的折扣金额： `base_discount_cancelled` 更改订单时，列计算不正确。 在折扣订单的发票中添加了对负值的支持。

<!-- ACP2E-2585 -->

* 其余的 `V1/orders` 现在，端点在导入订单时可按预期应用礼品卡。 以前，不会保存对此端点中礼品卡扩展属性的更改。
...
* 现在，当商家从此页面启动搜索时，“管理销售订单网格”页面会按预期加载。 以前，当商家尝试从此表中搜索订单时，页面冻结，Commerce显示 `Request-URI Too Long` 浏览器控制台中的错误。
...
* 在管理员用户无权访问客户网站的情况下，使用客户余额的离线退款创建贷项通知单时，Adobe Commerce不再引发错误。 以前，Adobe Commerce引发此错误： `More permissions are needed to view this item`，并且管理员用户无法创建商店退款。
...
* 现在，您可以在管理员>中使用页面生成器选项 **[!UICONTROL Content]** > **[!UICONTROL Pages]** 若要在所有样式中插入复制的文本，请使用 `text` 元素。 以前，页面生成器只显示纯文本，不保存样式。
...
* 加载管理员产品编辑页面时，产品不再生成间断 `Uncaught TypeError` JavaScript错误。

<!-- ACP2E-1886 -->

* 现在，通过页面生成器编辑器上传的图像将标记为使用 **[!UICONTROL Content]** > **[!UICONTROL Media Gallery]**. 以前，这些图像标记为 **未在任何位置使用** 在媒体集中。
...
* 现在，Commerce可以通过阻止在发生此错误时创建订单，正确处理来自PayPal的502错误。 以前，尽管PayPal发生了502错误，导致Commerce记录与PayPal记录不一致，但仍会创建订单。
...
* 购物者现在可以在结账期间应用优惠券代码时选择配送方式。 以前，在这些条件下，Commerce会引发错误，并为订单分配统一运费。
...
* 中的多次点击 **销售** > **订购** 页面的成批活动下拉菜单不再生成多个POST请求。  [GitHub-37997](https://github.com/magento/magento2/issues/37997)

<!-- AC-10944 -->

* 仅 `200` 或 `404` 当购物者导航到触发301重定向的页面时，响应将缓存，除非页面的Cache-Control标头包含 `private `. 以前，具有的HTTP状态200 `private` 缓存Cache-Control标头中的，并缓存HTTP状态301，除非它包含 `private` 在其Cache-Control标头中。 [GitHub-36492](https://github.com/magento/magento2/issues/36492)
...
* 性能 `addProductToCart` 改进了使用多个自定义产品属性执行时的突变。 以前，当使用许多产品属性（大约1000个）执行此突变时，Commerce会尝试加载所有产品属性。
...
* 积分过期后，将不再发送奖励积分过期电子邮件。 此 `magento_reward_balance_warning_notification` 和 `magento_reward_expire_points` cron任务已进行了重构，以增加灵活性和电子邮件发送容量。 以前，商家无法在短时间内发送由这些cron任务生成的许多电子邮件。 这可能会导致延迟提交对时间敏感的通知。
...
* 其余的 `POST /V1/returns` 请求现在为每个产品创建一个RMA。 以前，此请求可以绕过验证过程，为每个产品返回多个RMA。

<!-- ACP2E-2450 -->

* RMA属性现在会自动添加到默认EAV表单中，当 **[!UICONTROL Show on Storefront]** RMA属性设置为 **否**. 以前，在创建自定义返回属性时，RMA项目详细信息弹出窗口为空， **[!UICONTROL Show on Storefront]** RMA属性已设置为 **否**.

<!-- ACP2E-2211 -->

* 的值 **[!UICONTROL Enable RMA]** 属性现在与配置中指定的值匹配，当 **[!UICONTROL Use Config]** “产品编辑”页面上的复选框已启用。
...
* 为订单创建部分发运后，Commerce现在会按预期转移到新发运页面。 以前，在创建部分发运后，Commerce不会为订单的其余产品创建完整的发运页面。 [GitHub-38063](https://github.com/magento/magento2/issues/38063)
...
* 当管理员用户使用登录作为客户功能时，Commerce在签出多个送货地址期间不再引发异常。
...
* 当管理员用户创建重新订购时，初始订单中的账单地址和发货地址会在管理员用户单击 **[!UICONTROL Reorder]** 按钮（管理员） **[!UICONTROL Sales]** > **[!UICONTROL Orders]** > **[!UICONTROL View]** )。 如果购物者的通讯簿中保存了相同的地址，则重新排序将始终包含这些地址的更新版本。 以前，送货地址字段预填充了更新的数据，但帐单地址字段则未预填充。
...
* 现在，如果购物者在计划更新过程中将捆绑产品添加到购物车，则会正确更新捆绑产品。 以前，如果在计划任务处理过程中将带选项的捆绑产品添加到购物车，则Commerce会引发此异常： `Some of the products below do not have all the required options. Please select all required options`.
...
* Commerce现在为显示正确的值 **[!UICONTROL Regular Price]** 为具有下列属性的产品选择自定义选项时的值 **[!UICONTROL Special Price]** 值条件 **[!UICONTROL Price Display Settings]** 设置为包括税。
...
* 修复了以下内容的弃用通知 `StorefrontDeleteItemFromRequisitionListPrintView`.
...
* 更正了礼品注册表中的光标行为 **[!UICONTROL Message]** 文本字段。
...
* 改进了模板渲染期间发生的错误消息传递。 Commerce现在会显示一条信息性消息。 以前，发生PHP错误时，Commerce显示HTML泄漏。
...
* 此 `crossorigin` 属性已添加到 `linkTypes` 对象 `HEAD` 元素。[GitHub-32509](https://github.com/magento/magento2/issues/32509)
...
* 商业不再为根类别和设置为的产品生成产品URL重写 **[!UICONTROL Not Visible Individually]** 在多存储设置中，为自定义存储生成缺少的URL重写时。
...
* 当您尝试在以下情况下为产品URL创建301/302重定向时，Commerce不再引发错误 **生成类别/产品URL重写** 已启用。 创建产品并将其分配给类别后，将创建自动URL重写：一个针对直接URL，一个针对通过类别路径的产品。 创建新产品URL重写时，将检查产品、类别和商店之间的关联。 现在，此检查基于关联的实体属性（类别和产品）。 以前，此检查是在预先存在的URL重写时执行的。
...
* Commerce不再显示 `Integration not secure` 管理员中的警告 **[!UICONTROL System]** > **[!UICONTROL Extensions]** > **[!UICONTROL Integrations]** 当 **[!UICONTROL Callback URL]** 和 **[!UICONTROL Identity link URL]** 字段为空。 [GitHub-36407](https://github.com/magento/magento2/issues/36407)
...
* 其余的 `V1/carts/mine/estimate-shipping-methods` 方法现在会验证所有级别的有效负载字段数据，并为无效数据返回400 HTTP代码。 以前，仅验证第一级API有效负载，如果在有效负载的非验证级别中返回无效数据，则Commerce返回500状态代码。

<!-- ACP2E-2053 -->

* 现在，用于更新具有层价格和不同属性集的产品的异步批量REST API操作已成功完成。 以前，无法以这种方式更新具有不同属性集的产品，并且Commerce会显示此错误： `The product was unable to be saved. Please try again`.

<!-- AC-7002 -->

* 此 `POST /V1/products/tier-prices` 现在，如果在层价格创建过程中使用了无效的客户组名称，则端点会返回一条信息更丰富的错误消息。 以前，Adobe Commerce返回如下错误： `Notice: Undefined index: XXXXX in ~/public_html/vendor/magento/module-catalog/Model/Product/Price/Validation/TierPriceValidator.php on line 474"`. [GitHub-36431](https://github.com/magento/magento2/issues/36431)
...
* 当购物者多次尝试将同一项目添加到愿望清单时，Commerce现在显示一条消息，指示该项目已被添加到愿望清单。 以前，在这些条件下，Commerce会增加愿望清单中的产品数量。
...
您可以使用安装Adobe Commerce 2.4.7 [Composer](https://experienceleague.adobe.com/docs/commerce-operations/installation-guide/composer.html).
