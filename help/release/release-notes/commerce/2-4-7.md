...
description: 了解Adobe Commerce 2.4.7版的新增功能、增强功能、错误修复和已知问题。
...
Adobe Commerce 2.4.7引入了对PHP 8.3的支持。它引入了数百个质量修复和增强功能。 核心编辑器依赖项和第三方库已升级到最新可用版本。 此版本包括显着的性能改进，包括更快地加载具有许多选项的复杂产品的产品列表页面，以及增强索引器管理。 此版本增加了GraphQL对自定义属性和GraphQL解析器缓存的覆盖范围。 它还引入了对最近更新的FedEx和UPS服务的支持。

{{bics}}

## 其他发行信息

虽然这些功能的代码与Adobe Commerce核心代码版本捆绑在一起，但是其中一些项目(例如B2B和Progressive Web Application(PWA)Studio)也单独发布。 每个项目的文档中都提供了特定于项目的单独发行信息，其中记录了这些项目的错误修复。
...
* **增加了子资源完整性(SRI)支持**&#x200B;以符合PCI 4.0对验证支付页面上的脚本完整性的要求。 子资源完整性(SRI)支持为驻留在本地文件系统中的所有JavaScript资源提供完整性散列。 默认SRI功能仅在管理员和店面区域的付款页面上实施。 但是，商家可以将默认配置扩展到其他页面。 请参阅&#x200B;_Commerce PHP Developer Guide_<!--AC-1153-->中的[子资源完整性](https://developer.adobe.com/commerce/php/development/security/subresource-integrity/)。

* **内容安全策略(CSP)的更改** — 符合PCI 4.0要求的Adobe Commerce内容安全策略(CSP)的配置更新和增强功能。 有关详细信息，请参阅&#x200B;_Commerce PHP Developer Guide_&#x200B;中的[内容安全性策略](https://developer.adobe.com/commerce/php/development/security/content-security-policies/)。<!--AC-11513-->

   * Commerce管理员和店面区域付款页面的默认CSP配置现在为`restrict`模式。 对于所有其他页面，默认配置为`report-only`模式。  在低于2.4.7的版本中，CSP在所有页面中均配置为`report-only`模式。

   * 添加了一个nonce提供程序，以允许在CSP中执行内联脚本。 nonce提供程序有助于为每个请求生成唯一的nonce字符串。 然后，将字符串附加到CSP标头中。

   * 添加了选项，用于配置自定义URI以报告管理员中“创建订单”页面和店面中“签出”页面的CSP违规。 您可以从管理员添加配置，也可以通过将URI添加到`config.xml`文件来添加配置。
...
* **通过REST和GraphQL API传输的付款信息的本机费率限制**。 商家现在可以[为使用REST和GraphQL传输的付款信息配置速率限制](https://experienceleague.adobe.com/en/docs/commerce-admin/config/sales/sales#rate-limiting)。 这一新增的保护层支持对信用卡分拣攻击的防范，并有可能减少一次测试多个信用卡号的分拣攻击的数量。 这是现有REST端点的默认行为的更改。 请参阅[速率限制](https://developer.adobe.com/commerce/webapi/get-started/rate-limiting/)。
...
* **每个订单支持多张优惠券**。 商家现在可以使用新的&#x200B;**[!UICONTROL Maximum number of coupons per order]**&#x200B;配置选项[配置每个订单可以应用的最大优惠券数](https://experienceleague.adobe.com/en/docs/commerce-admin/config/sales/sales#multicoupon-settings)。 此值默认设置为1。 您现在可以使用[REST](https://developer.adobe.com/commerce/webapi/rest/modules/multicoupon/)或[GraphQL](https://developer.adobe.com/commerce/webapi/graphql/schema/cart/mutations/apply-coupons/)将多张优惠券应用于购物车。

* Commerce UPS XML API网关已迁移到新的[Commerce UPS REST API](https://experienceleague.adobe.com/en/docs/commerce-admin/stores-sales/delivery/shipping-carriers/ups)，以支持UPS正在对其[API安全模型](https://developer.ups.com/oauth-developer-guide)进行的更新。 (UPS正在为所有API实施OAuth 2.0安全模型（持有令牌）。) 所有以前的Commerce UPS XML API已从Adobe Commerce 2.4.7代码库中删除。<!-- AC-8918 -->

* Adobe Commerce [与FedEx的集成](https://experienceleague.adobe.com/en/docs/commerce-admin/stores-sales/delivery/shipping-carriers/fedex)已从旧版FedEx WSDL Web服务迁移到最新的FedEx RESTful API。 FedEx Web服务跟踪、地址验证和验证邮政编码WSDLS将于2024年5月停用。<!-- AC-1470 -->

* 增加了对新的USPS Ground Advantage配送方法的支持。 这是与USPS的新配送方法USPS Ground Advantage（2023年7月发布）的开箱即用集成。 这一新的集成可用于检索发货费率以及通过USPS发货服务安排交货和退货。 USPS Ground Advantage发运方法替代了以下发运方法，这些发运方法在USPS Ground Advantage发运方法发布时已停用：

   * USPS零售场
   * 一流包服务
   * 包裹选择地面<!-- AC-9182 -->

* Temando发货模块已从核心Commerce代码库中删除。 此功能在Adobe Commerce 2.4.4中已弃用。<!-- AC-8130 -->

### 性能和可扩展性增强

Commerce 2.4.7包括以下对Commerce性能和可伸缩性的增强：

* 企业商家现在可以在Adobe Commerce中配置最多&#x200B;**1百万条基于优惠券的有效购物车价格规则**，购物车和结账操作性能没有显着下降。<!-- ACPT-1198 -->

* **增强索引器管理**。 新的`indexer:set-status`命令支持索引器状态的动态管理。 管理员用户可以使用此命令将索引器状态更改为`suspended`、`invalid`或`valid`。 在大量批量操作（如产品导入或更新）期间，此功能通过允许控制索引器何时由系统的cron作业自动触发，对管理系统性能尤为有用。 请参阅[管理索引器](https://experienceleague.adobe.com/docs/commerce-operations/configuration-guide/cli/manage-indexers.html)。

* **具有许多选项的复杂产品的产品列表页面**。 包含超过100个选项的复杂产品的产品列表页面的加载时间已缩短。 GraphQL按类别列出产品的请求的性能也得到了提高。<!-- ACPT-1683 -->
...
* **对于具有许多存储区的部署，存储级配置的保存操作速度更快**。 在具有500个以上存储的部署中保存配置设置可能非常耗时。 新的[异步配置](../../../performance/configuration.md#asynchronous-configuration-save)模块通过运行使用使用者在消息队列中处理保存操作的cron作业来启用异步配置保存操作。 默认情况下，AsyncConfig处于禁用状态。

* **更快生成大型配置的配置缓存**。 现在，`bin/magento cache:clean config`命令在启用配置缓存时预热配置缓存。 这样可以减少为大型配置生成配置缓存所需的停机时间。 在将数据写入缓存之前，配置保存操作不再清理`config_scopes`缓存，这还缩短了写入配置数据时锁定其他请求的时间。

### GraphQL应用程序服务器

[GraphQL Application Server](../../../performance/application-server.md)使Adobe Commerce能够在Commerce GraphQL API请求之间保持状态，并且消除了引导需求，这将为所有GraphQL API带来更高的吞吐量、更短的延迟和更高效的资源使用。 通过在进程之间共享应用程序状态，GraphQL API请求的效率显着提高，GraphQL API响应的速度平均提高30%。<!-- ACPT-27 -->

GraphQL Application Server仅适用于Adobe Commerce。 不适用于Magento Open Source。 您必须[提交Adobe Commerce支持](https://experienceleague.adobe.com/en/docs/commerce-knowledge-base/kb/help-center-guide/magento-help-center-user-guide)票证，才能在Pro项目上启用GraphQL Application Server。

>[!NOTE]
>
>GraphQL Application Server当前与[[!DNL Amazon Simple Storage Service (AWS S3)]](https://aws.amazon.com/s3/)不兼容。 当前将[!DNL AWS S3]用于[远程存储](../../../configuration/remote-storage/cloud-support.md)的Adobe Commerce on cloud infrastructure客户在Adobe于2024年晚些时候发布修补程序之前，无法使用GraphQL Application Server。<!-- ACPT-1840 -->

### Adobe Commerce扩展元包

此版本引入了Adobe Commerce扩展元包v2.0.0，它可自动将选定的Adobe Commerce扩展与此核心版本捆绑在一起。 运行`composer update`时安装此中继包中包含的这些扩展的版本，从而简化了在升级到最新核心版本时升级这些扩展的流程。 这些扩展会维护独立的发放计划。

Adobe Commerce 2.4.7的Adobe Commerce扩展元包包含以下扩展：
...
* **GraphQL支持除[!DNL Venmo]**&#x200B;之外的所有[!DNL Braintree]付款方法 — 更多配置将在GraphQL API中公开。 这对于无头应用尤其有用。

* 帐户区域&#x200B;**中的**[!DNL Vaulting]&#x200B;付款 — 已登录的客户现在可以[保管库/存储](https://experienceleague.adobe.com/en/docs/commerce-admin/stores-sales/payments/braintree#step-3-complete-the-advanced-settings)新信用卡/借记卡以及[!DNL PayPal]个帐户帐户区域。 以前，客户只能保管/存储其付款以备在结账时完成交易时使用，现在他们可以保管新的信用卡/借记卡和PayPal帐户，而无需创建新交易。

* **无摩擦交易** — 通过减少客户完成在线信用/借记卡交易的点击数/步骤，无摩擦交易可加快付款流程。 以前（启用3DS时），每个客户都面临着3DS挑战。 凭借新的无摩擦交易功能，客户只有在银行要求时才能获得3DS。 这减少了购物车的弃置率，提高了转化率，并带来了更多的销量。
...
* **支持取消订单**。 [cancelOrder](https://developer.adobe.com/commerce/webapi/graphql/schema/orders/mutations/cancel-order/)突变允许客户取消订单，传递其标识符和取消原因。<!-- LYNX-197 -->
...
PWA Studio版本14.0与Adobe Commerce 2.4.7兼容。它包括多项增强功能，旨在提高可访问性。 有关错误修复的信息，请参阅[PWA Studio版本](https://github.com/magento/pwa-studio/releases)。 有关PWA Studio版本及其兼容的Adobe Commerce核心版本的列表，请参阅[版本兼容性](https://developer.adobe.com/commerce/pwa-studio/integrations/adobe-commerce/version-compatibility/)。
...
* 不必要的缓存操作已从设置过程中移除。 以前，Commerce在运行`bin/magento setup:db-data:upgrade`或`bin/magento setup:upgrade`时不必要将其配置写入磁盘，这会导致某些模块在安装过程中出现问题。 [GitHub-38124](https://github.com/magento/magento2/issues/38124)

<!-- ACP2E-1675 -->

* 解决了由于内存不足和表过大导致的部署问题。 由于与大型MySQL表相关的内存超出错误，`bin/magento  setup:upgrade`命令不再失败。

<!-- ACP2E-2198 -->

* 删除`app/etc/config.php`后，`bin/magento setup:install`现在成功完成。 以前，在安装过程中不会重新生成缺失的文件，Commerce会引发错误。 [GitHub-37805](https://github.com/magento/magento2/issues/37805)
...
* `bin/magento setup:upgrade`命令不再失败，出现此类型的MySQL内存限制相关错误： `PHP Fatal error:  Allowed memory size of 4294967296 bytes exhausted (tried to allocate 20480 bytes)`。 多选属性迁移已优化为在`setup:upgrade`期间占用更少内存。
...
* 当启用&#x200B;**[!UICONTROL Checkout Async]**&#x200B;时，购物者现在可以在实现拆分数据库的部署中下订单。 以前，Adobe Commerce引发此错误： `An error occurred on the server. Please try to place the order again`。

<!-- https://github.com/magento/magento2/pull/33092 -->

* 当安装同时安装表和关联的`mview`索引器的新模块时，`bin/magento setup:upgrade`现在可以成功完成。
...
* 当产品设置了&#x200B;**[!UICONTROL together]**&#x200B;装运类型时，您现在可以使用`GET V1/shipment/` API路由创建具有捆绑产品的装运。 捆绑产品现在还可根据其装运类型属性进行验证。
...
* Commerce现在可以在启用永久购物车的情况下，在生成整页可缓存页(如“产品详细信息页”(PDP)和CMS页)时不加载特定于用户的报价数据，从而优化性能。 以前，系统会在生成这些页面期间不必要地加载特定于用户的会话和引用数据，从而影响性能。
...
* 如果在移动环境中登录期间重新加载页面，则对`POST V1/guest-carts/<cartId>/shipping-information`的Guest API请求将不再返回`cancel`状态。 以前，Adobe Commerce抛出500错误并记录此异常： `TypeError: Argument 2 passed to Magento\CustomerCustomAttributes\Model\Plugin\ProcessCustomerShippingAddressCustomAttributes::beforeSaveAddressInformation() must be of the type string, null given`。
...
* 当此字段留空时，Commerce不再将产品描述文本插入到产品详细信息页&#x200B;**[!UICONTROL Meta Description]**&#x200B;字段中。 以前，当产品描述包含由页面生成器或其他HTML编辑器生成的HTML代码时，该HTML将显示在元&#x200B;**[!UICONTROL Description]**&#x200B;字段中，并且不能将此字段留空。

<!-- ACP2E-2308 -->

* 可配置产品PDP页面上的可配置产品选项不再受页面上的其他可配置产品选项的影响。 以前，当产品详细信息页面上存在产品轮播时，具有下拉属性的可配置产品不会再触发例外。 当页面上存在其他具有色板选项的可配置产品时，不可选择可配置产品选项。
...
* REST `V1/products/<sku>/media`终结点现在可以处理多个媒体图像的同时上传。 以前，当同时处理多个请求时，会创建不一致的数据，当此数据保存到数据库时，Commerce会引发错误。

<!-- ACP2E-2259 -->

* 现在，当使用新创建的客户帐户的购物者注销时，客户区段横幅的行为与预期一致。 以前，即使在该帐户会话结束后（即已注册的购物者已注销），也显示为已登录客户群创建的横幅。

<!-- ACP2E-2219 -->

* 改进了Category Products和Product Category索引器的部分重新索引性能。 以前，`indexer_update_all_views` cron作业针对单个产品或类别多次运行重新索引器。
...
* 现在，当管理员用户在清除缓存但未重新加载类别页面后将新产品添加到类别时，产品位置和ID选择可正确显示。 以前，产品位置和选择不匹配且显示不正确。

<!-- ACP2E-1779 -->

* 商家现在可以更新可下载产品的计划变更的未来开始日期或时间。 以前，Adobe Commerce在更新可下载产品的未来开始日期或时间期间引发此错误： `The downloadable link isn't related to the product. Verify the link and try again`。

<!-- ACP2E-1943 -->

* 现在，店面可以正确显示产品延交状态。 以前，可供发运的产品被错误地标识为延交订单。

<!-- ACP2E-1948 -->

* 当管理员用户将商店视图从产品创建期间使用的视图更改为其他商店视图时，从管理员访问可下载的产品不会再导致错误。 现在，当管理员用户将商店视图从所有商店视图切换到默认商店视图时，产品价格已成功转换为浮点数。 以前，Adobe Commerce引发错误。 [GitHub-37519](https://github.com/magento/magento2/issues/37519)

<!-- ACP2E-1994 -->

* 当`use_config_manage_stock`值等于`1`时，导出的产品现在具有正确的`manage_stock`值。 以前，此默认值不正确。
...
* 禁用主题`view.xml`中的`product_image_white_borders`后，产品详细信息页面上的主要产品图像在页面加载期间不再明显下移。 以前，由于Fotorama JS中的高度设置不正确，产品图像在页面加载期间明显向下偏移。

<!-- ACP2E-1361 -->

* 现在，当客户订阅价格下降警报时，客户会收到有关产品价格下降的通知。 以前，由于应用程序级别的缓存，并不总是发送降价通知。
...
* 使用片段的`routes`查询现在会按预期返回类别信息。 以前，类别页面上发生内部服务器错误。 [GitHub-35906](https://github.com/magento/magento2/issues/35906)
...
* 当通过计划更新启用目录规则时，目录规则现在可以正确索引。 因此，现在可以正确地生成折扣价格并编制索引。 以前，当两个目录规则可用，且一个规则在第一条规则之后激活且第一条规则的目录规则索引操作尚未完成时，不会为第二条目录规则生成目录规则折扣价。

<!-- ACP2E-1579 -->

* 现在，目录规则已按预期方式应用于多网站环境。
...
* 购物者现在可以使用由同一购物车价格规则生成的两个单独的自动生成优惠券代码。 当优惠券类型选定为`Auto`时，`Uses per Coupon`字段现在处于启用状态（与优惠券类型=`Specific Coupon`类似）。 反过来，如果代码不同，购物者也可以将多个优惠券代码应用于相同的购物车价格。

<!-- ACP2E-1496 -->

* 现在，当购物者的单次使用优惠券值所应用的顺序被取消时，该值将按预期恢复。
...
* 客户电子邮件地址现在可以包含变音符。 以前，前端验证禁止在其电子邮件地址中创建带有变音符的用户。 [GitHub-12075](https://github.com/magento/magento2/issues/12075)

<!-- ACP2E-1682 -->

* 现在，系统会根据为网站默认商店视图配置的语言翻译库存和价格预警电子邮件。 以前，不翻译这些电子邮件。
...
* 已删除未使用的`AlgorithmProviderFactory`类以及包括`JweAlgorithmManagerFactory.php`、`JweContentAlgorithmManagerFactory.php`和`JwsAlgorithmManagerFactory`在内的类中对此类的依赖项。 [GitHub-37783](https://github.com/magento/magento2/issues/37783)
...
* 当`exec()`返回false时，Redis预加载不再失败。 已在预加载过程中添加验证，以防止`array_combine`将`false`作为第二个参数接收。 因此，当`exec()`返回`false`时，预加载将继续运行。 [GitHub-37509](https://github.com/magento/magento2/issues/37509)
...
* 在可用货币列表中添加了新尼加拉瓜货币(NIO)。

<!-- AC-10591 -->

* 已在`app/code/Magento/CatalogRule/Controller/Adminhtml/Promo/Catalog/NewConditionHtml.php`中将`reponseBody`变量的名称更改为`responseBody`。[GitHub-38093](https://github.com/magento/magento2/issues/38093)

<!-- AC-9221 -->

* 解决了启用了小数数量的产品的交易记录中发生的小计舍入错误。 [GitHub-37817](https://github.com/magento/magento2/issues/37817)

<!-- ACP2E-2127 -->

* 现在，管理员客户网格的日期筛选器根据指定的区域设置按预期工作。
...
* `User`模块中的JSON序列化程序已替换为一个新的、更准确的序列化程序(`JsonHexTag`)。 以前，当包含未转义的元素时，Commerce在尝试使用由常规序列化程序序列化的数据时有时会崩溃。 [GitHub-31377](https://github.com/magento/magento2/issues/31377)

<!-- ACP2E-2138 -->

* 运行完全重新索引(`bin/magento indexer:reindex`)时不再发生中断的管道错误。 AMPQ连接现在会在进程管理器分叉进程之前关闭。

<!-- ACP2E-2244 -->

* 现在，队列消息验证器会验证所有数组元素的子类型。 以前，Commerce引发异常。

<!-- ACP2E-2066 -->

* `app/etc/env.php`中每个主题的队列连接配置现在可以正常工作。 以前，使用自定义Exchange创建新消息队列时，产品警报队列无法正确生成。 如果在`app/etc/env.php`中用自定义Exchange配置了主题的队列连接，则会为该自定义Exchange中的所有主题创建一个新绑定。 商家无法处理产品警报。

<!-- ACP2E-2144 -->

* 将MariaDB服务器升级到10.5.1或更高版本时，用`/* mariadb-5.3 */`注释标记的时间格式在`bin/magento setup:install`期间不再导致错误。

<!-- ACP2E-2147 -->

* 现在，AwsS3驱动程序可按预期工作，取代基本默认文件存储实施。 以前，将此驱动程序配置为文件存储时，数据可能会损坏。 [GitHub-37844](https://github.com/magento/magento2/issues/37844)
...
* 长宽比值已添加到目录图像，不必要的脚本已从UI框架中删除。 [GitHub-37691](https://github.com/magento/magento2/issues/37691)
...
* 现在，当管理员用户的Commerce实例连接到S3存储时，管理员用户可以生成发票。 以前，Commerce显示以下错误： `Cannot create image resource. File not found`。 [GitHub-35706](https://github.com/magento/magento2/issues/35706)
...
* 当使用&#x200B;**产品进行配置以匹配**&#x200B;设置时，相关产品规则的&#x200B;**是**&#x200B;条件之一现在可以正常工作。 以前，此规则不识别任何产品。
...
* 启用&#x200B;**[!UICONTROL RSS Feed Top Level Category]**&#x200B;后，在请求类别的RSS源期间不再发生错误。 以前，在启用&#x200B;**[!UICONTROL RSS Feed Top Level Category]**&#x200B;时，浏览RSS馈送类别页面会导致日志文件中的Elasticsearch `CRITICAL`错误。

<!-- ACP2E-1490 -->

* 现在，如果在创建用户角色期间更改了角色范围，则会为`gws_store_groups`保存正确的值。 以前，将角色范围选为`All`时，`gws_store_groups`值会在用户创建期间保存为null，但在角色编辑期间不会保存为null。 此外，如果角色范围被选为`Custom`，则`gws_store_groups`值始终保存为null。

<!-- ACP2E-1446 -->

* 启用&#x200B;**[!UICONTROL Add Store Code to Urls]**&#x200B;设置后，现在可以通过单击管理员登录页上的管理员重置密码链接来访问重置密码页。 管理员重置密码链接以前曾打开登录页面或404页面。

<!-- ACP2E-1274 -->

* 管理员统计信息仪表板中的日期范围现在会根据配置设置中设置的时区进行计算，然后转换为UTC以便从数据库中获取数据。

<!-- ACP2E-1665 -->

* 在筛选商店视图，然后单击管理员&#x200B;**[!UICONTROL Content]** > **[!UICONTROL Hierarchy]**&#x200B;上的&#x200B;**[!UICONTROL Save]**&#x200B;后，CMS层次结构筛选器可按预期工作。 以前，单击&#x200B;**[!UICONTROL Save]**&#x200B;刷新页面，但上下文已丢失，并且选定的商店视图不再显示。
...
* `setGiftOptionsOnCart`突变现在会按预期从订单中删除礼品包装。 现在，在购物车级别取消设置`gift_wrapping_id`的值。 以前，即使在购物车级别取消设置值，`gift_wrapping_id`也将在下单后更新为其先前的值。

<!-- ACP2E-1866 -->

* 现在，当从应用礼品卡的订单处理简单产品的部分退款时，礼品卡帐户将按预期保持活动状态。 以前，已删除礼品卡帐户。

<!-- ACP2E-1805 -->

* 现在，当从购物车中移除礼品邮件时，`setGiftOptionsOnCart`变异可以成功更新数据库。 以前，数据库中的消息详细信息未更新，并且礼品消息未从购物车中删除。
...
* 现在，在使用多个地址的结账过程中可以正确应用礼品卡。 以前，礼品卡金额不适用于多地址发运，导致礼品卡金额无效。

<!-- ACP2E-1341 -->

* 现在，在`getInfoBlockHtml()`调用期间完成模拟后，`send()`调用期间将开始模拟，并且不会记录任何错误。 以前，`system.log`文件会因以下错误而泛滥： `main.ERROR: Environment emulation nesting is not allowed`。 [GitHub-35603](https://github.com/magento/magento2/issues/35603)

<!-- ACP2E-1612 -->

* 您现在可以使用`updateCartItems`变异来更新购物车中的礼品卡。 以前，只有卡数量可以通过此突变在购物车中更新。

<!-- ACP2E-1669 -->

* 礼品包装现已按预期包含在基于合并购物车的报价中。 以前，礼品消息已成功合并，但不会包装选区。

<!-- AC-7530 -->

* `addProductsToCart`突变现在已成功将礼品卡添加到购物车中。 以前，此查询需要礼品卡接收者姓名，但返回的是发件人姓名。 Adobe Commerce引发此错误： `Please specify a recipient name`。 [GitHub-36665](https://github.com/magento/magento2/issues/36665)
...
* `customerCart`查询现在可按预期返回捆绑产品上所有应用的折扣。 以前，应用于捆绑产品的总折扣返回为零。
...
* GraphQL库组件的地址错误处理现在可按预期工作。

<!-- AC-9058 -->

* 现在，当动态定价设置为“否”并应用折扣时，`products`查询会返回所有相关店面定价详细信息。 以前，不返回产品的价格范围。 [GitHub-35649](https://github.com/magento/magento2/issues/35649)
...
* 当产品价格配置为含税时，`customer`查询现在返回一个包含订单税的`product_sale_price`字段。 [GitHub-36946](https://github.com/magento/magento2/issues/36946)

<!-- ACP2E-1584 -->

* `DateTime`类型的产品属性现在映射到产品查询中的`FilterRangeTypeInput`筛选器。 以前，这些属性映射到`FilterMatchTypeInput`。 作为此更改的结果，筛选`DateTime`属性的查询需要`from`和`to`值，而不是匹配值。
...
* 当`items.redirect_code`响应值包含null值时，`categories`查询不再返回错误。 [GitHub-36675](https://github.com/magento/magento2/issues/36675)
...
* `changeCustomerPassword`变异现在会在成功重置密码后按预期触发电子邮件。

<!-- ACP2E-1494 -->

* 当URL重写的`Target Path`是绝对URL时，`urlResolver`和`route`查询现在会返回结果。 以前，`urlResolver`查询在用于检索重定向数据时返回NULL。
...
* `categoryList`查询现在按预期返回捆绑产品的子产品在`options`块中填充的`product`节。 以前，不会返回有关捆绑产品子产品的信息。
...
* 在`ProductInterface`上查询`related_products`、`upsell_products`和`crosssell_products`时，您现在可以使用片段`ProductCard`。 [GitHub-29769](https://github.com/magento/magento2/issues/29769)
...
* 产品导入过程现在使用正确的分隔符分析值。 以前，导入过程使用管道(“|”)运算符来解析多选值，直到确定的值分隔符不等于缺省值为止。 导入过程随后失败。
...
* 价格索引器性能问题已得到解决。 索引器的性能不再逐渐下降。 索引器已重构，通过改变临时表的创建方式来提高索引器处理速度。 这些临时表现在已重新创建，而不是由更耗时的DELETE语句来处理。
...
* 修复了在`int`上调用成员函数`getId()`时可能发生的错误，该错误有时会在管理员中查看客户时触发异常。
...
* 商家现在可以为订单创建贷项通知单，其中删除了与可配置产品关联的简单产品。 以前，商家无法创建贷项通知单，Commerce引发了`TypeError`异常。

<!-- ACP2E-2310 -->

* 现在，在应用筛选器后，管理网格源计数会返回有效记录。 以前，`getSourcesCount`方法始终在应用筛选器后返回源总数。

<!-- ACP2E-1364 -->

* 使用“从Stock移至底部”选项对产品进行排序时，“类别”页面不再重定向到“管理员”信息板。 通过将SQL联接语句放在条件语句中来解决该问题，前提是确保该语句尚未包括在内。 以前，页面已重定向到管理员信息板，并显示消息： `Invalid security or form key. Please refresh the page`。

<!-- ACP2E-1488 -->

* `inventory_source_item`表的`AUTO_INCREMENT`不再随每`UPDATE`操作增加。 以前，每次更新都会增加此表的`AUTO_INCREMENT`，这最终会在向`inventory_source_item table`添加新记录时导致`AUTO_INCREMENT`值超出范围。 因此，管理员用户在尝试从管理员创建新产品时收到以下错误： `Numeric value out of range...`。
...
* 客户现在会在按预期激活帐户后重定向到登录页面。 以前，客户会自动登录。

### 媒体集

<!-- ACP2E-2568 -->

* 商家现在可以将具有相同名称但大小写不同的图像（混合大小写字符）上传到媒体库。 以前，图像文件名在Page Builder中不区分大小写，而文件名不同于大小写的文件在Commerce媒体存储中将被覆盖，这些存储托管在区分大小写的文件系统上。
...
* 商家现在可以按预期创建贷项通知单，以部分返还通过奖励积分付款的订单。 以前，当商家创建部分退货时，订单状态错误地标识为“已关闭”，且“管理员”订单页面未显示贷项通知单选项。
...
* 现在，当商家从此页面启动搜索时，将按预期加载“管理员销售订单网格”页面。 以前，当商家尝试从此表格搜索订单时，页面会冻结，并且Commerce在浏览器控制台中显示了`Request-URI Too Long`错误。

<!-- ACP2E-2294 -->

* 现在，当已用商店积分加上其他付款方式退还整个订单金额时，订单状态将标记为“已关闭”而不是“完成”。 以前，由于退款金额计算错误，订单状态被标记为“完成”。

<!-- ACP2E-2213 -->

* 通过减少`OrderRepository::get()`方法从数据库中加载订单的次数，改进了该方法的性能。 以前，此方法多次加载订单。 [GitHub-36636](https://github.com/magento/magento2/issues/36636)

<!-- ACP2E-1918 -->

* 当其他订单项目可以退款时，商家现在可以为包含某些项目的订单生成总和为零的贷项通知单。 以前，当客户对订单中的某些项目享有100%折扣时，商家无法执行多次退款。
...
* 当管理员用户无权访问客户网站时，Adobe Commerce在创建具有客户余额的脱机退款的贷项通知单时不再引发错误。 以前，Adobe Commerce引发此错误： `More permissions are needed to view this item`，并且管理员用户无法创建商店退款。

<!-- ACP2E-1714 -->

* 当对包含捆绑产品的订单进行了部分退款时，订单状态现在是正确的。 以前，在发放部分退款后，即使剩余的订单未完成，订单状态仍列为“完成”。 [GitHub-37377](https://github.com/magento/magento2/issues/37377)
...
* 现在，`GET V1/orders/<OrderID>`端点按顺序返回有关可配置和简单产品的信息，而不管简单产品的Stock状态如何。 仅当所选项目（产品）已库存或可销售时，管理员用户才能立即下订单。 以前，此API仅返回有关现有产品的信息。

<!-- AC-7901 -->

* 在启用免费发运的情况下，如果发运价格含税，并且应用了折扣后的税计算，贷项通知单页面将不再崩溃。 以前，Adobe Commerce引发`Division by zero`异常。 [GitHub-36800](https://github.com/magento/magento2/issues/36800)

<!-- AC-1342 -->

* 使用`rest/V1/orders/{id}/comments`端点在不提供订单状态的情况下发布有关订单的注释不再影响订单的显示。 以前，`sales_order`和`sales_order_grid`表中的订单状态记录为NULL，并且“我的订单”页面或“管理员”订单网格均未显示订单。 [GitHub-34180](https://github.com/magento/magento2/issues/34180)
...
* 现在，在页面生成器中调整大小时图像可保持正确的长宽比。 [GitHub-37493](https://github.com/magento/magento2/issues/37493)

<!-- AC-10919 -->

* 当您尝试上传的图像超过指定的最大文件大小时，页面生成器现在会根据需要显示错误消息。 以前，对于小于指定最大值的已上传图像文件，Commerce会显示一条警报弹出消息。
...
* 改进了Chrome浏览器中页面生成器保存操作的性能。

<!-- ACP2E-1954 -->

* 您现在可以使用“管理”>“**[!UICONTROL Content]**”>“**[!UICONTROL Pages]**”中的“页面生成器”选项以使用“`text`”元素在所有样式中插入复制的文本。 以前，页面生成器只显示纯文本，不保存样式。
...
* 产品小组件和页面生成器预览现在可正确处理HTML特殊字符。 以前，产品小组件未正确显示包含`&`字符的SKU。

<!-- ACP2E-1441 -->

* 现在，当您在预览模式下选择并更新完整的内容块时，页面生成器文本元素内容会按预期方式更新。

<!-- ACP2E-1362 -->

* 将文本从一个页面生成器文本编辑器复制并粘贴到另一个页面生成器文本编辑器，现在可按预期工作。 以前，您尝试粘贴文本的页面无法保存，Adobe Commerce显示此控制台错误： `[ERROR] Page Builder was rendering for 5 seconds without releasing locks.`。
...
* 已从`Magento\Catalog\Model\ProductRepository`中删除未使用的`Magento\Catalog\Controller\Adminhtml\Product\Initialization\Helper`。 以前，`ProductRepository`加载此帮助程序，它是一个`Adminhtml`控制器。 [GitHub-37864](https://github.com/magento/magento2/issues/37864)
...
* 已优化将产品从&#x200B;**[!UICONTROL Products in Category]**&#x200B;选项卡分配到类别的过程。 在产品完全加载到网格中之前，**[!UICONTROL Add Product by SKU]**&#x200B;选项卡中的&#x200B;**[!UICONTROL Assign]**&#x200B;和&#x200B;**[!UICONTROL Remove]**&#x200B;按钮现在处于禁用状态。 以前，管理员用户无法按SKU在包含数百万个已注册产品的目录中添加产品。

<!-- ACP2E-2098 -->

* 在用多个自定义产品属性执行时，`addProductToCart`变异的性能已得到提高。 以前，当使用多个产品属性（大约1000个）执行此变异时，Commerce会尝试加载所有产品属性。

<!-- ACP2E-1965 -->

* 解决了加载Cart规则使用的产品属性时的性能问题。 现在只加载活动购物车规则的产品属性。

<!-- ACP2E-1864 -->

* 多次运行时，`bin/magento setup:perf:generate-fixtures setup/performance-toolkit/profiles/ee/small.xml`命令现在可以成功执行。 以前，当运行多次时，它引发此错误： `Coupon with the same code already exists`。

### 促销

<!-- ACP2E-1290 -->

* 在与&#x200B;**[!UICONTROL Apply to shipping amount]**&#x200B;规则一起应用时，**[!UICONTROL Fixed amount discount for whole cart]**&#x200B;价格规则的效果现在按预期在产品和装运金额之间按比例分配。 以前，折扣计算不正确。

<!-- ACP2E-1300 -->

* **[!UICONTROL Discard Subsequent Rules]**&#x200B;设置现在已按预期应用。 启用此设置后，其他低优先级的规则只能应用于其他产品。 [GitHub-35707](https://github.com/magento/magento2/issues/35707)
...
* reCAPTCHA小组件现在位于付款页面上的&#x200B;**[!UICONTROL Place Order]**&#x200B;按钮之前。 以前，小组件显示在页面顶部。
...
* 现在，当启用产品编辑页面上的&#x200B;**[!UICONTROL Use Config]**&#x200B;复选框时，**[!UICONTROL Enable RMA]**&#x200B;属性的值与配置中指定的值匹配。
...
* 现在，仅当&#x200B;**[!UICONTROL Use in Search]**&#x200B;设置设置为&#x200B;**[!UICONTROL Yes]**&#x200B;时，才会显示&#x200B;**[!UICONTROL Use in Search Results Layered Navigation]**&#x200B;产品属性属性。
...
* 管理员&#x200B;**[!UICONTROL Stores]** > **[!UICONTROL Attributes]** > **[!UICONTROL Product]**&#x200B;上`Search Weight`属性的`Storefront Properties`输入字段下的信息性文本现在描述了可能的值。 [GitHub-36646](https://github.com/magento/magento2/issues/36675)
...
* 现在，启用&#x200B;**[!UICONTROL Apply custom Shipping Policy]**&#x200B;并且自定义配送策略可用时，**[!UICONTROL Shipping Policy]**&#x200B;文本框会在“管理应用商店视图”范围中可见。 [GitHub-36425](https://github.com/magento/magento2/issues/36425)

<!-- AC-7550 -->

* `cart`查询现在返回与`available_shipping_methods`运营商和方法代码之一匹配的`selected_shipping_method`值。 以前，当其中一个选定的配送方式包含下划线时，这些承运人/方式代码不匹配。 [GitHub-36606](https://github.com/magento/magento2/issues/36606)
...
* XML站点地图现在在多层部署中可按预期工作。 以前，站点不包含为每个商店分配的默认页面。 [GitHub-35001](https://github.com/magento/magento2/issues/35001)
...
* 如果未指定结束日期，则更新计划更新不再删除暂存项目的先前版本。 以前，更新暂存项目时，如果未指定结束日期，则会删除项目的回滚版本。 Commerce在删除之前未引发错误或警告管理员用户。

<!-- ACP2E-1923 -->

* 计划更新购物车价格规则后，不再擦除商店视图标签值。 以前，即使更新中未指定这些标签，这些标签也会被删除。

<!-- ACP2E-1803 -->

* 内容暂存预览现在可正确用于链接的页面。 以前，您无法导航到暂存预览中的链接页面，但被错误地定向到店面页面。
...
* 当没有要处理的临时更新时，`magento_bulk`表中的`staging.synchronize_entity_period`使用者队列不再增加。 以前，如果将`cron`配置为每分钟运行一次，则每分钟会向此表添加一条与`staging.synchronize_entity_period`使用者相关的新记录。
...
* 目标规则现在可正确用于不同的存储视图。 以前，由于相关产品规则缓存存在问题，因此无法在多存储部署中显示推荐的产品。
...
* 现在，当将&#x200B;**[!UICONTROL Price Display Settings]**&#x200B;设置为含税时，为具有&#x200B;**[!UICONTROL Special Price]**&#x200B;值的产品选择自定义选项时，Commerce会显示&#x200B;**[!UICONTROL Regular Price]**&#x200B;值的正确值。

<!-- ACP2E-2303 -->

* 现在，按照配置设置中的指定，税费会按预期包含在结账工作流程发货价格中。

<!-- AC-9758 -->

* `cart`查询`applied_taxes`响应值现在按预期包括所有个别税则。 以前，在配置多个税则时，它会返回总税值。
...
* 通过引入一个显式设置要在搜索中使用的属性的额外步骤，修复了`AdminSimpleProductwithTextandVisualSwatch`测试。
...
* 改进了`AdminCreateOrderWithVirtualProductFromDefaultSourceWithBackorders`测试，以消除由于过早完成电子邮件字段而导致的失败。

<!-- ACP2E-2202 -->

* 已重构`Magento\GraphQl\Catalog\ProductSearchTest::testFilterProductsBySingleCategoryId`测试以处理`products`个查询响应中的随机类别顺序。
...
* `CollectionTest::testGetDateRangeFirstPart()`现在占用夏令时。
...
* 图像上传器现在可在动态行中按预期工作。 现在以多阵列形式发送图像时，图像可以正确上传。 [GitHub-37957](https://github.com/magento/magento2/issues/37957)

<!-- AC-10877 -->

* 亮度主题中不再复制文件上传样式。 [GitHub-32373](https://github.com/magento/magento2/issues/32373)
...
* 更新了店面页脚模板文件中GitHub问题列表页面的链接。 [GitHub-37214](https://github.com/magento/magento2/issues/37214)

<!-- AC-6282 -->

* “最近查看”小组件现在可以按产品价格分组显示正确的税额。 [GitHub-35881](https://github.com/magento/magento2/issues/35881)
...
* 现在，产品下拉属性类型的顺序与可配置产品页面上的图像顺序相匹配。 [GitHub-37285](https://github.com/magento/magento2/issues/37285)

<!-- ACP2E-1969 -->

* 系统消息现在加载速度更快。 仅加载正在显示的消息。 以前，所有消息都是从数据库加载的，非常耗时。
...
* 现在，当包含多选客户属性时，可按预期加载客户注册表单。 以前，Adobe Commerce引发此异常： `[2023-03-05T05:57:40.032602+00:00] report.CRITICAL: Exception: Deprecated Functionality: explode(): Passing null to parameter #2 ($string) of type string is deprecated in /magento2ee/app/code/Magento/CustomAttributeManagement/Block/Form/Renderer/Multiselect.php on line 53 in /lib/internal/Magento/Framework/App/ErrorHandler.php:62`。 [GitHub-37189](https://github.com/magento/magento2/issues/37189)

<!-- ACP2E-1602 -->

* 现在，包含单引号的欢迎消息在店面中可正确呈现，不会出现控制台错误。 以前，如果欢迎消息包含单引号，则在店面渲染期间不会加载欢迎消息。 Adobe Commerce还显示控制台错误。
...
* `HEAD`元素的`crossorigin`属性已添加到`linkTypes`。[GitHub-32509](https://github.com/magento/magento2/issues/32509)
...
* 产品URL后缀和类别URL后缀配置现在与`url_rewrite`表中的后缀匹配。 以前，更新URL后缀配置时出现争用条件，并且配置的URL后缀与`url_rewrite`表不匹配。

<!-- ACP2E-1703 -->

* 现在，在创建类别URL重写时，Adobe Commerce会在所有级别渲染类别树。 以前，仅显示前三个级别的类别。 [GitHub-36973](https://github.com/magento/magento2/issues/36973)
...
* 您现在可以对Visual Merchandiser规则使用`Categories`属性。 以前，Adobe Commerce引发此异常： `Something went wrong while saving the category`。

### Web API框架

<!-- AC-6980 -->

* 当&#x200B;**[!UICONTROL Callback URL]**&#x200B;和&#x200B;**[!UICONTROL Identity link URL]**&#x200B;字段为空时，Commerce在“管理员&#x200B;**[!UICONTROL System]**”>“**[!UICONTROL Extensions]**”>“**[!UICONTROL Integrations]**”中不再显示`Integration not secure`警告。 [GitHub-36407](https://github.com/magento/magento2/issues/36407)
...
* Tier价格REST API现在支持小数数量。 以前，即使启用了&#x200B;**[!UICONTROL Use Decimal Quantity]**&#x200B;设置，此API也不允许使用小数数量。

<!-- AC-8998 -->

* 尝试向待定订单的状态添加注释时，REST `V1/orders/{id}/comments`终结点不再更改订单的状态。 现在它提供了一个信息性错误。 以前，此端点更改了添加注释时的订单状态。

<!-- ACP2E-2275 -->

* REST API `GET`和`POST V1/products/attributes`终结点不再为&#x200B;**[!UICONTROL Use in Layered Navigation]**&#x200B;选项的`Filterable(with results)`和`Filterable(no results)`选项的`is_filterable`属性返回相同的值。 以前，此端点返回相同的值，因为`is_filterable`属性属性是`Boolean`类型，不允许将此属性设置为`Filterable(no results)`。 [GitHub-37979](https://github.com/magento/magento2/issues/37979)
...
* REST `V1/carts/mine/estimate-shipping-methods`方法现在将验证所有级别的有效负载字段数据，并为无效数据返回400 HTTP代码。 以前，仅验证第一级API负载，如果以未验证的负载级别返回无效数据，则Commerce会返回500状态代码。

<!-- ACP2E-2053 -->

* 现在，使用层价格和不同的属性集更新产品的异步批量REST API操作已成功完成。 以前，无法以这种方式更新具有不同属性集的产品，Commerce显示以下错误： `The product was unable to be saved. Please try again`。
...
* 您现在可以执行授权的异步批量API请求，以创建具有非默认组ID的客户。 以前，这些请求将组ID重置为默认值。

<!-- ACP2E-1543 -->

* 现在，使用Bulk API创建的客户会收到其分配网站配置的语言版本的客户注册电子邮件。 以前，所有电子邮件都发送到默认区域设置，而不是客户分配到的网站区域设置。

<!-- ACP2E-1339 AC-7577-->

* 禁用&#x200B;**[!UICONTROL Allow Guest Checkout]**&#x200B;后，`POST V1/guest-carts`调用不再提交来宾订单。

<!-- ACP2E-1336 -->

* 您现在可以在未启用`AsyncOrder`时从管理员处打开状态为`Received`的订单。 以前，当订单状态由`POST V1/orders`更改时，无法从管理员打开订单详细信息页面。

<!-- ACP2E-1526 -->

* Adobe Commerce现在在`GET V1/products/sku/media`产品媒体API响应中成功返回`content`属性（base64图像代码）。 `content`属性以前未出现在响应中。

### 愿望列表

<!-- ACP2E-1784 -->

* 当购物者多次尝试将同一项目添加到愿望列表时，Commerce现在会显示一条消息，指示该项目已添加到愿望列表。 以前在这些情况下，Commerce会增加愿望列表中的产品数量。

<!-- AC-8610 -->

* Adobe Commerce现在已成功将包含必需上传的可自定义选项文件的产品添加到愿望列表。 以前，Commerce向愿望列表添加了两个产品，而不是一个。 [GitHub-37437](https://github.com/magento/magento2/issues/37437)


## 系统要求

我们的技术栈栈是基于PHP和MySQL构建的。 有关详细信息，请参阅[系统要求](https://experienceleague.adobe.com/docs/commerce-operations/installation-guide/system-requirements.html)。

## 安装和升级说明

您可以使用[编辑器](https://experienceleague.adobe.com/docs/commerce-operations/installation-guide/composer.html)安装Adobe Commerce 2.4.7。
