---
title: Adobe Commerce 2.4.7-beta2发行说明
description: 了解2.4.7-beta2 Adobe Commerce版本中的新增功能、增强功能、错误修复和已知问题。
...
# Adobe Commerce 2.4.7-beta2发行说明
...
Adobe Commerce 2.4.7-beta2包括200多项质量修复和增强功能。 核心编辑器依赖项和第三方库已升级到最新可用版本。
...
Adobe Commerce 2.4.7-beta2已针对以下组件版本进行了测试：
...
Adobe Commerce 2.4.7-beta2引入了十个安全修复和平台安全改进。 Adobe Commerce 2.4.7之前的测试版引入了13项安全修复和平台安全改进。
...
请参阅 [Adobe安全公告](https://helpx.adobe.com/security/products/magento/apsb23-50.html) 以获取有关这些已修复问题的最新讨论。
...
* Composer 2.5.x.与Composer 2.2.x的兼容性得以保留。

* **支持Varnish缓存7.3**. 此版本与最新版本的Varnish Cache 7.3兼容。与6.0.x和7.2.x版本的兼容性保持不变，但我们建议仅将Adobe Commerce 2.4.7-beta2与Varnish Cache版本7.3或版本6.0 LTS一起使用。

* **RabbitMQ 3.11支持**. 此版本与最新版本的RabbitMQ 3.11兼容。兼容性保持与RabbitMQ 3.9的兼容，该版本在2023年8月之前受支持，但我们建议仅将Adobe Commerce 2.4.7-beta2与RabbitMQ 3.11一起使用。

#### 其他升级和更换

Adobe Commerce 2.4.7-beta2引入了以下平台升级：
...
* Commerce FedEx REST API已更新，以便与更新的FedEx Web服务兼容，该服务将于2024年5月发布。 此版本中不包含任何FedEx SOAP支持。 <!-- AC-1470 -->
...
之前推出的Adobe Commerce 2.4.7测试版包括：

* **针对具有许多商店的部署，加快存储级配置的保存操作**. 在具有500个以上存储的部署中保存配置设置可能非常耗时。 新 [异步配置](https://experienceleague.adobe.com/docs/commerce-operations/performance-best-practices/configuration.html?lang=en#asynchronous-configuration-save-2.4.7-beta1) 模块通过运行使用使用者处理消息队列中的保存操作的cron作业来启用异步配置保存操作。 默认情况下，AsyncConfig处于禁用状态。
...
Commerce 2.4.7-beta2引入了新的用于GraphQL API的PHP应用程序服务器，该服务器在Openswoole PHP框架上实现。 借助此应用程序服务器，Adobe Commerce可以维护Commerce GraphQL API请求中的状态，并且无需引导，进而提高所有GraphQL API的吞吐量、降低延迟以及高效使用资源。 通过在进程之间共享应用程序状态，GraphQL API请求可以显着提升效率，GraphQL API响应速度平均提高30%。

此 `ApplicationServer` 模块实现PHP应用程序服务器。 如果开发人员希望在内部部署中实施此模块，则还必须安装和配置Swoole和Nginx。 使用Adobe Commerce进行云基础架构项目的开发人员无需下载这些组件，因为基础架构已包含Nginx和OpenSwool。 云开发人员必须手动启用应用程序服务器，并且无法像其他服务一样启动或停止模块。 必须重新部署Adobe Commerce才能启动或停止模块，这需要站点停机时间。

请参阅 [适用于GraphQL API的应用程序服务器](https://experienceleague.adobe.com/docs/commerce-operations/performance-best-practices/performance-best-practices/application-server.html) 用于内部部署和云部署。

这是PHP Application Server for GraphQL API模块的测试版。 包含自定义项和扩展的部署可能与模块不兼容。 启用该模块的商家应该准备好解决与已部署的自定义项和扩展可能的不兼容问题。 <!-- ACPT-27 -->

### Adobe Commerce扩展中继包

此版本引入了Adobe Commerce扩展metapackage v2.0.0-beta2，它自动将所选Adobe Commerce扩展与此核心版本捆绑在一起。 在以下情况下安装此中继包中包含的这些扩展的版本： `composer update` 运行，从而简化在升级到最新核心版本时升级这些扩展的过程。 这些扩展维护独立的发放计划。

适用于Adobe Commerce 2.4.7-beta2的Adobe Commerce扩展元包包含以下扩展：
...
Commerce 2.4.7-beta1中包含的Bolt checkout扩展已从此Beta版本中删除。

此扩展metapackage的未来版本可能包含其他扩展。

### B2B

此版本随B2B 1.4.2一起打包。它仅包含错误修复，尽管与Adobe Commerce 2.4.6-beta2打包的PHP Application Server版本引入了对B2B GraphQL API的覆盖。

**2.4.7-beta1中引入的B2B增强功能**

Adobe Commerce 2.4.7-beta1包含针对B2B可转让报价、REST API更新以及多个错误修复的新功能和增强功能。
...
* REST和GraphQL的更新
   ...
   * 添加了端点，以便为本版本中引入的卖方启动的报价功能提供Web API覆盖范围。 请参阅 [管理可转让报价](https://developer.adobe.com/commerce/webapi/rest/b2b/negotiable-manage/)： `POST /V1/negotiableQuote/draft`.
...
* 通过减少每个请求调用解析方法的次数，GraphQL解析器的性能得到了改进。 现在只调用一次。 以前，解析器至少被调用三次。
...
此版本引入了对以下支付方法的存储支付的支持：

* ApplePay  <!-- BUNDLE-3246 -->
* GooglePay  <!-- BUNDLE-3270 -->
* PayPal点数  <!-- BUNDLE-3272 -->

此版本还包括对这些支付方法的支持：

* 使用Braintree卡付款的银联卡 <!-- BUNDLE-3268 -->
* 带3D安全支付的GooglePay <!-- BUNDLE-3271 -->
...
PWA Studio v.13.3.x与Adobe Commerce 2.4.7-beta2兼容。 它包括多项增强功能，可改善辅助功能。 有关错误修复的信息，请参见 [PWA Studio版本](https://github.com/magento/pwa-studio/releases). 请参阅 [版本兼容性](https://developer.adobe.com/commerce/pwa-studio/integrations/adobe-commerce/version-compatibility/) 有关PWA Studio版本及其兼容的Adobe Commerce核心版本的列表。

## 修复的问题

我们已在Adobe Commerce 2.4.7-beta2核心代码中修复了数百个问题。 此版本中包含的已修复问题的子集如下所述。 还介绍了Commerce 2.4.7-beta1中包含的修复。
...
* 解决了在将部署数据发送到New Relic模块时处理标头数据中的错误。

<!-- ACP2E-1833 -->

* 通过以下方式启用Google Tag Manager **[!UICONTROL Configuration]** > **[!UICONTROL Sales]** > **[!UICONTROL Google API]** > **[!UICONTROL Google Analytics]** 现在，触发GA Analytics 3标记。在运行Google Chrome扩展开发人员工具时，不会在产品详细信息页面中触发。
...
* 当您取消选择并选择新创建的CMS页面时，Adobe Commerce不再引发约束冲突错误。
...
* 在计划更新购物车价格规则后，商店视图标签值不再被擦除。以前，即使这些标签未在更新中指定，它们也会被擦除。
...
* Adobe Commerce现在为具有特殊价格的固定捆绑产品显示正确的价格。 这是2.4.7-beta1的一个已知问题。
...
Adobe Commerce 2.4.7-beta3将修复以下已知问题：

**问题**：从产品中删除分配给迷你购物车的共享目录后，不会按预期从迷你购物车中删除产品。 此问题是2.4.7-beta1的一个已知问题，计划在2.4.7-beta3中解决。 <!-- AC-8935 -->
...
您可以使用安装Adobe Commerce 2.4.7-beta2 [Composer](https://experienceleague.adobe.com/docs/commerce-operations/installation-guide/composer.html).
