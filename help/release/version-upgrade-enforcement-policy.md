...
当对Adobe Commerce版本的常规支持和扩展支持终止时，Adobe保留在仍运行该不受支持版本的云环境中停用Adobe Commerce的权利。 版本升级实施仅适用于云环境上的Adobe Commerce 。 本地客户管理他们自己的基础架构。 Adobe在这些日期之前提供提前通知和支持资源，以帮助您规划升级或迁移。
...
>[!NOTE]
>
>本主题仅介绍云升级实施。 请参阅[生命周期策略](lifecycle-policy.md)以了解支持层定义、[支持结束日期](lifecycle-policy.md#end-of-support-dates)、[仅限安全性的过渡条款](lifecycle-policy.md#security-only-transitional-period)、[第三方软件依赖项](lifecycle-policy.md#platform-dependencies)、[PHP生命周期结束和PCI合规性](lifecycle-policy.md#php-end-of-life-and-pci-compliance)以及[升级和迁移选项](lifecycle-policy.md#upgrade-and-migration-options)。 除了升级到支持的[!DNL Adobe Commerce version]之外，Adobe还要求您在主动支持的版本上保留第三方软件依赖项。 有关适用于[!DNL Adobe Commerce on Cloud]版本2.4.4到2.4.9的特定所需操作和截止日期，请参阅[安全和合规性声明](security-enforcement-policy.md)。

## Adobe为何要推出这项政策

Adobe负责托管平台基础设施的安全性和合规性，Adobe Commerce on Cloud客户可以在该基础设施上运行。 这包括使所有基础软件依赖项保持最新，应用安全补丁程序，以及满足客户所依赖的合规性标准（如PCI）。

当供应商正式终止对基础软件依赖项的安全支持时，Adobe将不再提供所需级别的安全保护范围和平台支持。 如果继续在不支持的基础设施上运营商店，客户、购物者和Adobe将面临不可接受的风险。 因此，Adobe将引入正式的版本升级实施策略，该策略以及Adobe提供的支持来定义何时停止支持运行不支持的Commerce版本的Cloud环境上的Adobe Commerce，以帮助您规划升级或迁移。 有关详细信息，请参阅[安全和合规性通知]。
