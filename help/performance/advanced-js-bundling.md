...
有关第三方工具、HTTP/2以及有关已弃用的JS和CSS合并的指南，请参阅&#x200B;*配置最佳实践*&#x200B;中的[捆绑提示](configuration.md#bundling-tips)。
...
- [生成.js](../assets/performance/code-samples/build.js)
...
要获取[!DNL RequireJS]以使用您的包，请在`build.js`文件中的`modules`节点之后添加`onModuleBundleComplete`回调：
...
在`pub/static/frontend/Magento/luma/en_US`目录中打开`requirejs-config.js`以验证[!DNL RequireJS]是否将该文件附加到捆绑配置调用：
