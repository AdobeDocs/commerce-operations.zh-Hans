...
description: 了解Adobe Commerce中的graphql应用程序服务器。 了解实施指导和优化策略。
...
1. 取消注释`.magento/services.yaml`文件中的`files`节。
...
1. 取消注释`.magento.app.yaml`文件中装载配置的`TEMPORARY SHARED MOUNTS`部分。
...
1. 注释掉`.magento.app.yaml`文件中装载配置的`DEFAULT MOUNTS`和`TEMPORARY SHARED MOUNTS`部分。
...
1. 取消注释`.magento.app.yaml`文件中装载配置的`OLD LOCAL MOUNTS`和`SHARED MOUNTS`部分。
...
1. 注释掉`.magento.app.yaml`文件中装载配置的`OLD LOCAL MOUNTS`部分。
...
GraphQL Application Server将值为`graphql_server`的`X-Backend`响应标头添加到其处理的每个请求中。 要检查GraphQL应用程序服务器是否已处理请求，请检查此响应标头。
...
默认情况下，GraphQL Application Server支持`app/etc/di.xml`文件中定义的`x-forwarded-for`标头的标准配置，从而能够准确检索典型代理和CDN环境中的客户端IP地址。

如果您需要支持其他或自定义标头（如`x-client-ip`、`fastly-client-ip`或`x-real-ip`），则可以扩展或覆盖`app/etc/di.xml`文件中的`alternativeHeaders`参数。 仅当您的环境使用`x-forwarded-for`以外的标头传递客户端IP地址时，才需要执行此操作。

例如，要添加对其他标头的支持，请更新您的`app/etc/di.xml`，如下所示：
...
您可以根据需要添加、删除或重新排序标头，以确保从正确的源中检索客户端IP以进行设置。

>[!NOTE]
>
>如果您将Adobe Commerce Cloud与Fastly CDN模块一起使用，则此配置会自动处理，无需手动更改。 只有自定义CDN、代理或非标准标头设置才需要手动配置。
