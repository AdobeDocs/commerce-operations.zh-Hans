...
- 您使用 `nginx.conf.sample` 由Commerce提供，具有本教程中讨论的修改内容。
...
1. 传递以下项的值： [图像变量](ms-overview.md) `$MAGE_RUN_TYPE` 和 `$MAGE_RUN_CODE` 到nginx，使用Magento提供的 `nginx.conf.sample` （具体步骤详见下文）。
...
只使用一个 [nginx虚拟主机文件](#step-2-create-nginx-virtual-hosts)，您可以保持简单干净的nginx配置。 通过使用多个虚拟主机文件，您可以自定义每个存储（以便为使用自定义位置） `french.mysite.mg` 例如)。
...
有关map指令的更多详细信息，请参见 [nginx有关map指令的文档](http://nginx.org/en/docs/http/ngx_http_map_module.html#map).
...
>[!TIP]
>
>请勿编辑 `nginx.conf.sample` 文件；它是一个核心Commerce文件，可能会随每个新版本进行更新。 相反，请复制 `nginx.conf.sample` 文件，重命名该文件，然后编辑复制的文件。
...
您必须更新的基础URL `french` 和 `german` Commerce管理员中的网站。
...
1. 展开 **基本URL（安全）** 部分并更新 **安全基础URL** 和 **安全基本链接URL** 值至 `https://french.magento24.com/`.
1. 单击 **保存配置** 并保存配置更改。
...
1. 展开 **基本URL（安全）** 部分并更新 **安全基础URL** 和 **安全基本链接URL** 值至 `https://german.magento24.com/`.
1. 单击 **保存配置** 并保存配置更改。
