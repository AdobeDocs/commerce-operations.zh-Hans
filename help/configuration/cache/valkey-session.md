---
title: 为会话存储配置Valkey
description: 了解如何在Adobe Commerce中为会话存储配置Valkey。 了解CLI设置、会话参数和连接验证技术。
...
Commerce提供了命令行选项来配置Valkey会话存储。 虽然您可以通过编辑`<Commerce-install-dir>/app/etc/env.php`文件来配置会话存储，但建议使用命令行方法，尤其是对于初始配置。 命令行提供验证，确保配置语法正确。

>[!IMPORTANT]
>
>在配置会话存储之前，必须安装[Valkey](config-valkey.md#install-valkey)。
