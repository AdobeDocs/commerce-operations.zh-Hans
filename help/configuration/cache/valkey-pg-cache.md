...
- **Zend缓存（2.4.8及更早版本）** — Commerce 2.4.8及更早版本的Valkey后端：
  - **基于Zend的Valkey后端** — 使用完整的类路径(`Magento\Framework\Cache\Backend\Valkey`)
  - **预加载密钥** — 支持预加载常用的缓存密钥
  - **Lua脚本** — 用于垃圾回收的Lua
  - **压缩** — 支持数据压缩
...
- **Symfony缓存(2.4.9+)** — 从Commerce 2.4.9开始，Symfony缓存为Valkey提供了符合PSR-6的缓存实现，并显着提高了性能：
