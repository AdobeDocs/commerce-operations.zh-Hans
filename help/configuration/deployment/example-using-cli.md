...
此示例说明如何在开发系统中设置共享、系统特定的敏感值，然后将这些值部署到生产系统。
这是通过使用共享配置、 `config.php` 文件和Commerce CLI命令。

此示例使用以下配置设置：

- **增值税号** 和 **存储名称** 共享配置设置的信息。

   这些规则位于 **商店** >设置> **配置** >常规> **常规**.

- **发送电子邮件至** 敏感配置值的标头。

   此函数位于 **商店** >设置> **配置** >常规> **联系人**.

- **默认电子邮件域** （对于系统特定的配置值）。

   此函数位于 **商店** >设置> **配置** >客户> **客户配置** > **创建新帐户选项**.
...
1. 在右窗格中，展开 **电子邮件选项**.
1. 如有必要，请清除 **使用默认值** 复选框。 **发送电子邮件至** 字段。
1. 在字段中输入电子邮件地址。
1. 单击 **保存配置**.
...
现在，您已在Admin中更改了配置，请使用以下步骤将共享配置写入文件：
...
现在，您已将对共享配置的更改提交到源代码管理，接下来可以将这些更改提取到构建系统中，编译代码并生成静态文件。
...
- 每个设置的范围
   ...
   如果您按照步骤1中的说明进行操作，则要检查的域 **发送电子邮件至** 是网站和范围 **默认电子邮件域** 是全局的（即默认配置范围）。
...
要设置值 **发送电子邮件至**&#x200B;但是，您必须知道作用域类型(`website`)和范围代码，每个网站上的范围代码可能不同。
...
此部分讨论如何将您在开发和构建系统时所做的所有更改提取到生产环境中，这会更新共享的配置设置（商店名称和VAT编号）。
