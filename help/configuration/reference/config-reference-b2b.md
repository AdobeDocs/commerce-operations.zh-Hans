...
本主题列出了Commerce Enterprise B2B扩展的配置路径。 此 [`magento app:config:dump` 命令](../cli/export-configuration.md) 将这些值写入共享配置文件， `app/etc/config.php`，它应位于源代码控制中。

>[!INFO]
>
>此参考资料列表 _仅限_ Adobe Commerce的B2B的唯一配置路径。 此扩展包含Adobe Commerce的所有配置路径。
...
此部分列出了“管理员”中选项可用的变量名称和配置路径。 **[!UICONTROL Stores]** >设置> **[!UICONTROL Configuration]** > **[!UICONTROL General]**.

### B2B功能路径

管理员中提供了这些配置值，位置如下： **[!UICONTROL Stores]** >设置> **[!UICONTROL Configuration]** > **[!UICONTROL General]** > **[!UICONTROL B2B Features]**.
...
此部分列出了“管理员”中选项可用的变量名称和配置路径。 **[!UICONTROL Stores]** >设置> **[!UICONTROL Configuration]** > **[!UICONTROL Customers]**.

### 公司配置路径

管理员中提供了这些配置值，位置如下： **[!UICONTROL Stores]** >设置> **[!UICONTROL Configuration]** > **[!UICONTROL Customers]** > **[!UICONTROL Company Configuration]**.

| 名称 | 配置路径 | 是否加密？ | 特定于系统？ | 敏感？ |
|--------------|--------------|--------------|--------------|--------------|
| 允许从店面进行公司注册 | `company/general/allow_company_registration` |  |  |  |
...
| 默认的公司管理员已更改为成员电子邮件 | `company/email/customer_remove_super_user_template` |  |  |  |
...
| 已更新电子邮件模板 | `company/email/credit_updated_email_template` |  |  |  |
| 已报销电子邮件模板 | `company/email/credit_reimbursed_email_template` |  |  |  |
| 已退款电子邮件模板 | `company/email/credit_refunded_email_template` |  |  |  |
...
管理员中提供了这些配置值，位置如下： **商店** >设置> **配置** > **客户** > **申请列表**.

| 名称 | 配置路径 | 是否加密？ | 特定于系统？ | 敏感？ |
| -------------- | -------------- | -------------- | -------------- | -------------- |
| 申请列表数量 | `requisitionlist/general/number_requisition_lists` |  |  |  |
...
| 已更新报价模板（发送给采购员） | `sales_email/quote/updated_buyer_template` |  |  |  |
| 已拒绝报价模板（发送给买方） | `sales_email/quote/declined_buyer_template` |  |  |  |
| 新建报价模板（向销售方） | `sales_email/quote/new_seller_template` |  |  |  |
| 更新了报价模板（向销售方） | `sales_email/quote/updated_seller_template` |  |  |  |
| 报价到期（48小时内） | `sales_email/quote/expire_two_days_template` |  |  |  |
| 报价到期（24小时内） | `sales_email/quote/expire_one_day_template` |  |  |  |
| 到期日期重置 | `sales_email/quote/expire_reset_template` |  |  |  |
...
管理员中提供了这些配置值，位置如下： **商店** >设置> **配置** > **销售** > **引号**.

| 名称 | 配置路径 | 是否加密？ | 特定于系统？ | 敏感？ |
| -------------- | -------------- | -------------- | -------------- | -------------- |
| 最小金额 | `quote/general/minimum_amount` |  |  |  |
| 最小金额消息 | `quote/general/minimum_amount_message` |  |  |  |
| 默认过期期限 | `quote/general/default_expiration_period` |  |  |  |
