...
配置 **frontName** 的Commerce管理员url，使用 `backend` env.php中的节点。
...
了解关于不同内容的更多信息 [缓存类型](../cli/manage-cache.md).

## consumers_wait_for_messages

指定如果处理的消息数小于 `max_messages` 值。 默认值为 `1`.
...
- `1` — 使用者继续处理来自消息队列的消息，直到到达 `max_messages` 中指定的值 `env.php` 文件，然后关闭TCP连接并终止消费进程。 如果队列在到达 `max_messages` 值，则消费者会等待更多消息到达。

   我们建议大型商户使用此设置，因为需要稳定的消息流，并且不希望出现延迟处理。

- `0` — 使用者处理队列中的可用消息，关闭TCP连接，然后终止。 即使已处理的消息数小于 `max_messages` 中指定的值 `env.php` 文件。 这有助于防止由于消息队列处理长时间延迟而导致cron作业出现问题。

   我们建议将此设置应用于较小的商家，他们不希望持续的消息流并且更愿意节约计算资源以换取在连续几天没有消息的情况下出现的轻微处理延迟。

## cron

启用或禁用Commerce应用程序的cron作业。 默认情况下，将启用cron作业。 要禁用它们，请添加 `cron` 配置 `env.php` 文件并将值设置为 `0`.
...
>[!WARNING]
>
>禁用cron作业时要小心。 禁用后，Commerce应用程序所需的基本流程将不会运行。

详细了解 [Crons](../cli/configure-cron-jobs.md).
...
定义消息队列的默认连接。 该值可以是 `db`， `amqp`或自定义队列系统，如 `redismq`. 如果指定的值不是 `db`，必须先安装和配置message queue软件。 否则，将无法正确处理消息。
...
如果 `queue/default_connection` 在系统中指定 `env.php` 文件，此连接用于通过系统的所有消息队列，除非中定义了特定连接 `queue_topology.xml`， `queue_publisher.xml` 或 `queue_consumer.xml` 文件。
例如，如果 `queue/default_connection` 是 `amqp` 在 `env.php` 但是 `db` 连接是在模块的队列配置XML文件中指定的，模块将使用MySQL作为消息代理。

## 目录

将Web服务器配置为从提供Commerce应用程序时需要设置的可选目录映射选项 `/pub` 目录 [提高了安全性](../../installation/tutorials/docroot.md).
...
此节点中可用的可下载域列表。 可以使用CLI命令添加、删除或列出其他域。
...
使用此节点，Commerce会锁定 `env.php` 文件，然后禁用管理员中的字段。
