...
description: 了解自托管安全理念以及要考虑的概念和最佳实践。 了解只读文件系统、恶意软件扫描等概念，以及托管Adobe Commerce时要考虑的许多其他主题。
landing-page-description: 了解自行托管Adobe Commerce时的一些安全概念和注意事项。
...
对于任何与电子商务项目相关的内容，安全都应始终是一个强有力的考虑因素。 如果没有强大的安全态势，可以攻击的表面面积将呈指数级增长。 所提出的概念和想法提供了经证实可降低通常所利用的常见漏洞的方法。

以下概念不是按任何特定顺序排列的。 它们旨在提供一些想法和概念以供考虑。 其中许多是免费的，或者需要最低限度的设置和配置以及后续的监控。 在本教程之外研究这些主题，以确保您对此处介绍的概念有足够深入的了解。

## 只读文件系统

只读文件系统的概念借鉴自 [云基础架构上的Adobe Commerce](https://experienceleague.adobe.com/docs/commerce-learn/tutorials/getting-started/cloud/1-overview.html){target="_blank"}. 这完全删除了错误操作者使用的一个主要区域。 许多漏洞利用更改商务应用程序中预期存在的文件来避免检测。 坏操作者不会创建一个，而是更改现有文件的内容以执行意外操作。 使文件系统为只读可显着减少此攻击向量。
...
恶意软件扫描通常可以从尝试在Adobe Commerce中专门化的托管提供商找到。 随着新威胁的发现、分类和诊断，这个已知的恶意软件和漏洞库不断增长。 查询托管提供程序是否具有此类服务，以及它们是可以自动运行还是仅应请求运行。 此外，您还可以订阅一些服务，这些服务可以使用他们自己的已知利用漏洞库来不断检查您的商业应用程序中的利用漏洞。 其中有些只是外部的，有些可以添加到基础结构中以对所有文件夹、文件甚至数据库进行内部深度扫描。 从Sansec.io到Sucuri，当然还有MageReport，在此领域拥有多年经验的供应商不胜枚举。 有些是免费的，有些是有相关成本的。 了解这一点并与Adobe Commerce架构师和DevOps团队进行深思熟虑的对话将确保您找到合适的解决方案。

## 适用于商务的网站范围分析工具

此 [站点范围分析工具](https://experienceleague.adobe.com/docs/commerce-operations/tools/site-wide-analysis-tool/intro.html){target="_blank"} 是一个主动式自助服务工具和中央存储库，其中包含详细的系统分析和建议，以确保Adobe Commerce安装的安全性和可操作性。 它提供全天候的实时性能监控、报告和建议，以确定潜在问题，并更好地了解站点运行状况、安全和应用程序配置。 它有助于缩短解决时间并改进站点稳定性和性能。

## 启用和验证管理操作日志记录设置

登录Adobe Commerce管理员并导航到“商店” > “配置” > “高级” > “管理员” > “管理员操作日志记录”后，即可找到该标记。 这将提供监视和记录的事件的列表。 如果在受攻击的网站上进行取证分析，并且怀疑这些网站获取了商务管理员的访问权限，则此功能非常有用。 此日志记录和报告有助于查看不良操作者执行了哪些事件。 如果禁用了任何管理操作日志记录，则表明有人可能出于保护目的禁用了这些操作日志记录，请在执行某些操作时删除日志记录。

## 用于进行ssh访问的堡垒服务器

这很难解释“安全概念”教程中的大多数主题。 其基本思想是提供一个服务器作为ssh访问的中间人。 这样，您的生产服务器就永远不会允许外部ssh连接。 您可以创建此堡垒服务器并使用本地IP掩码，以确保只允许指定的服务器通过SSH连接到这些服务器。

## 查看ACL角色和权限

Adobe Commerce的每位管理员用户都分配有一个ACL角色。 应创建此角色，以仅提供必须执行作业的功能。 应经常评估ACL角色，以确保它们不会提供超出必要的权限。 如果需要，请创建多个ACL角色，并赋予其职责。 如果出于某种原因将访问权限授予第三方，则应尽快停用这些权限。 询问他们在创建管理员用户时绝对需要访问多长时间，并设置自动过期日期。
...
限制对生产数据的访问。 这些指定的队友应该能够拉下生产数据库，并清除其真实数据。 如果删除数据是一个选项，请截断相应的表，如订单、报价和客户。 但是，有时您需要完整的数据集，但可以对这些值进行匿名处理。 在暂存环境中通常会出现这种情况。 在升级之前，此插件也很有用。 通过拥有真实的数据量，但匿名化可以确保您测试和验证正确执行部署以进行升级的时间。 如果您的数据集有限，则可能会低估升级过程和时间。

+++随机化客户信息示例以下示例介绍了如何在Adobe Commerce存储数据的某些标准表中使用随机字符串以及所有名字和姓氏字段更改客户电子邮件地址。 **请记得检查所有表是否包含敏感数据，此列表并非包含所有可能存储客户数据的表**
...
+++完全删除信息示例以下是在启动之前删除所有订单、报价单、贷项通知单等的示例，适用于较低级别的开发环境
...
使用环境变量有助于您设置某些值，这些值可以而且应该为每个环境更改。 例如，您可能希望为每个环境使用不同的管理员URL。 通过将此值设置为环境变量，您可以配置此值，并在必要时从Cloud UI中快速引用此值。
...
CI/CD管道可以是一个功能强大的工具，并帮助自动执行某些任务。 特别是，开发人员提交可能被利用的代码的可能性始终很大。 同伴代码审查通常会捕捉到此类项目，但由于它是人，因此会出错。 自动代码扫描有助于减少新引入的功能中出现意外漏洞的机会。 这些工具甚至可以阻止将代码合并到实时代码库中。 提供自动化代码安全和质量扫描有许多方法和工具。 可以有强大的自定义开发工具，但是它们需要不断更新和调整。 另一种方法是应用主动更新的工具，如synk.io和Amazon的代码检查器。

## Web应用程序防火墙

与DevOps或托管提供商通话时通常使用的Web应用程序防火墙或WAF。

Web应用程序防火墙(WAF)通过根据一组安全规则过滤通信量来阻止恶意通信量进入站点和网络。 触发任何规则的流量会先被阻止，然后才可能损坏您的网站或网络。

Adobe Commerce的cloud WAF提供了WAF策略，以及旨在保护Adobe Commerce Web应用程序免受各种攻击的规则集。 如果选择自托管选项，则查找WAF和配置规则是您的责任。 一些托管提供程序和WAF提供程序具有一组通用规则，这是一个良好的开始，但有一些工作有望使您的项目取得成功。

WAF检查Web和管理通信以识别任何可疑活动。 它会评估GET和POST流量（HTTP API调用），并应用规则集来确定要阻止的流量。 WAF可以阻止各种攻击，包括SQL注入攻击、跨站点脚本攻击、数据过滤攻击和HTTP协议违规。

作为基于云的服务，WAF不需要安装或维护任何硬件或软件。 Fastly是一家现有的技术合作伙伴，负责提供软件和专业知识。 它们的高性能、始终开启的WAF驻留在Fastly全球交付网络的每个缓存节点中。
