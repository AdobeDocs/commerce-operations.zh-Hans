...
在设计布局时，请使用灵活的格式来在此区域以最佳方式显示优先级最高的产品、功能、销售、通知、选项等。

对于移动设备和平板电脑，折页上方区域存在很大差异，尤其是在屏幕大小和尺寸以及查看时的方向（纵向与横向）上。
...
活动分支或环境是指连接到有权访问服务的已部署或正在运行的实例的分支或环境。
取消激活时，与服务和正在运行的实例的连接将被删除，但代码会保留。
它会变成一个普通的Git分支。
...
例如，数据库适配器、缓存适配器、文件系统适配器、后处理器库适配器以及其他类型的计算适配器。
...
在Adobe Commerce中，管理员用户拥有对“管理员”中所有功能、选项和功能的完全权限和访问权限。
他们还可以创建用户和角色。
...
描述产品某些方面的产品特性或属性。
Adobe Commerce或Magento Open Source用户可创建自定义属性以添加到默认属性集或自定义属性集。
通过管理员或以编程方式创建这些属性。
示例：颜色、大小、重量、价格、年龄、性别等。

自定义属性是一种Entity-Attribute-Value (EAV)属性。
...
产品价格，减去优惠券或折扣，加上运费和适用的税款。
平均成本的计算方法是：将每月库存的起始成本加上期间最后一个月库存的终止成本。
...
一种页面输出单位，可为最终用户呈现一些独特内容（一段信息、一个用户界面元素）以及任何视觉上可见的内容。
[个块](https://experienceleague.adobe.com/docs/commerce-admin/content-design/elements/blocks/blocks.html) 由模块实现和提供。
块使用模板生成HTML。
阻止的示例包括类别列表、迷你购物车、产品标记和产品列表。

[动态块](https://experienceleague.adobe.com/docs/commerce-admin/content-design/elements/dynamic-blocks/dynamic-blocks.html) 提供基于逻辑的内容，例如价格规则。
...
这些品牌包括服装、家电、奢侈品等名牌。
您的公司也可以是品牌，根据业务单元、目标受众等以多个品牌销售产品。

使用自定义属性保存产品的品牌信息。

某些扩展和集成可能会为您的产品使用或要求使用品牌，例如Google Shopping Ads渠道和AmazonSales Channel。
...
具有永久物理位置的零售企业，与几乎或仅通过互联网运作的企业不同。

对象 [Inventory management](https://experienceleague.adobe.com/docs/commerce-admin/inventory/sources/sources-manage.html) 和 [Order Management](https://omsdocs.magento.com/getting-started/terminology/)，此商店是跟踪产品数量、发货订单和支持店内提货的来源。
...
批量操作是大规模执行的操作。
批量操作任务示例包括导入或导出物料、更改批量价格，以及将产品分配给仓库。
...
在Zend的默认框架中，将缓存记录存储到两级后端系统中。
第一级缓存是快速的（例如，APC或Memcached后端），但它是有限的，不支持标记和分组缓存条目。
...
每个Commerce商店只有一个产品目录，所有商店视图都共享该目录。
在多存储安装中，每个存储可以有一个单独的目录，也可以共享该目录。
当前商店目录由默认根目录类别确定，用户可在商店的顶部导航（主菜单）中看到该目录。
（术语“根类别”可能会让人困惑，因为“根类别”实际上根本不是类别，而是菜单的容器，由类别和子类别组成。）

您可以创建任意多个根类别，但一次只能使用一个（默认）。
...
客户审查信息并提交订单后，会向客户发送电子邮件确认。

签出具有许多现成的选项和配置，并通过扩展实现。
...
特殊变体的 [块](https://experienceleague.adobe.com/docs/commerce-admin/content-design/elements/blocks/blocks.html) 只能在Admin中创建，不能通过布局文件引用。
...
每个选项实际上都是一个单独的简单产品，具有唯一的SKU，因此可以跟踪每个产品变体的库存。
为了达到类似的效果，简单的产品可以与自定义选项一起使用，但无法跟踪每个变体的库存。
具有多个选项的产品有时称为复合产品。

虽然可配置产品使用的SKU较多，并且最初可能需要更长的时间进行设置，但最终可为您节省时间。
如果您计划拓展业务，对于具有多种选项的产品来说，可配置产品类型可能是更好的选择。

示例：有两种颜色和三种尺寸的T恤。
必须将6个变体创建为单个产品（每个产品都具有自己的SKU）。
然后，所有变体都会添加到可配置产品中，客户可以在其中选择大小和颜色，然后将其添加到购物车中。
...
核心层扩展包括三个用于数据存储、缓存和服务的服务节点，如OpenSearch、Elasticsearch、MariaDB、Redis。
...
商家向客户出具的单据，用于核销因超额收费、返点或退货而产生的未清余额。
备忘会将资金还原到客户的帐户。
...
购物车旁边显示的产品。
当客户导航到购物车页面时，这些产品显示为与购物车中已有的商品交叉销售。
...
数据库中数据的结构。
定义如何组织数据以及如何管理数据关系，包括应用于数据的所有限制。
如果某个模块包含必须存储在数据库中的数据，则该模块可以包含数据库模式的片段。
...
一种电子邮件验证过程，要求潜在的订阅者完成确认其订阅意向的第二步。
...
可下载的文件可以驻留在您的服务器上，也可以作为URL提供给任何其他服务器或内容交付网络。
...
要使用静态URL，请更改配置设置。
但是，如果在禁用设置时目录中包含了Dynamic Media URL，则目录中的每个引用都会变为断开的链接。
...
一组用于管理和部署Commerce应用程序的脚本和工具。 此软件包简化了许多Adobe Commerce在云基础架构方面的流程，包括部署到Docker环境、管理crons、验证项目配置和应用Adobe修补程序。
...
编程中的唯一单位或对象。
包含可修改的属性或参数。
示例包括暂存（更新可以更改价格规则、产品或类别等实体）和数据库记录（服务合同包括发送和接收的数据结构）。
...
对于数据库实体，是一种有效编码实体的数据模型。
将实体ID、属性名称和值存储为三次，这样可以随时创建新的实体属性名称。
在编码中，可用于描述实体的属性数量可以大幅缩放，但适用于给定实体的数量会最小化。
此数据模型虽然灵活，但速度可能较慢。
...
前端组件（或客户端）是一个界面，允许用户处理底层后端代码或与之交互。
后端代码在服务器上运行。
用户无法直接访问后端代码。
...
可在商店中购买的预付卡或礼品证书。
每个礼品卡都分配有一个唯一代码，该代码在结帐时输入。
礼品卡的价值反映在礼品卡帐户余额中。
礼品卡有三种类型：

* “实物”礼品卡可以用塑料或卡片材生产，然后发运给客户。
* “虚拟”礼品卡通过电子邮件发送。
* “组合”礼品卡是这两种礼品卡的组合，作为实体卡发运给收件人，并通过电子邮件发送。
礼品卡可配置，包括产品资格选项和选择未结或固定金额。

在后端创建订单时，商店管理员还可以根据客户请求兑换礼品卡。

礼品卡还有助于促销活动，因为商店管理员可以在后端手动创建礼品卡帐户，并将礼品卡代码发送到特定客户区段。
礼品卡可以用作忠诚度计划，针对在节假日从网络商店购买大量商品或进行特定促销活动的最活跃客户。
...
一种产品类型，将多个类似的独立产品分组到一个页面上。
可以随单个产品的变体提供，也可以通过按季节或主题分组它们以创建协调集。
每个产品可以单独购买，也可以作为群组的一部分购买。
...
为了拦截函数，插件实施了要调用的其他代码。
插件通过依赖关系注入配置文件(di.xml)与拦截点相关联。
如果同一函数上定义了多个插件，则依赖项注入配置将定义这些插件的调用顺序，从而允许使用多个插件而不会发生冲突。
...
在构建Commerce页面时，布局是指一组以层次结构装配的块，表示页面的结构。
...
布局文件中的标记，描述要应用于块、容器和UI组件的结构化元素树中所做的更改。
单个布局文件可以包含多个布局指令。
布局指令在布局文件中以XML格式编码。
...
用于为修改XML布局或从其他文件导入布局说明而添加的代码片段。
...
许可证所有者（也称为帐户所有者）是商业组织中的指定人员，负责管理Adobe Commerce on cloud infrastructure帐户的付款和其他商业相关问题。
此人充当与Adobe的联系人。
企业购买Adobe Commerce on cloud infrastructure订阅后，初始项目和代码访问权限仅供指定为许可证所有者的人员使用。
...
MAGEID通常是Adobe Commerce帐户上的计费联系人(可能不是Adobe Commerce on cloud infrastructure项目的项目所有者)。
要获得对Adobe Commerce和Adobe Commerce在云基础架构包上的访问权限，您必须使用与有权访问这些包的MAGEID关联的访问密钥。
...
[配置标记](https://experienceleague.adobe.com/docs/commerce-admin/catalog/products/settings/settings-advanced-custom-options.html)通过产品可自定义选项查看产品的、或Markdown。
...
在云基础架构上的Adobe Commerce上，Pro项目使用名为主控的活动Platform as a Service (PaaS)环境，其中包括生产环境数据库和Web服务器的副本。
...
它为Commerce开发人员和软件工程师（例如QA专家、PHP开发人员和系统集成商）提供了一个测试框架。
...
模块包含在目录结构中，该目录结构包含与特定功能相关的PHP和XML文件（配置、块、控制器、帮助程序、模型等），以提供独特的产品功能集合。
每个模块的目的是通过实施新功能或扩展其他模块的功能来提供特定的产品功能。
每个模块均设计为独立运行，因此包含或排除特定模块不会影响其他模块的功能。

模块还可以实施构件，构件是业务用户在管理员中自定义的页面元素。

可以禁用或删除模块，而不会破坏Magento应用程序的一致性。
一个例外：当模块依赖于其他模块时，这要求禁用或删除依赖的模块。

_术语属性：_

* _字段：商业软件_
* _相关术语：php、xml、block_
...
OMS是一种灵活且经济实惠的解决方案，可用于管理、销售和完成任何销售渠道中的库存。
...
源遮盖是一项安全功能，它允许云基础架构上的Adobe Commerce阻止任何流向云基础架构（源）的非Fastly流量以防御DDoS攻击。

了解详情： [Fastly起源遮盖](https://experienceleague.adobe.com/docs/commerce-knowledge-base/kb/faq/fastly-origin-cloaking-enablement-faq.html)
...
Page Builder是一个Commerce扩展，用于通过拖放预建控件来定义自定义布局来创建内容丰富的页面。
这些控件也称为“内容类型”。
商家可以设计版面和页面，而无需编码体验。
...
支付网关是一种第三方服务，它无缝地处理信用卡交易，无需客户离开商家网站。
...
一种产品选择，以奖励购买其他商品。
例如，如果客户正在查看相机的产品页面，则相关产品可能包括其他类似相机、相机盒和三脚架。
...
在Adobe Commerce中，范围描述了设置可能影响的存储层级的范围。
范围可以应用于以下内容：
...
在层次结构中，在较低级别应用的设置可以覆盖某些更高级别的设置。
...
为模块定义的一组PHP接口。
服务合同包括保持数据完整性的数据接口和服务接口，服务接口隐藏业务逻辑细节，使业务逻辑细节对服务请求者如控制器、Web服务和其他模块隐藏。
...
一种功能，允许商家创建一个目录，用作其整个目录或该目录的子集，然后为一个或多个产品指定自定义价格。
...
* 主目录
* 客户1目录（可能只有三个SKU，主目录中的这些SKU折扣很大）
* Customer 2目录（可以是整个目录，享受10%的折扣）
* Customer 3目录（主目录折扣为5%至60%的几十种产品）。
...
“商店”的商务范围级别是网站层次结构的第二级，如下所示：网站>商店>商店视图。
可以将商店组织为一个或多个。 每个商店可能都有自己的根类别，并且所有商店都共享目录和客户数据。

每个商店可以有多个商店视图，这些视图通常用于以不同的区域设置和语言呈现店面。
...
“店面视图”的商务范围层级是指网站、店面和店面视图层级的第三层。
...
产品税分类、客户税分类和税率的组合。 此规则定义要应用的计税方法。
...
HTML模板或PHTML模板的简称。
PHTML文件包含HTML标记和PHP代码的混合，用于将动态内容插入HTML。
大多数块至少有一个PHTML（模板）文件，该文件包含块生成的静态HTML。
...
包含图形和外观信息。
自定义商店的外观。
Adobe Commerce可以在(Composer)包中发送主题。
但是，主题可以放置在应用程序/设计下，该应用程序未随包提供。
包是Composer的下载单元，并且 — 通过Commerce Marketplace—Commerce用户可以下载CE或EE作为一系列包，其中包包含模块、主题或语言包。
...
UPLOAD服务器使用定义文件来确定应用程序shell请求的适当进程或服务。
它描述了服务器应如何处理请求并为其构建响应。

PWA项目可以包括UPLOAD定义文件以指定其服务依赖关系。

_术语属性：_

* _字段：设计、商业软件、编程_
* _同义词：统一渐进式Web应用程序响应定义PWA Studio_
* _相关术语： pwa_
...
供应商生成的代码，用于扩展或自定义商业行为并作为第三方扩展运行，这被视为供应商捆绑扩展(VBE)。
VBE经过全面测试，并包含在每个受支持的Magento Open Source和Adobe Commerce版本中。
VBE可以包括模块、主题和语言包。

了解详情，请参阅 [供应商捆绑的扩展主题](https://experienceleague.adobe.com/docs/commerce-operations/upgrade-guide/modules/upgrade.html).
...
表示可以销售的非物理产品，如会员资格、服务、保修或订阅。
虚拟产品可以单独销售，也可以作为以下产品类型的一部分包括在内：分组产品和捆绑销售产品。
不需要装运或盘存。
...
虚拟类型是一种在不影响其他类且无需创建类文件的情况下将不同依赖项插入到现有PHP类中的方法。
...
您可以使用小组件创建营销活动的登陆页面，并在整个商店中的特定位置显示促销内容。
小组件还可用于为外部审阅系统、视频聊天、投票和订阅表单添加交互式元素和操作块，或者为标记云和图像滑块提供导航元素。
