...
* __管理员用户的密码重置模板问题__
使用正确的密钥解决了该问题，现在，该密钥在电子邮件模板中包含管理员用户名并正确填写主题。 以前，该问题源于使用的密钥已过时。
...
* __“我的订单”部分上的不必要的边框__
以前，创建了一个应用了其他CSS类的附加容器（订单参考），这会导致不必要的边框行显示在“我的订单”部分中的订单编号下方，而此部分现在不可见。
  _ACP2E-3044 - [GitHub代码贡献](https://github.com/magento/magento2/commit/9af794a4)_
* __sales_clean_quotes cron删除尚未批准的采购订单中的报价__
...
系统现在可以正确处理自定义发运方法，而无需使用运费验证规则，从而确保在结帐期间更改发运地址中的邮编后，加载程序不会阻止发运方法。 以前，在结帐期间更改发运地址中的邮编将导致装入程序阻止发运方法，并且在使用没有发运费率验证规则的自定义发运方法时不会消失。
...
* __[问题]使ImageMagick适配器与php8（从浮点到整点的隐式转换）兼容__
现在，系统在计算图像尺寸时正确处理浮点数，从而确保与PHP8的兼容性，从而防止由于从浮点到整点的隐式转换而产生任何错误。 以前，图像尺寸的计算可能会导致浮点数，当对其进行隐式舍入时，会导致错误。
...
将Magento\Framework\App\PageCache\Version类构造函数属性的可见性从private还原为protected
  _LYNX-581_
* __将默认GraphQL查询的最大复杂性增加到1000__
将默认的最大GraphQL查询复杂性从300增加到1000。
  _LYNX-600_
...
解决了启用了单独链接购买选项的可下载产品在itemsV2 >原始行总计时返回$0的问题，对于具有单独价格的文件选项的产品，价格范围返回为$0.00。
  _LYNX-620_
* __PHP-8.4版本__的GraphQl兼容性
修复了多个解析器之间的GraphQL与PHP 8.4的兼容性问题，从而确保了流畅的功能。 在CatalogRule、Customer、GiftMessage、GiftCard和GiftWrapping模块中更新了受影响的文件。
  _LYNX-772_

### GraphQL、清单/MSI

* __当源购物车和目标购物车具有相同的捆绑包项目时，MergeCart变异引发异常__
&#39;-
